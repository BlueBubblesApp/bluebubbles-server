...
export class ContactValidator {
        ...
        for (const item of body) {
            if (!item.id && !item.externalId) {
                throw new BadRequest({
                    error: "Each contact update must include either an 'id' or 'externalId' field!"
                });
            }

            // Ensure there's at least one field to update
            const hasUpdateFields =
                isNotEmpty(item.firstName) ||
    ...
    static async validateImportVcf(ctx: RouterContext, next: Next) {
        ...
        if (!vcfFile) {
            throw new BadRequest({
                error: "VCF file is required! Upload using multipart/form-data with field name 'vcf'"
            });
        }

...
// import { Device } from "@server/databases/global/entity/Device";
...
//      */
//     constructor() {
//         this.server = io(Server().db.getConfig("socket_port") as number, {
...
//     startStatusListener() {
//         setInterval(async () => {
//             const port = Server().db.getConfig("socket_port");
...
//             const cfgPass = String((await Server().db.getConfig("password")) as string);
...
//             const encrypt = Server().db.getConfig("encrypt_coms") as boolean;
//             const passphrase = Server().db.getConfig("password") as string;
...
//                 const device = await Server().db.devices().findOne({ name: params.deviceName });
//                 if (device) {
//                     device.identifier = params.deviceId;
//                     device.last_active = new Date().getTime();
//                     await Server().db.devices().save(device);
//                 } else {
...
//                     await Server().db.devices().save(item);
//                 }

//                 Server().db.purgeOldDevices();

...
## The data protection keychain is asserted at signing; the app still cannot say which it uses

`Packaging/sign-app.sh` now runs the nested CLI's `--check-keychain` after signing and fails
the build if it fell back to the legacy store. That closes the dangerous half: a release whose
provisioning profile did not embed, expired, or was issued for a different App ID can no longer
ship silently putting every secret in the weaker store.

The check writes a probe item, reads it back and deletes it before answering, because
`usingDataProtection` resolves lazily: on the first call that returns
...
What is still missing is the runtime half:
...
## The § Security #1 claim is met by the entitlement, not by an ACL

§ Security #1 asks for Keychain items "bound to the app's code signature rather than being
readable by any same-user process". This entry used to ask for a `SecAccessControl` to get
there. That is no longer the shape of the answer, and the old ask would have been the wrong
work: an ACL is the LEGACY keychain's mechanism, it authorises by prompting, and a prompt on
the TLS bind path is invisible in a launchd process.

`KeychainSecretStore` writes to the data protection keychain now, where access is decided by
the `keychain-access-groups` entitlement and a process outside the group is refused
categorically, with no prompt and no dialog to click through. That is a stronger property than
the ACL this entry asked for. `kSecAttrAccessibleAfterFirstUnlockThisDeviceOnly`: not
`WhenUnlocked`, because `auto_lock_mac` means the server has to keep serving behind a locked
screen, is honoured here, where the legacy keychain ignored it entirely.

So #1 is met **in a signed release build**, and only there. What is left is not an ACL:

- [ ] Prove the release build is actually using it. Tracked above under "The data protection
      keychain has no test that proves it is in use": an unsigned build falls back to the
      legacy store by design, and a release build that lost its profile would do the same
      silently. Until that assertion exists, the regression suite cannot honestly claim #1.
...
## The dev bundle loses Keychain access after a rebuild

`Tools/dev-bundle.sh` ad-hoc signs on every build, and the login-Keychain ACL on
`app.bluebubbles.server / password` is granted to a specific code identity. A rebuild changes
that identity, the read fails, and `AuthenticationScheme` reports "No server password is
configured"; every authenticated request 500s while the item itself is intact (verified: 20
characters, still present).

It fails SILENTLY. Nothing is logged: `grep -ic "secret|keychain"` over the whole server log
returns 0, and the failure surfaces only as a request error. That is the worst property here:
the server looks broken for a reason it never states.

- [ ] Report a Keychain read failure distinctly from "no password set". `SecretStore.get`
      throws on anything but `errSecItemNotFound`; the auth path should say which happened.
      Related to the already-open item about surfacing `usingDataProtection`.
- [ ] Say it in `Tools/dev-bundle.sh`'s header, which explains the TCC consequence of ad-hoc
      signing and not the Keychain one.
...
**What has to match here, and what does not.** The OUTPUT is the contract: the 400, the
`error.type`, and the generated sentence in `error.message`, because a client shows that string
and some branch on it. The rule sets are the contract too, in the sense that WHICH inputs are
rejected is observable. Nothing else is. So this is not a port of validatorjs: do not clone its
API, its rule-string parser, or its internals; build the validation layer this codebase would
want on its own terms (declarative per-route rules next to `RouteTable`, typed, testable) and
make it EMIT what the reference emits.

- [ ] Build a declarative validation layer beside `RouteTable` covering the dozen rules actually
      used (`required`, `string`, `numeric`, `boolean`, `array`, `present`, `in:`, `min:`,
      `max:`). Reproduce validatorjs's message text and its FIRST-error-only behaviour as
      OUTPUT (`ValidationFailure` already documents the second half) without reproducing its
      structure. Done = the `post_api_v1_message_query-5baa61-400` baseline entry is deleted,
      and the deletion is what proves the output matches.
...
every client: the filename is derived from this id. A string is the right default precisely
because the client stringifies whatever it gets: an integer and a UUID are already the same
value to it, so sending the UUID costs nothing and sending an integer would buy nothing.

**This is closed, and the replay baseline says so.** `post_api_v1_contact-5baa61-200.json` was
listed as `OPEN`, which reads as pending work and made the ratchet report three real divergences
when there are two. It is now `DECIDED`: a fourth reason class, for a divergence that was
examined and kept. The entry carries the decision so the baseline can be read without coming
here first.
...
## `ServerComposition.build` is 346 lines

By a wide margin the longest function in the package; the next is 157. It is a deliberately linear
composition root and reading it top to bottom is the point, so this is not a defect, but the
phases inside it are already marked with `// MARK:` comments and would read the same as
`buildStorage`, `buildCrossCutting`, `buildTransports` returning into one another.

- [ ] Low priority. Only worth doing if a phase needs to be testable on its own.

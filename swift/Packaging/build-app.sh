...
# Whether a missing Sparkle key is fatal. Off for a local build, which legitimately has no
# signing key and produces an app that simply cannot verify an update. ON for a release, where
# the same silence ships installs that can never auto-update again; see below.
REQUIRE_SPARKLE_KEY=0

while [ $# -gt 0 ]; do
    case "$1" in
        --output) OUTPUT="$2"; shift 2 ;;
        --configuration) CONFIGURATION="$2"; shift 2 ;;
        --require-sparkle-key) REQUIRE_SPARKLE_KEY=1; shift ;;
...
# Sparkle. SwiftPM places the framework beside the binary (which is how `swift run` finds
# it, through the @loader_path rpath) and does nothing further; the bundle finds it through
# the `@executable_path/../Frameworks` rpath Package.swift adds to the app target. `cp -R`
# rather than ditto: it keeps the `Versions/` symlinks, and a framework whose symlinks were
# resolved into copies has a broken seal that notarization rejects.
SPARKLE="$BIN_PATH/Sparkle.framework"
if [ ! -d "$SPARKLE" ]; then
    echo "error: no Sparkle.framework at $SPARKLE. The app links it; without it the" >&2
    echo "error: bundle dies at load with 'Library not loaded'." >&2
    exit 1
fi
rm -rf "$APP/Contents/Frameworks/Sparkle.framework"
cp -R "$SPARKLE" "$APP/Contents/Frameworks/"
SPARKLE_ARCHS="$(lipo -archs "$APP/Contents/Frameworks/Sparkle.framework/Sparkle")"
for required in arm64 x86_64; do
    case " $SPARKLE_ARCHS " in
        *" $required "*) ;;
        *) echo "error: Sparkle.framework is missing the $required slice (got: $SPARKLE_ARCHS)" >&2; exit 1 ;;
    esac
done
echo "==> Bundled Sparkle.framework ($SPARKLE_ARCHS)"
...
# An absent key is a NOTE locally and an ERROR for a release, and the difference is the whole
# point of the flag. `SUPublicEDKey` is what every installed copy uses to verify the signature
# on the next update: an empty one ships an install that rejects every future update forever,
# and there is no way to repair it remotely because the repair would itself be an update.
#
# It was a note in both cases, which is how it could have shipped. The release workflow exports
# the key from `bb-appcast public-key`, and `export VAR="$(cmd)"` does NOT propagate the
# command's exit status under `set -e` (verified: `bash -ec 'export F="$(exit 7)"; echo $?'`
# prints 0). So a rotated or absent signing secret produced an empty key, this printed a note,
# and the build went on to sign, notarize, staple and publish.
if [ -z "$SPARKLE_PUBLIC_KEY" ]; then
    if [ "$REQUIRE_SPARKLE_KEY" -eq 1 ]; then
        echo "error: SPARKLE_PUBLIC_KEY is empty and --require-sparkle-key was given." >&2
        echo "error: This build would ship installs that can never auto-update." >&2
        echo "error: Check that SPARKLE_EDDSA_PRIVATE_KEY is set and that" >&2
        echo "error: \`swift run bb-appcast public-key\` succeeds." >&2
        exit 1
    fi

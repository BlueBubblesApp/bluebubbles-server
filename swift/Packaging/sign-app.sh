...
# ASSERTED BY NAME, not discovered by glob.
#
# This was `for item in "$APP/Contents/Frameworks/"*.dylib; do [ -f "$item" ] || continue`,
# and an unmatched glob in sh stays the literal pattern, which the file test then skips: the
# whole loop was a silent no-op. A bundle missing either helper signed, notarized, stapled
# and shipped green, and every Private API and FaceTime route was simply absent at runtime
# with no log line, which is the same invisible failure the arm64e rule exists for.
#
# Sparkle, the launcher and the CLI are all hard-asserted a few lines below and above; these
# two, the highest-consequence payload in the bundle, were the ones that were not.
echo "==> Signing the injected helpers"
for helper in libBlueBubblesHelper.dylib libBlueBubblesFaceTimeHelper.dylib; do
    item="$APP/Contents/Frameworks/$helper"
    if [ ! -f "$item" ]; then
        echo "error: $helper is missing from $APP/Contents/Frameworks." >&2
        echo "error: build-app.sh builds it arm64e and copies it; without it the Private" >&2
        echo "error: API is absent at runtime and nothing says so. Refusing to sign." >&2
        exit 1
    fi
    echo "    $item"
...
# Sparkle, in the order its documentation gives: the XPC services and the two helpers
# inside the framework first, then the framework. Three things are deliberate here.
#
# NO entitlements. This app's file claims `keychain-access-groups`, a restricted
# entitlement that is authorised by the embedded provisioning profile; an XPC service
# signed with it and carrying no profile of its own is a binary the kernel kills at launch,
# silently, and Sparkle then reports only that the installer could not be reached.
#
# `--preserve-metadata=entitlements` on Downloader.xpc, which ships with the entitlements
# it needs and must keep them.
#
# No `--deep`, for the same reason Sparkle gives: the pieces need different treatment, and
# `--deep` would give them all the outermost one's.
...
if [ -d "$SPARKLE" ]; then
    ...
    echo "    $SPARKLE/Versions/B/XPCServices/Downloader.xpc"
    ...
    echo "error: no Sparkle.framework in $APP/Contents/Frameworks; build-app.sh copies it." >&2
    exit 1
fi

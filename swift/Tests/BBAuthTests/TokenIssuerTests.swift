...
struct BearerSchemeTests {
  ...
  ///
  /// This exercises the SCHEME. What it does not exercise, and what its name implied it did,
  /// is the socket's own normalisation — which stripped every parameter but `password` and
  /// `guid`, so under `auth_mode = token` the chain was handed nothing and no client could
  /// open a socket at all. `SocketHandshakeNormalizationTests` below covers that half.
  @Test("A Socket.IO handshake token is accepted by the scheme")
...
/// The half the suite above did not reach.
///
/// `SocketHandshakeScheme.normalize` is what the transport applies BEFORE the chain sees
/// anything, and it is a separate, static function precisely so both the scheme and
/// `SocketServer.authenticate` share one implementation. A test that calls the scheme
/// directly skips it, which is why "the socket cannot authenticate under token mode" sat
/// behind a green test named for exactly that case.
@Suite("Socket handshake normalisation")
struct SocketHandshakeNormalizationTests {

  @Test("A bearer token survives normalisation, or no socket can connect in token mode")
  func bearerTokenSurvives() {
    for name in ["auth_token", "access_token"] {
      let normalized = SocketHandshakeScheme.normalize(query: [name: "tok3n"])
      #expect(
        normalized[name] == "tok3n",
        Comment(rawValue: "`\(name)` was stripped; BearerTokenScheme reads exactly this"))
    }
  }

  @Test("A password still normalises the way the socket has always required")
  func passwordIsUnchanged() {
    #expect(
      SocketHandshakeScheme.normalize(query: ["password": "hunter2"]) == ["password": "hunter2"])
    // `guid` is the legacy spelling and maps onto the same key.
    #expect(SocketHandshakeScheme.normalize(query: ["guid": "hunter2"]) == ["password": "hunter2"])
    // NOT decoded a second time; the transport already decoded once.
    #expect(
      SocketHandshakeScheme.normalize(query: ["password": "a%20b"]) == ["password": "a%20b"])
  }

  @Test("`token` is still not a credential the socket accepts")
  func referenceEraTokenIsStillRefused() {
    #expect(
      SocketHandshakeScheme.normalize(query: ["token": "tok3n"]).isEmpty,
      "the socket has never accepted `token`, and that is reproduced rather than harmonised")
  }

  @Test("Nothing usable normalises to nothing, rather than to a partial credential")
  func emptyStaysEmpty() {
    #expect(SocketHandshakeScheme.normalize(query: [:]).isEmpty)
    #expect(SocketHandshakeScheme.normalize(query: ["password": ""]).isEmpty)
    #expect(SocketHandshakeScheme.normalize(query: ["auth_token": ""]).isEmpty)
  }
}

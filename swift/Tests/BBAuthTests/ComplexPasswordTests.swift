...
struct ComplexPasswordTests {
  ...
  /// `Basic` is `user:password`, and the PASSWORD half is the credential — including when
  /// it is empty, and including when the username is not.
  ///
  /// `split(separator: ":", maxSplits: 1)` omits empty subsequences by default, so `"user:"`
  /// splits to `["user"]` and `.last` was the USERNAME. A header carrying an empty password
  /// therefore compared the username against the server password, which accepted
  /// `base64("thepassword:")` and read the wrong field on every header of that shape. Never
  /// a false accept of a wrong credential, and never the parse it reads as.
  @Test(
    "Basic reads the password half, not whichever half survived the split",
    arguments: [
      // encoded pair, what the credential should be
      ("user:secret", "secret"),
      (":secret", "secret"),
      ("user:", ""),  // the bug: this used to yield "user"
      ("user:pass:word", "pass:word"),  // only the FIRST colon separates
      (":", ""),
    ] as [(String, String)])
  func basicReadsThePasswordHalf(pair: String, expected: String) {
    let header = "Basic \(Data(pair.utf8).base64EncodedString())"
    let extracted = PasswordQueryScheme.extractCredential(
      from: CredentialPresentation(authorizationHeader: header))
    // An empty password is extracted as "" and refused later by the digest comparison,
    // which is a different thing from being read as the username.
    #expect(extracted == expected, "`\(pair)` read as \(String(describing: extracted))")
  }

  /// A Basic payload with no colon is not a `user:password` pair at all. It used to be taken
  /// as a bare password; now it is no credential, and the chain says "missing" rather than
  /// "wrong".
  @Test("A Basic payload with no colon is not a credential")
  func basicWithoutAColon() {
    let header = "Basic \(Data("justastring".utf8).base64EncodedString())"
    #expect(
      PasswordQueryScheme.extractCredential(
        from: CredentialPresentation(authorizationHeader: header)) == nil)
  }

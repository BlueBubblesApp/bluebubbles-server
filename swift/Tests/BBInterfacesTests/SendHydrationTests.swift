...
struct SendHydrationTests {
  ...
  func hydratesWhenTextColumnIsNull() async throws {
    // Writer first, and WAL; see `ChatFixtureCopy`. Two connections to a rollback-journal
    // copy is what made this suite fail intermittently with "SQLite error 10: disk I/O error".
    let (path, writer) = try ChatFixtureCopy.make()
    defer { ChatFixtureCopy.remove(path) }
  ...
  func waitsForTheChatJoin() async throws {
    ...
    // Sidecars too: a WAL copy leaves `-wal` and `-shm` beside it.
    defer { ChatFixtureCopy.remove(path) }
    ...
    //
    // Held rather than detached-and-forgotten, and NOT wrapped in `try?`. The write can fail
    // (SQLite is being read by the poll on the same file) and swallowing that turned a
    // failed insert into a full-limit timeout whose message said "answered before the join",
    // which is a true statement about a completely different cause. Awaited below so a write
    // that failed fails this test as itself.
    let rejoin = Task {
    ...
    // An explicit policy, where this test alone used to take the sixty-second default that
    // every sibling overrides. Two things it buys, and the first is the correctness one:
    //
    //   - A 50ms first attempt lands well before the 300ms re-insert, so the poll is
    //     GUARANTEED to see the un-joined row at least once. That is the property under
    //     test, and the 250ms default left barely 50ms of margin for it.
    //   - Five seconds is ample for a 300ms insert under any load this suite creates, and
    //     caps what a genuine regression costs at five seconds rather than a minute.
    let message = try await interface.awaitSentMessage(
      guid: Self.sentGUID,
      policy: .init(initialDelay: .milliseconds(50), multiplier: 1.5, limit: .seconds(5))
    )
    // Before the assertion: if the insert threw, say so, rather than reporting the timeout
    // it caused.
    try await rejoin.value
  ...
  static func writableInterface() async throws -> (MessageInterface, DatabaseQueue, String) {
    // Writer first, and WAL; see `ChatFixtureCopy`.
    let (path, writer) = try ChatFixtureCopy.make()

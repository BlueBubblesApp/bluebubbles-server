...
struct AppcastTests {
  ...
  private func item(
    ...
    AppcastItem(
      ...
      fullReleaseNotesLink: history,
      channel: channel
    )
  }

  /// A beta is invisible unless asked for, and never outranks a newer release.
  @Test("channels: a beta is offered only to installs that allow it")
  func channels() throws {
    let feed = Appcast(items: [
      item("1.2.0"), item("1.3.0-beta.1", channel: "beta"),
    ])
    #expect(feed.newestItem?.shortVersion == "1.2.0")
    #expect(feed.newestItem(allowingChannels: ["beta"])?.shortVersion == "1.3.0-beta.1")

    // Round trip: the channel is written and read as Sparkle's element.
    let parsed = try AppcastParser.parse(Data(feed.xmlString().utf8))
    #expect(parsed.items.first { $0.shortVersion == "1.3.0-beta.1" }?.channel == "beta")
    #expect(parsed.items.first { $0.shortVersion == "1.2.0" }?.channel == nil)

    // A beta older than the current release is not what "beta" means.
    let stale = Appcast(items: [item("1.4.0"), item("1.3.0-beta.1", channel: "beta")])
    #expect(stale.newestItem(allowingChannels: ["beta"])?.shortVersion == "1.4.0")
  }

  /// The two things a person reads in Sparkle's window. Neither is required, and a feed
  /// written without them parses back without them rather than as empty strings.
  @Test("release notes and the history link survive the round trip")
  func notesAndHistoryRoundTrip() throws {
    let notes = "<h2>1.3.0</h2><ul><li>Fixed a thing &amp; another</li></ul>"
    let history = "https://example.com/releases"
    let feed = Appcast(items: [item("1.3.0", notes: notes, history: history), item("1.2.0")])
    let parsed = try AppcastParser.parse(Data(feed.xmlString().utf8))

    let newest = try #require(parsed.newestItem)
    #expect(newest.releaseNotesHTML == notes)
    #expect(newest.fullReleaseNotesLink == history)
    let older = try #require(parsed.items.first { $0.shortVersion == "1.2.0" })
    #expect(older.releaseNotesHTML == nil)
    #expect(older.fullReleaseNotesLink == nil)
    #expect(feed.xmlString().contains("<sparkle:fullReleaseNotesLink>"))
  }

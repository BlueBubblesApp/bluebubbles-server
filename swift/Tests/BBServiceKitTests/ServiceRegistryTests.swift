...
//  The lifecycle the registry owns: dependency-ordered start and stop, settings-change
//  routing, gating, supervision, and the serialisation that keeps those from overlapping.
//
//  This replaces hand-maintained ordering. The Electron server started services in one
//  literal order and stopped them in a second, separately maintained one, so a service could
//  be torn down while something still depended on it; here stop order is asserted to be
//  exactly the reverse of start order, and a dependency cycle is rejected at build time
//  rather than deadlocking at launch.
//
//  Two things to know before editing:
//
//  - **Every suite is `.serialized`, and the recorder is per test.** A file-scope recorder
//    reset at the top of each test was shared by suites Swift Testing runs in PARALLEL, so
//    one suite erased another's events mid-run. The tests failed intermittently and in
//    varying combinations, which reads exactly like a product race and was not one. The
//    recorder now arrives through the `Service.init(context:)` the registry already threads,
//    so there is no shared state left to erase.
//  - **The ordering tests gate rather than sleep.** A fake holds its `start()` open until the
//    test releases it, so an assertion about "the stop waited for the start" is about
//    causality and not about the machine being fast enough that day.
...
actor PermissionGatedService: RecordingService {
  static var manifest: ServiceManifest {
    .minimal(
      id: "needs-fda",
      permissions: [.init(.fullDiskAccess, .required, purpose: "to read the database")]
    )
  }
  let recorder: LifecycleRecorder
  init(host: TestContext) { recorder = host.recorder }
...
struct SupervisionTests {
  ...
  /// Exhausting the policy raises an alert. It does NOT relaunch the application, which is
  /// what the reference does: `proxyServices/proxy.ts` gives up after `maxTries = 10` and
  /// calls `Server().relaunch()`, restarting everything because one tunnel failed.

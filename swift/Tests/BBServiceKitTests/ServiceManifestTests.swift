...
@Suite("Declared macOS permissions")
struct ServicePermissionTests {

  private func manifest(_ permissions: [ServicePermission]) -> ServiceManifest {
    .minimal(id: "app.example.thing", permissions: permissions)
  }

  @Test("Required permissions are derived from the manifest, recommended ones are not")
  func onlyRequiredGatesStartup() {
    // The registry refuses to start a service whose REQUIRED permission is missing. A
    // service that merely works better with Contacts has to start without them, so
    // `.recommended` and `.feature` must not reach that gate.
    let declared = manifest([
      .init(.fullDiskAccess, .required, purpose: "to read the database"),
      .init(.contacts, .recommended, purpose: "to show names"),
      .init(.notifications, .feature("Alerts"), purpose: "to post alerts"),
    ])
    #expect(declared.permissions.count == 3)
    #expect(
      declared.permissions.filter(\.requirement.isRequired).map(\.id) == [.fullDiskAccess])
  }

  @Test("A permission declared twice is a manifest problem")
  func duplicateIsRefused() {
    let problems = ManifestValidator.validate(
      manifest([
        .init(.contacts, .required, purpose: "one"),
        .init(.contacts, .recommended, purpose: "two"),
      ]),
      secretKeys: []
    )
    #expect(
      problems.contains(
        .duplicatePermission(
          service: ServiceIdentifier("app.example.thing"), permission: .contacts)))
  }

  @Test("A permission with no stated reason is a manifest problem")
  func purposeIsRequired() {
    // The reason is the whole of what a user has to judge a permission by. A declaration
    // without one puts a bare "Contacts" on the approval screen and asks for a decision.
    let problems = ManifestValidator.validate(
      manifest([.init(.contacts, .required, purpose: "   ")]),
      secretKeys: []
    )
    #expect(
      problems.contains(
        .permissionWithoutPurpose(
          service: ServiceIdentifier("app.example.thing"), permission: .contacts)))
  }

  @Test("An unknown permission renders as its raw identifier rather than vanishing")
  func unknownPermissionStillShows() {
    // The set of permissions is open: BBSystem adds `sip-disabled` in an extension, and a
    // third-party manifest can name one this binary has never heard of. Dropping it from
    // the list would hide exactly the declaration the list exists to show.
    let permission = ServicePermission(
      PermissionID("screen-recording"), .required, purpose: "to capture the screen"
    )
    let sentence = permission.userFacingDescription(namingPermission: { $0.rawValue })
    #expect(sentence.contains("screen-recording"))
    #expect(sentence.contains("to capture the screen"))
  }
}

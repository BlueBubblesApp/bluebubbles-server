...
struct SocketTransportTests {
  ...
  func handshakeOpens() async throws {
    ...
    // The three timeouts are the reference's explicit values, not defaults: a
...
// MARK: - The integration switch

@Suite("Socket integration switch")
struct SocketIntegrationSwitchTests {

  private func engine(
    accepting: @escaping @Sendable () async -> Bool
  ) -> (EngineIOServer, SocketServer) {
    let sockets = SocketServer()
    return (
      EngineIOServer(
  ...
  @Test("A switched-off socket closes the handshake instead of refusing it")
  func closesRatherThanErrors() async {
    // Closed, not errored, and the difference is not cosmetic: clients RETRY a connect
    // error and STOP on a silent close. The socket service stopping used to leave the
    // transport mounted and the reaper cancelled, so a client connected, heard nothing,
    // never got a ping, timed out and reconnected, forever leaving a session behind
    // each time. An error here would rebuild that loop from the other end.
    let (engine, _) = engine(accepting: { false })
    let outcome = await engine.open(
      query: ["password": "hunter2hunter2", "EIO": "4"], clientAddress: "1.2.3.4"
    )
    let sent = packets(outcome)
    #expect(sent.count == 2, "expected the handshake and a close")
    #expect(sent.first?.hasPrefix("0") == true)
    #expect(sent.last == "1", "the last packet must be a bare Engine.IO CLOSE")
  }

  @Test("A refused handshake leaves no session behind")
  func leavesNoSession() async {
    // The leak this whole gate exists to stop: a client that reconnects must not cost a
    // session per attempt.
    let (engine, _) = engine(accepting: { false })
    for _ in 0..<5 {
      _ = await engine.open(query: ["password": "hunter2hunter2"], clientAddress: nil)
    }
    #expect(await engine.sessionCount == 0)
  }

  @Test("Switched on, the same handshake establishes a session")
  func acceptsWhenEnabled() async {
    let (engine, _) = engine(accepting: { true })
    _ = await engine.open(query: ["password": "hunter2hunter2"], clientAddress: nil)
    #expect(await engine.sessionCount == 1)
  }

  @Test("The switch is read per handshake, not captured once")
  func gateIsLive() async {
    // The engine outlives every start and stop of the socket service, and a socket
    // switched off before the server started never runs `SocketService.start` at all, so
    // a flag only that method could set would say "accepting" forever.
    let enabled = SwitchFlag()
    let (engine, _) = engine(accepting: { await enabled.value })
    ...
    _ = await engine.open(query: ["password": "hunter2hunter2"], clientAddress: nil)
    #expect(await engine.sessionCount == 1, "the second handshake must have been refused")
  }
}

/// A switch somebody flips while the server is running.
private actor SwitchFlag {

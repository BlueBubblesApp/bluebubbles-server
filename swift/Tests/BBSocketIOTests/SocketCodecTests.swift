...
struct SocketIOEncodingTests {
  ...
  func nullPayload() throws {
    // `2["hello-world"]` and `2["hello-world",null]` are different arities to a client:
    // the handler's first parameter is `undefined` in one and `null` in the other. The
    // vector replay does cover this (the two parse to different JSON arrays) but the
    // case is worth naming, because "drop the argument when it is nil" is the obvious
    // encoder simplification and it silently changes what every listener receives.
...
struct EngineIOTests {
  ...
  @Test("The handshake carries the reference's timeouts")

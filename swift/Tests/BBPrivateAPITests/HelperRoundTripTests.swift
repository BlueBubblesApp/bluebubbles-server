...
/// Waits for the helper to connect, and FAILS if it does not.
///
/// The loop this replaces recorded nothing on expiry, so a helper that never arrived ran the
/// body anyway and every request in it timed out — reported as whatever assertion happened to
/// come first, never as "nothing was connected".
private func waitForHelper(
  on transport: SocketTransport,
  ...
  await cleanUp()
  Issue.record("the helper never connected", sourceLocation: sourceLocation)
...
private func withHelperAndServer(
  ...
  try await waitForHelper(on: transport) {
    helper.stop()
        ...
        for request in requests {
          group.addTask {
            do {
              ...
              // Named, not bucketed. Collapsing every other error into one token is what
              // made a dropped reply indistinguishable from a correlation failure.

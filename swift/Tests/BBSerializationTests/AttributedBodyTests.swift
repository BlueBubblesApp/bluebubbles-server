...
struct PropertyListWireTests {
  ...
  ///
  /// This asserted `first["amc"] == 2` for a while, under that very sentence: the comment
  /// named the long-form contract and the expectation pinned the short Apple key, so the
  /// renaming the reference performs was missing and nothing said so. The shape itself is
  /// `MessageSummaryInfoWireTests`; what is left here is the array wrapper.
  ...
  func decodesToArrayWrappedObject() throws {
    ...
    // An unmapped key is passed through as Apple wrote it.
    #expect(first["ust"] == .int64(1))
    // A mapped one arrives under the name the client reads, and only that one.
    #expect(first["associatedMessageContent"] == .int64(2))
    #expect(first["amc"] == nil)
  }

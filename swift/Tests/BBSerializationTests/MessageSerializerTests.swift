...
@Suite("Notification size ceiling")
struct NotificationSizeCapTests {

  private let profile = SchemaProfile.baseline(messageColumns: modernColumns)

  private func context(participants: Int) -> MessageSerializer.Context {
    let chat = Rows.chat(style: 43, displayName: "Group")
    return MessageSerializer.Context(
      chats: [chat],
      participantsByChatGUID: [
        chat.guid: (0..<participants).map {
          Rows.handle(rowID: Int64(100 + $0), id: "+1202555\(String(format: "%04d", $0))")
        }
      ]
    )
  }

  private var capping: MessageSerializerConfig {
    // Node's notification path inherits `loadChatParticipants: true` from
    // DEFAULT_MESSAGE_CONFIG and adds `enforceMaxSize: true`, which is the only
    // combination where the cap can do anything: with participants already empty there is
    // nothing left to drop.
    MessageSerializerConfig(
      loadChatParticipants: true, includeChats: true, enforceMaxSize: true
    )
  }

  private func participantCount(_ payload: JSONValue) -> Int {
    payload["chats"]?[0]?["participants"]?.arrayValue?.count ?? 0
  }

  /// The common case. A short message with a handful of participants is under budget, so
  /// nothing is dropped — this is the assertion that stops the cap from being "always on",
  /// which would silently strip participants from every notification.
  @Test("A small payload keeps its participants")
  func smallPayloadIsUntouched() {
    let serializer = MessageSerializer(profile: profile)
    let payload = serializer.serialize(
      Rows.message(Rows.messageColumns(text: "hey")),
      context: context(participants: 3),
      config: capping,
      isForNotification: true
    )

    #expect(participantCount(payload) == 3)
  }

  /// The regression. The message TEXT is what blows the budget here; the chats array is the
  /// same size it is above. Measuring chats alone finds nothing wrong with this payload.
  @Test("A long message trips the cap even though its chats array is small")
  func longTextTripsTheCap() {
    let serializer = MessageSerializer(profile: profile)
    let long = String(repeating: "a", count: 5_000)

    let payload = serializer.serialize(
      Rows.message(Rows.messageColumns(text: long)),
      context: context(participants: 3),
      config: capping,
      isForNotification: true
    )

    #expect(participantCount(payload) == 0)
    // The chat itself survives — a notification still has to say which conversation it
    // belongs to. Only the participants go.
    #expect(payload["chats"]?.arrayValue?.count == 1)
  }

  /// And the other direction: many participants on a short message trip it too, which is
  /// the case the old measurement did catch.
  @Test("A large participant list trips the cap")
  func manyParticipantsTripTheCap() {
    let serializer = MessageSerializer(profile: profile)
    let payload = serializer.serialize(
      Rows.message(Rows.messageColumns(text: "hey")),
      context: context(participants: 60),
      config: capping,
      isForNotification: true
    )
  ...
  /// The cap is opt-in. The socket payload has no size limit and must never be trimmed —
  /// a desktop client reading a group's participants off the socket would lose them.
  @Test("Without enforceMaxSize nothing is dropped, however large")
  func fullProjectionIsNeverCapped() {
    let serializer = MessageSerializer(profile: profile)
    let payload = serializer.serialize(
      Rows.message(Rows.messageColumns(text: String(repeating: "a", count: 5_000))),
      context: context(participants: 60),
      config: MessageSerializerConfig(loadChatParticipants: true, includeChats: true)
    )

    #expect(participantCount(payload) == 60)
  }
}

...
struct LaunchOptionsTests {
  ...
  /// The launcher and the app have to agree on this string, and nothing else checks that
  /// they do: the launcher writes it into `NSWorkspace.OpenConfiguration.arguments` and this
  /// parser reads it back. A rename on one side alone would make every login look like a
  /// person opening the app, silently, and `start_minimized` and `start_delay` would both
  /// stop working with nothing to show for it.
  @Test("the launcher's automatic-launch argument is the one this parses")
  func automaticLaunch() {
    let options = LaunchOptions.parse(["app", LauncherContract.automaticLaunchArgument])
    #expect(options.wasLaunchedAutomatically)
    #expect(!LaunchOptions.parse(["app"]).wasLaunchedAutomatically)
    // And it must not leak into the server's configuration: it says who opened the window,
    // which nothing below the app has an opinion about.
    #expect(options.compositionOptions.overrides.isEmpty)
    #expect(!options.compositionOptions.headless)
  }

  @Test("a login item carries its own arguments too")
  func automaticLaunchAlongsideOverrides() {
    // The launcher passes exactly one argument today, but a login item is configured ONLY by
    // its arguments, so the two must compose rather than one swallowing the other.
    let options = LaunchOptions.parse([
      "app", LauncherContract.automaticLaunchArgument, "--headless",
      "--set", "log_level=debug",
    ])
    #expect(options.wasLaunchedAutomatically)
    #expect(options.isHeadless)
    #expect(options.overrides == ["log_level": "debug"])
  }

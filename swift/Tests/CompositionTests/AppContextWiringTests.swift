...
struct AppContextWiringTests {
  ...
  /// The only conforming type is the app's `SparkleUpdater`, supplied by `AppModel.start`
  /// and only on a build that carries a signing key. The server composed on its own, which
  /// is this fixture and every headless install, has nothing to relaunch and stays nil, so
  /// `POST /server/update/install` refuses there rather than pretending.
  @Test("The server on its own wires no update installer")

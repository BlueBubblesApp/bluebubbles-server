...
//
//  This suite is the point of `HandlerCapabilities`. Every handler group used to take the
//  whole `AppContext`, so exercising one meant opening two databases, building a codec
//  negotiator, a token auth service, a service registry and a Private API client — which is
//  why none of them had a test of their own. Each group now takes a composition of small
//  protocols, and the fakes below are what that buys: a struct with one or two members.
//
//  The second thing it guards is the handler IDs. `buildRouter` refuses to mount a route
//  whose handler is unregistered, but nothing exercised that in tests — so a mistyped ID was
//  caught at server start-up and nowhere else. Diffing the registered IDs against the route
//  table's is the check that was missing.
...
struct HandlerCapabilityTests {
  ...
  /// routes must refuse with a clear 503 rather than fail obscurely. `MessageDataProviding`
  /// used to sit here vending a raw repository — it was deleted once every handler went
  /// through the interfaces layer, which is the door being removed rather than left unused.
  ...
  func schedulingNeedsOnlyItsOwnInterface() throws {
    ...
    for id in ["schedule.list", "schedule.find", "schedule.create", "schedule.update"] {
      #expect(registry.handler(for: HandlerID(id)) != nil, "\(id) was not registered")
    }
  }
  ...
  func othersAreEquallyNarrow() async throws {
    ...
    #expect(registry.handler(for: "security.listBlocked") != nil)
    #expect(registry.handler(for: "message.hydrate") != nil)
    #expect(registry.handler(for: "ui.index") != nil)
  }

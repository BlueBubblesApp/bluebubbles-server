...
struct HandlerCapabilityTests {
  ...
  /// Two capabilities, not one, and the second is deliberate. `auth/token` checks a
  /// `client_secret` that lives in the request BODY, which the auth chain never sees, so the
  /// middleware cannot record a failed attempt on this route and the handler has to. Without
  /// the access controller here, guessing a client secret was neither counted nor blocked.
  private struct AuthHost: TokenAuthProviding, AccessControlProviding {
  ...
  func othersAreEquallyNarrow() async throws {
    ...
    AuthHandlers.register(
      into: &registry,
      context: AuthHost(tokenAuth: TokenAuthService(), accessControl: .init())
    )

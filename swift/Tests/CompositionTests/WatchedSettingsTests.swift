...
struct WatchedSettingsTests {
  ...
  static let readPerUseOrUIOnly: Set<String> = [
    ...
    "update_feed_url", "check_for_updates", "auto_install_updates", "auto_install_hour",
    "receive_beta_updates", "landing_page_path",
    ...
    // Read by the handler on every `POST facetime/call`, so turning it off refuses the very
    // next call. Deliberately NOT watched: nothing about the helper or its injection changes,
    // so restarting the Private API service (which terminates Messages and FaceTime) would
    // be a violent way to apply a permission check.
    "facetime_outgoing_calls",
    ...
    // Bookkeeping, never user-set. `last_run_version` is the app's own note to itself.
    "last_run_version",
    "last_fcm_restart", "legacy_config_imported",
    // Migration bookkeeping. Read once BEFORE the server is composed: the whole point is
    // that they decide whether it may be composed at all, so there is no running service
    // for one of them to restart, and watching them would be watching for a change that
    // can only happen while nothing is listening. See `MigrationStateStore`.
    "migration_settings_state", "migration_push_state", "migration_certificates_state",
    "migration_plaintext_secrets_state",
    // TLS provenance. WRITTEN by the HTTP service and read back by it on the next start:
    // there is nothing for a service to react to, because the only thing that changes them
    // is a certificate being installed, which already restarts the listener.

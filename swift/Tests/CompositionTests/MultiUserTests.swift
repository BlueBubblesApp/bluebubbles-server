...
//
//  Scope: only the per-USER half lives here. `SocketLocation`'s own properties — that the
//  path sits inside Messages' container, and that it fits in `sun_path` — are asserted in
//  `Tests/HelperTests/SocketLocationTests.swift`, which is where the reasoning for them is
//  written down. They were duplicated here verbatim, comments included, and both copies
//  called the same statics; re-adding them buys nothing.

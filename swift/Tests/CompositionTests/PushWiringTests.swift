...
struct PushWiringTests {
  ...
  /// An empty directory standing in for Application Support.
  ///
  /// These two tests used to assert an IMPLICATION: "if this machine has no legacy
  /// credentials, then…", because the real path was baked in and a developer's own Mac may
  /// genuinely have an Electron install. That made the assertion different depending on who
  /// ran the suite, and on the machine where it mattered it asserted nothing at all. Now the
  /// directory is injected, so both can state the fact directly.
  private func emptyDirectory() -> URL {
    let url = URL(fileURLWithPath: NSTemporaryDirectory())
      .appendingPathComponent("bb-support-\(UUID().uuidString)", isDirectory: true)
  ...
  func gateDeclinesWhenUnconfigured() async {
    let base = emptyDirectory()
    defer { try? FileManager.default.removeItem(at: base) }
    #expect(PushCredentialMigration.hasLegacyCredentials(in: base) == false)
  }

  @Test("Credentials waiting in Application Support are seen")
  func gateAccountsForUnmigratedCredentials() throws {
    // The gate has to see plaintext waiting on disk, not just the Keychain: declining on
    // exactly the installs that have credentials to migrate would leave the plaintext
    // readable forever, since the migration is what deletes it.
    let base = emptyDirectory()
    defer { try? FileManager.default.removeItem(at: base) }
    #expect(PushCredentialMigration.hasLegacyCredentials(in: base) == false)
    ...
    try "{}".write(
      to: firebase.appendingPathComponent("server.json"), atomically: true, encoding: .utf8)
    #expect(PushCredentialMigration.hasLegacyCredentials(in: base) == true)
  }

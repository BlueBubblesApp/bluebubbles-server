...
struct FileSinkFollowTests {
  ...
  @Test("Clearing empties the file, its rotations and the tail, and writing carries on")
  func clearing() async throws {
    ...
    let (_, updates) = sink.follow(tail: 10)

    sink.clear()

    #expect(sink.tail(lines: 10) == [""], "the file holds nothing but its final newline")
    #expect(!FileManager.default.fileExists(atPath: rotated.path))
    #expect(sink.follow(tail: 10).tail.isEmpty)

    // The follower opened before the clear is still fed.
    sink.write("two\n")
    #expect(try await Self.next(1, from: updates).map(\.text) == ["two"])
    #expect(sink.tail(lines: 10) == ["two", ""])
  }

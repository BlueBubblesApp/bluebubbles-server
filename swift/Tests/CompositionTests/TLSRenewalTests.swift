...
struct TLSRenewalTests {
  ...
  /// The SAN is the whole point of a certificate this server generates, and the renewal
  /// used to drop it.
  ///
  /// `generate` passes the published address into `hostnames(publishedAddress:)`; the
  /// renewal called `hostnames()` with no argument, so a renewed certificate covered
  /// `localhost` and the Mac's `.local` name and nothing else. On the one deployment this
  /// feature exists for (dynamic DNS, where the server terminates TLS itself rather than a
  /// tunnel doing it) the certificate stopped matching the name clients dial at the moment
  /// it renewed, and nothing said so: renewal logged success and the server bound TLS.
  @Test("A renewed certificate still covers the published address")
  func renewalKeepsThePublishedAddress() async throws {
    let secrets = InMemorySecretStore()
    let (scope, store) = try await makeScope(secrets)
    let disk = diskStore()
    defer { disk.clear() }
    ...
    try await store.set(Settings.serverAddress, to: "https://bb.example.com")
    ...
    let names = try CertificateAuthority.subjectAlternativeNames(
      certificatePEM: renewed.certificatePEM)
    #expect(
      names.contains("bb.example.com"),
      "a renewed certificate must still match the address clients dial; got \(names)")
    // And the local names are not lost in the process.

...
struct ServiceGraphTests {
  ...
  func dependenciesAreDeclared() {
    // The socket shares the HTTP listener, and the edge runs socket -> HTTP.
    //
    // It ran the other way, on the grounds that the websocket upgrade is a channel-level
    // decision made before the router sees a request, so the transport must exist by the
    // time the port is bound. The first half is true; the conclusion did not follow.
    // `HTTPService.start` CONSTRUCTS the `SocketIOTransport` itself, from the engine the
    // composition root built, and hands it to the listener, so the upgrade channel does
    // not need `SocketService` to have started. What that service adds is the event sink
    // and the session reaper, neither of which is needed to bind a port.
    //
    // Pointed this way, the edge says the thing that is actually true: the socket has no
    // port of its own and is unreachable without the listener. The registry then does the
    // rest: switching the HTTP API off takes the socket down with it, and switching the
    // SOCKET off leaves the API alone. Pointed the old way, turning the socket off would
    // have taken the whole REST API and every tunnel with it.
    #expect(SocketService.dependencies.contains(BuiltInManifests.ID.http))
  ...
  /// This replaces the reference's proxy recovery path, which retries ten times
  /// (`proxyServices/proxy.ts`, `maxTries = 10`) and then calls `Server().relaunch()`,
  /// restarting the entire application because one tunnel failed.

...
struct HTTPEdgeTests {
  ...
  /// The ORIGIN is wide open, deliberately, and recorded as residual risk: locking it down
  /// would break browser clients that cannot be enumerated. Pinned so that "fixing" it is a
  /// decision rather than an accident, which is exactly what the comment on `allowedOrigin`
  /// warns about.
  ///
  /// The HEADERS are not, and used to be. `Access-Control-Allow-Headers: *` looked like the
  /// same decision and was a different one: it tells a browser that a cross-origin request
  /// may carry any header, including `X-Forwarded-For`, which this server believes from a
  /// trusted proxy and loopback is one by default. A page in the operator's browser could
  /// therefore have a failed login attributed to an address of its choosing. A response now
  /// carries no allow-headers at all, and a preflight is answered with what it asked for
  /// minus the forwarding headers; see `CORSHeaderReflectionTests`.
  @Test("Every response carries the permissive origin header and no header wildcard")
  func corsOnNormalResponses() async throws {
    ...
    try await withServer(registry: registry, groups: [group(routes)]) { port in
      ...
      #expect(
        reply.headers["access-control-allow-headers"] == nil,
        "a wildcard here lets a browser assert a header this server believes")
    }
  }

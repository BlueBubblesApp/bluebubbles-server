...
struct ConnectionMethodToolTests {
  ...
  @Test("Every connection method's program says which versions it can drive")
  func toolsDeclareACompatibleRange() {
    // Read from the manifests rather than from `BuiltInTools.all`, so a connection method
    // that brings its own descriptor is covered too. Without a range, discovery is off for
    // that method and the only symptom is a duplicate download nobody notices.
    for manifest in connectionMethods {
      for tool in manifest.tools {
        #expect(
          tool.acceptsPreexistingCopies,
          "\(manifest.name)'s '\(tool.id)' would never use a copy already on this Mac")
      }
    }
  }

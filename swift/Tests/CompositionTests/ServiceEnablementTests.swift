...
struct ServiceEnablementTests {
  ...
  func catalogAgreesWithTheServer() {
    ...
    // And so is the socket. It was always-on on the grounds that it is how a connected
    // client hears anything, which is true and is not a reason: a REST-only server is a
    // real deployment, and the API it depends on has always been switchable. What the
    // socket needs is an ORDER, which it declares; see `dependsOnTheListenerItShares`.
    #expect(!BuiltInManifests.alwaysOn.contains(BuiltInManifests.ID.socket))
  }

  @Test("The socket depends on the listener it shares, not the other way round")
  func dependsOnTheListenerItShares() {
    // The direction is what keeps the two switches independent in the one direction that
    // makes sense. Reversed, turning the socket off would take the REST API and every
    // tunnel with it, because the registry stops a service's dependents with it.
    #expect(BuiltInManifests.socket.dependencies.contains(BuiltInManifests.ID.http))
    #expect(!BuiltInManifests.http.dependencies.contains(BuiltInManifests.ID.socket))
  }

  @Test("Switching off the socket leaves the HTTP API enabled")
  func socketCanBeDisabledAlone() {
    let disabled = ServiceEnablement.disabledIdentifiers(in: BuiltInManifests.ID.socket.rawValue)
    #expect(!ServiceEnablement.isEnabled(BuiltInManifests.ID.socket, disabled: disabled))
    // The half that matters: a REST-only server is the point of this switch.
    #expect(ServiceEnablement.isEnabled(BuiltInManifests.ID.http, disabled: disabled))
  }

...
//  reachable: a scope built in a protocol extension resolves `Self.manifest` STATICALLY,
//  against the type that declared the conformance (the base) rather than dynamically to the
//  subclass. Anything reading a core setting through the scope traps.

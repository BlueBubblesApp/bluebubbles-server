//  SettingsStoreTests
//  The store as it behaves against a real database: typed reads, layering, batching, and the
//  boundary that keeps secrets out of `app.db`.
//
//  The type coercion is the reason this file is as detailed as it is. The Electron server
//  inferred a setting's type from the shape of its value, so "1" came back as a Bool, which
//  made a `db_poll_interval` of 1 stop being a number, and is why `start_delay` is defaulted
//  to the STRING "0.0" there with a comment saying a numeric default would parse as a
//  boolean. The nastiest case is a password of "1", which coerces to `true` and then fails
//  its comparison against the client's string for a reason nothing logs. Each of those is
//  pinned here through a live store rather than through the descriptor: a `Setting<Int>`
//  handing back the `Int` it was constructed with proves nothing about what a round trip to
//  SQLite does. → `SettingTests.swift` for the descriptor half and why it is thin.
//
//  Layering is CLI over config file over stored value over declared default, and neither of
//  the two top layers is written to the database; `--set` is for a run, not a change.
//
//  `SecureString`'s comparisons are constant-time and are asserted not to short-circuit on
//  length, since the obvious early return leaks the password's length to a timing attacker.

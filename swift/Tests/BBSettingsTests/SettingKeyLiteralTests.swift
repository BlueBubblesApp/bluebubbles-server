...
struct SettingKeyLiteralTests {
  ...
  func noLiteralKeys() throws {
    ...
    for directory in ["Sources", "Helper"] {
      ...
      // `continue` on a root that will not resolve means this scan finds nothing and
      // passes, which is why the floor below exists rather than only the key count above.

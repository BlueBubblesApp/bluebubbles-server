//  UnreadableSecretTests
//  A Keychain that CANNOT be read is not a Keychain with nothing in it, and the store has to
//  tell the two apart.
//
//  The regression: `secret()` swallowed the read error and returned an empty `SecureString`,
//  which the auth path could not distinguish from a password that was never set. A locked
//  login keychain (or a signature the item's ACL no longer trusts, which is what an ad-hoc
//  rebuild produces) was therefore reported to every client as "Failed to retrieve password
//  from the database", pointing at the wrong subsystem entirely. Worse, the swallowed read
//  recorded "there was nothing here before", so rolling back a failed write DELETED the
//  existing secret instead of restoring it.
//
//  It survived as long as it did because every test agreed with the code:
//  `InMemorySecretStore` only ever succeeds, so there was no way to write the failing case.
//  `UnreadableSecretStore` below is that missing fake: it throws
//  `errSecInteractionNotAllowed`, the status a locked keychain returns when there is no UI to
//  prompt with, and it can be switched back to succeeding, because a Keychain that fails, is
//  fixed, and fails again has to report the second time too.
//
//  The alert half has two traps of its own. The alert centre is built AFTER the settings
//  store, so a failure during start-up (the one likeliest to matter) happens when there is
//  nowhere to report it yet and must be held and raised on attach. And `resolve` runs on the
//  authentication path, so without a per-key latch a locked Keychain raises once per request.
//
//  `RecordingAlerts.waitForRaised` waits for a COUNT rather than sleeping: the store
//  dispatches its raise into an unstructured `Task`, and a fixed sleep here passed in
//  isolation and failed under a full-suite run.
...
struct UnreadableSecretTests {
  ...
  // MARK: - Presence: what is known without reading

  /// The settings screen reads a secret only when the person asks it to, so what a secure
  /// field draws itself from until then is this — and it has to reach the same three-way
  /// verdict `readSecret` does, without materialising anything.
  ///
  /// The failure it is guarding against is the one at the top of this file wearing a
  /// different hat: a row that took "not stored" from a Keychain that had merely refused
  /// would show an empty password field, and the only control offering to do anything about
  /// that is Generate, which cannot be undone.

  @Test("A stored secret is present without being read")
  func presenceOfStored() async throws {
    let store = try await makeStore(InMemorySecretStore(seed: ["password": "hunter2"]))
    #expect(await store.presence(of: Settings.password) == .stored(.persistedStore))
  }

  @Test("A secret that was never set is absent")
  func presenceOfAbsent() async throws {
    let store = try await makeStore(InMemorySecretStore())
    #expect(await store.presence(of: Settings.password) == .absent)
  }

  /// NOT `.absent`, which is the whole point of the case existing.
  @Test("A Keychain that refuses is unreadable, not absent")
  func presenceOfUnreadable() async throws {
    let store = try await makeStore(UnreadableSecretStore(seed: ["password": "hunter2"]))
    #expect(await store.presence(of: Settings.password) == .unreadable)
  }

  /// And it joins the set the settings screen consults, so a reveal that comes back empty
  /// can be reported as a refusal rather than as an empty value.
  @Test("A refused presence check marks the key unreadable and alerts")
  func presenceFailureAlerts() async throws {
    let store = try await makeStore(UnreadableSecretStore(seed: ["password": "hunter2"]))
  ...
  /// An override is in effect whatever the Keychain holds, and answering that must not cost
  /// a Keychain call — a password on the command line is readable from a store that is
  /// refusing everything.
  @Test("A command-line override is present without touching the Keychain")
  func presenceOfOverride() async throws {
    let database = try AppDatabase.inMemory(contributors: [SettingsSchema.self])
    let store = try await SettingsStore(
      database: database,
      secrets: UnreadableSecretStore(),
      commandLineValues: [Settings.password.key: "from-the-command-line"]
    )
    #expect(await store.presence(of: Settings.password) == .stored(.commandLine))
    #expect(await store.unreadableSecretKeys.isEmpty)
  }

  /// A secret genuinely set to the empty string is STORED, not absent. The two are
  /// different states — `remove` exists precisely because "configured, to nothing" and
  /// "never configured" are not the same row — so a reveal that comes back blank is not on
  /// its own evidence of anything going wrong.
  @Test("A secret set to the empty string is stored, not absent")
  func presenceOfEmptyValue() async throws {
    let store = try await makeStore(InMemorySecretStore(seed: ["password": ""]))
    #expect(await store.presence(of: Settings.password) == .stored(.persistedStore))
  }

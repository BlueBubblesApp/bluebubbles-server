...
struct SettingApplicationTests {
  ...
  func legacySettingsAreReservedButHidden() {
    ...
    // Six, not seven: `auto_install_updates` left this list when Sparkle started reading
    // it. A legacy row that gains a reader is promoted, not duplicated.
    #expect(Settings.Legacy.all.count == 6)

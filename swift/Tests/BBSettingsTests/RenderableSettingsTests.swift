...
struct RenderableSettingsTests {
  ...
  func presentations() {
    for setting in Settings.renderable {
      ...
      // The section is a `SettingSection` case, so "has no section" cannot compile; its
      // title and summary are checked by `SettingSectionTests`.
  ...
  func coverage() {
    ...
    #expect(
      Settings.renderable.count == 38 + Features.all.count,
      """

...
struct LegacyMigrationTests {
  ...
  @Test("The Electron auto-install answer carries across to the live setting")
  func autoInstallUpdatesIsAdopted() async throws {
    // `auto_install_updates` was a reserved legacy row until Sparkle read it. An upgraded
    // install that had opted in keeps installing without a person at the Mac.
    let url = try makeLegacyDatabase(["auto_install_updates": "1"])
    defer { try? FileManager.default.removeItem(at: url) }
    let secrets = InMemorySecretStore()
    let store = try await makeStore(secrets)
    ...
    #expect(await store.get(Settings.autoInstallUpdates) == true)
  }

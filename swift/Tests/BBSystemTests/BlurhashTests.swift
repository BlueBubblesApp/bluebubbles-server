...
struct BlurhashTests {
  ...
  // MARK: - The single-pass transform
  //
  // The transform used to make one pass over the pixels PER COMPONENT -- up to 81 of them --
  // and each pass called `cos` twice and `pow` three times for every pixel, so the same
  // pixel's colour conversion was recomputed 81 times. It is one pass now, with two cosine
  // tables and a 256-entry sRGB lookup.
  //
  // The arithmetic is unchanged: the same operands in the same order, so the same IEEE-754
  // results. These hashes were recorded from the OLD implementation before it was replaced,
  // which is what makes them a check on that claim rather than a restatement of it.

  /// A deterministic image, so a recorded hash means something.
  private static func noise(width: Int, height: Int, seed: Int) -> [UInt8] {
    var out = [UInt8](repeating: 0, count: width * height * 4)
    var state = UInt64(seed &+ 1)
    for i in 0..<(width * height) {
      state = state &* 6_364_136_223_846_793_005 &+ 1_442_695_040_888_963_407
      out[i * 4] = UInt8((state >> 33) & 0xFF)
      out[i * 4 + 1] = UInt8((state >> 41) & 0xFF)
      out[i * 4 + 2] = UInt8((state >> 49) & 0xFF)
      out[i * 4 + 3] = 255
    }
    return out
  }
  ...
  @Test(
    "The one-pass transform produces exactly what the per-component passes produced",
    arguments: [
      (64, 64, 4, 3, "L2HoB?=hKa?q=tV~rftKxJu.I.-:"),
      (64, 48, 3, 3, "K1HoB=:Kvg}_+CJnVnO?w8"),
      (128, 96, 5, 4, "V0HV9x^lMc{GIM~kDuN|M2BUGjEVEUI%EL~EvocXPl0g"),
      (32, 32, 1, 1, "00HLo7"),
    ])
  func matchesTheRecordedHashes(
    _ width: Int, _ height: Int, _ componentsX: Int, _ componentsY: Int, _ expected: String
  ) throws {
    let pixels = Self.noise(width: width, height: height, seed: width &* height &+ componentsX)
    let hash = try Blurhash.encode(
  ...
  /// The lookup table has to be the function, not an approximation of it. There are only
  /// 256 possible inputs, so this is exhaustive rather than a sample.
  @Test("The sRGB table is exact for every possible input")
  func srgbTableIsExact() {
    for value in 0...255 {
      #expect(Blurhash.linearFromSRGB[value] == Blurhash.sRGBToLinear(value), "\(value)")
    }
  }
}

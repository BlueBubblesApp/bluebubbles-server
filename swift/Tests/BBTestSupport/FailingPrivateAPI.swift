...
public struct FailingPrivateAPI: PrivateAPI {
  ...
  /// The two FaceTime call operations the cleanup sweep drives, for the same reason.
  ///
  /// `FaceTimeCleanup` has to be exercised with a helper that REPORTS a call and then either
  /// leaves it or refuses, which is the difference between "the Mac left the call" and "the
  /// Mac says it left the call". Default nil keeps the refuse-everything behaviour.
  public var onActiveCalls: (@Sendable () async throws -> [FaceTimeCall])?
  public var onLeaveCall: (@Sendable (String) async throws -> Void)?

  /// The five READ operations the fixture replay needs to answer rather than refuse.
  ///
  /// Same trick as the three above, and the same reason: the alternative is a second
  /// conformance to a sixty-eight-member protocol. These five are what
  /// `ReplayServer` sets, because they are the helper-backed routes the recorded corpus
  /// captured a 200 for — `icloud/account`, `icloud/contact`, both
  /// `handle/availability/*`, and `chat/:guid/share-contact-status`. Without them those
  /// responses were never compared to anything and two of them had drifted.
  ///
  /// Left nil for the helper routes the reference itself answered with a 500: making those
  /// succeed here would invent a difference rather than find one.
  public var onAccountInfo: (@Sendable () async throws -> AccountInfo)?
  public var onNicknameInfo: (@Sendable (String?) async throws -> NicknameInfo)?
  public var onIMessageAvailability: (@Sendable (String) async throws -> Bool)?
  public var onFaceTimeAvailability: (@Sendable (String) async throws -> Bool)?
  public var onShouldOfferNicknameSharing: (@Sendable (ChatIdentifier) async throws -> Bool)?
  ...
  public func checkIMessageAvailability(address: String) async throws -> Bool {
    try await onIMessageAvailability?(address) ?? { throw error }()
  }
  public func checkFaceTimeAvailability(address: String) async throws -> Bool {
    try await onFaceTimeAvailability?(address) ?? { throw error }()
  }
  ...
  public func leaveFaceTimeCall(callUUID: String) async throws {
    guard let onLeaveCall else { throw error }
    try await onLeaveCall(callUUID)
  }
  ...
  public func faceTimeActiveCalls() async throws -> [FaceTimeCall] {
    guard let onActiveCalls else { throw error }
    return try await onActiveCalls()
  }

...
@testable import BBPrivateAPICatalog
...
struct CapabilityCatalogTests {
  ...
  private static var headersDirectory: URL {
    URL(fileURLWithPath: #filePath)
      .deletingLastPathComponent()  // BBPrivateAPICatalogTests
  ...
  func copyIsPlainLanguage() {
    ...
    for capability in PrivateAPICapability.all {
      for (field, text) in [("title", capability.title), ("summary", capability.summary)] {
        let location = "\(capability.id).\(field)"
        #expect(!text.contains(":"), "\(location) looks like it names a selector: \(text)")
        #expect(!text.contains("()"), "\(location) names a function: \(text)")
        #expect(
          !text.contains(".framework"), "\(location) names a framework: \(text)")
        for word in text.split(whereSeparator: { !$0.isLetter && !$0.isNumber && $0 != "_" }) {
          ...
          for prefix in prefixes where token.hasPrefix(prefix) && token.count > prefix.count {
            ...
            #expect(
              !next.isUppercase,
              "\(location) names what looks like an Apple type (\(token)): \(text)")
          }
          #expect(
            !token.contains("_"), "\(location) contains an identifier (\(token)): \(text)")
        }
      }
...
struct FeatureListingTests {
  ...
  func connectedOnNewest() {
    ...
    #expect(
      !entries.contains {
        if case .heading(let t, _) = $0 { return t.contains("Upgrade") } else { return false }
      })
  }

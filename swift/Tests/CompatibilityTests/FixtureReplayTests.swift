...
struct FixtureReplayTests {
  ...
  func replayCorpus() async throws {
    ...
    // A fixture whose REQUEST threw never reaches a comparison, so without these two the
    // suite's assertions run over an empty set and pass vacuously. Proved by pointing the
    // replay at a dead port: every fixture was refused, and this test went green in 75
    // milliseconds, taking the staleness check below with it, which then approved all
    // thirty-one baseline entries for fixtures it had not replayed.
    //
    // A transport error is a HARNESS failure, not a contract finding, which is exactly why
    // it must be loud: it means this suite stopped measuring the thing it exists to measure.
    let errored = results.filter { $0.skipped == nil && $0.error != nil }
    #expect(
      errored.isEmpty,
      """
      \(errored.count) fixture(s) could not be replayed at all. The harness is broken, not \
      the contract.

      \(errored.map { "\($0.fixture): \(String(describing: $0.error))" }.joined(separator: "\n"))
      """
    )

    let compared = results.filter { $0.skipped == nil && $0.error == nil }
    // The floor, in the shape `LogRedactionPolicyTests` uses. Measured: 117 v1 fixtures, 56
    // skipped by `FixtureReplay` (44 destructive, 7 self-recorded, 2 binary, 2 lossy, 1
    // non-JSON), 61 compared. Held a little below that so recording a fixture for a
    // destructive route does not fail the build, and far enough above zero that a corpus
    // that stopped loading, a server that stopped starting, or a filter that stopped
    // matching cannot pass as agreement.
    #expect(
      compared.count >= 55,
      """
      only \(compared.count) of \(results.count) fixtures were actually compared. \
      Something upstream stopped feeding this suite; it is not measuring the contract.
      """
    )
  ...
  private static func replay() async throws -> [ReplayResult] {
    ...
    await server.seedRecordedEntities()

    let replay = FixtureReplay(
      baseURL: server.baseURL, password: server.password, mode: .shape
    )
    return await replay.run(fixtures.filter(\.isV1).map(Self.pointedAtSomethingThatExists))
  }

  /// Entities from the recording Mac, swapped for ones the synthetic database has.
  ///
  /// A recorded `GET /message/<guid>` names a message that exists on the Mac it was recorded
  /// on and nowhere else, so the replay answered 404 against the reference's 200 and the diff
  /// compared two error envelopes. The ENTITY shape — the whole reason that fixture is in the
  /// corpus — went unchecked, and six fixtures sat in the baseline saying so.
  ///
  /// The substitution is sound because the shape does not depend on which row was found; see
  /// `RecordedFixture.asking(about:)`. The identifiers below are read out of
  /// `ChatDBFixtures/chat-sonoma.db`, which is what `ReplayServer` mounts, and
  /// `substitutionsStillResolve` fails if one of them stops existing — otherwise a
  /// regenerated fixture database would quietly put these routes back to comparing 404s.
  ///
  /// Only chat.db-backed entities are here. `message/schedule/:id`, `backup/*` and
  /// `contact/:id` are app.db-backed and would need the replay to seed rows before it starts,
  /// which is a larger change and is recorded in the baseline rather than half-done.
  static let liveEntities: [String: String] = [
    "get_api_v1_message_:id-5baa61-200.json":
      "/api/v1/message/11111111-0000-0000-0000-000000000001",
    "get_api_v1_message_:id-0002fc-200.json":
      "/api/v1/message/11111111-0000-0000-0000-000000000002",
    // The seeded schedule is row 1 of a fresh `app.db`; the recording Mac's was row 4.
    // `ReplayServer.seedRecordedEntities` creates it, and the guard below fails if it stops
    // resolving.
    "get_api_v1_message_schedule_:id-5baa61-200.json": "/api/v1/message/schedule/1",
    "get_api_v1_chat_any;+;bcb9a1843dfc4b65bb47ce50afec8d32-5baa61-200.json":
      "/api/v1/chat/iMessage;+;chat000000000000000001",
    "get_api_v1_attachment_at_0_0A23AC44-5B9E-46F8-B494-B4630F309644-5baa61-200.json":
      "/api/v1/attachment/22222222-0000-0000-0000-000000000001",
  ]

  static func pointedAtSomethingThatExists(_ fixture: RecordedFixture) -> RecordedFixture {
    guard let path = liveEntities[fixture.name] else { return fixture }
    return fixture.asking(about: path)
  }

  @Test("Every substituted identifier still exists in the fixture database")
  func substitutionsStillResolve() async throws {
    // The substitutions are only sound while the rows are there. A regenerated fixture
    // database that dropped one would send these back to comparing 404 envelopes, silently,
    // which is the state this was written to get out of.
    let server = try await ReplayServer.start()
    defer { Task { await server.stop() } }
    // Seeded first: one of the substitutions names a row `seedRecordedEntities` creates.
    await server.seedRecordedEntities()

    for (name, path) in Self.liveEntities.sorted(by: { $0.key < $1.key }) {
      var components = try #require(URLComponents(string: server.baseURL + path))
      components.queryItems = [URLQueryItem(name: "password", value: server.password)]
      let url = try #require(components.url)
      let (_, response) = try await URLSession.shared.data(from: url)
      let status = (response as? HTTPURLResponse)?.statusCode ?? 0
      #expect(
        status == 200,
        "\(name): the substituted entity \(path) answered \(status); it is no longer in the fixture database"
      )
    }
  }

...
private enum Sources {
  ...
  static func scannable() throws -> [URL] {
    ...
    // A floor: a root that will not resolve `continue`s above, and a scan of nothing
    // passes while looking exactly like a tree with no real addresses in it.
    precondition(
      files.count > 200, "the test-data scan found \(files.count) files; it is not reading the tree"
    )
    return files
  }
}

...
/// they are loaded explicitly, once, and every suite below reads the answer.
enum IMCorePrivateFrameworks {

  static let areLoaded: Bool = {
    for path in [
  ...
  static let findMyIsLoaded: Bool = {
    _ = dlopen(
  ...
  static let telephonyIsLoaded: Bool = {
    _ = dlopen(
...
/// The loads themselves, asserted rather than assumed.
///
/// Every test in the suites below used to open with a bare `guard … else { return }` on its
/// framework, so a machine where the `dlopen` failed ran the whole thing to a green tick
/// having checked nothing at all. That is precisely the failure mode these suites exist to
/// catch, wearing their own clothes: a canary that cannot die is not a canary.
///
/// So the gate is now a suite-level condition, which the test runner REPORTS as skipped, and
/// each load is a test of its own. A failure rather than a skip because this package is
/// macOS-only and all three frameworks are part of macOS: there is no supported host where
/// those paths are legitimately absent, and if one appears the right answer is to find out
/// why, not to have been quietly passing all along.
@Suite("Private framework availability")
struct PrivateFrameworkAvailabilityTests {

  @Test("IMCore and its siblings load in a test host")
  func imCoreLoads() {
    #expect(
      IMCorePrivateFrameworks.areLoaded,
      "IMCore did not load, so every selector it pins went unchecked on this run")
  }

  @Test("TelephonyUtilities loads in a test host")
  func telephonyLoads() {
    #expect(
      IMCorePrivateFrameworks.telephonyIsLoaded,
      "TelephonyUtilities did not load, so no FaceTime selector was checked on this run")
  }

  @Test("The FindMy wrapper loads in a test host")
  func findMyLoads() {
    #expect(
      IMCorePrivateFrameworks.findMyIsLoaded,
      "FindMyLocateObjCWrapper did not load, so no FindMy selector was checked on this run")
  }
}

@Suite("IMCore selectors", .enabled(if: IMCorePrivateFrameworks.areLoaded))
struct IMCoreSelectorTests {
  ...
  @Test(
    "FindMyLocateSession still has the refresh and invite selectors",
    .enabled(if: IMCorePrivateFrameworks.findMyIsLoaded))
  ...
  @Test(
    "TUConversationManagerXPCClient still has the link and member selectors",
    .enabled(if: IMCorePrivateFrameworks.telephonyIsLoaded))
  ...
  @Test(
    "TUCallCenter still has the call-control selectors",
    .enabled(if: IMCorePrivateFrameworks.telephonyIsLoaded))
  ...
  @Test(
    "The FaceTime value types still expose what the bridge reads",
    .enabled(if: IMCorePrivateFrameworks.telephonyIsLoaded))
  ...
  @Test(
    "The renamed/removed FaceTime classes really are absent",
    .enabled(if: IMCorePrivateFrameworks.telephonyIsLoaded))
  ...
  func pinning() {
    ...
    guard let controller = NSClassFromString("IMPinnedConversationsController") else {
      Issue.record("IMPinnedConversationsController is missing on this macOS")
      return
    }
    let modern =
      class_getInstanceMethod(

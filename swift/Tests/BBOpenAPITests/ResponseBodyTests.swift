...
struct ResponseBodyTests {
  ...
  func fieldsAreSound() {
    for (handler, body) in ResponseBodies.byHandler {
      ...
      case .empty, .mirrors:
        ...
        #expect(body.variants.isEmpty, "\(handler.rawValue) declares fields it does not use")
      case .object, .list, .oneOf:
        #expect(!body.variants.isEmpty, "\(handler.rawValue) declares an empty response")
      }
      // Per VARIANT, not over a merged list: a route that answers with two shapes has two
      // sets of fields to get right, and merging them would let a field declared on one
      // shape vouch for the other.
      for variant in body.variants {
        #expect(
          !variant.properties.isEmpty,
          "\(handler.rawValue) declares a shape with no fields")
        #expect(
          Set(variant.properties.map(\.name)).count == variant.properties.count,
      ...
      // A route with alternatives has to say what distinguishes them, or the `oneOf` is a
      // list a client cannot choose from.
      if case .oneOf(let variants) = body.kind {
        #expect(variants.count > 1, "\(handler.rawValue) declares oneOf with a single shape")
        for variant in variants {
          #expect(
            variant.summary.count > 10,
            "\(handler.rawValue) has a shape that does not say when it applies")
        }
      }
    }
  }
  ...
  func examplesMatchTheirShape() {
    ...
    for (handler, body) in ResponseBodies.byHandler {
      // Each shape carries its own example, and each is checked against its OWN fields. A
      // route that answers two ways needs two examples: one of them describes a response
      // the other's client code would fail on.
      for variant in body.variants {
        guard let example = variant.example, !variant.properties.isEmpty else { continue }
        ...
        let declared = Set(variant.properties.map(\.name))
        ...
        for property in variant.properties where property.isRequired {
          #expect(

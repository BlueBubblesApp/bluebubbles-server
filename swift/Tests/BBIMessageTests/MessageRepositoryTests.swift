...
struct MessageRepositoryTests {
  ...
  func dateWindow() async throws {
    ...
    // INCLUSIVE at both ends, which is what the reference compares with
    // (`message.date >= :after`, `message.date <= :before`). This asserted the exclusive
    // form, and the exclusive form is why a message whose timestamp is exactly the boundary
    // was dropped: the incremental sync passes its last sync time as `after`, so a message
    // landing on that millisecond was never sent and never asked for again.
    //
    // The fixture seeds messages at +0, +30, +60, +90, +120 and +300. A window of [60, 300]
    // therefore holds four, where the exclusive reading held two.
    let rows = try await fixture.repository.messages(
      .init(
        after: base.addingTimeInterval(60), before: base.addingTimeInterval(300)
      ))
    #expect(Set(rows.map(\.guid)) == ["MSG-0002", "MSG-0003", "MSG-0005", "MSG-0006"])

    // And the boundary itself, stated on its own: a message exactly at `after` is in.
    let boundary = try await fixture.repository.messages(
      .init(after: base.addingTimeInterval(300)))
    #expect(Set(boundary.map(\.guid)) == ["MSG-0005"])
  }
  ...
  func updatedCount() async throws {
    ...
    #expect(
      try await fixture.repository.updatedMessageCount(
        MessageRepository.MessageQuery(after: base)) == 6)
    // After +240: only MSG-0005 (delivered at +360) qualifies.
    #expect(
      try await fixture.repository.updatedMessageCount(
        MessageRepository.MessageQuery(after: base.addingTimeInterval(240))
      ) == 1)
    #expect(
      try await fixture.repository.updatedMessageCount(
        MessageRepository.MessageQuery(after: base.addingTimeInterval(3600))
      ) == 0)
  }

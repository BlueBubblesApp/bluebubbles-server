...
struct ServiceAccountParsingTests {
  ...
  /// a Realtime URL is present, which is how the reference decides too.
...
struct CredentialStorageTests {
  ...
  /// A stored document that has stopped parsing is not "no credential". Reading it as nil
  /// made the status screen offer setup for what was a damaged key.
  @Test("A stored key that no longer parses is an error, not an absent credential")
  func damagedStoredKeyThrows() async throws {
    let secrets = InMemorySecretStore()
    try secrets.set(PushCredentialStore.serviceAccountKey, value: "{ not a service account")
    let store = PushCredentialStore(secrets: secrets)
    await #expect(throws: PushConfigurationError.self) {
      _ = try await store.serviceAccount()
    }
    // The gate still answers "not configured": a damaged key must not start the service.
    #expect(await !store.isConfigured())
  }
...
struct FCMRequestTests {
  ...
  /// The legacy library took a TTL in milliseconds: the reference passes

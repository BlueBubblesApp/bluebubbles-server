...
struct OAuthConfigurationTests {
  ...
  func authorizationURLShape() throws {
    let configuration = OAuthConfiguration()
    let url = configuration.authorizationURL(for: .firebase, nonce: "testnonce")
    ...
    // The purpose AND a nonce. `state` used to be the bare purpose: a constant, set and
    // never verified, which left the callback accepting a token from any local origin.
    #expect(items["state"] == "firebase.testnonce")
...
struct OAuthCallbackTests {
  ...
  /// The check the callback did not have.
  ///
  /// With `state` a constant and unverified, the listener accepted any body containing an
  /// `access_token`, for the five minutes it is up. The landing page posts with no content
  /// type, so a cross-origin `fetch` is a CORS-simple request needing no preflight: any page
  /// open in the operator's browser could inject a token, and Firebase provisioning would
  /// then run against the ATTACKER's project, routing the operator's notification payloads
  /// through infrastructure they do not control.
  @Test("A nonce is unguessable, fresh per flow, and compared in constant time")
  func nonceShape() {
    let first = OAuthConfiguration.makeNonce()
    let second = OAuthConfiguration.makeNonce()
    #expect(first != second, "a nonce reused across flows is not a nonce")
    #expect(first.count == 32, "128 bits, hex encoded")

    #expect(OAuthCallbackParsing.constantTimeEquals(first, first))
    #expect(!OAuthCallbackParsing.constantTimeEquals(first, second))
    #expect(!OAuthCallbackParsing.constantTimeEquals(first, first + "x"))
  }

  @Test("The state and the token are both read out of the posted fragment")
  func fragmentParsing() {
    let fragment = "#access_token=ya29.tok&state=firebase.abc123&token_type=Bearer"
    #expect(
      OAuthCallbackParsing.value(named: "state", inFragment: fragment)
        == "firebase.abc123")
    #expect(OAuthCallbackParsing.accessToken(inFragment: fragment) == "ya29.tok")
    // A body carrying no state cannot be this flow's answer.
    #expect(
      OAuthCallbackParsing.value(named: "state", inFragment: "#access_token=ya29.tok")
        == nil)
  }
}

...
//  problems, and each needs a lock and a flag to solve them:
//
...
//
//  One type, so the lock-and-flag is written once and cannot drift between call sites. Built on `OSAllocatedUnfairLock`, which is `Sendable` on its own; no

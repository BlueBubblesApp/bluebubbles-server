...
BOOL BBAllocateAndInitialize(Class cls,
    ...
    if (cls == Nil) {
        if (error) { *error = BBErrorWithReason(8, @"Class is nil"); }
        return NO;
    }
    if (![cls respondsToSelector:@selector(alloc)]) {
        if (error) {
            *error = BBErrorWithReason(
                9, [NSString stringWithFormat:@"%@ cannot allocate", NSStringFromClass(cls)]);
        }
        return NO;
    }

    // ARC owns this one for the duration of the function; see the header for why the
    // allocation never leaves it.
    id allocated = [cls alloc];
    if (allocated == nil) {
        if (error) {
            *error = BBErrorWithReason(
                9, [NSString stringWithFormat:@"%@ allocated nil", NSStringFromClass(cls)]);
        }
        return NO;
    }

    // The count `-init…` consumes. ARC's own release on the way out is NOT it: that one
    // balances `+alloc`, and spending the same count twice is what freed a live object
    // under the caller.
    CFTypeRef consumedByInit = CFRetain((__bridge CFTypeRef)allocated);

    id initialized = nil;
    if (!BBInvoke(allocated, selector, arguments, &initialized, error)) {
        // The initialiser never ran, so nothing consumed it.
        CFRelease(consumedByInit);
        return NO;
    }

    if (initialized == nil) {
        // A failed initialiser has already released the receiver, which spent the extra
        // count; ARC still holds its own. Reported rather than returned, because a caller
        // that gets nil here has no object and no reason.
        if (error) {
            *error = BBErrorWithReason(
                10, [NSString stringWithFormat:@"%@ returned nil", NSStringFromSelector(selector)]);
        }
        return NO;
    }

    // `initialized` arrives at +1 by the initialiser's own convention, and `BBInvoke`
    // retained it again for its strong local. One release hands the extra count back; what
    // reaches the caller is a single ordinary +1.
    CFRelease((__bridge CFTypeRef)initialized);

    if (outResult) { *outResult = initialized; }
    return YES;
}

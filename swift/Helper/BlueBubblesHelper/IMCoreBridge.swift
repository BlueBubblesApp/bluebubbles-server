...
public final class IMCoreBridge: PrivateAPI {
  ...
  ///
  /// **The failure names IMCore, not ChatKit, and that is measured rather than assumed.**
  /// This used to report "ChatKit does not know a conversation with GUID …", which sent a
  /// day of investigation into ChatKit's conversation tracking. Disassembled on 26.5.2:
  ///
  ///     -[CKConversationList conversationForExistingChatWithGUID:]
  ///       nil if the GUID is nil or empty
  ///       else [[IMChatRegistry sharedRegistry] existingChatWithGUID:] -> _conversationForChat:
  ///     -[CKConversationList _conversationForChat:]
  ///       nil ONLY when the chat argument is nil (cbz x19); otherwise it returns the
  ///       tracked conversation or begins tracking one
  ///
  /// So a nil here means IMCore found no chat, exactly as `IMChatRegistry.requireChat`
  /// would report on the text path. ChatKit is never the thing that is missing, and saying
  /// it is points the reader at the wrong framework.
  func requireConversation(_ chat: ChatIdentifier) throws -> CKConversation {
    guard let conversation = try CKConversationList.conversation(guid: chat.rawValue) else {
      // The same error the text path raises, from the same helper, because it is the same
      // miss: a mangled GUID explains itself there too.
      throw PrivateAPIErrorBridge.noSuchChat(chat.rawValue)
    }
    return conversation
  }
  ...
  /// Always empty, and that is the design: inbound events do not travel this way.
  ///
  /// They are observed by `EventObservation`, which attaches to `IMDaemonController`'s
  /// listener and pushes each event straight to the server over the helper socket. Nothing
  /// consumes this stream, so it finishes immediately rather than pretending to be a source.
  ///
  /// The previous note here described "swizzled Messages.app methods" and pointed at an
  /// `EventHooks` type for "the hook table and its fragility". There is no such type and no
  /// swizzling in this helper; see `docs/OBSERVATION_LADDER.md` for what is actually
  /// attached and which rung it reached.

...
extension IMCoreBridge {
  ...
  public func accountInfo() async throws -> AccountInfo {
    try translating {
      ...
      // `activeIMessageAccount`, which is what the ObjC helper asked for, rather than the
      // first of `activeAccounts`. The two are not the same list: the SMS account is also
      // active, and `activeAccounts.first` is whichever IMCore happens to order first.
      // Falls back to the old shape so a release that drops the accessor still answers.
      let account =
        ((try? IMCoreRuntime.send(controller, "activeIMessageAccount")) ?? nil)
        ?? ((try? IMCoreRuntime.objects(controller, "activeAccounts")) ?? []).first
      guard let account else {
        throw PrivateAPIErrorShim.rejected(
          "no active iMessage account: this Mac is signed out of Messages"
        )
      }
      // A SEPARATE account, and the only place the forwarding flags live. Optional: a Mac
      // with no SMS account reports both as false, which is what the ObjC helper's
      // `?: FALSE` produced for the same case.
      let smsAccount = (try? IMCoreRuntime.send(controller, "activeSMSAccount")) ?? nil

      // Each alias string, enriched into the object the wire carries.
      //
      // IMCore hands back plain strings; `_aliasInfoForAlias:` turns one into
      // `{Alias, Status, IsUserVisible}`. The ObjC helper did exactly this, and its
      // fallback — the bare `{"Alias": …}` — is reproduced too, because an alias IMCore
      // will not describe still has to reach the client under the key it reads.
      let aliasObjects: (String) -> [AccountAlias] = { selector in
        ((try? IMCoreRuntime.objects(account, selector)) ?? [])
          .compactMap { $0 as? String }
          .map { alias in
            guard
              let info =
                (try? IMCoreRuntime.send(
                  account, "_aliasInfoForAlias:", alias)) ?? nil,
              let dictionary = info as? [String: Any]
            else {
              return AccountAlias(alias: alias)
            }
            return AccountAlias(
              // The dictionary's own `Alias` where it has one, so a canonicalised form
              // IMCore prefers wins over the string we looked it up by.
              alias: dictionary[WireKey.aliasName.rawValue] as? String ?? alias,
              status: (dictionary[WireKey.aliasStatus.rawValue] as? NSNumber)?.intValue,
              isUserVisible: (dictionary[WireKey.aliasIsUserVisible.rawValue] as? NSNumber)?
                .boolValue
            )
          }
      }
      let aliases = aliasObjects("aliases")
      ...
      return AccountInfo(
        ...
        // The PERSON's name, off the login handle, not the account's display name: that
        // one is the active alias and is reported separately below.
        accountName: ((try? IMCoreRuntime.send(account, "loginIMHandle")) ?? nil)
          .flatMap { ((try? IMCoreRuntime.string($0, "fullName")) ?? nil) },
        activeAlias: ((try? IMCoreRuntime.string(account, "displayName")) ?? nil)
          ?? aliases.first?.alias,
        aliases: aliases,
        vettedAliases: aliasObjects("vettedAliases"),
        loginStatusMessage: (try? IMCoreRuntime.string(account, "loginStatusMessage")) ?? nil,
        smsForwardingEnabled:
          smsAccount
          .flatMap { (try? IMCoreRuntime.bool($0, "allowsSMSRelay")) } ?? false,
        smsForwardingCapable:
          smsAccount
          .flatMap { (try? IMCoreRuntime.bool($0, "isSMSRelayCapable")) } ?? false
      )
    }
  }

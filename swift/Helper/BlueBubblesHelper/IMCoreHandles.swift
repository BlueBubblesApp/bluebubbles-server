...
enum IMChatHistory {
  ...
  static func messagePartChatItem(guid: String, partIndex: Int) async throws -> AnyObject {
    ...
    // The LOWER bound, which the check below does not cover: it guards the end of the list
    // and lets a negative index fall straight through to `list[partIndex]`, which is an
    // out-of-bounds read inside Messages.app rather than an error anybody sees. Refused the
    // same way the upper bound is, so the server gets a message naming the field.
    guard partIndex >= 0 else {
      throw PrivateAPIErrorShim.rejected("part index \(partIndex) is negative")
    }
  ...
  private static func load(guid: String, selector: String) async throws -> AnyObject {
    ...
    // BOUNDED. `ResumeOnce`'s own header states the contract: "It may never fire... The
    // wait is bounded, and a timeout is reported as a value." Twelve call sites honoured
    // that and four did not, and this was the worst of them: `IMChatHistoryController.load`
    // sits on the path of every reply, tapback, sticker, edit, unsend and delete, so a
    // completion that never fired stranded the request task and leaked the retained IMCore
    // objects and the heap block for the life of the process. Twenty seconds, matching every
    // other completion site in the helper.
    let loaded = await once.wait(timeout: .seconds(20), onTimeout: Box(nil)).value

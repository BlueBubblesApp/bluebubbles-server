...
enum IMPolls {
  ...
  private static func composition(
    ...
    let session: AnyObject =
      if let uuid {
        try IMCoreRuntime.create(sessionClass, "initWithIdentifier:", [uuid as NSUUID])
      } else {
        try IMCoreRuntime.create(sessionClass, "init")
      }
    let messageClass: AnyClass = try requireMessagesClass("MSMessage")
    let message = try IMCoreRuntime.create(messageClass, "initWithSession:", [session])
    ...
    let template = try IMCoreRuntime.create(templateClass, "init")
    ...
    let live = try IMCoreRuntime.create(
      liveClass, "initWithAlternateLayout:", [template])
  ...
  static func voteMessage(_ request: PollVoteRequest) throws -> AnyObject {
    ...
    let sessionClass: AnyClass = try requireMessagesClass("MSSession")
    let session = try IMCoreRuntime.create(
      sessionClass, "initWithIdentifier:", [uuid as NSUUID])
    let ackClass: AnyClass = try requireMessagesClass("_MSMessageCustomAcknowledgement")
    let ack = try IMCoreRuntime.create(
      ackClass, "initWithSession:isFromMe:time:", [session, true, NSDate()])

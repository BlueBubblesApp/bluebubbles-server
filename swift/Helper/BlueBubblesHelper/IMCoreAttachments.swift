...
enum CKCompositions {
  ...
  static func empty(subject: NSAttributedString?) throws -> AnyObject {
    ...
    return try IMCoreRuntime.create(
      type, "initWithText:subject:",
  ...
  static func stageBytes(for media: AnyObject, from source: String) throws {
    ...
    if !manager.fileExists(atPath: destination) {
      ...
      try Self.place(source, at: destination)
    }
  ...
  /// Puts the file where the transfer expects it, cloning rather than copying where it can.
  ///
  /// **This runs on Messages.app's main thread and cannot simply move off it.**
  /// `IMCoreBridge` is `@MainActor`, and the send path's header explains at length why the
  /// composition and the send must not be separated by a suspension. So a `copyItem` here
  /// froze the user's Messages interface for as long as the copy took, which for a video
  /// sent through `POST /message/attachment` is seconds.
  ///
  /// `copyfile` with `COPYFILE_CLONE_FORCE` makes that O(1) on APFS: the destination is a
  /// copy-on-write clone, no bytes move, and the source may be deleted afterwards with the
  /// clone unaffected. Every supported Mac boots APFS, and the source is written by this
  /// server into its own directory, so the common case is same-volume and costs a syscall.
  ///
  /// The fallback is the honest part. A clone needs one APFS volume: a source on an external
  /// disk, a network mount or a non-APFS volume cannot be cloned, and there the bytes are
  /// copied and the interface does stall. That case is not fixed here, because fixing it
  /// means restructuring the send around a suspension the file argues against. It is
  /// narrower than what it replaces, and it is stated rather than hidden.
  static func place(_ source: String, at destination: String) throws {
    if copyfile(source, destination, nil, copyfile_flags_t(COPYFILE_CLONE_FORCE)) == 0 {
      return
    }
    // Not an error worth reporting on its own: a clone is an optimisation, and every
    // reason it can fail (different volumes, a filesystem without clones) is one a plain
    // copy handles.
  ...
  static func setSendLater(_ composition: AnyObject, _ date: Date?) throws {
    ...
    let info = try IMCoreRuntime.create(
      type, "initWithSelectedDate:", [date as NSDate])
    guard

...
extension IMCoreBridge {
  ...
  public func balloonBundleMediaPath(for guid: MessageGUID) async throws -> String {
    ...
    if IMCoreRuntime.responds(source, to: NSSelectorFromString("generateMedia:")) {
      ...
      // Bounded; see `ResumeOnce`'s header. Media generation that never calls back lets the
      // send proceed with whatever the source already holds, which is the same degradation
      // as a source that does not respond to the selector at all.

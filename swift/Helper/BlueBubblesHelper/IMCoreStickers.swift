...
enum IMStickers {
  ...
  static func sticker(path: String) throws -> AnyObject {
    ...
    // Asked of the CLASS: which initialiser this release has is the question, and an
    // uninitialised allocation is never vended to ask it of. See `IMCoreRuntime.create`.
    guard
      let (selector, arguments) = candidates.first(where: {
        IMCoreRuntime.instancesRespond(type, to: NSSelectorFromString($0.0))
      })
    else {
      throw PrivateAPIError.unavailableOnThisOS(
        method: "sendSticker", requires: "an IMSticker initWithStickerID: initializer")
    }
    let sticker = try IMCoreRuntime.create(type, selector, arguments)
...
enum IMStickerStore {
  ...
  static func representation(data: Data, role: String = singleImageRole) throws -> AnyObject {
    ...
    return try IMCoreRuntime.create(
      type, "initWithData:type:size:role:",
  ...
  static func attribution() throws -> AnyObject {
    ...
    return try IMCoreRuntime.create(
      type, "initWithAdamID:bundleIdentifier:name:",
  ...
  static func donateToRecents(
    ...
    let facade = try IMCoreRuntime.create(type, "init")

...
enum CKChatControllers {
  ...
  static func forConversation(_ conversation: CKConversation) throws -> AnyObject {
    ...
    // Through `allocate`, not `invoke`. `BBInvoke` reads an object return into an
    // `__unsafe_unretained` and assigns it to a strong `__block id`, so the +1 from
    // `+alloc` is never consumed and ARC adds a retain on top: one leaked `CKChatController`
    // inside the user's Messages per message delete.
    return try IMCoreRuntime.create(
      type, "initWithConversation:", [conversation.object]
    )
  }
}

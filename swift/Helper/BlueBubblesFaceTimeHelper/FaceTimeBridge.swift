...
enum FaceTimeBridge {
  ...
  // MARK: - The idle camera

  /// Turns off the self-preview FaceTime starts when we relaunch it for injection.
  ///
  /// **The camera is a side effect of injection, not something the Private API needs.**
  /// Injection RELAUNCHES FaceTime.app, and FaceTime comes up previewing you, so turning
  /// the FaceTime Private API on lit the user's camera and the recording indicator for as
  /// long as the server ran, with no call in progress. What this helper drives is
  /// `TUConversationManagerXPCClient` and the daemons behind it, which do not care whether a
  /// preview is running. Stopping it is undoing our own side effect.
  ///
  /// **It has to be stopped at the layer that OWNS it.** `AVConferencePreview` is the object
  /// that runs the camera, and stopping it there does nothing lasting: its delegate (a
  /// `TUVideoDeviceControllerProvider`) still believes a preview should exist and starts a
  /// new one about 1.2 seconds later. Sending `stopPreview` to the DELEGATE instead turns the
  /// camera off and it stays off. The delegate is reached through the preview because
  /// `TUVideoDeviceControllerProvider` has no singleton; FaceTime owns the instance.
  ///
  /// **This is ONE SHOT, and that is a known limitation rather than a decision.** FaceTime's
  /// own `PowerManager` cycles the camera as the app gains and loses focus ("Want camera
  /// paused because of power saver"), and a later resume starts it again with nothing here
  /// to stop it, which presents as the preview being absent from the window while the green
  /// indicator is lit. See `TODO.md`. One shot is what keeps this from fighting a user who
  /// opened FaceTime to actually make a call.
  ///
  /// `isPreviewRunning` is what this waits for rather than a delay: the preview does not
  /// exist at `didFinishLaunching`, and a fixed sleep is a race dressed as a constant. That
  /// flag turns true when the first frame arrives, so the camera is live for about fourteen
  /// seconds after a server start and off thereafter.
  ///
  /// Every failure is silent and survivable. This runs inside somebody's FaceTime, and a
  /// macOS that renames any of it must cost them a camera indicator, not their FaceTime.
  static func stopIdlePreview(attemptsRemaining: Int = 60) {
    guard attemptsRemaining > 0 else {
      BlueBubblesFaceTimeHelper.Logging.log(
        "Gave up waiting for FaceTime's preview to start; the camera may stay on")
      return
    }
    guard
      let preview = try? IMCoreRuntime.sharedInstance(
        ofClass: "AVConferencePreview", accessors: ["AVConferencePreviewSingleton"]
      ),
      let owner = try? IMCoreRuntime.send(preview, "delegate"),
      owner.responds(to: NSSelectorFromString("stopPreview"))
    else {
      retryStoppingIdlePreview(attemptsRemaining: attemptsRemaining - 1)
      return
    }

    // `isPreviewRunning` returns BOOL, so it goes through `bool`; `send` would read the
    // byte as a pointer. False simply means "not started yet".
    guard (try? IMCoreRuntime.bool(owner, "isPreviewRunning")) == true else {
      retryStoppingIdlePreview(attemptsRemaining: attemptsRemaining - 1)
      return
    }

    do {
      try IMCoreRuntime.send(owner, "stopPreview")
      BlueBubblesFaceTimeHelper.Logging.log("Turned off FaceTime's idle camera preview")
    } catch {
      BlueBubblesFaceTimeHelper.Logging.log("Could not turn off the idle preview: \(error)")
    }
  }

  private static func retryStoppingIdlePreview(attemptsRemaining: Int) {
    DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
      MainActor.assumeIsolated { stopIdlePreview(attemptsRemaining: attemptsRemaining) }
    }
  }

  // MARK: - Keeping it off while nobody is looking

  /// Whether the background watchdog is currently scheduled.
  private static var isWatching = false

  /// Keeps the camera off for as long as FaceTime is not the app the user is looking at.
  ///
  /// **The one-shot stop is not enough, because FaceTime's `CameraController` is a
  /// RECONCILER.** It re-derives `wantCamera` on its own events and restarts anything that
  /// does not match; its log line for undoing our stop is literally "Unpausing preview". So
  /// the camera comes back the moment the user switches windows, which is exactly when they
  /// notice: no preview in the window, green indicator lit.
  ///
  /// **The rule is `NSApp.isActive`, not "the preview started".** Hooking the start and
  /// stopping again would also fight somebody who opened FaceTime to make a call: before
  /// they dial there is no active call to test for, so "is there a call" cannot tell the two
  /// apart. "Is FaceTime the frontmost app" can: not frontmost means the only reason it is
  /// running is that WE relaunched it for injection, and its camera is our side effect to
  /// undo. Bring FaceTime to the front and this stops touching it entirely.
  ///
  /// WHY A WATCHDOG AND NOT A HOOK. The observation ladder was walked first
  /// (`docs/OBSERVATION_LADDER.md`) and the top two rungs are genuinely closed here:
  ///
  ///   - **Rung 1, a notification: absent.** The dyld cache carries only the
  ///     `AVCaptureVideoPreviewLayer…` family; observing all five of them across three real
  ///     preview restarts produced zero callbacks. AVConference does not post one; its
  ///     pipeline is not `AVCaptureVideoPreviewLayer`.
  ///   - **Rung 2, registering as an additional observer: no such API.**
  ///     `AVConferencePreview` has a single `delegate`, already FaceTime's own
  ///     `TUVideoDeviceControllerProvider`, and `registerBlocksForDelegateNotifications`
  ///     takes no arguments; it is AVConference wiring its own internals.
  ///
  /// That leaves rung 3, a swizzle, and a poll is strictly better than one here: it replaces
  /// no method inside somebody's FaceTime, it cannot recurse against the reconciler, and a
  /// selector that vanishes costs a camera indicator rather than the host. It only runs while
  /// FaceTime is in the background, so the cost when the user is actually using FaceTime is
  /// zero.
  static func keepPreviewOffWhileInactive() {
    for name in [
      NSApplication.didResignActiveNotification,
      NSApplication.didBecomeActiveNotification,
    ] {
      NotificationCenter.default.addObserver(forName: name, object: nil, queue: .main) { _ in
        MainActor.assumeIsolated { activeStateChanged() }
      }
    }
    activeStateChanged()
  }

  private static func activeStateChanged() {
    if NSApplication.shared.isActive {
      // The user is looking at FaceTime. Their camera, their business.
      isWatching = false
    } else if !isWatching {
      isWatching = true
      // Every activation change starts the watch over at its shortest interval: a camera
      // that comes on does so when FaceTime's state changes, which is exactly what these
  ...
  /// How long the next check waits. Doubles after each quiet one.
  private static var pollInterval = shortestPollInterval
  /// Consecutive checks that found nothing to stop.
  private static var quietPolls = 0

  private static let shortestPollInterval: TimeInterval = 2
  private static let longestPollInterval: TimeInterval = 60
  /// Quiet checks before the watch stops altogether and waits for an activation change.
  private static let quietPollsBeforeStopping = 8

  /// Checks for a camera FaceTime left running, backing off and then stopping.
  ///
  /// This used to re-arm itself every two seconds for as long as FaceTime was not frontmost
  /// -- which is the NORMAL state, because the server relaunches FaceTime precisely in order
  /// to inject into it, and nobody is looking at it afterwards. So it woke FaceTime's main
  /// thread thirty times a minute, forever, doing four Objective-C runtime lookups including
  /// two malloc/free pairs each time, in a process that would otherwise be idle enough for
  /// App Nap. The header's claim that the cost is zero while the user is actually using
  /// FaceTime is true and is the wrong half to measure.
  ///
  /// The camera coming on is not a silent event: it happens when FaceTime activates or a
  /// call starts, and `keepPreviewOffWhileInactive` is already observing activation. So the
  /// poll exists only to catch the moments just after one of those, and backing off to
  /// nothing is safe -- an activation change re-arms it at two seconds.
  private static func pollWhileInactive() {
    guard isWatching, !NSApplication.shared.isActive else {
      isWatching = false
      return
    }
    if stopPreviewIfRunning() {
      // Something was running, so stay attentive: a preview that restarts usually restarts
      // soon after.
      pollInterval = Self.shortestPollInterval
      quietPolls = 0
    } else {
      quietPolls += 1
      guard quietPolls < Self.quietPollsBeforeStopping else {
        isWatching = false
        return
      }
      pollInterval = min(pollInterval * 2, Self.longestPollInterval)
    }
    let delay = pollInterval
    DispatchQueue.main.asyncAfter(deadline: .now() + delay) {
      MainActor.assumeIsolated { pollWhileInactive() }
    }
  }

  /// One stop, only if something is actually running. Silent on every failure.
  ///
  /// - Returns: whether anything was stopped, which is what decides how soon to look again.
  @discardableResult
  private static func stopPreviewIfRunning() -> Bool {
    guard
      let preview = try? IMCoreRuntime.sharedInstance(
        ofClass: "AVConferencePreview", accessors: ["AVConferencePreviewSingleton"]
      ),
      let owner = try? IMCoreRuntime.send(preview, "delegate"),
      owner.responds(to: NSSelectorFromString("stopPreview")),
      (try? IMCoreRuntime.bool(owner, "isPreviewRunning")) == true
    else { return false }
    _ = try? IMCoreRuntime.send(owner, "stopPreview")
    BlueBubblesFaceTimeHelper.Logging.log("Stopped the camera FaceTime restarted in the background")
    return true
  }
  ...
  /// The registration in flight, so concurrent callers wait on one rather than racing.
  ///
  /// `Task<Void, …>`, not `Task<AnyObject, …>`: an IMCore object is not `Sendable`, and it
  /// does not need to cross anything here — everything on this type is main-actor isolated,
  /// so a waiter reads `conversationClient` itself once the task completes.
  private static var clientSetup: Task<Void, any Error>?

  /// Where event frames go. Set by `FaceTimeHelperMain` from the dylib constructor's
  /// thread before the socket connects, and read from the conversation delegate's queue,
  /// neither of which is the main actor, so it is lock-guarded rather than isolated.
  nonisolated private static let emitter = OSAllocatedUnfairLock<
    (@Sendable (HelperEventName, WireObject) -> Void)?
  >(initialState: nil)

  nonisolated static func setEmitter(
    _ emit: @escaping @Sendable (HelperEventName, WireObject) -> Void
  ) {
    emitter.withLock { $0 = emit }
  }

  /// Emits an event frame to the server, or drops it if nothing is listening yet.
  nonisolated static func emit(_ event: HelperEventName, _ payload: WireObject) {
    emitter.withLock { $0 }?(event, payload)
  }
  ...
  ///
  /// CONCURRENT callers share one registration through `clientSetup`. `conversationClient`
  /// is assigned before the first await, so without this a second caller arriving in that
  /// window took the reuse branch and re-ran `registerWithCompletionHandler:` and
  /// `fetchInitialStateWithCompletionHandler:` against a client that was mid-registration,
  /// then set `clientReady` on its own. Awaiting the in-flight task is the standard shape
  /// and is what an older version of this comment claimed already happened.
  @discardableResult
  static func ensureClient() async throws -> AnyObject {
    ...
    // Someone else is already doing this: wait for theirs rather than starting a second.
    if let inFlight = clientSetup {
      try await inFlight.value
      guard let client = conversationClient else {
        throw PrivateAPIError.rejectedByMessages(
          reason: "the FaceTime conversation client went away during setup"
        )
      }
      return client
    }

    let setup = Task<Void, any Error> { _ = try await performClientSetup() }
    clientSetup = setup
    defer { clientSetup = nil }
  ...
  private static func performClientSetup() async throws -> AnyObject {
    ...
    if let existing = conversationClient {
      ...
      // Allocated and initialised in ONE call: `-init` consumes the allocation's +1, so
      // an uninitialised object held here would be a reference the initialiser has already
      // disposed of. See `IMCoreRuntime.create`.
      guard let initialized = try? IMCoreRuntime.create(type, "init") else {
        throw PrivateAPIError.rejectedByMessages(
          reason: "could not construct the FaceTime conversation client"
        )
      }
  ...
  ///
  /// **NOT CALLED, and that is deliberate.** `ensureClient` above explains why and MEASURED
  /// the reason: `TUConversationManagerXPCClient` invokes delegate methods without
  /// `respondsToSelector:` guards, so an incomplete shim is `doesNotRecognizeSelector:` and
  /// `abort()`, which crashes FaceTime.app. Every protocol method would have to be
  /// implemented before this could be wired.
  ///
  /// So `.faceTimeMembershipChanged` never fires, and the "wait for join" flow polls
  /// `faceTimeMembers` on the server instead. Kept rather than deleted because the polling
  /// is the fallback and this is the clean path; two comments used to describe it as
  /// installed, which is the part that was wrong.
  ///
  /// If you wire it: implement the whole protocol first, and delete this paragraph.
  ...
  ///
  /// Three steps: build the `TUDialRequest` (`makeDialRequest`), hand it to the call
  /// center through whichever completion selector this release has (`performDial`), then
  /// find the resulting call and silence the Mac's side of it.
  static func dial(_ request: FaceTimeStartRequest) async throws -> FaceTimeCall {
    try requireFaceTimeHost()
    let center = try callCenter()
    let dialRequest = try makeDialRequest(request, on: center)
    ...
    let validity =
      (try? IMCoreRuntime.send(dialRequest, "validityErrors"))
      .map { String(describing: $0) } ?? "none"
    let requestDescription = (try? IMCoreRuntime.string(dialRequest, "description")) ?? nil

    let box = try await performDial(dialRequest, on: center)
  ...
  /// A `TUDialRequest` for the FaceTime provider, with its handles and media set.
  private static func makeDialRequest(
    _ request: FaceTimeStartRequest, on center: AnyObject
  ) throws -> AnyObject {
    ...
    // `initWithProvider:`, allocated and initialised together; see `IMCoreRuntime.create`.
    // An explicit nil would travel as `NSNull()`, which is why this no longer needs the
    // typed-IMP initialiser it used to call.
    guard
      let request0 = try? IMCoreRuntime.create(dialType, "initWithProvider:", [provider])
    else {
      throw PrivateAPIError.rejectedByMessages(
  ...
  /// Hands the request to the call center and waits for its completion. The box holds the
  /// call the completion returned, if any, and the error where the selector reports one.
  private static func performDial(
    _ dialRequest: AnyObject, on center: AnyObject
    ...
    {
      await callAwaitingCompletion2(
        center, "dialWithRequest:completionWithError:", leading: [dialRequest]
      ) { result, error in
        box.value = result
        box.error = error
      }
      ...
      center, to: NSSelectorFromString("dialWithRequest:completion:"))
    {
      await callAwaitingCompletion(
        center, "dialWithRequest:completion:", leading: [dialRequest]
      ) { result in
        box.value = result
      }
    } else {
  ...
  private(set) static var lastLinkSnapshotCount = 0

  static func invalidateLinks(matching urls: [String]?) async throws -> [String] {
    let client = try await ensureClient()
    let links = await linkSnapshot(on: client)
    ...
    for link in links {
      ...
      if let failure = await invalidate(link, on: client, selector: invalidateSelector) {
        lastError = failure
      } else {
        invalidated.append(url)
      }
    }
    // If links were found but NONE could be invalidated, say so. An empty list here would
    // be indistinguishable from "no links exist", and orphaned links would look cleaned up.
  ...
  /// Every link the client knows about, from every source, in one list.
  ///
  /// Gathered from both accessors because no single one is reliable without the data-source
  /// delegate: the XPC list (all links, not just created-this-session) and the client's own
  /// `activatedConversationLinks`. De-duplication by URL is the caller's, since it is
  /// also where the URL filter lives.
  private static func linkSnapshot(on client: AnyObject) async -> [AnyObject] {
    var links: [AnyObject] = []
  ...
  /// Invalidates one link. Nil on success; otherwise the reason, so the caller can report
  /// a batch that invalidated nothing as the failure it is rather than an empty success.
  private static func invalidate(
    _ link: AnyObject, on client: AnyObject, selector: String
  ) async -> String? {
    ...
    await callAwaitingCompletionBool(
      client, selector,
      leading: selector.contains("deleteReason:") ? [link, NSNumber(value: 1)] : [link]
    ) { success, error in
      box.success = success
      box.error = error
    }

    if box.success { return nil }
    if let error = box.error as? NSError { return error.localizedDescription }
  ...
  private static func callAwaitingCompletion(
    ...
    // THE BLOCK IS DECLARED WITH A PARAMETER ONLY IF SOMEBODY READS IT.
    //
    // Block arity is not symmetric. Declaring FEWER parameters than the callee passes is
    // harmless: the extra arguments sit in registers nobody reads. Declaring MORE reads an
    // uninitialised register AS AN OBJECT, and Swift retains it:
    // `swift_unknownObjectRetain` on a value that is not a pointer, which is `SIGSEGV` or
    // `EXC_ARM_PAC_FAIL` and takes FaceTime.app with it. Measured: a crash report naming
    // this exact thunk, from `registerWithCompletionHandler:` / `fetchInitialState…` during
    // a cold start. The headers record these as `id /* block */` and cannot tell us the
    // arity, so the safe default is the one that reads nothing.
    //
    // `callAwaitingCompletion2` says the same thing about its second parameter; this is
    // that rule applied to the FIRST one, which was assumed rather than checked.
    let erased: AnyObject
    if let onResult {
      let block: @convention(block) (AnyObject?) -> Void = { value in
        onResult(value)
        sentinel.finish()
      }
      erased = unsafeBitCast(block, to: AnyObject.self)
    } else {
      let block: @convention(block) () -> Void = { sentinel.finish() }
      erased = unsafeBitCast(block, to: AnyObject.self)
    }

    do {
      try IMCoreRuntime.invoke(target, selector, leading + [erased])
    } catch {
      sentinel.finish()
    }
...
private final class FaceTimeDelegateShim: NSObject {
  private let onMembership: (String, [WireObject]) -> Void

  init(onMembership: @escaping (String, [WireObject]) -> Void) {
...
extension FaceTimeBridge {
  ...
  nonisolated static func membersPayload(of conversation: AnyObject) -> [WireObject] {
    func decode(_ value: AnyObject?, pending: Bool) -> [WireObject] {
      let set: [AnyObject]
      ...
      return set.compactMap { member in
        ...
        return [
          .handle: value,
          .isPending: pending,
          .nickname: (try? IMCoreRuntime.string(member, "nickname")) ?? nil,
        ]
      }
    }

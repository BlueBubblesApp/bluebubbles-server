...
public enum WireKey: String, Sendable, Hashable, CaseIterable, CustomStringConvertible {
  ...
  case loginStatusMessage = "login_status_message"
  case smsForwardingEnabled = "sms_forwarding_enabled"
  case smsForwardingCapable = "sms_forwarding_capable"
  // The three keys inside an alias object. CAPITALISED, because they are IMCore's own and
  // the reference passed `_aliasInfoForAlias:`'s dictionary through untouched; the app
  // reads `e['Alias']`. See `AccountAlias`.
  case aliasName = "Alias"
  case aliasStatus = "Status"
  case aliasIsUserVisible = "IsUserVisible"
  case handle
  case name

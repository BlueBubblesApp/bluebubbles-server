...
//
//  One type, in the contract both sides depend on. Two spellings of one wire format (one
//  per target) is a drift waiting to happen: the day one side starts treating a number as a
//  boolean and the other does not, the difference shows up as a helper "rejecting" a request
...
public enum WireJSON: Sendable, Equatable {
  ...
  /// A whole number, or nil when the value is not one this side can represent.
  ///
  /// `Int(exactly:)` and not `Int(_:)`, and the difference is a crash INSIDE MESSAGES.APP.
  /// The plain conversion traps on infinity, on NaN, and on any magnitude beyond `Int`, and
  /// this wire carries doubles: the encoder below only shortens whole numbers below 2^53,
  /// so a request body carrying `9223372036854775807` arrives here as a `Double` just past
  /// `Int.max` and takes the host process down with it. That host is the user's Messages,
  /// the crash report says Messages, and nothing in it names BlueBubbles.
  ///
  /// Returning nil instead puts the field on the path every absent field already takes:
  /// `HelperDispatch` throws `rejectedByMessages` for a required one, which the server turns
  /// into an error the client can read. Rejecting a nonsense number is also the server's job
  /// (a 400 says which field, and says it before any of this), but a boundary that cannot
  /// trap is what makes the server's checks a convenience rather than the only thing
  /// standing between a malformed request and somebody's Messages.
  ...
  /// A field of an object, by its typed key. The string form above stays for the frame
  /// decoder and for tests that spell the wire out on purpose; production code on either
  /// side of the socket reads through this one; see `WireKey`.
...
extension WireJSON {
  /// An object keyed by `WireKey`: the form every request payload is built in.
  ///
  /// Overloads the case rather than taking a new name so a call site reads the same
  /// whichever it uses; the literal's key type is what picks between them, and a `.x`
  /// member cannot be a `String`, so `[.chatGuid: …]` can only mean this one.
  public static func object(_ pairs: [WireKey: WireJSON]) -> WireJSON {
    .object(Dictionary(uniqueKeysWithValues: pairs.map { ($0.key.rawValue, $0.value) }))
  }
  ...
  public static func object(dropping pairs: [WireKey: WireJSON?]) -> WireJSON {
    object(pairs.compactMapValues { $0 })
  }

  /// The string-keyed form of `object(dropping:)`, for tests that spell the wire out.
  public static func object(dropping pairs: [String: WireJSON?]) -> WireJSON {
    .object(pairs.compactMapValues { $0 })
  }

  /// `{}`: an action with no fields. Named because `.object([:])` is ambiguous now that
  /// two key types can build one.
  public static let emptyObject: WireJSON = .object([String: WireJSON]())
}

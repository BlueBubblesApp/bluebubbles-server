...
//
//  Spelled as string literals on each side independently, a typo on either side compiles and
//  surfaces as a runtime rejection from Messages, and adding a command means remembering to
//  edit a file in a different target with no compiler to remind you.
//
//  These enums are the connection. The client sends a case, each helper switches over its

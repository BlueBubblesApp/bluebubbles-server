...
// ports are shippable.
//
// SPLIT INTO ROLES, and the reason is testing rather than tidiness. `PrivateAPI` was one
// protocol of seventy-six members with no default implementations, so a conformance was an
// all-or-nothing seventy-six-member obligation. The practical consequence was that the only
// double in the package was `FailingPrivateAPI`, which throws from every member — cheap to
// write precisely because no return value has to be constructed. There was no SUCCEEDING
// double, and therefore no test anywhere that drove a Private API operation to a result.
//

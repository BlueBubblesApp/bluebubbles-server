...
//  over yet. That only works if they derive the SAME path.
//
...
//  `connect` to a Unix socket OUTSIDE its container. It is easy to read that as "the sandbox
//  refuses Unix sockets" and fall back to a loopback TCP bridge, which works, and which
//  cannot identify its peer, so any local process could drive the Private API.
//
...
//      which is why both go through it. A helper computing the path from `NSHomeDirectory()`
//      connects to a path that does not exist, retries forever, and reports nothing.
...
public enum SocketLocation {
  ...
  ///
  /// **ONE SOCKET CANNOT SERVE TWO APPS.** The sandbox restriction documented above is
  /// SYMMETRIC: measured with both

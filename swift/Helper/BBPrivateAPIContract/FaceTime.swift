...
/// Names carried on the injected app's environment.
///
/// The injection environment is the only channel the helper can read at LOAD time: it runs
/// inside the target app's sandbox with no reach into the settings store, and its socket is
/// not connected yet. Declared in the CONTRACT because both sides read it: the server sets it
/// when relaunching the app, the helper reads it in its constructor.
public enum HelperEnvironment {
  /// "1" or "0": whether the FaceTime helper keeps FaceTime's camera off while FaceTime is
  /// in the background. Absent means ON, matching the setting's default, so a helper injected
  /// by an older server does not leave a camera running because a variable was missing.
  public static let faceTimeIdleCameraOff = "BLUEBUBBLES_FACETIME_IDLE_CAMERA_OFF"
}

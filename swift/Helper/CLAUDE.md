...
**Construct with `IMCoreRuntime.create(_:_:_:)` — class, initialiser and arguments in ONE
call.** It delegates to `BBAllocateAndInitialize` in the Objective-C shim. There is no way to
get an uninitialised allocation from Swift, and that is deliberate.

Both halves of the convention have been got wrong here, in opposite directions:

- `+alloc` returns +1, and `perform(alloc)?.takeUnretainedValue()` left it unconsumed while
  ARC added a retain of its own: one permanently over-retained IMCore object per message,
  tapback, sticker, attachment and poll sent, accumulating inside the USER'S MESSAGES.
- `-init…` **consumes** the receiver's +1 — it may return the same object, release the
  receiver and return a different one, or return nil. Fixing the first bug by handing the
  allocation to Swift and initialising it in a second call therefore left Swift holding a
  reference to an object the initialiser had already disposed of. Every
  `POST /api/v1/message/attachment` hit it in `-[CKComposition initWithText:subject:]` and
  killed Messages with `EXC_ARM_PAC_FAIL` inside `swift_unknownObjectRelease`. Text sends
  survived the same pattern only because something else happened to hold a reference.

Nine sites had the first, thirteen the second, because the pattern gets copied. Both halves
now live in a file compiled with ARC, where the ownership is the compiler's to track, and
`AllocationPolicyTests` fails the build on a call that splits them again or names a vended
allocator.

Picking between two initialisers (a version ladder) asks the CLASS:
`IMCoreRuntime.instancesRespond(_:to:)`, never an instance that does not exist yet.
...
`HelperSocketClient.connection`, the event emitters in `EventObservation` and
`FaceTimeBridge`), never `NSLock` plus a `var` and never `nonisolated(unsafe)`. There are no
`nonisolated(unsafe)` statics left; a new one is a lock that has not been written yet.
...
**Dispatch is split by role, and the switch stays a switch.** `HelperDispatch.swift` holds the
exhaustive `switch` over `MessagesHelperAction`: one line per case, calling a static in the
role file it belongs to (`HelperDispatch+Sending`, `+Chats`, `+Muting`, …, one per role
`PrivateAPI` composes), and `RequestData`, the only reader of request fields. A new action is
a case in the switch and a function in its role file; a body written inline in the switch, or
a field read as `fields["chatGuid"]` instead of `data.chat()`, is the 516-line function this
replaced growing back. `HelperVocabularyTests` scans the role files for string-keyed reads.

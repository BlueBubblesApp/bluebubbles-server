...
- **A rule the compiler cannot check ships with a test that scans the source for it.** A
  comment saying "never X" is read once; a test that greps for X runs on every build. Six
  exist and are the pattern: `SettingKeyLiteralTests` (a declared setting key spelled as a
  literal outside the registry), `HelperVocabularyTests` (a wire field spelled as a string on
  either side of the helper socket), `IntegrationCatalogTests` (an app file reading `BuiltInManifests.all`),
  `TestDataPolicyTests` (a real address in a fixture), `RequestBodyPolicyTests` (a handler
  reading a request with `try?`), `HandlerBoundaryPolicyTests` (a handler taking the Private
  API client, the FaceTime coordinator or the FindMy runtime directly). Each locates the tree from `#filePath`,
  skips comment lines, and prints every offender with a file and line. When you add a "never"
  to this file, add the test that enforces it in the same change.

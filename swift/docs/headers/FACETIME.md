...
### The idle camera, and stopping it at the layer that owns it

Injection RELAUNCHES FaceTime.app, and FaceTime comes up previewing you, so enabling the
FaceTime Private API lit the user's camera and the recording indicator for as long as the
server ran, with no call in progress. `FaceTimeBridge.stopIdlePreview` turns it off.

**Stop it at the OWNER, not at the object running the camera.** `AVConferencePreview`
(`+AVConferencePreviewSingleton`) is what drives the capture, and stopping it there does
nothing lasting: its delegate, a `TUVideoDeviceControllerProvider`, still believes a preview
should exist and starts a new one. Measured on macOS 26.5.2:

    helper: stopPreview returned normally
    +0.6s   -[AVConferencePreview setLocalVideoLayer:front:]
    +1.2s   AVConferencePreview-didStartPreview
    +14s    didReceiveFirstPreviewFrameFromCameraUniqueID

Sending `stopPreview` to the DELEGATE instead turns the camera off and it stays off. The
delegate is reached through `AVConferencePreview.delegate` because
`TUVideoDeviceControllerProvider` has no singleton; FaceTime owns the instance.

`isPreviewRunning` on the provider is what the helper waits for, rather than a delay: there is
no preview at `didFinishLaunching`, and a fixed sleep is a race dressed as a constant. That flag
turns true when the first camera frame arrives: **1.7 seconds after the helper loads on a
healthy machine.** (An earlier reading of ~14s was taken on a Mac with 43 days of uptime, a
1 GB WindowServer and heavy paging; it was the machine, not FaceTime. Measure this kind of
thing on a freshly booted box or not at all.)

**What this does NOT cover, and why no preview-level API can.** FaceTime's `CameraController`
is a RECONCILER. It re-derives whether the camera should run and acts on the difference:

    Update camera state -> Check if want to unpause, wantCamera: 1, wantCameraPaused: 0,
                           isPreviewRunning: 0, previewStarted: 0
    Update camera state -> Unpausing preview

Every preview-level call attacks `isPreviewRunning`, which is the half the controller
recomputes. `wantCamera` is the half that decides, and it stays 1. Measured on a healthy Mac
(freshly booted, load < 8) with the PAC bug fixed, so these are clean results, not the earlier
contaminated ones:

| Attempt | Outcome |
|---|---|
| `stopPreview` on the provider | holds while FaceTime is left alone; undone by any focus change |
| `pausePreview` | same reconciler: the log's own word for the undo is "Unpausing" |
| `setLocalCameraWithUID:` nil | camera re-selected (`_isCameraValid` names it again), then unpaused |
| `NSApplication.hide(_:)` | NO-OP. `wasHidden=false nowHidden=false` |
| `orderOut` / `miniaturize` | FaceTime terminates (its `AppController`: "terminate app if needed") |

So the shipping stop is worth having (with FaceTime untouched the camera stays off, confirmed
by the indicator at 30s and 75s with zero unpause events after ours), but it cannot survive the
user switching windows, which is when they notice.

`wantCamera` is driven by FaceTime's own conditions: `screenShieldIsRaised`,
`hasNoActiveVideoCall`, `videoIsInitialized`, `appActive`, `isOccluded`.

**Lazy injection is NOT the answer**, and it is worth writing down why so nobody proposes it
again: the helper has to be injected BEFORE an incoming call arrives, so a server that injects
on first use cannot answer one.

**What is there instead: `keepPreviewOffWhileInactive`.** The rule is `NSApp.isActive`: if
FaceTime is not the app the user is looking at, the only reason it is running is that we
relaunched it, so its camera is our side effect to undo; if it IS frontmost, the helper does not
touch it. That rule is what makes this safe, and it is why hooking "the preview started" would
not be: before a user dials there is no active call, so "is there a call" cannot tell somebody
about to make one apart from a background restart.

The observation ladder was walked before settling on a poll, and the top two rungs are closed:

  - **Rung 1, a notification: absent.** The dyld cache carries only the
    `AVCaptureVideoPreviewLayer…` family (`DidStartRunning`, `DidBecomeVisible`,
    `DidDisplayFirstPreviewFrame`, plus `DisplayFirstPreviewFrame` and
    `AVCaptureSessionDidStartRunning`). Observing all five across three real preview restarts
    produced ZERO callbacks: AVConference's pipeline is not `AVCaptureVideoPreviewLayer`.
  - **Rung 2, an additional observer: no API.** `AVConferencePreview` has one `delegate`,
    already FaceTime's `TUVideoDeviceControllerProvider`, and
    `registerBlocksForDelegateNotifications` takes no arguments: AVConference wiring itself.

A poll beats the remaining rung-3 swizzle here: it replaces no method inside somebody's
FaceTime, it cannot recurse against the reconciler, and it runs only while FaceTime is in the
background.

**Confirmed working** by the user on 7 September 2026: focusing and unfocusing FaceTime, the
case a scripted focus cycle could not reproduce.

It is behind `facetime_idle_camera_off`, which **defaults ON**: the camera is a side effect of
something the SERVER did, so leaving it running as an opt-out is the wrong way round. The value
reaches the helper through the injection environment (`HelperEnvironment.faceTimeIdleCameraOff`,
declared in the contract because both sides read it): the only channel readable at load, since
the helper has no settings store and no socket yet. Absent means ON, so a helper injected by an
older server does not leave a camera running because a variable was missing.

**Changing it re-injects FaceTime ONLY.** `PrivateAPIGatedService` owns both helpers because
they share a transport, and it used to answer every watched settings change with `.restart`,
which stopped and started the service, re-injected both, and so quit the user's Messages.app
for a FaceTime setting. It now partitions its watched keys by helper (`messagesKeys` /
`faceTimeKeys`), rebuilds the configuration, re-injects just the affected app and returns
`.reconfigure`. FindMy rides in the Messages helper (`FindMyBridge.swift`, not a third dylib),
so a FindMy setting added to that set re-injects Messages and leaves FaceTime alone.

Verified in the UI on 7 September 2026, toggling the switch off on a running server:

    Messages PID   19316 -> 19316   (untouched)
    FaceTime PID   19322 -> 20013   (re-injected)
    server log     "Helper registered {process=com.apple.FaceTime}" only; no service restart
    new process    BLUEBUBBLES_FACETIME_IDLE_CAMERA_OFF=0
    helper         "Idle-camera control is switched off; leaving FaceTime's camera alone"

That also settles the other open question: a live settings change reaches the service and
applies without a server restart, so the setting needs no `application: .composition`.

**The crash that hid all of this was ours.** Every one of these selectors returns `void`, and
`IMCoreRuntime.send` read the return register anyway and had ARC retain it:
`swift_unknownObjectRetain` on a value that is not a pointer fails Pointer Authentication:
`EXC_ARM_PAC_FAIL`, SIGKILL, FaceTime gone. It looked exactly like "FaceTime quits whenever we
touch the camera". Fixed in `IMCoreRuntime.send`, which no longer touches a void result, and
the same latent bug was live on `addHandler:`/`removeHandler:` in the Messages helper.

Two things nearly buried it, both worth knowing:

  - **`log` is a zsh builtin.** A bare `log show` reports "too many arguments" and prints
    nothing, which reads as "the helper logs nothing". Use `/usr/bin/log`. The claim elsewhere
    in this project that "os_log from an injected dylib inside a sandboxed host does not
    reliably reach `log show`" is FALSE: with `/usr/bin/log`, `subsystem ==
    "com.bluebubbles.facetimehelper"` shows every helper line from inside FaceTime.
  - **The crash reports were there.** A `find -newermt` with a threshold later than the runs it
    was looking for produced "no crash report", which was then used as evidence that FaceTime
    exits cleanly. It does not; it is SIGKILLed.

The WINDOW side stays closed, and is a separate mechanism: `NSApplication.hide(_:)` and
`NSRunningApplication.hide()` both no-op (`hide()` returns `false`) because the injector
launches the executable directly rather than through LaunchServices (RunningBoard resolves the
process as `app<Unknown>`), and `orderOut`/`miniaturize` make FaceTime exit through its own
`AppController` ("Presentation changed, terminate app if needed"), with no crash report, which
is the one place that phrase was accurate.
...
Since built, and verified on a SIP-off Mac (macOS 26.5.2) on 7 September 2026:

1. **Per-process request routing in the transport.** `SocketTransport.write(process:)` targets
   exactly the named helper or fails clearly, `registeredProcesses` is keyed off the bundle id
   in the registration handshake, and `PrivateAPITransport` passes `HelperHost.faceTime` for
   FaceTime actions. Untargeted actions now prefer Messages explicitly rather than "most
   recently connected": the comment there records the bug that forced it, a plain
   `check-facetime-availability` answered `unknown action` by the FaceTime helper.
2. **Injecting into and supervising FaceTime.app.** `PrivateAPIRuntime` injects both dylibs at
   startup from `configuration.faceTimeDylibPath`. Observed repeatedly: `Verified helper
   connected` then `Helper registered {inboundEvents=facetime process=com.apple.FaceTime}`,
   about 1.5s after the Messages helper.

   Note the packaging half, which was NOT done and is easy to miss: neither
   `Packaging/build-app.sh` nor `Tools/dev-bundle.sh` copied
   `libBlueBubblesFaceTimeHelper.dylib` into `Contents/Frameworks/` at all, and both built
   `arm64` rather than the `arm64e` slice FaceTime runs. Every FaceTime route was dead in a
   shipped build for that reason alone. Both scripts now build the helpers separately and
   assert the slices with `lipo`.

Still needs a Mac:

3. **On-device validation of the reliability hypothesis.** Partly answered. The registered,
   state-synced client demonstrably returns real data: the startup sweep reported
   `FaceTime cleanup on helper registration {calls=0 links=1}`, i.e. it enumerated and
   invalidated an actual link, which is exactly what the throwaway-per-call client could not
   do. What remains unexercised is the CALL flows: dial, admit, hand-off, and the Flow C drop.
...
## Outgoing calls have their own switch

`facetime_outgoing_calls` (default ON) gates `POST facetime/call`, checked in the handler
immediately after `enable_ft_private_api`. Dialling is the one FaceTime capability that reaches
OUT: it makes this Mac ring somebody else's phone on a client's say-so. So it is refusable
without giving up links, admission and call control, which are all passive.

Checked in the handler rather than by unmounting the route: a client that asks gets a reason
(`403`, "Let clients place calls is disabled on this server") instead of a 404 it cannot tell
from an older server. It is deliberately unwatched by any service: nothing about the helper or
its injection changes, so restarting the Private API service to apply a permission check would
terminate Messages and FaceTime for no reason. `WatchedSettingsTests` records that.

**It is NOT the legacy `facetime_calling`, which meant the opposite half of the feature.** In
the Electron server that key gated INCOMING calls: the UI's own text was "the server will
detect incoming FaceTime calls and forward a notification to your device", and
`PrivateApiFaceTimeStatusHandler` branched on it to choose between the rich incoming payload
and the legacy `INCOMING_FACETIME` string. Its nearest equivalent here is
`facetime_incoming_handoff`, not this. Reusing the key would have read an upgraded install's
stored `true` (meaning "I want incoming hand-off") as "let clients dial out". The legacy
declaration has been deleted rather than repurposed.

...
- **`_for` for a time something is aimed at**: `scheduled_message.scheduled_for` and
  `first_scheduled_for`, the date a recurring series counts from. Neither is an event
  time: both may be in the future and may never arrive, and calling one `scheduled_at`
  would say the scheduling happened then, which is what `created_at` already says.

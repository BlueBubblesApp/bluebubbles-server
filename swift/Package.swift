...
let package = Package(
    ...
    products: [
        ...
        .executable(name: "BlueBubblesLauncher", targets: ["BlueBubblesLauncher"]),
    ...
    dependencies: [
        ...
        // Replaces google-libphonenumber. Verified against the macOS 14 floor as the
        // dependency policy requires: platforms: .macOS(.v10_15), and, per the lesson
        // Hummingbird taught, NO availability macros, so its declared floor is its real one.
        //
        // `PhoneNumberKit/PhoneNumberKit`, NOT `marmelroy/PhoneNumberKit`. The project moved
        // organisation and the old repository says so from inside the compiler: every
        // `PhoneNumberUtility()` against 4.x raised "no longer maintained here. See the
        // README to migrate", which is why `DeprecatedDeclaration` was held at warning level
        // for this one call site. The old repository still takes commits, so neither its
        // archived flag nor its last-pushed date says what its own deprecation message does.
        ...
        // Already resolved transitively through swift-websocket; declared because BBHTTPAPI
        // imports CompressNIO directly to gzip JSON responses.
        .package(url: "https://github.com/adam-fowler/compress-nio.git", from: "1.4.0"),
        // The app's updater. A prebuilt xcframework (macOS 10.13+, no availability macros
        // above the floor). Linked by the BlueBubblesApp target ONLY: the headless CLI has
        // nothing to relaunch, and the appcast tool writes the feed without the framework.
    ...
    targets: [
        ...
        .target(
            ...
            dependencies: ["BBPrivateAPIContract"],
            swiftSettings: swiftSettings
        ),
        ...
        .target(
            ...
            dependencies: [
                ...
                .product(name: "NIOCore", package: "swift-nio"),
                // gzip for JSON responses, negotiated per request. Already in the graph
                // under swift-websocket (permessage-deflate); declared here because this
                // target imports it directly. See `ResponseCompression.swift`.
                .product(name: "CompressNIO", package: "compress-nio"),
                // `FileBodySequence` runs its reads on `NIOThreadPool.singleton`: `read(2)`
                // is a blocking syscall and a large download issues thousands of them, which
                // inline would park a cooperative thread apiece.
                .product(name: "NIOPosix", package: "swift-nio"),
                .product(name: "Logging", package: "swift-log")
            ],
            swiftSettings: swiftSettings
        ...
        .target(
            name: "BBFaceTime",
            dependencies: [
                "BBCore", "BBDiagnostics", "BBPrivateAPI", "BBPrivateAPIContract", "BBSettings",
                "BBSystem",
                .product(name: "Logging", package: "swift-log")
            ],
            swiftSettings: swiftSettings
        ...
        .target(
            ...
            dependencies: [
                ...
                "BBPrivateAPICatalog", "BBIMessage", "BBContacts", "BBSerialization", "BBPersistence",
        ...
        .executableTarget(
            ...
            dependencies: [
                ...
                // BBCore and BBSettings are for `--check-keychain`, which reaches the secret
                // store directly: the entitlement is a property of this binary's signature,
                // so the question has to be asked without composing a server.
                "BBAuth", "BBCore", "BBDiagnostics", "BBPersistence", "BBServiceKit",
                "BBSettings",
        ...
        // The login item that supervises the app. Deliberately depends on BBCore alone:
        // a supervisor that fails to launch leaves nothing running and nobody informed, so
        // it carries no database, settings or logging stack to fail on.
        .executableTarget(
            name: "BlueBubblesLauncher",
            dependencies: ["BBCore"],
            swiftSettings: swiftSettings
        ),
        ...
        .target(
            ...
            // HelperObjC is named DIRECTLY as well as arriving through HelperShared, which
            // owns the runtime shim: the send paths call `BBSuppressSendSoundFor` and the
            // load path calls `BBInstallSendSoundSuppression`, so this target imports it
            // itself rather than relying on a transitive edge.
            dependencies: [
                "BBPrivateAPIContract", "HelperBootstrap", "HelperObjC", "HelperShared",
            ],
        ...
        .testTarget(
            name: "BBPrivateAPICatalogTests",
            dependencies: ["BBPrivateAPICatalog", "BBPrivateAPIContract"],
            swiftSettings: swiftSettings
        ),
        ...
        // Declared late: the directory existed, held tests, and was not a target, so
        // nothing in it had ever compiled. A test directory that implies coverage it does
        // not have is worse than no directory, because it is the thing a reader checks.
        .testTarget(
            name: "BBDiagnosticsTests",
            dependencies: [
                "BBCore",
                "BBDiagnostics",
                .product(name: "Logging", package: "swift-log")
        ...
        // BBProxy is every command line this server hands to a program that is not
        // installed on a CI runner, so nothing in it fails to compile and nothing in it is
        // exercised by another suite. It had no test target at all.
        .testTarget(
            name: "BBProxyTests",
            dependencies: [
                "BBProxy",
        ...
        // in what order.
        // Fixtures two test targets share.
        //
        // A LIBRARY target rather than files duplicated into both, because the split below is
        // what forced the question: `InterfaceFixtures` and `FailingPrivateAPI` are used by the
        // interface suites AND by `AppContextFixture`/`AppContextWiringTests`, which stay with
        // the composition root. Two copies of a fake is how the two copies drift, and a fake
        // that has drifted is worse than no fake: both suites go on passing.
        //
        // It lives under `Tests/` and belongs to no product, so nothing ships it. Everything in
        // it uses public API only; there is no `@testable` here, which is what lets it be a
        // plain target at all.
        .target(
            name: "BBTestSupport",
            dependencies: [
                "BBIMessage", "BBPersistence", "BBSerialization", "BBPrivateAPIContract",
                // `InterfaceFixtures` materialises the empty chat.db it hands back, and
                // creating a file is something `ReadOnlyDatabase` correctly cannot do.
                .product(name: "GRDB", package: "GRDB.swift")
            ],
            path: "Tests/BBTestSupport",
            swiftSettings: swiftSettings
        ),

        // The domain layer on its own.
        //
        // These suites were in `CompositionTests` because that is where the fakes were, not
        // because they needed a composition root, and BBInterfaces, the largest module in the
        // package, had no test target of its own as a result. Every suite here stands up one
        // interface against a repository and a helper double; none builds an `AppContext`.
        //
        // What that buys is a failure that points at the interface rather than at the wiring,
        // and a test that does not pay for the whole graph to ask one question.
        .testTarget(
            name: "BBInterfacesTests",
            dependencies: [
                "BBTestSupport",
                "BBInterfaces", "BBIMessage", "BBPersistence", "BBSerialization",
                "BBPrivateAPIContract", "BBContacts", "BBCore", "BBDiagnostics",
                // `UpdateAnnouncerTests` stands up a real bus with a capturing sink.
                "BBEvents",
                // `GroupChatCreationTests` covers the Shortcut path, which is how a group
                // is created without the Private API.
                "BBShortcuts",
                // `SendShapeTests` diffs the send response against the recorded reference
                // fixture; the send routes are deny-listed in the parity replay, so this is
                // the only place that comparison happens.
                "BBParity",
                .product(name: "GRDB", package: "GRDB.swift")
            ],
            swiftSettings: swiftSettings
        ...
        .testTarget(
            ...
            dependencies: [
                ...
                // `UpdateWaitTests` drives the update-install seam, whose types are here.
                "BBUpdates",
                ...
                // `ProxyAttentionAlertTests` drives a `ProxyService` through a fake
                // `ProxyProviding`, which is a BBProxy protocol.
                "BBProxy",
        ...
        .executableTarget(
            ...
            dependencies: [
                ...
                "BBOpenAPI",
                // `APIDocsRelay` holds the reference window's request relay to the same
                // forbidden-header list the CORS middleware enforces, rather than a second
                // copy of it that can drift. Linked already, through BlueBubblesServerCore.
                "BBHTTPAPI",
                "BBCore",
                .product(name: "Logging", package: "swift-log"),
                // The updater. See `SparkleUpdater.swift` for why it is an app concern.
                .product(name: "Sparkle", package: "Sparkle")
            ],
            // The module's own guide. Every other target with one excludes it; this one
            // did not, which is the "1 file(s) which are unhandled" warning the build
            // emitted.
            exclude: ["CLAUDE.md"],
            resources: [
                ...
                // The product logo, drawn at the head of the sidebar. `.copy` so the
                // lookup path is exactly `Branding/Logo.png`: `.process` may rename or
                // recompress an image asset, and `Branding.logo` resolves it by name.
                .copy("Resources/Branding")
            ],
            swiftSettings: swiftSettings,
            // Sparkle is a dynamic framework, and SwiftPM links it against the copy in
            // `.build/artifacts` with an rpath to THAT location, which is fine for
            // `swift run` and useless once the binary is inside a bundle. The packaging
            // scripts copy the framework into `Contents/Frameworks`; this is the rpath that
            // finds it there. Without it the packaged app dies at load with an
            // "image not found" for a path inside a build directory on the release runner.
            linkerSettings: [
        ...
        // The CLI's own surface: which flags exist, and what `--set` turns into. A test
        // target CAN depend on an executable target, and doing so tests the real command
        // rather than a copy of its option list.
        .testTarget(
            name: "BlueBubblesServerTests",
            dependencies: [
                "BlueBubblesServer",
                .product(name: "ArgumentParser", package: "swift-argument-parser"),
            ],
        ...
        .testTarget(
            ...
            dependencies: [
                ...
                // `DaemonLedgerTests` hands the ledger a logger.
                .product(name: "Logging", package: "swift-log"),
        ...
        .testTarget(
            ...
            dependencies: [
                ...
                .product(name: "NIOCore", package: "swift-nio"),
                .product(name: "NIOPosix", package: "swift-nio"),
                // `TransactionStore` is constructed with a logger, so the ordering tests
                // name one directly.
                .product(name: "Logging", package: "swift-log")
            ],
            swiftSettings: swiftSettings
        ...
        .testTarget(
            ...
            dependencies: [
                ...
                "BBTestSupport", "BBPrivateAPIContract",
                .product(name: "Hummingbird", package: "hummingbird"),
                .product(name: "Logging", package: "swift-log")
            ],
        ...
        .testTarget(
            name: "BBHTTPAPITests",
            // BBSerialization for `JSONValue`, which request validation reads; BBAuth for
            // `Scope` and `AuthenticatedPrincipal`, which the scope-enforcement tests build;
            // NIOCore for `ByteBuffer`, which `FileStreamingTests` reads chunks out of.
            dependencies: [
                "BBHTTPAPI", "BBSerialization", "BBAuth",
                .product(name: "NIOCore", package: "swift-nio"),
                // `ResponseCompressionTests` DECOMPRESSES what the server produced, so the
                // round trip is asserted rather than the header alone.
                .product(name: "CompressNIO", package: "compress-nio")
            ],
            swiftSettings: swiftSettings
        ),
        ...
        .testTarget(
            ...
            // `BBOpenAPI` so the response SHAPES a handler builds can be checked against
            // what the document declares. Nothing else connects the two: the schema tables
            // are hand-written for routes with no recorded fixture, so without a test that
            // executes the serializer they are testimony nothing cross-examines.
            dependencies: [
                "BBHandlers", "BBHTTPAPI", "BBSerialization", "BBIMessage", "BBSystem",
                "BBInterfaces", "BBPrivateAPIContract", "BBOpenAPI",
        ...
        .testTarget(
            ...
            dependencies: [
                ...
                // `SendLaterGuidanceTests` reads the Send Later capability's macOS floor.
                "BBPrivateAPICatalog",
                // `FirebaseSetupModelTests` stands the model up against a real `PushInterface`
                // over an empty in-memory store: `PushCredentialStore` is a BBPushKit type,
                // and file inspection needs no network.
                "BBPersistence", "BBPushKit",
                // `ContactFilteringTests` builds `ContactRecord` values: the contacts page's

...
//
//  Nothing else is scripted: GUI puppetry has Private API equivalents. See
...
//
//  Terms, on the macOS 14 floor
//  ----------------------------
//  Verified against the Messages `sdef` on macOS 26, whose only classes are `participant`,
//  `account`, `chat` and `file transfer`:
//
//      `service`   -> `account`      (Ventura+)
//      `buddy`     -> `participant`  (Big Sur+)
//
//  No pre-Sonoma spelling is needed, and `make new text chat` -> `make new chat` is moot: on
//  Big Sur+ neither works. See above.
...
public enum MessagesScripts {
  ...
  public static func supportedServices(
    ...
    // MEMOISED, and that is not an optimisation so much as a bug fix.
    //
    // This forks `/usr/bin/sdef` and blocks the calling thread on it, measured at roughly
    // twenty milliseconds. It is reached from a DEFAULT ARGUMENT on
    // `AppleScriptMessageSender()`, which `ChatInterface.messages` evaluated by constructing
    // a `MessageInterface` per call — so `GET /chat/:guid/message`, the route a full sync
    // pages every conversation through, forked a subprocess and blocked a cooperative pool
    // thread on every request, for an answer that cannot change while Messages.app is the
    // one on disk.
    //
    // Keyed by path because the argument exists and tests pass a fixture. The lock rather
    // than a `static let` for the same reason: a `static let` cannot take a parameter.
    cacheLock.lock()
    if let cached = serviceCache[applicationPath] {
      cacheLock.unlock()
      return cached
    }
    cacheLock.unlock()
    ...
    if let definition = scriptingDefinition(at: applicationPath),
      definition.contains("\"RCS\"") || definition.contains("name=\"RCS\"")
    {
      available.append(.rcs)
    }

    // Two callers racing here both probe and both store the same answer, which costs one
    // extra fork on a cold start and avoids holding a lock across a subprocess.
    cacheLock.lock()
    serviceCache[applicationPath] = available
    cacheLock.unlock()
    return available
  }

  private static let cacheLock = NSLock()
  private nonisolated(unsafe) static var serviceCache: [String: [MessagingService]] = [:]

  /// Drops the memoised answer. For tests that change what `sdef` would report.
  public static func forgetSupportedServices() {
    cacheLock.lock()

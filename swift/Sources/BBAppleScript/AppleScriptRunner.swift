...
//
//  Why compiled scripts with parameters
//  ------------------------------------
//  Shelling out to `osascript` with the message text interpolated into the source forces
//  escaping for TWO layers at once (the shell and the AppleScript parser) and every send
//  then re-parses the script, spawns a process, and depends on that escaping being exactly
//  right for arbitrary user text. A quote in a message is a broken send at best.
//
...
//  run-loop thread compiles, looks reasonable, and blocks forever.
//
...
//      seconds, on a real one, and deadlocks outright under a parallel test runner.
...
/// something outside it begins.
///
/// With a concrete runner the only paths a test could reach would be the validations that
/// run BEFORE the script does. What a `-1743` becomes, whether a failure is worth retrying
/// under a different spelling of the chat GUID, and what a caller is told when no spelling
/// works are the paths a user actually meets.
...
public actor AppleScriptRunner {
  ...
  public func run(
    ...
    // The handler and the time, never `arguments`: they carry the message text and the
    // attachment path.
    switch outcome {

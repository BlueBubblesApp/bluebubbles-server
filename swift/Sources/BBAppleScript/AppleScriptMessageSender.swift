...
public enum MessageSendError: BBError, Equatable, CustomStringConvertible {
  ...
  public var description: String {
    ...
    case .invalidChatGUID(let guid):
      // The hint is the same one `PrivateAPIErrorBridge.noSuchChat` gives on the helper
      // side, deliberately worded alike: the two backends fail the same mangled request,
      // and a reader who sees one sentence here and a different one there has to work out
      // whether they are the same problem. Kept short; the long version is in
      // `docs/api/README.md` § Multipart uploads.
      "\(guid) is not a valid chat identifier. Expected `service;-;address` or "
        + "`service;+;chat…`; a GUID cut short at a `;` usually means the tool that sent it "
        + "read the semicolon as a separator (`curl -F` does; use `--form-string`)."
...
public actor AppleScriptMessageSender {
  ...
  public func send(
    ...
    for candidate in normalized.lookupCandidates() {
      ...
      do {
        ...
        if attempted.count > 1 {
          // Worth a line: it means this database spells the GUID differently from the
          // client, and every send is paying for the misses before this one.
          logger.debug(
            "Chat resolved under a different service prefix",
            metadata: [
              "attempts": .stringConvertible(attempted.count),
              "chat": .string(Redaction.chatGUID(candidate)),
            ])
        }
        return candidate
    ...
    // The case name only: `invalidChatGUID` carries the GUID, which is an address.
    logger.debug(
      "No spelling of the chat GUID resolved",
      metadata: [
        "attempts": .stringConvertible(attempted.count),
        "lastError": .string(lastError.code),
      ])

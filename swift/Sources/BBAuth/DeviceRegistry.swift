...
public actor DeviceRegistry {
  ...
  public func enroll(
    ...
    // The client id rather than the name: a device name is the one the person gave their
    // phone, and is usually their own. The id is what every later line about this device
    // carries anyway, so it is also the one that joins them up.
    logger.info(
      "Enrolled a device",
      metadata: [
  ...
  ///
  /// Rate-limited to one write a minute per device. This ran on EVERY authenticated request:
  /// a read of a row the caller had already loaded, then a SQLite write transaction, to move
  /// a timestamp nobody reads at sub-minute granularity. On a spinning disk that is an fsync
  /// per request, and the Devices screen shows "last seen" as a relative time.
  ///
  /// Written when the stored value is older than the window, so a device that has been quiet
  /// still records its first request immediately.
  public func noteActivity(clientId: String, now: Date = Date()) async {
    ...
    if let seen = device.lastSeenAt, now.timeIntervalSince(seen) < Self.activityWriteInterval {
      return
    }

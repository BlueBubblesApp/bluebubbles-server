...
//  identically to the reference's AuthMiddleware. Everything else in BBAuth is dormant code
...
public enum AuthenticationFailure: BBError, Equatable {
  ...
  public var countsAsAttempt: Bool {
    ...
    case .serverMisconfigured: false
    }
  }
}
...
///
/// Two changes from the reference's middleware, neither observable to a client:
...
public struct PasswordQueryScheme: AuthenticationScheme {
  ...
  public func authenticate(
    ...
    guard let password = await passwordProvider() else {
      ...
      // unconfigured case below. Naming the Keychain matters: saying "database" for both
      // sends anyone debugging this at the wrong subsystem.
  ...
  static func extractCredential(from presentation: CredentialPresentation) -> String? {
      ...
      let pair = String(data: decoded, encoding: .utf8)
    {
      ...
      //
      // `omittingEmptySubsequences: false`, which is not the default and is the whole point.
      // With the default, `"user:"` splits to `["user"]` and `.last` is the USERNAME, so a
      // Basic header carrying an empty password compared the username against the server
      // password — accepting `base64("thepassword:")`, and reading the wrong field on every
      // header of that shape. Never a false accept of a wrong credential, but not the parse
      // it reads as.
      //
      // Requiring exactly two components also refuses a header with no colon at all, which
      // is not a `user:password` pair and was previously taken as a bare password.
...
public struct SocketHandshakeScheme: AuthenticationScheme {
  ...
  public static func normalize(query: [String: String]) -> [String: String] {
    // A BEARER TOKEN passes through, and without this the socket could not authenticate at
    // all under `auth_mode = token`.
    //
    // In that mode the chain holds only `BearerTokenScheme`, which reads `auth_token` or
    // `access_token`. Stripping everything but `password`/`guid` meant the chain was handed
    // an empty presentation, found no credential, and closed the session: no client could
    // open a socket on a token-mode server. The test named for this called the bearer scheme
    // directly with an `auth_token` and never touched this function or the transport, so it
    // passed throughout.
    //
    // Safe under the other modes rather than a widening of them: `password` mode's chain has
    // no scheme that reads these names, so a token presented there authenticates nothing,
    // exactly as before. What the header below says about `token` still holds — that is a
    // different, reference-era parameter name and it is still not accepted.
    for name in ["auth_token", "access_token"] {
      if let token = query[name], !token.isEmpty { return [name: token] }
    }
    ...
    //
    // Decoding again is precisely the bug the reference has: it calls `decodeURI`

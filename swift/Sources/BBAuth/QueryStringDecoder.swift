...
//
//  Passwords arrive in a URL, and three distinct mistakes all look like "the server rejected
//  my correct password":
//
//  1. **Double decoding.** A socket handshake query that engine.io has ALREADY
//     percent-decoded, decoded again. A password containing a literal `%` followed by two
//     hex digits is then decoded twice: the client correctly sends `a%2520b` for the
//     password `a%20b`, and the server ends up with `a b`. The user's password is unusable
//     and nothing explains why.
//
//  2. **`+` handled inconsistently.** Node's `querystring.parse` turns `a+b` into `a b`;
//     `decodeURI` does not. An HTTP path and a socket path using different decoders disagree
//     about a password containing a space.
//
...
public enum QueryStringDecoder {
  ...
  /// Taking a parsed one is how a double decode happens: the caller cannot tell whether

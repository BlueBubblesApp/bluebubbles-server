...
public actor TokenIssuer {
  ...
  public func issue(
    ...
    let header = ["alg": Self.algorithm, "typ": "JWT"]
  ...
  /// The one algorithm this server signs and accepts. Named once so the writer and the
  ...
  public func verify(_ token: String, now: Date = Date()) async throws -> TokenClaims {
    ...
    // The HEADER is checked, not merely used as signing input.
    //
    // This is not exploitable today: verification is unconditionally Ed25519 against this
    // server's own key, so `alg: none` and every other substitution is rejected by the
    // signature check below whatever the header claims. It is checked anyway because the
    // omission is the one that becomes a vulnerability the day a second algorithm is
    // accepted, and by then nothing here would remind anyone.
    guard let headerData = Self.decodeBase64URL(String(parts[0])),
      let header = try? JSONSerialization.jsonObject(with: headerData) as? [String: Any],
      header["alg"] as? String == Self.algorithm
    else { throw TokenError.malformed }

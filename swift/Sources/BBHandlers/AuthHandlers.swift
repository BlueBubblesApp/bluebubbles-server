...
public enum AuthHandlers {
  ...
  public static func register(
    ...
    context: some TokenAuthProviding & AccessControlProviding
  ) {
  ...
  private static func token(
    ...
    do {
      ...
      // COUNTED, and it has to be counted here rather than by the middleware.
      //
      // This is the one route whose entire job is checking a credential, and its credential
      // is a `client_secret` in the BODY, which the auth chain (query parameters and the
      // Authorization header) cannot see. So the middleware admits the caller and this
      // handler decides, which left secret-guessing neither blocked nor counted: the
      // `Unauthorized` below is thrown past the stage that records failures.
      //
      // Recording it here puts this route on the same lockout as every other credential in
      // the server, against the same identity the middleware already resolved.
      // `auditPath`, like the middleware. The two auth routes carry no path parameter, so
      // this is the same string either way; it is spelled this way so that NO call site
      // reads `.path` for a failure record and the rule needs no exceptions to remember.
      await context.accessControl.recordFailure(
        request.identity, path: request.auditPath, reason: "client credential rejected"
      )

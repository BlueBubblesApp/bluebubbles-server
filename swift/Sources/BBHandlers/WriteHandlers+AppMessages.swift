...
extension WriteHandlers {
  static func registerAppMessages(
    ...
    registry.register(.messageCreatePoll) { request in
      let interfaces = try await context.requireInterfaces()
      let values = try request.values()
      let options = try Self.strings(values["options"], named: "options")
    ...
    registry.register(.messageVotePoll) { request in
      ...
      let optionIDs = try Self.strings(values["optionIds"], named: "optionIds")
  ...
  /// A list of strings, with a mistyped element refused rather than dropped.
  ///
  /// `compactMap(\.stringValue)` turned `["a", 2, "b"]` into a two-option poll answered 200,
  /// so a client with an encoding bug got a poll missing an option and no way to know. These
  /// are v2 routes with no reference sentence to match, so the wording is ours; the rule is
  /// the project's own: apply what was sent or refuse it.
  static func strings(_ raw: JSONValue?, named key: String) throws -> [String] {

...
public enum StickerHandlers {
  ...
  /// SHAPE a client has to decode and it was invisible as a literal: the OpenAPI table
  /// documented only the readable-store shape, so the document declared `identifier`,
  /// `kind` and `effect` required on a response that carries none of them and omitted
  /// `saved`, which appears nowhere else in the API. `StickerResponseShapeTests` now holds
  /// this against `ResponseBodies`.

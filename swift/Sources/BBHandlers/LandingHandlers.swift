...
//
//  A port of `UiRouter.index` (packages/server/.../routers/uiRouter.ts). It matters more
//  than a landing page usually does: it is how people check a tunnel. The first thing
//  someone does after configuring cloudflared is paste the URL into a browser, and a 404
//  there reads as "the tunnel is broken" when the tunnel is fine. `landing_page_path`
//  carries across from an Electron install, so an operator's own page has to keep working.
//

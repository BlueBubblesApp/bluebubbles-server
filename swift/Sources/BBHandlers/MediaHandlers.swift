...
public enum MediaHandlers {
  ...
  /// The `only` filter, normalised the way the reference normalises it: comma-separated,
  /// lower-cased, and de-pluralised so `chats` and `chat` are the same ask.
  ///
  /// Not private: `AdminHandlers` applies the same parameter to
  /// `GET /server/statistics/totals`, and two copies of this would be two answers to
  /// "did the client say chats or chat".
  static func requestedCategories(_ only: String?) -> Set<String>? {
  ...
  private static func registerNicknames(
    ...
    registry.register(.chatShouldShareContact) { request in
      ...
      // A BARE BOOLEAN, not an object. The reference sends `data: canShare`
      // (`chatRouter.shouldShareContact`), and `get_api_v1_chat_…_share_contact_status`
      // records `"data": false`. Wrapping it in `{shouldShare: …}` made every answer
      // truthy to a client reading `data` directly, so "do not offer" read as "offer".
      return .data(.bool(should))
    }

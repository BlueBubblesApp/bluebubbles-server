...
extension InterfaceError: HTTPError {
  ...
  public var status: Int {
    ...
    // 403, the same answer `FindMyHandlers.requireFeature` gives for a switched-off
    // feature flag: the route exists and the caller may not use it, which is not a 404.
    // Not 503 either: that says "try later", and this will not resolve until somebody
    // flips a switch. The reference has no notion of a disabled service, so there is no
    // shipped response to diverge from here.
    case .serviceDisabled: 403
    }
  }
  ...
  public var errorType: ErrorType {
    ...
    // Matching `Forbidden`, so a disabled integration and a disabled feature flag are one
    // shape on the wire rather than two.
    case .serviceDisabled: .authenticationError
    }
  }

  /// The envelope's sentence: NOT the short `ResponseMessage` value.
  ///
  /// Deferred to the error types themselves rather than transcribed again here. Nearly every
  /// failure a handler produces arrives as an `InterfaceError`, so this decides the `message`
  /// on most of the error responses the server sends, and a second copy of six sentences
  ...
  public var responseMessage: String {
    ...
    // `Forbidden`'s own envelope sentence, for the same reason: one shape.
    case .serviceDisabled: Forbidden().responseMessage
    }
  }
  ...
  ///
  /// `helperUnavailable` does not: the reference's Private API gate sends the envelope and
  /// nothing else, and the two-way diff treats a key we add exactly like one we drop. The
  /// feature name goes to the log instead.
  ///
  ...
  public var data: JSONValue? {
    ...
    // Named so a client can say WHICH integration to switch on rather than repeating the
    // sentence. `requireFeature` sends no data, but it also names its flag in the sentence;
    // this is the same information in a form a client can act on.
    case .serviceDisabled(_, let service): .object(["service": .string(service)])
    default: nil
    }
  }
}

...
//
//  Off the view for the reason `SettingsTabs` and `WebhookEventCatalog` are, which is NOT that
//  the app cannot be tested: `BlueBubblesAppTests` depends on the executable target and runs
//  fine. The reason is that a decision embedded in a `body` can only be asserted by rendering
//  one, and a list's order, grouping and cut-off are facts about the list rather than about
//  how it is drawn. Here they are values a test reads directly.
//
//  So what is here is every decision worth getting wrong: which groups appear, in what order,
//  what each heading says, and where a collapsed list is cut. The view maps these to rows and
//  owns nothing but symbols and spacing.
//

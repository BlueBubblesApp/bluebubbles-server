...
///
/// Requiring `Actor` makes the service its own isolation domain, so a plain `private var` is
/// both safe and checked: rather than each service pushing its mutable state into private
/// single-purpose actors, or hiding it behind `@unchecked Sendable`. The registry awaits
/// every call into a service, so the requirement costs nothing above.
...
///
/// The registry routes a change only to services whose `watchedSettings` intersect it.
///

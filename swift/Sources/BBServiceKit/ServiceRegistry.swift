...
public actor ServiceRegistry<Host: Sendable> {
  ...
  /// Services whose `start()` is in flight, for `health()`.
  ...
  /// declined its gate and one with a missing permission would both report the same
  /// "not started".
  private var inactiveReasons: [ServiceIdentifier: String] = [:]
  /// Everyone following `healthChanges()`.
  private var healthSubscribers:
    [UUID: AsyncStream<[ServiceIdentifier: ServiceHealth]>.Continuation] = [:]
  ...
  /// Consulted before starting a service that declares a required permission, so a missing
  ...
  public func register<S: Service>(_ type: S.Type) where S.Host == Host {
    register(type, from: { $0 })
  }

  /// Registers a service built from a PROJECTION of the host rather than the whole thing.
  ///
  /// The registry is generic over one host so that "this service needs the application
  /// context" is checked by the compiler. That constraint is worth keeping, but stating it
  /// as `S.Host == Host` forced every service to name the whole container even when it
  /// touches two members of it, so the container was every service's declared dependency
  /// and none of them said what it actually used.
  ///
  /// This overload keeps the check and moves where it lands: `S.Host` is whatever the
  /// service asked for: commonly a composed capability existential such as
  /// `any SettingsProviding & AlertProviding`, and the closure is the one place that has to
  /// know the host can supply it. A host that cannot is still a compile error, raised at the
  /// registration site rather than inside an initialiser.
  public func register<S: Service>(
    _ type: S.Type,
    from project: @escaping @Sendable (Host) -> S.Host
  ) {
    let watched = (type as? any ConfigurableService.Type)?.watchedSettings ?? []
    let permissions = type.requiredPermissions
    registrations[type.id] = Registration(
      id: type.id,
      dependencies: type.dependencies,
      manifest: type.manifest,
      restartPolicy: type.restartPolicy,
      make: { host in S(host: project(host)) },
  ...
  private func markStart(_ id: ServiceIdentifier, inFlight: Bool) async {
  ...
  private func performStart(_ id: ServiceIdentifier) async {
    ...
    if let culprit = registration.dependencies.first(where: { blocked.contains($0) }) {
      ...
      // The same sentence `applyEnablement` records, so one condition reads one way
      // whether it arose at startup or from a switch. It also says no identifier: this
      // reaches a screen, and `app.bluebubbles.core.http` names nothing a person can act
      // on. Whoever renders it holds the manifests and can say "Needs HTTP API"; the
      // culprit is in the log line above for anyone reading a log.
      inactiveReasons[id] = "a service it depends on is switched off"
      blocked.insert(id)
      return
    }
  ...
  public func restart(_ id: ServiceIdentifier) async {
    logger.info("Restarting service", metadata: ["service": .string(id.rawValue)])
    supervisors[id]?.cancel()
    await serialized(id) {
  ...
  public func restartWithDependents(_ id: ServiceIdentifier) async {
    ...
    logger.info(
      "Restarting service with its dependents",
      metadata: [
        "service": .string(id.rawValue),
  ...
  public func apply(_ change: SettingsChange) async {
    ...
    // Services with NO INSTANCE whose gate reads one of the keys that just changed.
    //
    // The loop above can only route a change to `instances[id]`, and a `GatedService` that
    // declined at startup does not have one. Without this, turning the Private API on would
    // write `enable_private_api`, reach nothing, and leave the helper uninjected until the
    // next full restart of the server: indistinguishable, from the UI, from the switch not
    // working.
    //
    // `applyEnablement` covers the same shape for the Integrations switch; a gate is the
    // other way a service can be absent. A gate reads settings, so the keys that can open
    // it are the keys the service already declares it watches.
    //
    // `performStart` re-checks permissions, enablement and the gate, and returns early when
    // an instance exists, so an attempt that should still decline simply declines again,
    // and one `applyEnablement` has already started is a no-op.
    for id in startOrder
    where instances[id] == nil
      && registrations[id].map({ change.intersects($0.watchedSettings) }) == true
    {
      await start(id)
    }
  ...
  private func applyEnablement() async {
    ...
    // The REASON is recorded for everything in the set, and the stop only for what is
    // actually running. Those are different populations, and conflating them reported the
    // wrong thing: switch off the HTTP API and its socket together, then switch the SOCKET
    // back on, and the socket is enabled but still blocked: it cannot run without the
    // listener. It was not running, so it was not visited, so it went on saying "switched
    // off" about itself when the truth had moved to the thing it depends on.
    for id in startOrder.reversed() where toStop.contains(id) {
      if instances[id] != nil {
        logger.info(
  ...
  /// Every service's health, again whenever any of it changes.
  ///
  /// A snapshot rather than a per-service delta, because a consumer draws the whole table
  /// and a delta would leave it reconciling. Published after every lifecycle transition the
  /// registry performs and whenever a service reports through `noteHealthChanged()`; the
  /// newest snapshot is kept for a slow consumer, so it sees the current state rather than a
  /// backlog. A consumer that needs the state before the first change reads `health()`.
  public func healthChanges() -> AsyncStream<[ServiceIdentifier: ServiceHealth]> {
    let (stream, continuation) = AsyncStream.makeStream(
      of: [ServiceIdentifier: ServiceHealth].self, bufferingPolicy: .bufferingNewest(1)
    )
    let id = UUID()
    healthSubscribers[id] = continuation
    continuation.onTermination = { [weak self] _ in
      Task { await self?.removeHealthSubscriber(id) }
    }
    return stream
  }

  /// A service's health moved on its own (a tunnel dropped, a helper connected) and the
  /// registry did not perform the transition, so it is told.
  public func noteHealthChanged() async {
    await publishHealth()
  }

  private func removeHealthSubscriber(_ id: UUID) {
    healthSubscribers[id] = nil
  }

  private func publishHealth() async {
    guard !healthSubscribers.isEmpty else { return }
    let snapshot = await health()
    for continuation in healthSubscribers.values {

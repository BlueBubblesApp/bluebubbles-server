...
public struct SettingsScope: Sendable {
  ...
  ///
  /// It does not waive the checks below. A service compiled into this binary can read
  /// `app.db` directly, so containing it would be theatre: true as a SECURITY argument and
  /// beside the point. The entitlement list is what a person is shown, and a built-in
  /// reading a setting it never declared makes that list a description of a different
  /// program. Enforcing it for everyone is what keeps the list true; this was never a
  /// sandbox.
  public init(

...
public struct SettingsScope: Sendable {
  ...
  /// Accepted and ignored, so a manifest can still say what it is.
  ///
  /// It used to waive both checks below, on the reasoning that a service compiled into this
  /// binary can read `app.db` directly, so containing it would be theatre. That is true as a
  /// SECURITY argument and beside the point. The entitlement list is what a person is shown,
  /// and a built-in reading a setting it never declared makes that list a description of a
  /// different program. Enforcing it for everyone is what keeps the list true — the sandbox
  /// argument is unaffected, because this was never a sandbox.
  public init(
  ...
  public func checkRead(_ key: String) throws {
    ...
    if readable.contains(key) { return }
  ...
  public func checkWrite(_ key: String) throws {
    if ownsKey(key) { return }
    if writable.contains(key) { return }

...
/// The versions of a program this service's code can actually drive.
///
/// A field a BUILT-IN needs today, which is the bar the frozen manifest surface sets: zrok
/// needs the upper bound now, because zrok 2 removed `zrok share reserved` and `Tunnels.zrok`
/// invokes it, so a copy already on the Mac satisfies any floor and then fails at runtime on a
/// machine nobody is sitting at; and ngrok needs the floor now and can never derive one,
/// because a `.rollingURL` source cannot carry a `recommended` at all.
///
/// **This is not `recommended`, and the difference is the whole point.** `recommended` is the
/// one build we tested and will DOWNLOAD. This is the span of builds the code as written
/// drives, and it is strictly wider. Deriving the floor from `recommended` would make a
/// CalVer tool permanently ineligible — no copy already on a Mac is ever newer than a pin on
/// the day it is cut — which would switch this off for cloudflared, the default connection
/// method, without anything saying so.
///
/// A tool that declares no range is never adopted from disk. A manifest that has not said
/// which versions its code drives has not earned the right to have an unknown binary run on
/// its behalf.
public struct ToolVersionRange: Sendable, Codable, Equatable {

  /// Inclusive. Nil means no floor.
  public let minimum: String?

  /// EXCLUSIVE, because what is being excluded is a major that changed the command set, and
  /// an inclusive bound would have to name a version that does not exist yet.
  public let exclusiveMaximum: String?

  public init(atLeast minimum: String? = nil, below exclusiveMaximum: String? = nil) {
    self.minimum = minimum
    self.exclusiveMaximum = exclusiveMaximum
  }

  /// Whether a version this program reported is one this service can drive.
  ///
  /// Through `SemanticVersion`, never string comparison: lexically `2024.9.1` sorts above
  /// `2024.10.0`, and cloudflared is CalVer.
  public func contains(_ version: String) -> Bool {
    let candidate = SemanticVersion(version)
    if let minimum, candidate < SemanticVersion(minimum) { return false }
    if let exclusiveMaximum, candidate >= SemanticVersion(exclusiveMaximum) { return false }
    return true
  }

  /// Whether the range can be satisfied at all. A manifest declaring one that cannot is
  /// refused by `ManifestValidator` rather than silently refusing every copy.
  public var isSatisfiable: Bool {
    guard minimum != nil || exclusiveMaximum != nil else { return false }
    guard let minimum, let exclusiveMaximum else { return true }
    return SemanticVersion(minimum) < SemanticVersion(exclusiveMaximum)
  }

  /// Written for a person, because it goes in the sentence explaining a refusal.
  public var summary: String {
    switch (minimum, exclusiveMaximum) {
    case (.some(let low), .some(let high)): "\(low) or newer, below \(high)"
    case (.some(let low), nil): "\(low) or newer"
    case (nil, .some(let high)): "anything below \(high)"
    case (nil, nil): "any version"
    }
  }
}
...
public struct ManagedToolDescriptor: Sendable, Codable, Equatable, Identifiable {
  ...
  /// The versions of this program the declaring service can drive, for a copy that is
  /// ALREADY on the Mac. See `ToolVersionRange`.
  ///
  /// Nil switches off adoption of a pre-existing copy for this tool: nothing is scanned for,
  /// and an explicit choice is accepted on the user's say-so rather than on a version test
  /// there is no bar to run.
  public let compatible: ToolVersionRange?
  ...
  public init(
    ...
    compatible: ToolVersionRange? = nil,
  ...
  /// Whether a copy of this program already on the Mac may be used.
  ///
  /// False for a tool that declared no compatible range, which is the default: adopting a
  /// binary nobody here has vouched for, on no version evidence, is the thing the range
  /// exists to prevent.
  public var acceptsPreexistingCopies: Bool { compatible != nil }

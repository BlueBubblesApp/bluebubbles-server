...
public enum ServiceCategory: String, Sendable, CaseIterable, Codable {
  ...
  /// The glyph for a service in this category that declares none of its own.
  public var symbol: String {
    switch self {
    case .reverseProxy: "network"
    case .eventSink: "bell"
    case .messaging: "message"
    case .messageSource: "tray.full"
...
/// A macOS permission a service needs, why it needs it, and how much it matters.
///
/// **This is a declaration, not a control, and it cannot become one.** macOS grants TCC
/// permissions to an APPLICATION, not to code within it: once the user gives BlueBubbles Full
/// Disk Access, every line in the process has it. Nothing here can prevent a service from
/// benefiting from a grant it never declared.
///
/// What it is for is the thing the user actually needs: knowing, before they enable
/// something, what it will be able to reach. It is shown on a service's page today, and it is
/// what a third-party plugin's approval screen will be built from. That makes accuracy a
/// correctness property even though nothing enforces it: a manifest that under-declares is a
/// manifest that lies to the person deciding whether to trust it.
///
/// A struct rather than an enum on purpose. Permissions gain attributes over time: a
/// deep link, a minimum OS, a "this one needs a relaunch" flag, and a struct absorbs those
/// without breaking every exhaustive `switch` a plugin author wrote against last year's
/// version.
public struct ServicePermission: Hashable, Sendable, Codable {

  public let id: PermissionID
  /// Whether the service simply cannot run without it, runs in a reduced form, or needs it
  /// only for one named feature.
  public let requirement: PermissionRequirement
  /// THIS service's reason, in its own words, not the permission's generic description.
  ///
  /// "so it can watch your message database for new messages" says something the app-wide
  /// catalogue entry for Full Disk Access cannot: which of the things that permission allows
  /// is the one this service actually does. Required to be non-empty, because a permission
  /// with no stated reason is one a user can only refuse or blindly accept.
  public let purpose: String

  public init(_ id: PermissionID, _ requirement: PermissionRequirement, purpose: String) {
    self.id = id
    self.requirement = requirement
    self.purpose = purpose
  }

  /// One sentence for the permissions list.
  ///
  /// Takes a resolver for the same reason `Entitlement.userFacingDescription` does: the
  /// human title of a permission lives in the layer that can probe it, and a manifest must be
  /// describable without importing that layer. An unrecognised id renders as its raw
  /// identifier rather than vanishing from the list.
  public func userFacingDescription(namingPermission resolve: (PermissionID) -> String) -> String {
    let name = resolve(id)
    return switch requirement {
    case .required: "\(name): required, \(purpose)"
    case .recommended: "\(name): optional, \(purpose)"
    case .feature(let feature): "\(name): needed for \(feature), \(purpose)"
    }
  }
}
...
public enum Entitlement: Hashable, Sendable, Codable {
  ...
  ///
  /// A SETTING IS NAMED BY ITS STORAGE KEY, deliberately. This used to take a resolver that
  /// swapped in the label from the settings screen; three things were wrong with that.
  ///
  /// The key is the identity. It is unique, it exists for every setting, and it is the same
  /// string the reader meets in the config file, in `--set`, in the API and in the logs, so
  /// it is the one they can go and check. A label is none of those: it exists so a settings
  /// row has a heading, and two settings may reasonably carry similar ones.
  ///
  /// The label also lives inside a setting's PRESENTATION, which pointed a list about
  /// CAPABILITY at a decision about UI. Someone rewording a settings row would have quietly
  /// changed what this list claims a service can touch, with no test failing and nothing to
  /// connect the two.
  ///
  /// And the resolver could not answer for every key, which produced the state this
  /// replaced: a service's read line came back in friendly names and its write line in
  /// storage keys, because almost everything a service WRITES is internal bookkeeping with
  /// no presentation to take a label from. One list, two vocabularies, decided by whether a
  /// setting happened to have a screen.
  ///
  /// `Settings.label(forKey:)` still exists and still has a job: the restart notice names a
  /// setting the person has just edited, and a label is right there. This is a different
  /// surface, with a different reader and a different question.
...
/// A rule a field's value must satisfy before it is stored.
///
/// **Data rather than a closure, for the same reason `FieldCondition` is: a third-party
/// manifest has to be able to express it.** That rules out handing each service a validator
/// function, and it is why this carries a pattern instead.
///
/// The rule it exists for is zrok's reserved share name, which zrok states as "must be
/// lowercase alphanumeric, between 4 and 32 characters in length, screened for profanity".
/// Without it, an invalid name is accepted by the form, refused by zrok, and reported only
/// as an alert: on one Mac a `bb-test` typed into that field produced 175 failed reserve
/// attempts in 71 seconds behind a drawer nobody had open.
///
/// The profanity screen is deliberately NOT modelled. It is zrok's list, it is not published,
/// and a guess at it would refuse names zrok accepts. This checks what can be checked and
/// leaves the rest to the server, which is why a failure from zrok still has to be reported.
public struct FieldValidation: Sendable, Codable, Equatable {
  /// An `NSRegularExpression` the whole value must match. Anchoring is the pattern's job.
  public let pattern: String
  /// What is wrong, in the user's terms. Shown under the field, so it says what a valid
  /// value looks like rather than naming the rule that refused it.
  public let message: String
  /// Whether an empty value is acceptable, which for an optional field it usually is:
  /// zrok generates a name when none is given, so empty means "you choose" and not "invalid".
  public let allowsEmpty: Bool

  public init(pattern: String, message: String, allowsEmpty: Bool = true) {
    self.pattern = pattern
    self.message = message
    self.allowsEmpty = allowsEmpty
  }

  /// The reason this value is unacceptable, or nil when it is fine.
  ///
  /// Returns the REASON rather than a Bool so the caller has something to show; a Bool
  /// would send every call site looking for the message separately.
  ///
  /// A pattern that does not compile answers nil: a manifest with a bad rule must not make
  /// its field impossible to fill in, which would be a worse failure than the one this
  /// prevents. The rule is then simply not enforced, and the service still reports what its
  /// own backend says.
  public func failure(for value: String) -> String? {
    let trimmed = value.trimmingCharacters(in: .whitespacesAndNewlines)
    if trimmed.isEmpty { return allowsEmpty ? nil : message }
    guard let expression = try? NSRegularExpression(pattern: pattern) else { return nil }
    let range = NSRange(trimmed.startIndex..<trimmed.endIndex, in: trimmed)
    let match = expression.firstMatch(in: trimmed, options: [.anchored], range: range)
    // `.anchored` pins the START only, so the match must also reach the end, or `bb-test`
    // passes on the strength of its first two characters.
    guard let match, match.range == range else { return message }
    return nil
  }
}
...
public struct FieldDescriptor: Sendable, Codable, Equatable {
  ...
  /// A rule the value must satisfy before the form will store it.
...
public enum FieldKind: Sendable, Codable, Equatable {
  ...
  ///
  ///   Needed because an unset flag reads as `false`, so without it a manifest could
...
public struct ServiceManifest: Sendable {
  ...
  /// An SF Symbol name, shown wherever the service is offered as a choice.
  ///
  /// On the manifest rather than in a switch over identifiers in the app: a picker that
  /// chose glyphs by built-in id would show a third-party connection method a generic one
  /// while the registry ran it like any other. Defaults to the category's glyph.
  ...
  /// The macOS permissions it needs. Declared, shown, and never enforced; see
  /// `ServicePermission`.
  public let permissions: [ServicePermission]
  ...
  public init(
    ...
    // DEFAULTS TO THE PRIVILEGED VALUE, which is the wrong direction for a trust flag and
    // is kept deliberately rather than by oversight.
    //
    // Every one of the thirty call sites is in this repository and is a built-in, so the
    // default is right for all of them and removing it would be thirty edits asserting what
    // is already true. It is safe only because this type is NOT `Codable` and there is no
    // loader: a manifest cannot arrive from outside this binary, so nothing can decline to
    // state the flag and be trusted for it.
    //
    // **The day either changes, this default must go.** A decoded manifest omitting the key
    // would be trusted as a built-in, which is the one field where the generous answer is
    // the dangerous one. `ServiceManifest.swift`'s header already freezes this surface
    // against new fields; this is the same rule pointed at an existing one.
    isBuiltIn: Bool = true,
  ...
  /// a setting it never watches or watch one it never reads.
  ///

...
public enum ManifestProblem: Sendable, Equatable, CustomStringConvertible {
  ...
  /// The same permission declared twice.
  case duplicatePermission(service: ServiceIdentifier, permission: PermissionID)
  /// A permission declared with no stated reason.
  case permissionWithoutPurpose(service: ServiceIdentifier, permission: PermissionID)
  ...
  /// A compatible range no version could satisfy: a floor at or above the ceiling, or a
  /// range declaring neither bound.
  case incoherentVersionRange(service: ServiceIdentifier, tool: String)
  /// A tool that would download a version it then refuses to use.
  case recommendedVersionOutsideCompatibleRange(service: ServiceIdentifier, tool: String)
  ...
  public var description: String {
    ...
    case .duplicatePermission(let service, let permission):
      "\(service) declares the macOS permission '\(permission)' twice."
    case .permissionWithoutPurpose(let service, let permission):
      "\(service) declares the macOS permission '\(permission)' without saying what it "
        + "needs it for. The reason is the whole of what a user has to judge it by."
    ...
    case .incoherentVersionRange(let service, let tool):
      "\(service) declares a range of '\(tool)' versions that nothing could satisfy, so "
        + "every copy already on the Mac would be refused and no reason given."
    case .recommendedVersionOutsideCompatibleRange(let service, let tool):
      "\(service) recommends a version of '\(tool)' that falls outside the versions it "
        + "says it can drive, so it would download a build and then refuse to use the "
        + "same build found on disk."
    }
  }
...
public enum ManifestValidator {
  ...
  public static func validate(
    ...
    problems.append(contentsOf: validatePermissions(manifest))
  ...
  ///
  /// Nothing here checks that the permission EXISTS: the set is open by design, and a
  /// manifest naming one this binary does not know renders as its raw identifier rather than
  /// being refused. What is checked is that each declaration is usable: named once, and
  /// carrying the reason the whole list exists to show.
  private static func validatePermissions(_ manifest: ServiceManifest) -> [ManifestProblem] {
    var problems: [ManifestProblem] = []
    var seen: Set<PermissionID> = []
    for permission in manifest.permissions {
      if !seen.insert(permission.id).inserted {
        problems.append(
          .duplicatePermission(service: manifest.id, permission: permission.id))
      }
      if permission.purpose.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
        problems.append(
          .permissionWithoutPurpose(service: manifest.id, permission: permission.id))
      }
    }
    return problems
  }
  ...
  private static func validateTools(_ manifest: ServiceManifest) -> [ManifestProblem] {
    ...
    for tool in manifest.tools {
      ...
      // A range is optional; one that is declared has to mean something. Both of these are
      // refused rather than ignored for the same reason the check above is: a constraint
      // that silently never holds is indistinguishable from one that does, until someone
      // compares versions by hand and finds they never matched.
      if let compatible = tool.compatible {
        if !compatible.isSatisfiable {
          problems.append(.incoherentVersionRange(service: manifest.id, tool: tool.id))
        }
        // The check that earns the field its keep. Move zrok's pin to 2.0.4 without moving
        // the ceiling and the build goes red here, rather than a tunnel that installs
        // cleanly and then cannot open a reserved share.
        if let recommended = tool.recommended, !compatible.contains(recommended.version) {
          problems.append(
            .recommendedVersionOutsideCompatibleRange(service: manifest.id, tool: tool.id))
        }
      }
    }
    return problems

...
public enum RouteTable {
  ...
  static let server = RouteGroup(
    ...
    routes: [
      ...
      // A brute-force target: unauthenticated guessing is cheap here because it is a plain
      // GET with a small response. Failure-only rate limiting covers it.
...
public enum AdditiveRoutes {
  ...
  public static let auth = RouteGroup(
    ...
    routes: [
      ...
      // `.optionalAuthentication`, not `.unauthenticated`, and the difference is the
      // blocklist. `.unauthenticated` skips the ADMIT stage as well as the credential check,
      // so a client already blocked for guessing the server password could keep guessing
      // `client_secret` here, on the one route whose whole job is checking a credential.
      //
      // The credential half stays the handler's: it lives in the body as a `client_secret`,
      // which the auth chain does not read, so the best-effort check this requirement runs
      // finds nothing and proceeds, which is exactly what is wanted. The handler records its
      // own failures; see `AuthHandlers.token`.
      .init(.post, "token", .authToken, requires: .optionalAuthentication),

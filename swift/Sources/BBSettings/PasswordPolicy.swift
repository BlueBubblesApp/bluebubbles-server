...
//
//  The shipped default is an EMPTY password, which is what makes off-path brute-forcing
//  worth attempting at all. Setup generates a strong one and offers it, so the common case
//  is strong by default rather than strong by diligence.
//
//  It lives in BBSettings rather than BBAuth because the write path is here: attaching it
//  to the `password` descriptor is what makes it unbypassable. BBAuth re-exports it for
//  `import BBAuth` call sites.
//
...
public struct PasswordPolicy: Sendable {
  ...
  public static func generate(length: Int = 20) -> String {
    ...
    // `SystemRandomNumberGenerator` is the CSPRNG here.
    //
    // Corrected against the reference rather than assumed: `generateRandomString`
    // (`server/utils/CryptoUtils.ts`) uses `randomBytes`, which IS a CSPRNG, and it is what
    // mints the project-ID suffix at `oauthService/index.ts:343`. That half is fine and an
    // earlier version of this comment said otherwise. The reference's `Math.random` is in
    // `generateUuid` (`server/helpers/utils.ts:24`), whose output is the Private API
    // TRANSACTION id: a collision there crosses two in-flight helper calls, which is a
    // correctness problem rather than a secrecy one, and nothing about a password.
    //

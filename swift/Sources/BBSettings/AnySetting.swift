...
public struct AnySetting: Sendable, Identifiable {
  ...
  ///
  /// NOT for a secret: resolving one reads it, and a screen that renders secrets on demand
  /// must not. Use `presence` there, which answers the same question without the read.
  public let source: @Sendable (SettingsStore) async -> SettingSource

  /// What is known about a secret without reading it.
  ///
  /// `.absent` for a setting that is not one, which is not a claim about its value: no
  /// caller asks. A row reaches for this instead of `read` and `source` precisely because
  /// it is the pair of calls that would materialise the secret.
  public let presence: @Sendable (SettingsStore) async -> SecretPresence
  ...
  init<Value: SettingValue>(
    ...
    // Internal, so the section is never shown; Advanced is where a stray would land anyway.
    self.presentation =
      setting.presentation
      ?? SettingPresentation(
        label: setting.key, section: .advanced, control: .custom, isInternal: true
      )
    ...
    let isSecret = setting.isSecret
    let key = setting.key
    self.presence = { store in
      guard isSecret else { return .absent }
      return await store.presence(ofSecretKey: key)
    }
  }
}

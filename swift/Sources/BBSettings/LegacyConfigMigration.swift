...
//  beta still works.
//
//  **That includes the secrets, and for a long time this header said otherwise.** It claimed
//  they moved to the Keychain "and the plaintext row is deleted, because leaving them
//  readable is the vulnerability being fixed", and the doc comment on `run` repeated it. Both
//  connections are opened `readonly = true` and there was no `DELETE` anywhere in the file:
//  the password stayed in plaintext in the Electron `config.db` indefinitely, and the
//  vulnerability the sentence described as fixed was not.
//
//  It is fixed now, but as a SEPARATE and consented step — `removePlaintextSecrets` below,
//  offered by the migration wizard after the copy into the Keychain has succeeded. Separate
//  because it is the one irreversible thing here, and because deleting those rows is exactly
//  what breaks the downgrade path promised above. Nobody should get that as a side effect of
...
public struct LegacyConfigMigration: Sendable {
  ...
  /// irreversible step in the whole migration.** It is what makes the claim this header used
  /// to make falsely — that moving a secret to the Keychain removes the readable copy —
  /// actually true.
  ///

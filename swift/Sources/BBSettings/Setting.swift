...
public struct SettingPresentation: Sendable {
  ...
  /// Hidden from the GENERATED settings page.
  ///
  /// Two reasons a setting carries this, and they are worth telling apart. Most are
  /// bookkeeping nobody should edit (`last_fcm_restart`, the helper dylib paths) and
  /// have no screen at all. The rest are drawn by a BESPOKE screen instead: `bind_address`
  /// and `use_custom_certificate` belong to the HTTP listener, so they sit behind
  /// Configure HTTP Settings on the Connection page rather than loose among settings that
  /// have nothing to do with it. Either way the declaration is unchanged: the label, the
  /// help and the control are still here, and the bespoke screen renders them with the
  /// same `SettingRow` the generated page uses, so there is one description of each
  /// setting rather than two.
  ///
  /// The failure this invites is a setting marked internal and then drawn by nothing.
  /// A setting drawn elsewhere says where, here, and the screen that draws it declares
  /// its list off the view so a test can hold the two together.
  ...
  /// The storage key of the Bool setting this one does nothing without.
  ///
  /// Declared here rather than checked in the view, for the same reason the label and the
  /// control are: a screen that decides which rows to grey out by knowing the FaceTime
  /// group by name is a screen that will not know about the next group. The generated page
  /// disables the row and says which switch to turn on; nothing else has to be told.
  ///
  /// **Presentation only, and deliberately so.** It does NOT gate reads or writes: the
  /// server already treats each of these as unreachable when its parent is off (a FaceTime
  /// setting with `enable_ft_private_api` off is read by a service that never starts), and
  /// making the store refuse a write would break `--set` on a config being assembled before
  /// the switch it depends on. What it fixes is a row that looks live and is not: somebody
  /// turning "Incoming call hand-off" on with the FaceTime Private API off, watching the
  /// switch move, and concluding that hand-off is broken.
  ///
  /// Must name a renderable `Setting<Bool>` and must not chain: `SettingDependencyTests`
...
public struct Setting<Value: SettingValue>: Sendable {
  ...
  public init(
    ...
    // A secret is a String, and nothing else can be one.
    //
    // `SettingsBatch.set` computes `value as? String` to hand the Keychain; for any other
    // type that is nil, and `stageSecrets` then wrote the EMPTY STRING — a secret silently
    // destroyed at the moment it was set. No such setting exists, which is why this is a
    // precondition at declaration rather than a runtime check on every write: it fires
    // once, at launch, on the line that made the mistake.
    precondition(
      !isSecret || Value.self == String.self,
      "\(key) is marked isSecret but its value is \(Value.self); only a String can be a secret"
    )
    self.key = key
...
///
/// A CLI value is never written into the database; it wins for this process and is gone
/// with it.
...
///
/// Three cases rather than a `Bool`, for the reason `SettingsStore.SecretRead` has three:
/// "the Keychain refused" is not "nothing is stored". A settings row that conflated them
/// would offer to generate a replacement for a password that is sitting in the Keychain
/// perfectly intact, and generating is the one irreversible thing on that screen.
public enum SecretPresence: Sendable, Equatable {
  /// Nothing is stored under this key.
  case absent
  /// A value is stored, and this is the layer it comes from.
  case stored(SettingSource)
  /// The Keychain would not say. An alert has already been raised.
  case unreadable

  /// Where the value comes from, for a row deciding whether it may be edited here.
  /// `nil` when there is nothing to edit or nothing is known.
  public var source: SettingSource? {
    if case .stored(let source) = self { return source }
    return nil
  }
}

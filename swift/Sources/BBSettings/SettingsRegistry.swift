...
public enum Settings {
  ...
  public static let faceTimeOutgoingCalls = Setting<Bool>(
    ...
    presentation: .init(
      ...
      section: .faceTime, control: .toggle,
      requires: enableFaceTimePrivateAPI.key
    )
  )
  ...
  /// 40,000-message database, page queries stay flat from one concurrent reader to sixteen
  /// through a single connection, while a pool of four roughly doubles under contention. The
  /// ceiling is real, and multi-client is the normal case here.
  ///
  /// **Measured 2026-09-14: MacBook Pro M3 Max, macOS 26.5.2, arm64, release build.**
  /// queue(1) held 559-592 queries/second at every concurrency level; pool(4) went from 550
  /// at one reader to 1,096 at sixteen. Pool memory was +0.0 MB against a 29.5 MB baseline.
  ///
  /// Those conditions are stated because the previous figures here had none, and were wrong:
  /// "~26/second", "~95/second" and "about 2 MB" for the pool. Re-measuring gave twenty times
  /// the throughput and no measurable memory, and even the RATIO was off -- 3.7x claimed
  /// against 1.9x measured. A number with no machine beside it cannot be checked, and this
  /// one went unchecked long enough to be quoted as a settled fact. The suite's own
  /// `poolMemoryCost` test had been printing +0.0 MB all along; nobody compared the two.
  ///
  /// Expect a different answer on the hardware this server is often deployed to. This Mac
  /// has an NVMe fast enough that a single connection is rarely waiting on the disk, which
  /// is precisely the wait a pool exists to overlap; on a SATA or spinning disk the pool
  /// should help more, not less. Nobody has measured that.
  ///
  ...
  /// Whether a new release installs without anyone at the Mac.
  ///
  /// Off by default. On, Sparkle downloads a release in the background once the daily
  /// check finds one and installs it the next time the app relaunches; `POST
  /// /server/update/install` installs and relaunches at once. A server left running in a
  /// closet wants this; a person who reads release notes first does not. The Electron server
  /// had the same key (its `autoUpdater.autoInstallOnAppQuit`), so an upgraded install
  /// keeps its answer. Read by `SparkleUpdater` alone.
  public static let autoInstallUpdates = Setting<Bool>(
    "auto_install_updates", default: false,
    presentation: .init(
      label: "Install Updates Automatically",
      // NOT "Needs Check for Updates" any more: `requires` says that structurally, and the
      // row greys out and names the switch rather than describing a rule the person then
      // has to apply themselves.
      help: "Downloads a new release in the background and installs it at the hour below, "
        + "relaunching BlueBubbles.",
      section: .updates,
      control: .toggle,
      // Nothing installs automatically without the daily check that finds the release:
      // `SparkleUpdater` maps this to `automaticallyDownloadsUpdates`, which never fires
      // while `automaticallyChecksForUpdates` is off. The manual menu item is unaffected,
      // which is why `checkForUpdates` itself hangs off nothing.
      requires: checkForUpdates.key
    ))

  /// Whether pre-releases are offered.
  ///
  /// Off by default. On, the update check and Sparkle both read the `beta` channel of the
  /// same feed as well as the default one, so a beta is offered exactly as a release is:
  /// same signature check, same notes, same install paths. Turning it off does not roll
  /// back: an installed beta stays until the next release outranks it. This is what the
  /// read-only feed URL is FOR; a person who wants betas flips this rather than editing a
  /// URL. Read per check by `UpdateHandlers` and `SparkleUpdater`.
  public static let receiveBetaUpdates = Setting<Bool>(
    "receive_beta_updates", default: false,
    presentation: .init(
      label: "Receive Beta Releases",
      help: "Offers pre-release builds as updates. They are signed and installed exactly "
        + "like releases, and may have rough edges.",
      section: .updates,
      control: .toggle
    ))

  /// The version the app last ran as. Bookkeeping, never shown.
  ///
  /// Written by the app at every start and read once, before the write, so a start on a
  /// different version can say "Updated to X" with a link to its notes. Empty on a fresh
  /// install, which says nothing. See `UpdatesModel.noteVersionChange`.
  public static let lastRunVersion = Setting<String>("last_run_version", default: "")

  /// The hour of the day an automatically downloaded update relaunches the app.
  ///
  /// Local time on the Mac, 0 to 23. Sparkle's own behaviour is to install on quit, and a
  /// server that runs all day never quits; this is what makes the toggle above mean what
  /// it says. `InstallWindow` (in the app) does the arithmetic and holds the relaunch while
  /// a scheduled message is about to go out. Read by `SparkleUpdater` alone.
  public static let autoInstallHour = Setting<Int>(
    "auto_install_hour", default: 3,
    presentation: .init(
      label: "Install Updates At",
      help: "The hour (0 to 23, this Mac's time zone) when a downloaded update is installed "
        + "and BlueBubbles relaunches. The relaunch waits if a scheduled message is due "
        + "within fifteen minutes.",
      section: .updates,
      control: .number(range: 0...23),
      // The hour hangs off the AUTOMATIC install, not off the check: it is the answer to
      // "when does the thing you already agreed to happen", and it is meaningless if
      // nothing installs unattended. That makes this the one two-link chain in the
      // registry, and the reason `Settings.blockingRequirement` names the OUTERMOST switch
      // that is off rather than the nearest: with checks off, telling somebody to turn on
      // Install Updates Automatically points them at a row that is itself greyed out.
      requires: autoInstallUpdates.key
    ))
  ...
  public enum Legacy {
    ...
    public static let all: [AnySetting] = [
      ...
      tutorialIsDone.erased, encryptComs.erased, privateAPIMode.erased,
    ]
  }
  ...
  ///
  /// Shown, not editable. This is where the app fetches code it will run, so it is not a
  /// row to mistype; a release channel, or a local feed while developing the updater, is
  /// set from the CLI (`--set update_feed_url=…`), and that friction is the point. Both
  /// readers (`UpdateChecker` behind `GET /server/update/check`, and Sparkle through
  /// `SparkleUpdater`) take it from here, so the API and the installer cannot disagree about
  /// which feed is current.
  public static let updateFeedURL = Setting<String>(
    "update_feed_url", default: UpdateFeed.defaultURL,
    presentation: .init(
      label: "Update feed",
      help: "The Sparkle appcast this server checks for updates. Set from the command line "
        + "to point a development build at a local feed.",
      section: .advanced, control: .readOnly
    )
  )
  ...
  public static let rateLimitFailureThreshold = Setting<Int>(
    ...
    presentation: .init(
      ...
      // Counting failures IS the automatic half, and it is the half the toggle above
      // governs: with it off, `AccessControl.recordFailure` returns before the threshold is
      // ever compared.
      requires: rateLimitEnabled.key))

  // `trustLocalNetwork` below deliberately does NOT hang off `rateLimitEnabled`, however
  // much it reads like it should. It writes the private ranges into the ALLOWLIST, and
  // `AccessControl.evaluate` consults `isAlwaysAllowed` before it consults
  // `policy.isEnabled`, so with rate limiting switched off this still exempts a LAN address
  // from a block an administrator set by hand. Greying it out would leave a control that
  // still does something looking dead.
  public static let rateLimitBlockSeconds = Setting<Int>("rate_limit_block_seconds", default: 900)
  /// Whether a device on your own network can be auto-blocked for failed logins.
  ///
  /// **Defaults ON, and what that does NOT do is the point: it never bypasses the password.**
  /// Authentication runs in two halves (`AuthenticationMiddleware.authenticate` is `admit`
  /// then `verifyCredential`) and this touches only the first: the allowlist is read by
  /// `AccessControl.evaluate` and `recordFailure` and by nothing in the credential chain. A
  /// LAN client still presents the server password like any other, and a wrong one is still
  /// a 401. All this removes is the automatic LOCKOUT.
  ///
  /// It defaults on because the failure it prevents is invisible and the protection it drops
  /// is thin. Loopback can never be blocked (`ProxyTrustPolicy.permanentAllowlist`), so with
  /// this off a phone on the same desk locks itself out after ten bad passwords while `curl`
  /// on the Mac keeps working — and a blocked caller receives a 401, deliberately identical
  /// to a wrong password, so nothing on the wire says what happened. Reproduced exactly: ten
  /// wrong passwords from the LAN address, then the CORRECT password answered 401 from the
  /// LAN and 200 from loopback. It is the shape of a support report nobody can diagnose.
  ///
  /// Against that, the reference has no rate limiter, no blocklist and no such setting at
  /// all, so an install with this on is still strictly more hardened than every v1 server
  /// ever shipped. What it costs is that a hostile device already inside your network gets
  /// unlimited guesses at the password, which is the reason to turn it off and why the
  /// switch is a plain toggle on the Security page rather than something buried.
  public static let trustLocalNetwork = Setting<Bool>(
    "trust_local_network", default: true,
    presentation: .init(
      label: "Trust My Local Network",
      help: "Devices on your own network are never locked out for failed logins. They still "
        + "need the server password. Turn this off to rate-limit them too, if something on "
        + "your network might try to guess it.",
      section: .security, control: .toggle
    ))
...
extension Settings {
  ...
  public static let renderable: [AnySetting] =
    [
      ...
      autoInstallUpdates.erased,
      autoInstallHour.erased,
      receiveBetaUpdates.erased,
      authMode.erased,
  ...
  /// The declared setting a key names, or nil.
  ///
  /// Over `all` rather than `renderable`, so a caller resolving a `requires` key gets the
  /// same answer whether or not the parent happens to have a screen.
  public static func setting(forKey key: String) -> AnySetting? {
    all.first { $0.key == key }
  }

  /// The switches a setting hangs off, nearest first.
  ///
  /// Usually one, and `auto_install_hour` is why this is a walk rather than a lookup: it
  /// depends on the automatic install, which depends in turn on the daily check. A screen
  /// that read only the immediate parent would draw that row live while the switch above
  /// it was greyed out, which says the opposite of the truth.
  ///
  /// Cycle-guarded by construction. `SettingDependencyTests` proves the declarations
  /// contain no cycle, and this still refuses to loop on one: a settings screen that hangs
  /// is a worse failure than a row that is wrong, and the test is the thing that has to be
  /// true, not the only thing.
  public static func requirementChain(for setting: AnySetting) -> [AnySetting] {
    var chain: [AnySetting] = []
    var seen: Set<String> = [setting.key]
    var next = setting.presentation.requires
    while let key = next, !seen.contains(key), let parent = self.setting(forKey: key) {
      chain.append(parent)
      seen.insert(key)
      next = parent.presentation.requires
    }
    return chain
  }

  /// The switch to tell somebody to turn on, or nil when the setting is live.
  ///
  /// **The OUTERMOST one that is off, not the nearest**, and that is the whole reason this
  /// is a function rather than a comparison at the call site. With Check for Updates off and
  /// Install Updates Automatically stored on, the nearest unmet switch for the install hour
  /// is Check for Updates and both answers happen to agree. With BOTH off, the nearest is
  /// Install Updates Automatically, which is itself greyed out: the row would name a control
  /// the person cannot click. The outermost is always the one that can be.
  ///
  /// - Parameter isOn: the current value of a switch, by storage key. A key the caller
  ///   could not read answers `true`, so an unreadable parent leaves the row usable rather
  ///   than locking it on a failed read.
  public static func blockingRequirement(
    for setting: AnySetting, isOn: (String) -> Bool
  ) -> AnySetting? {
    requirementChain(for: setting).last { !isOn($0.key) }
  }

...
public enum AutoStartMethod: String, SettingValue, CaseIterable, Sendable {
  ...
  /// `launch-agent` was a third option and it never registered anything — every call site
  /// omitted the plist name the agent path required, and no plist was ever shipped.
  ///
  ...
  /// between the two options was supervision, which the launcher now provides for the login
...
public enum Settings {
  ...
  //
  // One row per step, rather than the single `legacy_config_imported` Bool these replace.
  //
  ...
  //
  // Where a certificate CAME FROM, and when it expires. Both were previously encoded by the
  // presence or absence of `Certs/expiration.txt` on disk: the file was written only when
  // this server generated a certificate, so its ABSENCE meant "the user installed this, never
  // touch it". That is the correct fail-safe direction and an unreadable way to express it —
  // a missing file is indistinguishable from a deleted one, and it cannot survive the move

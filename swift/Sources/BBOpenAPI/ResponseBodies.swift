...
public enum ResponseBodies {
  ...
  public indirect enum Kind: Sendable {
    ...
    /// is missing or a store is unreadable) has to be able to say so, or the table
    /// documents whichever shape its author had in mind and a client generating a decoder
    /// from it gets one that fails on a response the server legitimately sends.
    /// `sticker.save` is the standing example.
    case oneOf([Variant])
  }

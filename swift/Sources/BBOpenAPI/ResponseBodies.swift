...
public enum ResponseBodies {
  ...
  public indirect enum Kind: Sendable {
    ...
    /// One of several shapes, because the route answers differently depending on what it
    /// could reach.
    ///
    /// Not a convenience. A handler that degrades — answering with less when a permission
    /// is missing or a store is unreadable — had no way to say so, so the table documented
    /// whichever shape its author had in mind and the other one was invisible. A client
    /// generating a decoder from this document got one that fails on a response the server
    /// legitimately sends. `sticker.save` is the case that found it.
    case oneOf([Variant])
  }

  /// One of the shapes a route can answer with.
  ///
  /// Carries its own summary and example rather than borrowing the body's: the whole point
  /// is that the shapes differ, so a single example could only ever describe one of them.
  public struct Variant: Sendable {
    /// What has to be true for the route to answer with this shape. Written as a condition,
    /// not as a label — "when the sticker store is readable", not "full".
    public let summary: String
    public let properties: [Property]
    public let example: OrderedJSON?

    public init(summary: String, properties: [Property], example: OrderedJSON? = nil) {
  ...
  public struct Body: Sendable {
    ...
    /// Every shape this response can take, as a flat list.
    ///
    /// One entry for an ordinary object or array response, none for an empty or mirrored
    /// one, and one per variant for a route that degrades. Checks over a declaration run
    /// over THIS rather than over a merged field list, because merging is what would let a
    /// field declared on one shape vouch for an example of another.
    public var variants: [Variant] {
      switch kind {
      case .object(let properties), .list(let properties):
        [Variant(summary: summary, properties: properties, example: example)]
      case .empty, .mirrors:
        []
  ...
  /// What `sticker.save` answers with when the store cannot be read back.
  ///
  /// Four fields, and `saved` appears nowhere else in the API. It exists because the other
  /// three cannot say on their own that the operation SUCCEEDED — a client seeing a response
  /// with no `identifier` would otherwise have to guess whether the sticker was added.
  static let degradedStickerProperties: [Property] = [
    Property(
      "external_uri", .string,
      "The sticker's address in the store, which is what the donation was filed under. "
        + "The one identifier this response can offer that is not a lie.",
      required: true),
    Property(
      "byte_count", .integer,
      "Size of the sticker's bytes as the store received them.", required: true),
    Property(
      "shelf", .string,
      "Always `recent`: a newly added sticker lands there. Stated rather than omitted so "
        + "this shape and the full one agree about the field.",
      required: true),
    Property(
      "saved", .boolean,
      "Always true, and present only on this shape. It is how a client tells `the sticker "
        + "was added but could not be read back` from a failure — the identifier is absent "
        + "in both cases, and only this says the operation succeeded.",
      required: true),
  ]
  ...
  public static let byHandler: [HandlerID: Body] = [
    ...
    .stickerSave: Body(
      summary:
        "The sticker as the store now holds it, or an acknowledgement that it was added "
        + "and could not be read back. Which one arrives depends on whether this server "
        + "can read the sticker store, so a client has to handle both.",
      kind: .oneOf([
        Variant(
          summary:
            "When the sticker store is readable. Read back rather than echoed, so the "
            + "`identifier` is one the read routes accept and the representation carries "
            + "the UTI and size the store derived from the bytes. It lands on the "
            + "`recent` shelf.",
        ...
        Variant(
          summary:
            "When the store took the sticker but this server cannot read it back — almost "
            + "always a Mac that has not granted Full Disk Access. The sticker IS saved. "
            + "There is deliberately no `identifier`: the only one available is the "
            + "donation's, which every read route answers 404 for, so sending it would be "
            + "worse than sending nothing. `metadata.reason` says what to fix.",
          properties: degradedStickerProperties,
          example: .obj([
            (
              "external_uri",
              .string("sticker:///user/identifier/8A7657D3-E58A-440B-8FCA-7F4389F49DEA")
  ...
  public static func schema(for body: Body) -> OrderedJSON? {
    ...
    case .oneOf(let variants):
      // OpenAPI 3.1 `oneOf`. The body's summary describes the route; each variant's
      // describes the condition under which that shape is the answer.
      return .obj([
        ("description", .string(body.summary)),
        ("oneOf", .array(variants.map { element($0.summary, $0.properties) })),
      ])
    }
  }

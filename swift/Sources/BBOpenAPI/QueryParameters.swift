...
//  there is no type to reflect on and nothing in the route table records them: a generated
//  document would describe `?pretty` and nothing else, while `GET /message/query` quietly
//  takes six.
//
...
public enum QueryParameters {
  ...
  /// `extraProperties`, on the three contact reads that accept it.
  static let extraProperties = Parameter(
    "extraProperties", "string",
    "Comma-separated extra fields to include. `avatar` is the one the Flutter client asks "
      + "for, and it is expensive: the address-book index deliberately holds no image data, "
      + "so this fetches one per contact on the page.")

  /// What all three `/message/count*` routes accept, which is one identical list.
  ///
  /// Shared rather than repeated, because they drifted: `chatGuid` was documented and
  /// implemented on one of the three, and the row-id window on none.
  static let countParameters: [Parameter] = [
    after, before,
    Parameter("chatGuid", "string", "Count only within this conversation."),
    Parameter("minRowId", "integer", "Only messages with a ROWID at or above this."),
    Parameter("maxRowId", "integer", "Only messages with a ROWID at or below this."),
  ]
  ...
  public static let byHandler: [HandlerID: [Parameter]] = [
    ...
    .chatMessages: [
      with(
        "`handle`, `chat`/`chats`, `attachment`/`attachments`, "
          + "`message.attributedBody`, `message.messageSummaryInfo`, `message.payloadData`"),
    ...
    // The same five on all three, which is what the reference accepts on all three.
    .messageCount: countParameters,
    .messageCountUpdated: countParameters,
    .messageSentCount: countParameters,
    .handleFind: [with("`chat`/`chats`")],
    .contactList: [limit, offset, extraProperties],
    .contactFindByExternalID: [extraProperties],
    ...
    .attachmentBlurhash: [
      Parameter(
        "width", "integer",
        "Upper bound on the box the image is scaled into before hashing. A blurhash is a "
          + "very low-frequency summary, so a smaller box is the same hash for less work.",
        defaultValue: "64"),
      Parameter("height", "integer", "As `width`; the larger of the two is used."),
      Parameter(
        "quality", "string",
        "Accepted for compatibility and has no effect: it selects Electron's image resize "
          + "filter, which has no counterpart in the Core Graphics path used here."),
    ...
    .serverStatTotals: [
      Parameter(
        "only", "string",
        "Comma-separated totals to count. Omit for all. Each is a full table scan, so "
          + "naming the one you need is worth doing on a large database.",
        allowed: ["handle", "message", "chat", "attachment"])
    ],
    .serverStatMedia: [
      Parameter(
        "only", "string",

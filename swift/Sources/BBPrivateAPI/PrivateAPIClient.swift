...
//
//  This is the only place in the server that knows which helper action a contract method
//  maps to and which fields it carries. Everything above it calls typed methods; everything
//  below it moves bytes. That split is what lets the legacy Objective-C helper and the Swift
//  one coexist behind one API.
//
//  Actions are `MessagesHelperAction` / `FaceTimeHelperAction` and fields are `WireKey`, both
//  from the contract the helpers link, so a spelling exists once. Their raw values are the
//  SHIPPING helper's, verbatim: `backwardsCompatibilityMessage`, `selectedMessageGuid`,
//  `ddScan` as 0/1 rather than a boolean. They look inconsistent because they are; they are
//  also the wire, so they are reproduced rather than tidied. The Swift helper accepts
//  exactly these.
//
...
public actor PrivateAPIClient: PrivateAPI {
  ...
  public func rescheduleMessage(
    ...
    _ = try await transport.request(
      action: .rescheduleMessage,
      data: .object([
        .chatGuid: .string(chat.rawValue),
        .messageGuid: .string(guid.rawValue),
        .scheduledFor: .number(date.timeIntervalSince1970 * 1000),
      ]))
  }
  ...
  public func createChat(
    ...
    // flagged in the reference as "Yes this is correct". The caller resolves the
  ...
  public func pinnedChats() async throws -> [ChatIdentifier] {
    let reply = try await transport.request(
      action: .getPinnedChats, data: .emptyObject)
  ...
  public func checkIMessageAvailability(address: String) async throws -> Bool {
    try await availability(.checkIMessageAvailability, address: address)
  }

  public func checkFaceTimeAvailability(address: String) async throws -> Bool {
    try await availability(.checkFaceTimeAvailability, address: address)
  }
  ...
  public func accountInfo() async throws -> AccountInfo {
    let result = try await transport.request(
      action: .getAccountInfo, data: .emptyObject)

    /// An alias object, tolerating the bare string an older helper sends.
    ///
    /// The helper and the server ship together, so the string form is not a case this has
    /// to serve forever — but a helper is injected into a running Messages and survives a
    /// server restart, so the two versions DO meet in practice. Reading a string as
    /// `{alias}` keeps that pairing working instead of emptying the list.
    func alias(_ value: WireJSON) -> AccountAlias? {
      if let flat = value.stringValue { return AccountAlias(alias: flat) }
      guard let name = value[WireKey.aliasName]?.stringValue else { return nil }
      return AccountAlias(
        alias: name,
        status: value[WireKey.aliasStatus]?.intValue,
        isUserVisible: value[WireKey.aliasIsUserVisible]?.boolValue
      )
    }

    return AccountInfo(
      appleId: result?[.appleId]?.stringValue,
      accountName: result?[.accountName]?.stringValue,
      activeAlias: result?[.activeAlias]?.stringValue,
      aliases: result?[.aliases]?.arrayValue?.compactMap(alias) ?? [],
      vettedAliases: result?[.vettedAliases]?.arrayValue?.compactMap(alias) ?? [],
      loginStatusMessage: result?[.loginStatusMessage]?.stringValue,
      smsForwardingEnabled: result?[.smsForwardingEnabled]?.boolValue ?? false,
      smsForwardingCapable: result?[.smsForwardingCapable]?.boolValue ?? false
    )
  }
  ...
  public func nicknameInfo(for address: String?) async throws -> NicknameInfo {
    ...
    let result = try await transport.request(
      ...
      data: .object(dropping: [.address: address.map(WireJSON.string)])
    )
    return NicknameInfo(
      ...
      avatarPath: result?[.avatarPath]?.stringValue
    )
  }
  ...
  public func faceTimeActiveCalls() async throws -> [FaceTimeCall] {
    let result = try await transport.request(
      action: .faceTimeActiveCalls, data: .emptyObject
    )
  ...
  public func faceTimeWindows() async throws -> [String] {
    let result = try await transport.request(
      action: .faceTimeWindows, data: .emptyObject
    )
  ...
  public func dismissFaceTimeAlert() async throws -> Int {
    let result = try await transport.request(
      action: .faceTimeDismissAlert, data: .emptyObject
    )
  ...
  private static func link(from value: WireJSON?) -> FaceTimeLink? {
    ...
    return FaceTimeLink(
      url: url,
      groupUUID: value?[.groupUUID]?.stringValue ?? value?[.legacyGroupUUID]?.stringValue,
  ...
  private static func call(from value: WireJSON?) -> FaceTimeCall? {
    guard let uuid = value?[.callUUID]?.stringValue ?? value?[.legacyCallUUID]?.stringValue,
      !uuid.isEmpty
    else { return nil }
    return FaceTimeCall(
      callUUID: uuid,
      status: FaceTimeCallStatus(
        raw: value?[.callStatus]?.intValue ?? value?[.legacyCallStatus]?.intValue ?? 0
      ),
  ...
  private func availability(
    _ action: MessagesHelperAction, address: String
  ) async throws -> Bool {
    let result = try await transport.request(

...
//  to run and reports `.inactive`, so "off" is a state the UI can show rather than an
//  absence.
//
...
public struct PrivateAPIConfiguration: Sendable {
  ...
  /// Whether the FaceTime helper should keep FaceTime's camera off while it is in the
  /// background. Travels in the injection environment; see `Settings.faceTimeIdleCameraOff`.
  public var faceTimeIdleCameraOff: Bool
  ...
  public init(
    ...
    faceTimeIdleCameraOff: Bool = true
  ) {
    ...
    self.faceTimeIdleCameraOff = faceTimeIdleCameraOff
  }
}

public actor PrivateAPIRuntime {

  private var configuration: PrivateAPIConfiguration
  ...
  /// Registrations, and anyone waiting for one. Its own type because it is state with
  /// rules; see `RegistrationLatch`, which also carries why the count is a count.
  private let registrations = RegistrationLatch()
  ...
  /// What a screen draws: whether the last start finished, and whether a helper is talking.
  ///
  /// Both together, because the two questions have four answers and a screen needs to tell
  /// "switched on and waiting for the helper" from "switched on and the helper is here".
  public struct State: Sendable, Equatable {
    public let outcome: StartOutcome
    public let isConnected: Bool

    public init(outcome: StartOutcome, isConnected: Bool) {
  ...
  public var state: State {
    get async { State(outcome: startOutcome, isConnected: await transport.isConnected) }
  }

  private var stateSubscribers: [UUID: AsyncStream<State>.Continuation] = [:]

  /// The state, again whenever it changes: a start finishing or overrunning, a helper
  /// registering, a helper's socket closing. The newest is kept for a slow consumer. A
  /// consumer that needs the state before the first change reads `state`.
  ...
  private func publishState() async {
    guard !stateSubscribers.isEmpty else { return }
  ...
  private func injectManagedApp(
    ...
    let injector = DylibInjector(
      target: DylibInjector.Target(
        ...
        dylibPath: dylibPath,
        // One variable per host, and only the one that host reads: only FaceTime has a
        // camera to leave running, so setting this on Messages would be a variable nothing
        // reads.
        environment: bundleIdentifier == HelperHost.faceTime
          ? [
            HelperEnvironment.faceTimeIdleCameraOff:
              configuration.faceTimeIdleCameraOff ? "1" : "0"
          ]
          : [:]
      ),
    ...
    let baseline = await registrations.count(of: bundleIdentifier)
  ...
  /// Replaces the configuration without restarting anything.
  ///
  /// Exists so a settings change can re-inject ONE app. `reinject` reads the dylib paths and
  /// the injection environment out of `configuration`, so a stale one would relaunch the app
  /// with the value it already had, which looks exactly like the setting not working.
  public func reconfigure(_ configuration: PrivateAPIConfiguration) {
    self.configuration = configuration
  }

  /// Stops managing one app's injection, leaving every other app alone.
  ///
  /// For a helper being switched OFF. The app keeps running with the dylib it already loaded
  /// until something relaunches it: the same as before, since nothing un-injects a live
  /// process; what this avoids is disturbing the OTHER app to achieve it.
  public func stopInjecting(bundleIdentifier: String) async {
    await injectors[bundleIdentifier]?.stop()
    injectors[bundleIdentifier] = nil
  }
  ...
  public func stop() async {
    ...
    await registrations.releaseAll()
    await publishState()
  }
  ...
  private func startEventPump() {
    eventPump = Task { [weak self] in
      ...
      for await event in await transport.events {
        ...
        if case .helperDisconnected = event {
          await self.publishState()
        }
      }
    }
  ...
  private func registrationObserved(process: String) async {
    await registrations.observe(process)
  }

  private func awaitRegistration(
    process: String, after baseline: Int, timeout: Duration
  ) async -> Bool {
    await registrations.wait(for: process, after: baseline, timeout: timeout)
  }

...
actor TransactionStore {
  ...
  /// It used to be the caller's first statement — write the frame, then await the reply —
  /// which is a race the helper wins whenever it answers faster than this actor can record
  /// who is waiting. `resolve` drops a reply it has no continuation for, so the caller then
  /// waited out its entire timeout for an answer that had already arrived and been thrown
  /// away. The reply that arrives fastest is the one for an action the helper rejects
  /// immediately — an unknown action does no work before answering — so the failure showed
  /// up first in exactly the tests that assert on a rejection.
  ///
  ...
  /// nothing to resume.
  ///
  /// `action` is carried only to name the timeout. Reporting the transaction id there said

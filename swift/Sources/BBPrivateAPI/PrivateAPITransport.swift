...
extension PrivateAPITransport {
  ...
  ///
  /// A `process:` argument at every FaceTime call site would make "send a FaceTime command
  /// to the Messages helper" a plausible typo. It is not expressible here.
  @discardableResult
  public func request(
...
actor TransactionStore {
  ...
  /// Registers a transaction, THEN sends it, and suspends until it is answered, times out,
  /// or is cancelled.
  ///
  /// **The send happens in here, after registration, and the ordering is the whole point.**
  /// Writing the frame first and then awaiting the reply is a race the helper wins whenever
  /// it answers faster than this actor can record who is waiting: `resolve` drops a reply it
  /// has no continuation for, and the caller then waits out its entire timeout for an answer
  /// that has already arrived and been thrown away. The reply that arrives fastest is the one
  /// for an action the helper rejects immediately.
  ///
  /// Registering first makes it unrepresentable rather than unlikely: the continuation is in
  /// `pending` before the bytes leave, so there is no window in which a reply can find
  /// nothing to resume.
  ///
  /// `action` is carried only to name the timeout. Reporting the transaction id would say
  /// `timedOut(method: "3416CCD4-…")`, which identifies the attempt and not the operation.
  func await(
    id: String,
    action: String,
    timeout: Duration,
    ...
    try await withThrowingTaskGroup(of: WireJSON?.self) { group in
          ...
          try await withCheckedThrowingContinuation { continuation in
            Task {
              ...
              do {
                ...
                // The frame never left. Fail the transaction we just registered rather
                // than leaving it to time out over something already known.
      ...
      group.addTask {
        ...
        throw PrivateAPIError.timedOut(method: action)
      }
  ...
  private func register(id: String, continuation: CheckedContinuation<WireJSON?, any Error>) {
    ...
    // CANCELLED BEFORE WE GOT HERE, which is a real window and not a theoretical one.
    //
    // The cancellation handler is installed before the operation, but registration happens
    // inside a nested `Task` that has to hop to this actor. Cancellation landing in that gap
    // ran `fail` first, which found nothing in `pending` and returned WITHOUT REMEMBERING —
    // and then this stored a continuation nobody would ever resume. The timeout child is
    // cancelled with the group, so the whole task group waited on a stranded child
    // indefinitely. `FaceTimeCoordinator` cancelling a watcher mid-request is how it
    // happens in practice.
  ...
  func resolve(id: String, with value: WireJSON?) {
    guard let continuation = pending.removeValue(forKey: id) else {
      // The race the header describes, if it ever regresses: a reply with nobody waiting.
      logger.debug(
        "Reply for an unknown transaction",
        metadata: ["tx": .string(String(id.prefix(8)))])
      return
    }
  ...
  func fail(id: String, with error: any Error) {
    guard let continuation = pending.removeValue(forKey: id) else {
      // Nobody is waiting YET. For cancellation that is the window described in
      // `register`, so it is remembered rather than dropped; for anything else the
      // transaction is already finished and there is nothing to do.
      if error is CancellationError { cancelled.insert(id) }
      return
    }
    continuation.resume(throwing: error)
  }

  /// Transactions cancelled before they finished registering. Bounded by construction: an
  /// id is inserted only by `fail` and removed by the `register` that follows it, or by
  /// `failAll` when the helper goes away.
  ...
  func failAll(with error: any Error) {
    ...
    pending.removeAll()
    // The helper is gone, so a registration that has not happened yet never will. Nothing
    // would otherwise remove these.
    cancelled.removeAll()
    if !outstanding.isEmpty {
      // One line: `HelperVocabularyTests` exempts log metadata by the `metadata:` token.
      logger.warning(
        "Failing every outstanding Private API request",
        metadata: ["count": .stringConvertible(outstanding.count), "error": .string("\(error)")]
      )
    }
...
struct HelperResponse: Decodable {
  ...
  private static let reservedKeys: Set<String> = Set(
    [WireKey.transactionId, .error, .identifier].map(\.rawValue))
  ...
  var result: WireJSON? {
    ...
    // `event` is not a reserved key in the reference, so it survives the

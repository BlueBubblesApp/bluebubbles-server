...
public actor SocketTransport: PrivateAPITransport {
  ...
  public func request(
    ...
    // The write is handed to the store so it happens AFTER the continuation is registered.
    // Sending first is a race the helper wins on any reply it can produce immediately.
    return try await transactions.await(

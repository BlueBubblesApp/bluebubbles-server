...
//
//  Both migration identifiers keep their exact original spelling: `createContactIndex` and `addContactAddressRawAndAccount` are
...
//
//  The BBAuth/BBInterfaces tables cannot be contributed this way, for one reason:
...
public enum ContactsSchema: SchemaContributor {
  ...
  public static func registerSchema(in migrator: inout DatabaseMigrator) {
    ...
    migrator.registerMigration("addContactAddressRawAndAccount") { db in
      ...
      // lookup. `raw` keeps what was actually entered, so a read does not hand the
      // stripped form back: `person.name@example.com` as `personnameexamplecom`.
      //

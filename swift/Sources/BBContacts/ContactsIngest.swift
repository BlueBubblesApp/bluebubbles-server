...
//
//  `enumerateContacts` streams one contact at a time, only the keys needed to index are
//  requested, image data is never fetched in the bulk pass, and batches are flushed to
//  SQLite so peak memory is the batch rather than the address book. Fetching every contact
//  with every property (full-resolution images included) is the single biggest
//  allocation a server like this can make.
//
...
public actor ContactsIngestor {
  ...
  public static var authorizationStatus: ContactsAuthorization {
    ...
    /// Requesting either here is what makes the reference expensive, and the
    ...
    public func reindexAll() async throws -> ContactsIngestResult {
      ...
      // deliberately never reaches the store, so without this the app cannot trigger its
      // own permission prompt at all. No test can cover it: a test bundle's TCC status is
      ...
      try autoreleasepool {
        try store.enumerateContacts(with: request) { contact, stop in
          autoreleasepool {
            // CANCELLATION, checked per contact, because nothing else in this loop can see
            // it. The enumeration callback is synchronous, so there is no suspension point
            // for the cooperative cancellation to act on, and `ContactsService` cancelling
            // the ingest did nothing at all: the run continued to completion and its
            // `catch is CancellationError` was unreachable.
            //
            // What that cost is a real corruption rather than wasted work. A settings change
            // restarts the ingest, and the second run opens with `removeAll(source: .macOS)`
            // while the first is still flushing batches, so the second deletes rows the
            // first then writes on top of: the index ends up holding a mixture of two runs
            // and missing whatever the first had not reached. The comment on `cancel()`
            // describes preventing exactly this.
            guard !Task.isCancelled else {
              thrown = CancellationError()
              stop.pointee = true
              return
            }
      ...
      // Not flushed on cancellation: a partial tail written after `removeAll` is exactly the
      // half-built index the check above exists to avoid.

...
public actor FCMSender {
  ...
  public func send(
    ...
    // webhook has none. What Google weighs is the `data` map below (keys, JSON-string
    // escaping and all) so the cap is measured here, on the exact bytes sent, rather than
    // guessed at from the message object in the serializer.
    var shrunk = data
    if let size = Self.payloadSize(of: data), size > Self.maximumPayloadBytes {
      ...
      if trimmedSize > Self.maximumPayloadBytes {
        logger.warning(
          "Notification is still over FCM's size limit after trimming; not sent",
          metadata: [
            "bytes": .stringConvertible(trimmedSize),
            "limit": .stringConvertible(Self.maximumPayloadBytes),
  ...
  private func deliver(
    ...
    let message: [String: Any] = [
      "message": [
        ...
        "android": [
          ...
          // milliseconds: the reference passes `24 * 60 * 60 * 1000` with a
    ...
    do {
      ...
      // Status and code only. The token is the device's identity and never goes in a log.
      var metadata: Logger.Metadata = ["outcome": .string(String(describing: outcome))]
      if case .requestFailed(let status, let code, _) = error {
        metadata["status"] = .stringConvertible(status)
        metadata["code"] = .string(code ?? "-")
      }
      logger.debug("FCM rejected a notification", metadata: metadata)
      return outcome
    } catch {

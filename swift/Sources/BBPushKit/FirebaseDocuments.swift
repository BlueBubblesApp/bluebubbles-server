...
public actor RestartCommandReader {
  ...
  private func firestoreNextRestart() async throws -> Int64? {
    ...
    // Decoded, not `try?`-ed: this is polled for the life of the server, and a document
    // shape this build cannot read would otherwise disable remote restart in silence.
    let document = try GoogleAPIClient.decode(
      Document.self, from: data, describing: "reading the restart command document")
    guard let value = document.fields?["nextRestart"] else { return nil }

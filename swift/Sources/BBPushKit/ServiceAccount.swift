...
public actor PushCredentialStore {
  ...
  /// Nil when nothing is stored. A stored document that no longer parses THROWS (through
  /// the same `parse` that admitted it at import) rather than reading as nil, because
  /// "the key is damaged" and "no key was ever imported" call for different remedies and
  /// the status screen has to be able to tell them apart.
  public func serviceAccount() throws -> ServiceAccount? {
    guard let raw = try rawServiceAccount() else { return nil }
    return try ServiceAccount.parse(raw)
  }

  public func clientConfig() throws -> FirebaseClientConfig? {
    guard let raw = try rawClientConfig() else { return nil }
    return try FirebaseClientConfig.parse(raw)
  }
  ...
  ///
  /// `try?` because this is the startup GATE, whose only answers are start or do not, and
  /// a Keychain that refuses is reported on another path: `PushInterface.status()` carries
  /// it as `credentialProblem`, which the Firebase page headlines.
  public func isConfigured() -> Bool {
    ((try? serviceAccount()) ?? nil) != nil
  }

  /// Whether the service account specifically is stored. Used by the migration, which
  /// decides each file on its own, so a Keychain that will not answer THROWS rather than
  /// reading as "not stored", which would have the migration import over a credential it
  /// could not see.
  public func hasServiceAccount() throws -> Bool {
    try secrets.get(Self.serviceAccountKey)?.isEmpty == false
  }

  /// Whether the client config specifically is stored.
  public func hasClientConfig() throws -> Bool {
    try secrets.get(Self.clientConfigKey)?.isEmpty == false
  }

  /// Whether push can start.
  ///
  /// The same question as `isConfigured`, and that is the point. Adoption is a step the user
  /// takes before the server starts, so by the time this is asked the credentials are
  /// either in the Keychain or genuinely absent.
  ///
...
public enum PushCredentialMigration {
  ...
  ///
  /// **Each file is decided on its own.** A single guard over both has a hole: `server.json`
  /// imports and is deleted, `client.json` then fails, and the store reads as configured:
  /// so the next run would return early and `client.json` would never be migrated or
  /// deleted. The service account is skipped only if the service account is already stored,
  /// and likewise the client config, so a half-done move finishes on the next attempt.
  @discardableResult
  public static func migrateIfNeeded(
    ...
    if try await !store.hasServiceAccount(),
    ...
    if try await !store.hasClientConfig(),

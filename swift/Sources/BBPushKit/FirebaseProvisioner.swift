...
public actor FirebaseProvisioner {
  ...
  /// IAM call, so leaving it off breaks the step that the whole flow exists to reach. The
  /// reference server enables it in a method called `enableIdentityApi`, which is easy to
  /// misread as Identity Toolkit: Firebase Auth, a different service this server does not
  /// use.
  ...
  public func listProjects() async throws -> [FirebaseProjectSummary] {
    ...
    let listing = try GoogleAPIClient.decode(
      Listing.self, from: response, describing: "listing Firebase projects")
  ...
  public func inspectForAdoption(
    ...
    // `try?`: `firebaseAdminAccount` has already logged why at error level, and a plan
    // without an account is a real answer: the key decision is simply not offered.
    ...
    let existing: [String]
    do {
      existing = try await userManagedKeyNames(in: projectId, account: account.identifier)
    } catch {
      // Reported here, because the plan cannot carry it: a count of zero below means the
      // dialog will not offer to delete keys, which is the safe direction to be wrong in.
      logger.error(
        "Could not list the project's existing keys; the plan assumes none",
        metadata: [
  ...
  private func userManagedKeyNames(in projectId: String, account: String) async throws -> [String] {
    let response = try await api.send(
      method: "GET",
    ...
    let all =
      try GoogleAPIClient.decode(Keys.self, from: response, describing: "listing keys").keys ?? []
    ...
    if keyStrategy == .reuseHeld, let existingServiceAccountJSON {
      ...
      serviceAccountJSON = try await step(.creatingServiceAccount, onProgress) {
        ...
        if case .mintNew(deletingExisting: true) = keyStrategy {
          // Propagated: the user chose to delete the old keys, and a listing this build
          // cannot read must fail the run rather than quietly delete none of them.
          previous = try await userManagedKeyNames(
  ...
  private func createProject(projectId: String, displayName: String) async throws {
    ...
    try await awaitOperation(
      named: try operationName(in: response),
  ...
  /// Enabling them one at a time costs a POST plus its own polling loop per service:
  /// measured at 26 seconds of a 105-second run, for five services that Google is happy
  /// to switch on together. Enabling is idempotent, so the list is applied without checking first.
  ...
  private func firebaseAdminAccount(
    ...
    while ContinuousClock.now < deadline {
      ...
      do {
        ...
        // A shape this build cannot read lands in `lastError` with the rest, and is
        // named when the account never appears.
        let accounts =
          try GoogleAPIClient.decode(
            Accounts.self, from: response, describing: "listing service accounts"
          ).accounts ?? []
  ...
  private func generateServiceAccount(for projectId: String) async throws -> Data {
    ...
    for attempt in 1...12 {
      do {
        ...
        let encoded = try GoogleAPIClient.decode(
          Key.self, from: response, describing: "creating a service account key")
        guard let decoded = Data(base64Encoded: encoded.privateKeyData) else {
          throw GoogleAPIError.decodingFailed(reason: "the new key's data is not base64")
        }
  ...
  private func existingAndroidApp(in projectId: String, packageName: String) async -> String? {
    do {
      ...
      let apps =
        try GoogleAPIClient.decode(Listing.self, from: response, describing: "listing apps")
        .apps ?? []
      ...
      // is better handled by attempting the registration than by aborting here. Said out
      // loud, though: a listing this build could not read is worth a line before the
      // registration attempt explains itself.
      logger.warning(
        "Could not read the project's existing apps; registering instead",
        metadata: [
          "projectId": .string(projectId),
          "error": .string(String(describing: error)),
        ])
  ...
  static func appName(in data: Data?) -> String? {
    ...
    // `try?` as a probe: this looks for a name in whichever of two shapes carries it, and
    // "neither" is the nil the caller handles.
  ...
  private func fetchClientConfig(appName: String) async throws -> Data {
    ...
    let wrapper = try GoogleAPIClient.decode(
      Config.self, from: response, describing: "fetching the app's configuration")
    guard let decoded = Data(base64Encoded: wrapper.configFileContents) else {
      throw GoogleAPIError.decodingFailed(reason: "configFileContents is not base64")
    }
    // Parsed only to VALIDATE, and the failure is the typed one `parse` raises, which
    // names what is wrong with the file rather than reporting it as absent.
    _ = try FirebaseClientConfig.parse(decoded)
    return decoded
  }
  ...
  private func operationName(in response: Data) throws -> String? {
    struct Operation: Decodable {
      let name: String?
      let done: Bool?
    }
    // Both fields optional, so this only fails on a body that is not JSON at all, and a
    // call that answered with one is not "completed inline", it is broken.
    let operation = try GoogleAPIClient.decode(
      Operation.self, from: response, describing: "reading an operation")
  ...
  ///   Polling them all against Cloud Resource Manager 404s on every step after project
  ///   creation: enabling the APIs appears to hang and then times out with a message about
  ///   the wrong thing.
  @discardableResult
  private func awaitOperation(
    ...
    while ContinuousClock.now < deadline {
      ...
      // A poll answer this build cannot read used to keep polling until the timeout, which
      // then blamed the step for being slow. It fails as what it is.
      let operation = try GoogleAPIClient.decode(
        Operation.self, from: response, describing: "polling \(step)")
      ...
      // second or two of the first poll, and a fixed interval would spend the same three
      // seconds waiting on every one of them regardless, in a flow whose slowness is its
      // main usability problem. The interval still grows to three seconds for the
      // operations that genuinely take minutes, so nothing is polled harder for longer.

...
public actor PushService {
  ...
  public func start() async {
    // Credentials are NOT migrated here any more.
    //
    // They used to be, as the first thing `start` did — which forced the startup gate
    // (`PushDeliveryService.canRun`) to answer "there is plaintext waiting" as well as
    // "there are credentials", or push would decline on exactly the installs that had
    // something to move and the plaintext would never be deleted. Adopting an install is
    // now an explicit step the user takes before the server runs at all, so the gate can
    // ask the plain question and this can assume the answer.

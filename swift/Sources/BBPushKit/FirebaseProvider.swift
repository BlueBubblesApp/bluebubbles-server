...
//  event; this is what joins them. With no sink registered nothing fails: push is
//  configured, `server/info` reports it active, and Android clients receive nothing unless
//  they happen to hold a socket open.
//
...
public struct FirebaseProvider: NotificationProvider {
  ...
  /// Static as well as a member, so the service that DETACHES it names the same constant
  /// the provider declares. Two spellings of a provider id means one side detaches nothing
  /// and the other keeps delivering after its service has stopped.
  public static let identifier = "firebase"
  ...
  /// webhook under v1) receives them. Applied at the bus it would take them away from every
  /// other transport too.
  ///
  ...
  public func send(_ event: ServerEvent) async throws {
    ...
    guard !devices.isEmpty else {
      logger.debug(
        "No registered devices; not sending",
        metadata: ["event": .string(event.name.rawValue)])
      return
    }
    ...
    // client app can parse, which is nothing to do with ntfy or a webhook.

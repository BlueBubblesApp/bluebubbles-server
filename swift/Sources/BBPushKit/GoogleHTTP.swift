...
public struct GoogleAPIClient: Sendable {
  ...
  /// A response body as the shape a call expects, or `decodingFailed` naming the call.
  ///
  /// The provisioner read a dozen responses with `try?`, which turned a changed response
  /// shape into "no projects", "no keys", "no apps", a quiet empty answer where the true
  /// one was "Google said something this build does not understand". A shape this build
  /// cannot read is a failure of THIS build, and it has to say so.
  public static func decode<T: Decodable>(
    _ type: T.Type, from data: Data, describing call: String
  ) throws -> T {
    do {
      return try JSONDecoder().decode(T.self, from: data)
    } catch {
      throw GoogleAPIError.decodingFailed(
        reason: "\(call) answered in a shape this build cannot read: \(error)")
    }
  }
  ...
  static func decodeError(status: UInt, body: Data) -> GoogleAPIError {
    ...
    // `try?` as a probe: an envelope that is not Google's falls through to the raw body
    // below, which is the report.

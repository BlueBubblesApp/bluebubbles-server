...
//  Two properties:
//
//    1. **Length.** Google allows 30 characters for a project ID, so the suffix runs to the
//       full length the prefix leaves room for. Four hex characters is 16 bits.
//    2. **Source.** A CSPRNG. Predictable output makes the length irrelevant, because an
//       attacker who can predict the generator does not need to search at all.
//

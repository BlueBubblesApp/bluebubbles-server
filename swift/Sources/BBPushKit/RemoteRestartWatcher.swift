...
public actor RemoteRestartWatcher {
  ...
  /// How far ahead of this server's clock a command may be stamped and still be acted on.
  ///
  /// The timestamp comes from the client's clock and is judged against ours, so a small
  /// disagreement is normal. Everything beyond it is refused WITHOUT advancing
  /// `lastHonoured`; see `evaluate`, where a future timestamp used to latch the high-water
  /// mark forward and disable the button permanently.
  static let clockSkewTolerance = Duration.seconds(120)
  ...
  func evaluate(timestamp: Int64, now: Date) -> RestartDecision {
    ...
    // THE FUTURE, checked before freshness because freshness cannot see it.
    //
    // `age` is NEGATIVE for a timestamp ahead of now, so `age > freshnessWindow` was false
    // for every future value however absurd, and the command was honoured. Honouring it
    // then set `lastHonoured` to that value, and the `timestamp > lastHonoured` guard above
    // is what turns that into a permanent lockout: nothing a real client can write is newer
    // than a timestamp in the year 30000, so the restart button never works again on that
    // install. One write to a document the published rules make world-writable, which is
    // exactly the attacker this file exists to bound. The rate limit bounds the loop and
    // bounded nothing about this.
    //
    // A small tolerance rather than zero, because the two clocks are genuinely independent:
    // the timestamp is stamped by the CLIENT and compared against the SERVER's clock, and a
    // phone a few seconds ahead is ordinary rather than hostile. Anything further ahead is
    // refused as stale, which is the honest description (it is not a request this server
    // can act on) and, more importantly, leaves `lastHonoured` untouched.
    if age < -Self.clockSkewTolerance.seconds {
      return .stale(timestamp: timestamp)
    }

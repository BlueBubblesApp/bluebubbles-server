...
public enum SecurityRuleset {
  ...
  ///   revoked. Without this the rules check would report "nothing needed changing"
  ...
  public static func isPermissive(
    ...
    case .realtime:
      ...
      // shapes the reference publishes and both are too broad.
...
public actor SecurityRulesManager {
  ...
  private func currentFirestoreRules() async throws -> String {
    ...
    let name = try GoogleAPIClient.decode(
      Release.self, from: release, describing: "reading the cloud.firestore release"
    ).rulesetName
    ...
    let decoded = try GoogleAPIClient.decode(
      Ruleset.self, from: ruleset, describing: "reading the released ruleset")
    guard let content = decoded.source.files.first?.content else {
  ...
  private func publishFirestoreRules() async throws {
    ...
    let name = try GoogleAPIClient.decode(
      Created.self, from: created, describing: "creating a ruleset"
    ).name
    ...
    //
    // A release exists on any project that has ever had rules, and a freshly provisioned
    // project has never had any: its `cloud.firestore` release does not exist yet and
    // `PATCH` answers 404 NOT_FOUND.
    //

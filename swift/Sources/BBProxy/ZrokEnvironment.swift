...
public enum ZrokError: BBError, Equatable {
  /// One zrok command could not be run or failed; see `ToolCommandError`.
  ...
  /// A failed command, in the shape `run` throws: for the two places that construct one
  /// with a better sentence than zrok printed.
  static func commandFailed(command: String, output: String) -> ZrokError {
    .command(.commandFailed(tool: "zrok", command: command, output: output))
  }
...
public actor ZrokEnvironment {
  ...
  public func enableIfNeeded(accountToken: String) async throws -> Bool {
    ...
    do {
      ...
      guard case .command(.commandFailed(_, _, let output)) = error else { throw error }
  ...
  public func reserve(
    ...
    logger.info(
      ...
      metadata: [
        ...
        "share": .string(trimmedName.isEmpty ? "(generated)" : trimmedName),
      ])
    ...
    do {
      ...
      guard case .command(.commandFailed(_, _, let text)) = error else { throw error }
  ...
  public func release(token: String) async -> Bool {
    ...
    do {
      ...
      if case .command(.commandFailed(_, _, let output)) = error,
  ...
  private func run(
    ...
    do {
      ...
      throw ZrokError.command(failure)
    }
  }
}

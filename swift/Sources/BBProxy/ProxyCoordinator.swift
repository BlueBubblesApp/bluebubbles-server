...
/// tunnel with a browser sign-in be expressible at all.
...
public struct ProxyObserver: Sendable {
  ...
  /// The tunnel dropped and the provider is bringing it back. Carries what happened, for
  /// the log; the address is gone until `addressChanged` says otherwise.
  public let disconnected: @Sendable (String) async -> Void
...
public actor ProxyCoordinator {
  ...
  /// Called after anything `address` or `pendingReason` depends on has moved, so a health
  /// report taken then is current. Fires after the more specific callbacks, never instead.
  private let onStateChanged: @Sendable () async -> Void
  ...
  public init(
    ...
    onStateChanged: @escaping @Sendable () async -> Void = {}
  ) {
    self.policy = policy
    self.logger = logger
  ...
  public func start(_ provider: any ProxyProviding) async throws {
    ...
    await provider.observe(
      ProxyObserver(
        addressChanged: { [weak self, onAddressChanged, onStateChanged, logger] address in
          logger.info("The tunnel published an address on its own")
          await self?.clearPending()
          await onAddressChanged(address)
          await onStateChanged()
        },
        failed: { [onFailure, onStateChanged] reason in
          await onFailure(reason)
          await onStateChanged()
        },
        attentionRequired: { [weak self, onAttention, onStateChanged, logger] attention in
          logger.info(
        ...
        disconnected: { [onStateChanged, logger] reason in
          logger.info("The tunnel dropped", metadata: ["reason": .string(reason)])
          await onStateChanged()
        }
      )
    )

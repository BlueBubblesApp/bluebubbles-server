...
public actor BinaryTunnel: ProxyProviding {
  ...
  public func connect() async throws -> String {
    // This is what spends `DaemonProcess`'s restart budget. Without it a tunnel that
    // died would stay dead, with the server publishing an address that resolves to
    // nothing.
  ...
  private func handleUnexpectedExit(code: Int32) async {
    ...
    address = nil
    await observer?.disconnected("the \(identifier.shortName) tunnel exited with code \(code)")
...
public struct CloudflareOptions: Sendable, Equatable {
  ...
  public func arguments(forwardingTo port: Int) -> [String] {
    ...
    //
    // "Serve over HTTPS" on the Connection page makes the local server speak TLS, and
    // cloudflared pointed at http would retry against a port that will not talk to it in
    // plaintext.
...
public struct NgrokOptions: Sendable, Equatable {
  ...
  ///
  /// **The authtoken goes here rather than on the command line.** A process's arguments are
  /// world-readable on macOS: any local account could run `ps` and
  ...
  ///
  /// `CloudflareOptions.environment` does the same, for the same reason.
  public var environment: [String: String] {
...
public enum Tunnels {
  ...
  public static func cloudflare(
    ...
    return BinaryTunnel(
      ...
      readiness: ReadinessSignal { line in
        ...
        return Self.quickTunnelAddress(in: line)
      },
      logger: logger
    )
  }

  /// The address cloudflared was ASSIGNED, out of one line of its output.
  ///
  /// Specific rather than "the first https:// on a line mentioning trycloudflare.com",
  /// because that matched Cloudflare's own API endpoint. cloudflared names
  /// `https://api.trycloudflare.com/tunnel` in its errors, including the rate-limit error
  /// it returns when quick tunnels are requested too quickly, and a loose match would store
  /// `https://api.trycloudflare.com/tunnel":` as this server's published address, announced
  /// to Firebase and handed to every client as the way to reach the Mac.
  ///
  /// Three conditions:
  ///   - the host must be a SUBDOMAIN of trycloudflare.com, which `api.` is not;
  ///   - there must be no path, which the API endpoint has and an assigned address never
  ///     does;
  ///   - it must parse as a URL at all, which is what discards the trailing `":` left by a
  ///     JSON error body.
  ///
  /// Announced inside an ASCII box, hence `|` among the characters trimmed off the ends:
  ///
  ///     +------------------------------------------------------------+
  ///     |  Your quick Tunnel has been created! Visit it at ...        |
  ///     |  https://<assigned>.trycloudflare.com                       |
  ///     +------------------------------------------------------------+
  static func quickTunnelAddress(in line: String) -> String? {
    guard let start = line.range(of: "https://") else { return nil }
    let token = String(line[start.lowerBound...].prefix { !$0.isWhitespace })
      .trimmingCharacters(in: CharacterSet(charactersIn: "|\"', "))
    guard let url = URL(string: token), let host = url.host,
      host.hasSuffix(".trycloudflare.com"), host != "api.trycloudflare.com",
  ...
  public static func zrok(
    ...
    BinaryTunnel(
      ...
      readiness: ReadinessSignal { shareAddress(in: $0) },
      logger: logger
    )
  }

  /// The share URL in a line of zrok's output.
  ///
  /// **zrok logs JSON, and compact JSON has no whitespace after a value.** Reading to the
  /// next space therefore ran straight off the end of the URL and through the rest of the
  /// object, and the trailing `}` survived the end-trim because it was not in the set. The
  /// published address became
  ///
  ///     https://<assigned>.share.zrok.io","time":"2026-09-14T15:38:33.797Z"}
  ///
  /// which is written to `server_address`, sent to Firebase and handed to every client, so
  /// the tunnel came up and nothing could reach it. Stopping at the quote is what a JSON
  /// value needs; the `URL` parse is the same backstop `quickTunnelAddress` uses, and for
  /// the same reason.
  ///
  /// The host is NOT required to be `share.zrok.io`: a self-hosted controller
  /// (`ZROK_API_ENDPOINT`) hands out its own domain, and demanding zrok's would break
  /// exactly the people who went to the trouble of running their own.
  static func shareAddress(in line: String) -> String? {
    guard let start = line.range(of: "https://"), line.contains("zrok") else { return nil }
    let token = String(
      line[start.lowerBound...].prefix { !$0.isWhitespace && !"\"'`,".contains($0) }
    ).trimmingCharacters(in: CharacterSet(charactersIn: "|\"', "))
    guard let url = URL(string: token), let host = url.host, !host.isEmpty else { return nil }
    return token
  }

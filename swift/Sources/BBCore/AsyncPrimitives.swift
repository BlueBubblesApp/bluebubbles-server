...
///
/// Scope is per-instance: hold one on the service that owns the work, and two services
/// doing similar work cannot collide.
public actor AsyncCoalescer<Key: Hashable & Sendable, Value: Sendable> {
  ...
  public func run(
    ...
    // Clears the slot only if it is still OURS. An unconditional clear let a late-resuming
    // joiner wipe a different caller's in-flight task: the next caller then started a
    // second execution of work that was already running. It costs a duplicate run rather
    // than a wrong answer, which is why it survived, and it defeats the whole point of a
    // coalescer.
...
public struct BoundedCache<Key: Hashable, Value>: Sendable where Key: Sendable, Value: Sendable {
  ...
  /// removed key leaves its slot behind rather than being searched for. Both would
  /// otherwise be O(n), and the change detector evicts thousands of entries in one
  /// reconcile pass.
...
extension Duration {
  ...
  /// Whole milliseconds, for a log line's `ms`. Truncated, not rounded: a reader wants
  /// "how long", and 0 for a sub-millisecond call reads better than 1.

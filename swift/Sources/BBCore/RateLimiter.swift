...
//  every device at once) and the reference sleeps 250ms between them inside the
...
public actor IntervalGate {
  ...
  public func attempt() -> Decision {
    ...
    // A BACKWARDS CLOCK opens the gate rather than wedging it shut.
    //
    // `clock.now` is wall-clock, which can step backwards: an NTP correction after sleep,
    // or someone changing the date. Elapsed time then goes negative, the guard below fails,
    // `lastPassed` is never refreshed, and the gate stays shut until wall-clock catches up
    // to a future it was never at — hours, for the FindMy refresh this gates. Everything
    // else in this file uses a monotonic clock; this one cannot, because `lastPassedAt` is
    // reported to callers as a real date.
    //
    // Treating a negative interval as "the stored time is nonsense" is the safe direction:
    // the worst case is one extra call through a gate whose whole purpose is being polite
    // to a remote service, against a gate that stops answering for the rest of the day.

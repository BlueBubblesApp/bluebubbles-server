...
///
/// Trivial-looking, and two things about it are load-bearing.
///
/// **The edge has to survive a relaunch.** Writing the fresh observation straight back over
/// the expectation stores `false` right after a relaunch (the app has not started yet when
/// it is sampled) and the running → stopped transition can then never occur again: the
/// launcher would restart the server exactly once per login and then sit there, alive,
/// watching nothing.
///
/// **A boolean cannot do this job at all.** Immediately after a launch, "not

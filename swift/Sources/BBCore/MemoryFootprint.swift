...
public enum MemoryFootprint {
  ...
  /// Runs `body` and reports the HIGHEST footprint seen while it ran.
  ///
  /// `growth` measures after an autorelease drain, which makes it a leak detector: it answers
  /// "what did this workload keep", and a workload that allocates 300MB and frees it all
  /// reads as zero. That is the right question for "returns to baseline" and the wrong one
  /// for a machine with 4GB, where what matters is whether the peak fits in what is left --
  /// and where exceeding it means swapping to a spinning disk rather than getting slower.
  ///
  /// Sampled from a thread rather than read at the end, because the peak is by definition a
  /// moment the body is still inside. The sampling interval bounds how brief a spike can be
  /// and still be seen; a millisecond is far shorter than any allocation this measures.
  ///
  /// Process-wide, so concurrent work in the same process shows up here. Suites that use
  /// this should be `.serialized`.
  public static func peak<T>(
    during body: () async throws -> T
  ) async rethrows -> (result: T, peak: Int64, baseline: Int64) {
    let baseline = Int64(current() ?? 0)
    let sampler = PeakSampler(baseline: baseline)
    sampler.start()
    let result = try await body()
    sampler.stop()
    return (result, sampler.highWaterMark, baseline)
  }

  /// Watches the process footprint on its own thread.
  private final class PeakSampler: @unchecked Sendable {
    private let lock = NSLock()
    private var peak: Int64
    private var isRunning = true

    init(baseline: Int64) { peak = baseline }

    var highWaterMark: Int64 { lock.withLock { peak } }

    func start() {
      let thread = Thread { [self] in
        while lock.withLock({ isRunning }) {
          if let now = MemoryFootprint.current() {
            lock.withLock { peak = max(peak, Int64(now)) }
          }
          // Short enough to catch an allocation spike, long enough not to be the workload.
    ...
    func stop() {
      // One final reading, in case the peak landed between the last sample and here.
      if let now = MemoryFootprint.current() {
        lock.withLock { peak = max(peak, Int64(now)) }
      }

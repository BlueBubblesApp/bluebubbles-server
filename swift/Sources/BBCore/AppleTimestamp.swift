...
public struct AppleTimestamp: Hashable, Sendable, Comparable {
  ...
  ///
  /// TRUNCATED, not rounded, because the reference truncates and this is wire contract.
  /// `dateUtil.ts` builds `new Date(appleEpochMs + raw / multiplier)` from a fractional
  /// value, and `new Date(number)` applies `ToIntegerOrInfinity`, which discards the
  /// fraction. Rounding here made roughly half of all rows a millisecond later than the
  /// same row served by the reference, across all seven date fields, and the parity diff
  /// cannot see it: it compares keys and types, and both agree.
  ///
  /// `.towardZero` rather than `.down` so the two also agree for a date before 1970, where
  /// JavaScript truncates toward zero and flooring would not. No such row exists in a
  /// `chat.db`; matching exactly costs nothing and removes the question.
  ///
  /// This server already made the same argument in the other direction:
  /// `BlobWireFormat` truncates plist dates, citing the same measured one-millisecond
  /// disagreement.

...
public enum LauncherContract {
  ...
  /// The argument the launcher passes when IT started the app rather than a person.
  ///
  /// **There is no way to ask macOS this question.** A GUI app launched by launchd at login
  /// and one double-clicked in Finder differ in nothing the process can read: the
  /// environment, the arguments and the activation are the same, and
  /// `NSApplicationLaunchIsDefaultLaunchKey` answers a different question (whether a
  /// document was opened). So the one process that knows says so, in the only channel it
  /// has: `NSWorkspace.OpenConfiguration.arguments`.
  ///
  /// It covers every automatic start there is, because the launcher performs all of them:
  /// the one at login, and every relaunch after a crash. A launch WITHOUT it is treated as
  /// a person opening the app, which is the safe direction — the settings this decides
  /// (the startup delay, starting minimised) all hide the window or make it look stuck, and
  /// getting it wrong towards "a person is watching" costs a window that stays open.
  ///
  /// Two known gaps, both deliberate. A bundle assembled without the launcher registers
  /// `SMAppService.mainApp`, which cannot carry arguments, so a login there reads as a
  /// person; `Tools/dev-bundle.sh` and a release both ship the launcher, so this is a
  /// hand-assembled bundle only. And a `--headless` launch never has a window to minimise,
  /// so the question does not arise.
  public static let automaticLaunchArgument = "--auto-launched"

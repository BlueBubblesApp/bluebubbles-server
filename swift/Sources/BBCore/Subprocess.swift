//  Subprocess
//  The one way to run a child process.
//
//  Four decisions every caller would otherwise make on its own, and get wrong differently:
//
...
//      the parent blocks waiting for the child.
//    - **Timeouts.** A command with no deadline can hang the caller forever; an unpacker
//      that decides to prompt is enough.
//    - **stdin.** Detached, because tools prompt: `unzip` asks when an archive contains a
//      name that already exists, and a child waiting for an answer hangs the caller.
...
//      so the wait is taken off the pool.
//
...
public enum Subprocess {
  ...
  /// The most child output that is kept, per call.
  ///
  /// Every caller here runs a tool with a few lines of output, so nothing reaches this
  /// today. That is exactly why it needs a number: the type exists so a caller does not have
  /// to reason about an unbounded child, and an unbounded capture was the one hazard it did
  /// not cover.
  static let maximumCapturedBytes = 4 * 1024 * 1024
  ...
  public struct Result: Sendable {
    ...
    /// Whether the capture cap discarded anything, in which case `output` is the TAIL.
    ///
    /// Reported rather than left implicit: a caller diagnosing a failure from the last few
    /// lines should know whether there were earlier ones, and a silently shortened
    /// transcript is its own kind of wrong answer.
  ...
  ///
  /// - Parameter timeout: Required, with no default, deliberately. A command with no
  ///   deadline is one that can hang the caller forever, and a default is how that
  ///   happens by oversight. No default forces the decision to be made once, visibly, per
  ///   call site.
  ...
  public static func run(
    ...
    do {
      ...
      drain.finishBlocking(waitingForEOF: false)
    ...
    //
    // `terminate()` alone did not live up to that comment. It sends SIGTERM, which a child
    // is free to catch or ignore, so a child that ignores it left the wait suspended
    // forever: precisely the hang this file exists to prevent, reachable only on the path
    // that was supposed to prevent it. SIGKILL follows after a grace period, and cannot be
    // ignored.
    let killed = Flag()
    let watchdog = Task {
      ...
      killed.set()
      process.terminate()
      // Long enough for a well-behaved child to flush and exit, short enough that a caller
      // already past its deadline is not waiting on a process that will never cooperate.
      try? await Task.sleep(for: Self.terminationGrace)
      if process.isRunning { kill(process.processIdentifier, SIGKILL) }
    }

    // Cancellation reaches the CHILD, not just the wait.
    //
    // `exited.wait()` is a bare continuation resumed only by `terminationHandler`, and the
    // watchdog is unstructured, so a cancelled caller used to wait out the full timeout with
    // no way to shorten it. Killing the child is what makes the wait finish, which is the
    // same mechanism the deadline uses.
    ...
    let collected = await drain.finish()

    if killed.isSet {
      throw Failure.timedOut(executable: executable, seconds: timeout.seconds)
    }
    return Result(
      status: process.terminationStatus, output: collected.data,
      outputWasTruncated: collected.truncated)
  }
  ...
  public static func runSynchronously(
    ...
    if finished.wait(timeout: .now() + timeout.seconds) == .timedOut {
      ...
      drain.finishBlocking()
      throw Failure.timedOut(executable: executable, seconds: timeout.seconds)
    }

    let collected = drain.finishBlocking()
    return Result(
      status: process.terminationStatus, output: collected.data,
      outputWasTruncated: collected.truncated)
  }
  ...
  ///      closed, which is the same condition, so it deadlocks in exactly the case it
  ...
  private final class Drain: @unchecked Sendable {
    ...
    /// Set once the cap has discarded anything, so the caller can be told rather than
    /// handed a silently shortened transcript.
    ...
    init(pipe: Pipe?) {
      ...
      pipe?.fileHandleForReading.readabilityHandler = { [self] handle in
        ...
        lock.lock()
        // CAPPED. A child that spews grew the parent's heap without limit, and the whole
        // point of this type is that callers do not have to think about that. The TAIL is
        // kept rather than the head: every caller here reads output to diagnose a failure,
        // and a failure says what went wrong at the end.
        buffer.append(chunk)
        if buffer.count > Subprocess.maximumCapturedBytes {
          buffer.removeFirst(buffer.count - Subprocess.maximumCapturedBytes)
          truncated = true
        }
        lock.unlock()
      }
    ...
    /// The BLOCKING form, for `runSynchronously` and for the spawn-failed path where there
    /// is nothing to wait for. The async caller uses `finish()` below, which does not
    /// occupy a cooperative thread.
    @discardableResult
    func finishBlocking(waitingForEOF: Bool = true) -> (data: Data, truncated: Bool) {
      guard let pipe else { return (Data(), false) }
      if waitingForEOF {
    ...
    /// OFF THE COOPERATIVE POOL, which is what this file's fourth header bullet promises
    /// and what `finishBlocking` does not deliver.
    ///
    /// That bullet says "the wait is taken off the pool", and it was true of the exit wait
    /// and false here: `finishBlocking`'s first act is a `DispatchSemaphore.wait` of up to
    /// two seconds, called synchronously from an `async` function. In the common case EOF
    /// has already arrived and it returns at once; when a grandchild inherits the pipe —
    /// a daemonising tool, which `BBProxy` handles elsewhere and nothing here prevents —
    /// it is a full two-second stall of a thread the runtime expects back.
    func finish(waitingForEOF: Bool = true) async -> (data: Data, truncated: Bool) {
      guard waitingForEOF, pipe != nil else { return finishBlocking(waitingForEOF: false) }
      await withCheckedContinuation { (continuation: CheckedContinuation<Void, Never>) in
        // A thread of ours to block, rather than one of the pool's. Short-lived: it ends as
        // soon as EOF arrives or the bound expires.
        let waiter = Thread { [reachedEOF] in
          _ = reachedEOF.wait(timeout: .now() + 2)
          continuation.resume()
        }
      ...
      return finishBlocking(waitingForEOF: false)
    }
  }

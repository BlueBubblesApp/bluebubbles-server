...
struct BlueBubblesServerCommand: AsyncParsableCommand {
  ...
  @Flag(
    help: """
      Delete the credentials an older installation left readable in its config database.       Irreversible, and separate from --migrate on purpose.
      """)
  ...
  func run() async throws {
    ...
    //
    // Silently coming up without an upgrading user's port, password and tunnel is the
    // failure this replaces: the server answers on the wrong port with no password, and the
    // only symptom is a log line nobody is watching on a headless box. Certificates are
  ...
  ///
  /// Blocking only, and that distinction is load-bearing. This filtered on `isActionable`
  /// once, which swept in the optional steps too — so a second `--migrate` silently deleted
  /// the plaintext credentials, the one irreversible thing here, with nobody asked. The
  /// app has a wizard to ask in; the CLI's equivalent of consent is a separate flag the
  /// user has to type.
  ///

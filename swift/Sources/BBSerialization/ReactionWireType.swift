...
//
//  The reference maps the column through a TypeORM transformer
//  (`databases/transformers/MessageTypeTransformer.ts`): declared `type: "text"` on the
//  entity and mapped through `ReactionIdToString` on the way out, so what a client has always
//  received is `"love"`, `"-like"`, or `null`. Measured against a live Electron server.
//

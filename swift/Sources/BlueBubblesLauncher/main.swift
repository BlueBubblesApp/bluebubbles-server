...
final class Launcher {
  ...
  private func launchMainApplication() {
    ...
    // WHO started it. The app cannot tell a login from a double-click any other way, and it
    // has to: `start_minimized` and `start_delay` are both answers to "nobody is looking",
    // and applied to a person who just opened the app they read as a hang. Passed on a
    // crash relaunch too, which is the same situation. See `LauncherContract`.
    configuration.arguments = [LauncherContract.automaticLaunchArgument]

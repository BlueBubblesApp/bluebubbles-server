...
public struct AppDatabase: Sendable {
  ...
  ///
  /// Two phases, and the split is deliberate. **Contributors come FIRST**, which is the
  /// opposite of what this comment used to say, and the order is load-bearing:
  ///
  ///   1. **Contributed schema**, in the order given. Each module that owns a table declares
  ///      its own migrations, so a table's creation sits beside the code that reads it.
  ///
  ///   2. **The frozen tail**, registered last. One migration lives here rather than in a
  ///      module because it cannot be split: `normaliseTimestampColumnNames` renames columns
  ///      on `device` (BBInterfaces) and on `blocked_client`, `paired_client` and
  ///      `auth_failure` (BBAuth) in a single step, and a released migration's identifier is
  ///      already recorded on every install. Running last is what makes it work at all: the
  ///      tables it renames are created by the contributors above.
  ///
  /// There is NO core baseline registered here. There was one when this comment was written
  /// and the tables have since moved out to their owners; the description outlived them, and
  /// it described the reverse of what the code does. Migration order is the one thing in
  /// this file that is destructive to get wrong, so it is worth restating: read `migrate`
  /// below rather than trusting a summary, this one included.
  ///

...
//
//  The guarantee here is structural rather than a flag, because a flag can be dropped,
//  overridden or forgotten in a new code path and nothing catches it until a user's history
//  is corrupted. This type exposes no write API at all: no write, no execute, no migrator,
//  so a write does not compile.
//
...
public struct ReadOnlyDatabase: Sendable {
  ...
  ///   reads it.
  /// Called with every statement this database executes.
  ///
  /// For tests that assert a page costs a bounded NUMBER of queries rather than a bounded
  /// time. A page of messages used to run three statements per ROW, and no assertion in the
  /// suite could see it: the answers were right and the tests were fast, because the fixture
  /// has seven messages. Counting statements is the only shape of test that notices.
  public typealias StatementObserver = @Sendable (String) -> Void

  /// - Parameter observingStatements: nil everywhere but a test. GRDB's tracing costs a
  ///   closure call and a string per statement, and nothing in the server reads it.
  public init(
    path: String, maximumReaders: Int = 1, observingStatements: StatementObserver? = nil
  ) throws {
    var configuration = Configuration()
    configuration.readonly = true
    if let observingStatements {
      configuration.prepareDatabase { db in
        db.trace { observingStatements("\($0)") }
      }
    }
    ...
    if maximumReaders > 1 {
      do {
        ...
        // `ServerComposition` logs the reader count it ended up with; this is the reason.
        logger.warning(
          "Could not open a reader pool; falling back to one connection",
          metadata: [
            "requestedReaders": .stringConvertible(maximumReaders),
  ...
  ///
  /// Preferred for anything unbounded: `fetchAll` on a large message query is how memory
  /// grows with the size of the user's history.
  ...
  ///
  /// Written for a CI check that fails any query full-scanning `message`, and that check does
  /// not exist yet: this has no caller outside tests. Recorded as outstanding work in
  /// `docs/TESTING.md` rather than described there as coverage, which is what it used to be.

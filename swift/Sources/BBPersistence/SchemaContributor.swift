...
//
//  ORDER. Migrations are applied in registration order. `AppDatabase` registers each
//  CONTRIBUTOR FIRST, in the order it was given, and then its own frozen tail. Within a
//  contributor, its own declared order holds. Cross-contributor ordering is therefore the
//  caller's to fix and must stay stable; see `AppDatabase.migrate`.
//
//  This said "registers the core baseline first and then each contributor", which was the
//  reverse and named a baseline that no longer exists: the tables moved out to their owners
//  and the sentence did not follow. It also contradicted `CLAUDE.md` in this directory,
//  which had it right. Migration order is destructive to get wrong.
//

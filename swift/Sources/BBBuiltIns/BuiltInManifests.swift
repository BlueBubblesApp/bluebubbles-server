...
//
//  Three things the manifests say that an enum of connection methods could not: that the
//  tunnels are the same KIND of thing and
...
public enum BuiltInManifests {
  ...
  ///
  /// Only Permissions, which reports what macOS granted: there is nothing in it to
  /// disable.
  ///
  /// The socket USED to be here, on the grounds that it is how a connected client is told
  /// anything at all. That is true and is not a reason: a REST-only server is a real
  /// deployment, and the HTTP API (which is strictly more essential to reachability) has
  /// always been switchable. What the socket needs is not permanence but an ORDER, which
  /// it now declares: it depends on the HTTP API, whose listener carries it.
  ///
  /// The HTTP API is the interesting case: switching it off stops the server serving, which
  /// is a thing an operator may genuinely want and is also the one switch that can leave a
  /// headless install unreachable. It is recoverable from the CLI:
  /// `bluebubbles-server --set disabled_services=`, and the app's own UI keeps working
  /// either way, because it talks to the interfaces in-process rather than over HTTP. The
  /// six reverse proxies and the socket declare a dependency on it, so switching it off
  ...
  public static let alwaysOn: Set<ServiceIdentifier> = [ID.permissions]

  // MARK: - Reverse proxies
  //
  // The category is exclusive, so declaring all six and enabling one gives the validator
  // something to check, and gives a plugin something to join, which an enum could never
  // be.

  // MARK: - What every connection method declares

  /// The entitlements a binary tunnel declares. Every binary tunnel runs a program: naming
  /// it is the point: "runs a program on this Mac" is something a user can weigh, and it is
  /// true of ngrok and cloudflared equally. It reaches the hosts named per method, with the
  /// reason for each at the call site. It reads the port to forward to, and whether this
  /// server terminates TLS (which decides the scheme the tunnel proxies to) and NOT the
  /// password: a tunnel has no business with it, and there is no entitlement that would
  /// hand it over anyway. And it publishes the address clients use, declared as a WRITE,
  /// which is the entitlement the model calls a hijack when a plugin asks for it, exactly
  /// why a person should see it on the list.
  ///
  /// One function rather than four copies, so a key a tunnel starts reading is declared
  /// once and the validator cannot catch three of them out.
  static func tunnelEntitlements(hosts: Set<String>) -> [Entitlement] {
    [
      .spawnProcess,
      .network(hosts: hosts),
      .readSettings(keys: [
        Settings.connectionMethod.key, Settings.socketPort.key, Settings.useCustomCertificate.key,
      ]),
      .writeSettings(keys: [Settings.serverAddress.key]),
    ]
  }

  /// What a method with no process and no network egress declares: Local Network and
  /// Custom URL. Worth noticing how little it is, which is exactly the comparison a
  /// permission list is meant to make possible.
  static let publishingEntitlements: [Entitlement] = [
    .readSettings(keys: [Settings.connectionMethod.key, Settings.socketPort.key]),
    .writeSettings(keys: [Settings.serverAddress.key]),
  ]
  ...
  public static let zrok = ServiceManifest(
    ...
    entitlements: tunnelEntitlements(hosts: [
      "api.zrok.io", "zrok.io",
    ...
    settings: [
      ...
      .field(
        FieldDescriptor(
          key: "reserved_name",
          label: "Reserved Name",
          help: "A name to ask zrok for: 4 to 32 characters, lowercase letters and "
            + "digits only. Leave empty and zrok generates one. Changing it releases the "
            + "old share and reserves a new one, which changes the address clients use.",
          // NOT "my-server". zrok refuses a hyphen, so the placeholder was an example of
          // a name it would reject, in the field whose rule it was demonstrating.
          kind: .text(placeholder: "myserver"),
          visibleWhen: FieldCondition(field: "reserve_tunnel", equals: "true"),
          // zrok's own rule, checked here so it is answered while the field is on screen.
          // Empty is allowed and means "zrok picks one"; see `ZrokMethod`.
          validation: FieldValidation(
            pattern: "[a-z0-9]{4,32}",
            message: "zrok names must be 4 to 32 characters, lowercase letters and digits "
              + "only: no capitals, spaces, hyphens or underscores."
          )
        )),
      .field(
        FieldDescriptor(
  ...
  public static let ngrok = ServiceManifest(
    ...
    dependencies: [ID.http],
    // `bin.equinox.io` is where the agent itself is downloaded from, and it is on this
    // list because `ManifestValidator` requires every host a declared tool reaches to
    // appear here. That is the point of the rule: "runs a program" and "downloads that
    // program from ngrok's distribution host" are two different things to agree to, and a
    // permissions list that mentioned only the first would be describing something other
    // than what happens.
    entitlements: tunnelEntitlements(hosts: [
  ...
  public static let tailscale = ServiceManifest(
    ...
    details: """
      ...
      unless Funnel is on. The Tailscale daemon is downloaded on first use from Homebrew's \
      package registry; Homebrew itself does not need to be installed.
      """,
    category: .reverseProxy,
    symbol: "lock.shield",
    dependencies: [ID.http],
    entitlements: tunnelEntitlements(hosts: [
      // Signing in, the coordination server, and the DERP relays that carry traffic
      // when a direct path cannot be found. What a host list CANNOT name is the direct
      // path itself: a tailnet peer is reached by its own address once one is found,
      // and this list is what the permissions screen shows, not what is enforced.
      ...
      // redirects to. See the note on ngrok above.
  ...
  public static var lan: ServiceManifest {
    ServiceManifest(
      ...
      entitlements: publishingEntitlements,
      settings: [
        .paragraph(
  ...
  public static let dynamicDNS = ServiceManifest(
    ...
    name: "Custom URL",
    summary: "Publish an address you manage yourself (i.e. Dynamic DNS).",
    details: """
  ...
  public static let webhooks = ServiceManifest(
    ...
    entitlements: [
      ...
      .readSettings(keys: [
        Settings.ntfyTopic.key, Settings.ntfyServer.key, Settings.ntfyEvents.key,
      ]),
    ]
  )
  ...
  public static let push = ServiceManifest(
    ...
    entitlements: [
      ...
      // `remote_restart_enabled` and `last_fcm_restart` have no presentation, so the
      // permissions sentence names them by their storage keys. That is the honest answer:
      // a setting a service reads is a setting a person is entitled to see on its
      // permissions list, and an ugly name is a better outcome than an undeclared read.
      // `last_fcm_restart` is also written: it is this service's own bookkeeping, and a
      // write it never declared would be exactly the hijack the model exists to surface.
      .readSettings(keys: [
        Settings.serverAddress.key, Settings.remoteRestartEnabled.key, Settings.lastFcmRestart.key,
      ]),
      .writeSettings(keys: [Settings.lastFcmRestart.key]),
    ]
  )
  ...
  public static let http = ServiceManifest(
    ...
    category: .networking,
    // NOT the socket, which now depends on this; see `BuiltInManifests.socket`. The
    // dependency used to point this way for ordering, because HTTP mounts the socket
    // transport; it does not need the socket SERVICE to have run to do that, since the
    // engine it mounts is built by the composition root either way.
    dependencies: [ID.permissions],
    entitlements: [
      .authenticateRequests,
      // `use_custom_certificate` decides whether the listener terminates TLS, and
      // `server_address` goes in the generated certificate's SAN: a client connecting by
      // the name the server published needs a certificate that matches it.
      .readSettings(keys: [
        Settings.socketPort.key, Settings.bindAddress.key, Settings.useCustomCertificate.key,
        Settings.serverAddress.key,
        // Where the installed certificate came from, and when it expires. The renewer acts
        // on these: it regenerates ONLY a self-signed certificate that is genuinely due, and
        // an unknown origin means the user installed it and it must never be replaced.
        Settings.tlsCertificateOrigin.key, Settings.tlsCertificateExpiresAt.key,
      ]),
      // Written when a certificate is generated or imported. Declared as a WRITE so it
      // appears on the permissions list: a service that can rewrite what your server
      // presents as its identity is worth seeing there.
      .writeSettings(keys: [
        Settings.tlsCertificateOrigin.key, Settings.tlsCertificateExpiresAt.key,
      ]),
    ]
  ...
  /// The live event channel, carried on the HTTP API's listener.
  ///
  /// It declares a dependency on the HTTP API rather than the other way round, and the
  /// direction is the whole point. The socket has no port of its own: `HTTPService` binds
  /// once and mounts `/socket.io/` on the same router, so the socket is unreachable without
  /// it. Declared this way, switching the HTTP API off takes the socket with it: the
  /// registry does that for any dependency, and switching the SOCKET off leaves the REST
  /// API alone, which is the independence this exists for. Declared the other way, as it
  /// was, turning the socket off would have taken the whole API and every tunnel down.
  public static let socket = ServiceManifest(
    id: ID.socket,
    name: "Socket",
    summary: "Pushes live events to connected clients.",
    details: """
      Runs on the HTTP API's listener rather than a port of its own, so turning it off \
      closes nothing: the address stays up and the REST API is unaffected. What stops is \
      live delivery: events are no longer pushed, and new socket connections are refused \
      rather than accepted and left silent. Clients fall back to asking over the REST API.
      """,
    category: .networking,
    dependencies: [ID.http],
  ...
  public static let changeDetection = ServiceManifest(
    ...
    dependencies: [ID.permissions],
    entitlements: [.readMessages, .readSettings(keys: [Settings.dbPollInterval.key])],
    // The one permission that genuinely gates a service. Without it there is no database to
    // watch, so the registry reports that precisely rather than letting the first read fail
    // obscurely.
    permissions: [
      .init(
        .fullDiskAccess, .required,
        purpose: "so it can watch your message database for new and changed messages"
      )
    ]
  )
  ...
  public static let contacts = ServiceManifest(
    ...
    // Recommended, not required: without it the server runs and shows phone numbers where it
    // would have shown names. Declaring that distinction is the point: a user weighing
    // "should I grant this" needs to know what refusing costs.
    permissions: [
      .init(
        .contacts, .recommended,
        purpose: "so messages show names and photos instead of phone numbers"
      )
    ]
  )
  ...
  public static let privateAPI = ServiceManifest(
    ...
    entitlements: [
      ...
      .readSettings(keys: [
        ...
        // Carried to the FaceTime helper through the injection environment, so a change
        // re-injects exactly like a dylib path change.
        Settings.faceTimeIdleCameraOff.key,
      ]),
    ],
    // Not a TCC permission and not something the app can prompt for: the user has to
    // disable SIP from Recovery. Declared as required anyway, because the permissions list
    // is where someone finds out what a service will need BEFORE they turn it on, and
    // "reboot into Recovery" is the least welcome thing to discover afterwards.
    permissions: [
      .init(
        .systemIntegrityProtection, .required,
        purpose: "because macOS only allows loading a library inside Messages without it"
      )
    ]
  )
  ...
  public static let sleepPrevention = ServiceManifest(
    ...
    category: .system,
    entitlements: [.readSettings(keys: [Settings.autoCaffeinate.key])],
    isUserManageable: false
  )
  ...
  public static let launchAtLogin = ServiceManifest(
    ...
    category: .system,
    entitlements: [.readSettings(keys: [Settings.autoStartMethod.key])],
    isUserManageable: false
  )
  ...
  public static let toolUpdates = ServiceManifest(
    ...
    category: .system,
    entitlements: [
      .readSettings(keys: [Settings.checkForUpdates.key]),
      // It now looks for copies of these programs already on this Mac, and asking a binary
      // its version means RUNNING it. That is the most consequential thing this model does,
      // and the permissions list is the only place a person sees it, so it is declared here
      // rather than left implicit in what `ToolManager` happens to do.
      .spawnProcess,
    ],
    isUserManageable: false
  )

...
//
//  The files are not a fallback for an unsigned build either: an unsigned build falls back
//  to the LEGACY keychain, which is a different store, so it could never read what a signed
//  build wrote regardless: it generates its own self-signed certificate into its own
//  keychain. A second source of truth would only be one more thing to keep in step forever,
//  with a renewal writing to one copy while the reader consults the other.
//

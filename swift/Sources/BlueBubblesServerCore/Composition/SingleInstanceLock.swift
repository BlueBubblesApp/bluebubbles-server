...
public enum SingleInstanceLock {
  ...
  private static let logger = Logger(label: "bluebubbles.lifecycle.lock")
  ...
  public static func acquire(path: String = SingleInstanceLock.defaultPath) throws -> Int32 {
    ...
    // CLOSE-ON-EXEC, and this is what makes `POST /server/restart/hard` survivable.
    //
    // `ServerLifecycle` replaces the process image with `execv`. A file lock is held per open
    // file DESCRIPTION, and an inherited description keeps its lock across an exec, so
    // without this the replacement image inherits a descriptor that still holds `LOCK_EX`
    // while its own `descriptor` static resets to -1 and `acquire()` opens a fresh one. The
    // new fd is then refused by the lock the old one is still holding, and the server exits
    // with "already running", naming its own pid. Reproduced outside this codebase: a process
    // that flocks a file and re-executes itself without this flag is denied re-acquisition
    // with EWOULDBLOCK.
    //
    // Belt and braces with the explicit `release()` before the exec: this covers any other
    // path that might exec, including one nobody has written yet.
    if fcntl(fd, F_SETFD, FD_CLOEXEC) == -1 {
      logger.warning(
        "Could not set close-on-exec on the single-instance lock; a hard restart may refuse to re-acquire it",
        metadata: ["errno": .stringConvertible(errno)])
    }
  ...
  /// Releases the lock.
  ///
  /// Two callers: tests, which take and drop it repeatedly in one process, and the hard
  /// restart, which must not carry the lock into the image that replaces it. Ordinary exit
  /// still relies on the process ending.

...
//
//  Two rules, both learned from the reference:
//
...
public enum TLSProvisioning {
  ...
  /// question. (A comment here once asserted that a renewal "writes through the same store it
  /// came from"; it was false, and the re-read it justified returned the superseded copy.)
  ...
  /// adopt what an older build left there, and then deleted; nothing writes them. Keeping them
  /// as a live second copy meant keeping two stores in step forever to buy a fallback that
  /// only an unsigned build could reach — and an unsigned build cannot read what a signed one
  /// wrote in any case, because the two are talking to different keychains.
  ///

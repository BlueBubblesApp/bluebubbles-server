...
public enum TLSProvisioning {
  ...
  public static func material(
    ...
    //
    // REFUSED and ABSENT are different answers and this used to collapse them. `try?` turned
    // "the Keychain would not let me read" into "there is nothing stored", and the imported
    // branch below then warns, generates a self-signed certificate and PERSISTS it over
    // `tls.certificate` and `tls.private_key`. A transient `errSecInteractionNotAllowed`, or
    // an entitlement lost to a re-sign, therefore destroyed a certificate the user supplied,
    // and nothing gets it back: rule one of this file is that a user-supplied certificate is
    // never regenerated or deleted.
    let stored: CertificateStore.Material?
    do {
      stored = try await keychain.load()
    } catch {
      logger.error(
        "The Keychain refused to return the TLS certificate",
        metadata: ["error": .string(String(describing: error))])
      await alerts?.raise(
        UserAlert(
          severity: .error,
          title: "Your TLS certificate could not be read",
          body: "The Keychain refused to return it, so the server is starting without "
            + "TLS rather than replacing a certificate it cannot see. Nothing has been "
            + "changed. Unlock the login keychain and restart the server; if this "
            + "persists, re-import the certificate on the Security page.",
          source: "Certificates",
          actions: [.openSettings(.security)],
          dedupeKey: "tls.keychain-refused"
        )
      )
      // Plain HTTP for this run. The alternative is generating over material that is very
      // probably still there and merely unreadable right now.
      return nil
    }

    if let material = stored, material.certificatePEM.isEmpty == false {
  ...
  /// question.
  /// Whether the recorded certificate is inside its renewal window RIGHT NOW.
  ///
  /// `renewIfNeeded` is reached only from `material`, which is reached only from
  /// `HTTPService.start`. Certificates last a year with a thirty-day window, so a server
  /// left up for eleven months serves an expired certificate and the failure is total and
  /// silent on the server side: every client stops connecting and nothing here says why.
  ///
  /// Renewing in place would mean swapping the material under a bound listener, which is a
  /// restart in everything but name. So this reports instead, and the restart the user makes
  /// is what renews — the path that already works. Called on a timer by `HTTPService`.
  static func isDueForRenewal(settings: ScopedSettings) async -> Date? {
    ...
    // Same fail-safe direction as everywhere else here: only a certificate this server
    // generated is ever spoken about, because only that one is ours to replace.
    guard recordedExpiry > 0, recordedOrigin == .selfSigned else { return nil }
    let expiry = Date(timeIntervalSince1970: TimeInterval(recordedExpiry))
    return CertificateAuthority.needsRenewal(notValidAfter: expiry) ? expiry : nil
  }
  ...
  private static func renewIfNeeded(
    ...
    do {
      // The PUBLISHED ADDRESS goes in too, exactly as `generate` puts it there, and leaving
      // it out was the bug: a renewal quietly produced a certificate for `localhost` and the
      // Mac's `.local` name only. On a dynamic-DNS install (the one deployment this feature
      // is for, since every tunnel terminates TLS itself) the certificate stopped matching
      // the hostname clients dial at the moment it renewed, and nothing said so: renewal
      // logged success, the server bound TLS, and every client started failing to verify.
      //
      // `HTTPService` watches `server_address` for this same reason, so the value is always

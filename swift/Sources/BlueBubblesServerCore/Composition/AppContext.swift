...
//    - isolated `var` or `func`: state that changes while the server runs. These are the
//      only members that cost a hop here, and the hop is what protects them.
//
//  What is built on first use: the call log, the sticker store, the FaceTime and restart
//  coordinators, lives on `LazyCollaborators`, its own actor, and is reached through a
//  `nonisolated func` that hops there rather than here.
//
//  Constructed from the four groups `ServerComposition` builds: storage, the read path,
//  the shared services and the transport, taken whole rather than as a parameter each.
//
...
public actor AppContext {
  ...
  /// `server-update`, once per version, whichever path found it. See `UpdateAnnouncer`.
  public nonisolated let updateAnnouncer: UpdateAnnouncer
  ...
  /// FaceTime coordinator and the application-restart coordinator. See `LazyCollaborators`.
  private nonisolated let collaborators: LazyCollaborators

  public nonisolated func callHistory() async -> CallHistoryRepository? {
    await collaborators.callHistory()
  }

  public nonisolated func stickerLibrary() async -> StickerRepository? {
    await collaborators.stickerLibrary()
  }
  ...
  ///
  /// **WEAK, and that is what breaks a retain cycle.** The registry holds its host strongly
  /// (`ServiceRegistry.host`), and its host is this context, so a strong reference here
  /// closes the loop and neither side is ever released. The app builds a fresh composition
  /// on every start, so a stop and start from the window leaked the whole graph each time:
  /// the context, all eighteen services, the event bus, the socket server and the tool
  /// manager. `HTTPServiceHost` already weakens its captures of the context for this reason;
  /// this is the same hazard at the other end of the same edge.
  ///
  /// Safe to weaken THIS side specifically because the registry's owner is `RunningServer`,
  /// which holds it for the life of the server. This is a back-reference for one call
  /// (`noteHealthChanged`), not ownership.
  private weak var registry: ServiceRegistry<AppContext>?
  ...
  ///
  /// The cache above is derived from these, and the `didSet` is what keeps that true:
  /// mutating any part of the struct invalidates the cache, whether or not the author thought
  /// about it. A stale interface is silent: one built before the helper connected holds a
  /// nil reference and goes on reporting the Private API as unavailable for the life of the
  /// process, which is how a working helper looks broken.
  ...
  /// Readable so a test can assert a service published what it owns.
  ...
  /// Assembled from the four groups `ServerComposition` builds, in the order it builds them.
  ///
  /// The groups are taken whole rather than unpacked into a parameter each: the composition
  /// root already produces them as units, and a context that named every member again would
  /// have twenty parameters that all had to be threaded through the same three call sites.
  /// What is not in a group (the route surface and the tool manager) is decided by the
  /// root from configuration, and arrives on its own.
  init(
    ...
    self.appDatabase = storage.appDatabase
    self.settings = storage.settings
    self.secrets = storage.secrets
    self.logger = storage.logger
    self.chatDatabase = readPath.database
    self.schemaProfile = readPath.profile
    self.messages = readPath.messages
    self.serializer = readPath.serializer
    self.systemInfo = SystemInfoProvider(messages: readPath.messages, logger: storage.logger)
    self.contacts = shared.contacts
    ...
    self.socketServer = transport.socketServer
    self.engineIO = transport.engineIO
    self.tokenAuth = transport.tokenAuth
    self.passwordDigests = transport.passwordDigests
    self.additionalRouteGroups = additionalRouteGroups
    self.tools = tools
    self.collaborators = LazyCollaborators(settings: storage.settings, logger: storage.logger)
  }
  ...
  ///
  /// ONE method: two independent late-binding points that must both fire is two chances to
  /// fire only one. `isWired` below turns "somebody remembered" into something a test can
  /// assert; see `AppContextWiringTests`.
  ...
  /// FaceTime link bookkeeping, hand-off tracking and cleanup. Built on first use; see
  /// `LazyCollaborators`.
  /// The FaceTime interface over the helper connected right now.
  public nonisolated func faceTimeInterface() async -> FaceTimeInterface {
    FaceTimeInterface(
      coordinator: await faceTime(), privateAPI: await privateAPIClient(), logger: logger)
  }

  /// The FindMy interface over the helper connected right now and the shared runtime.
  public nonisolated func findMyInterface() async -> FindMyInterface {
    FindMyInterface(runtime: findMy, privateAPI: await privateAPIClient(), logger: logger)
  }

  public nonisolated func faceTime() async -> FaceTimeCoordinator {
    await collaborators.faceTime(
  ...
  /// Restarting Messages or FaceTime with the helper re-injected. Built on first use; see
  /// `LazyCollaborators`.
  public nonisolated func applicationRestart() async -> ApplicationRestartCoordinator {
    await collaborators.applicationRestart(
  ...
  public func interfaces() -> ServerInterfaces? {
    ...
    let built = ServerInterfaces(
      ...
      // The enablement is a CLOSURE, not a value: these interfaces are cached until the
      // Private API republishes, so a flag read here would answer with whatever the
      // setting said at that moment for the rest of the server's life.
      contact: ContactInterface(
        index: contacts,
        ingestor: published.contactsIngestor,

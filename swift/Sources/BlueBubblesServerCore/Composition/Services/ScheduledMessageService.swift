...
//
//  Polled rather than timer-per-row, which is what the reference's service does. A timer per
...
actor ScheduledMessageService: Service {
  ...
  private func send(_ record: ScheduledMessage, now: Date) async {
    ...
    // ordering is what the reference's service does, and for the same reason.

...
actor PushDeliveryService: Service, GatedService, ConfigurableService {
  ...
  func stop() async {
    ...
    // DETACH the provider; do NOT unregister the lane.
    //
    // `NotificationSink` is one object with one `SinkID`, and BOTH this service and the
    // webhook service register it — ntfy is a provider on the same sink. `EventBus.unregister`
    // removes the whole lane, so switching push off in Integrations took ntfy's delivery
    // with it while the webhook service went on reporting `.running`, and nothing came back
    // until that service happened to restart.
    //
    // The lane belongs to whichever services still have a provider attached, so the sink is
    // unregistered only when the last one leaves. Firebase is detached by name for symmetry
    // with ntfy, which the webhook service already does.
    await notifications.detach(providerID: FirebaseProvider.identifier)

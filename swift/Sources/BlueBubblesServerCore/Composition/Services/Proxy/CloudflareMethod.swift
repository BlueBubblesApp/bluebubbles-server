...
enum CloudflareMethod: ProxyMethod {
  ...
  static func makeProvider(_ host: ProxyHost) async -> (any ProxyProviding)? {
    ...
    let configFile =
      mode == .configuration ? imported : Self.generatedConfigFile(logger: host.logger)
    guard let configFile else {
      await host.complain(
        title: "Cloudflare's configuration file could not be written",
        body: "This server could not write the empty config.yml it hands cloudflared under "
          + "\(SocketLocation.supportDirectory)/cloudflared. Check that the folder is "
          + "writable, then start the tunnel again.",
        key: "config-write"
      )
      return nil
    }
  ...
  private static func generatedConfigFile(logger: Logger) -> String? {
    ...
    do {
      ...
      // Not silent. Failing to write it is not fatal — cloudflared still starts — but it
      // starts reading `~/.cloudflared/config.yml` instead, and Cloudflare documents quick
      // tunnels as unsupported once cloudflared finds a configuration of its own. That is
      // this server's default connection method failing for a reason nothing on screen
      // explains, which is the exact failure this file exists to prevent.
      logger.warning(
        "Could not write the cloudflared configuration file",

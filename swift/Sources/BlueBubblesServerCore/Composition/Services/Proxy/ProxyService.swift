...
struct ProxyHost: Sendable {
  ...
  func complain(title: String, body: String, key: String) async {
    // The log says why the method declined, because the line that follows it in
    // `ProxyService.start` cannot: it only knows the provider was nil.
    logger.warning(
      "Connection method cannot start",
      metadata: [
        "connectionMethod": .string(manifest.id.rawValue),
        "reason": .string(key),
      ])
    await alerts.raise(
      UserAlert(
  ...
  GatedService
{
  ...
  func start() async throws {
    let host = proxyHost
    guard let provider = await Method.makeProvider(host) else {
      // ASKED, not assumed. A method returns nil for several reasons and says which through
      // `complain`; exactly one of them is "there is no binary", and this alert is only true
      // of that one. Raised unconditionally, it told a user with zrok installed and running
      // to install zrok — beside the accurate alert the method had just raised, so one
      // failure produced two notices and the louder one was the false one. It was the more
      // visible half of a reserve that failed 175 times: 178 copies of "zrok is selected but
      // not installed" for a program on disk.
      guard await host.toolExecutable() == nil else { return }
    ...
    // The address it obtains is never logged, here or in the coordinator: the public URL
    // of this server is the one string a pasted log must not carry.
    logger.info(
      "Connection method starting",
      metadata: ["connectionMethod": .string(Self.manifest.id.rawValue)])

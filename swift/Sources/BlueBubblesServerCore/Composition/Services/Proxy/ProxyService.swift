...
//
//  Worse, it produced a real bug that is now unrepresentable. A scope built in a protocol
...
//  crashed the app at launch. `ProxyHost` carries the manifest as a stored property and
  ...
  GatedService
{
  ...
  /// Replaces the reference's recovery path, which retries ten times and then relaunches the

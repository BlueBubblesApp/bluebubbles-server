...
enum ZrokMethod: ProxyMethod {
  ...
  static func makeProvider(_ host: ProxyHost) async -> (any ProxyProviding)? {
    ...
    do {
      if try await environment.enableIfNeeded(accountToken: accountToken) {
        host.logger.info("Linked this Mac to a zrok account")
      }
      ...
      // Says SOMETHING. `ProxyService` no longer speaks for a method that returned nil while
      // its binary is on disk, so a silent nil here would be a connection method that
      // declines to start and reports it nowhere — the state that generic alert existed to
      // prevent, and the reason it was worth being wrong about.
      await host.complain(
        title: "zrok could not be set up on this Mac",
        body: "Linking this Mac to your zrok account did not finish, so no address is "
          + "being published. " + DiagnosticText.sentence(for: error),
        key: "enable-failed"
      )
      return nil
    }
    ...
    if await host.ownFlag("reserve_tunnel") {
      ...
      // Reserving is off, so anything reserved earlier is now unused. Leaving it would
      // quietly go on occupying a name on the user's account that they can no longer see
      // from here.
  ...
  private static func reservedShareToken(
    ...
    do {
      ...
      host.logger.info("Reserved a zrok share")
      return token
    } catch let error as ZrokError {
      await host.complain(
      ...
      // As above: nil is always explained, because nothing downstream explains it any more.
      await host.complain(
        title: "zrok could not reserve a share",
        body: "Reserving the share did not finish, so no address is being published. "
    ...
    async
  {
    ...
    if await host.ownFlag("keep_share") {
      host.logger.info("Leaving a reserved zrok share in place, as configured")
    } else {

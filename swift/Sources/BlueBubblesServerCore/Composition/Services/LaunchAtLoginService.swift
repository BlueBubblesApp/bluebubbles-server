...
//
//  `LaunchAtLogin` wraps `SMAppService`; this service is what calls it when the setting
//  changes. A setting that is stored and does nothing is the worst shape for this particular
//  feature: the user finds out at the next reboot, when the server they expected to be
//  running is not.
//
...
actor LaunchAtLoginService: Service, ConfigurableService {
  ...
  /// The three things this service touches, rather than the container that holds them.
  typealias Host = any SettingsProviding & AlertProviding & LoggerProviding
  /// What the reconciliation helpers need: they report, they do not read configuration.
  typealias Reporting = any AlertProviding & LoggerProviding
  ...
  func start() async throws {
    let requested = try await scoped.get(Settings.autoStartMethod)
    await Self.reconcile(requested, context: host)
  }
  ...
  var health: ServiceHealth {
    get async {
      let mode = Self.mode(for: await scoped.valueOrDefault(Settings.autoStartMethod))
  ...
  static func reconcile(_ method: AutoStartMethod, context: Reporting) async {
    let mode = mode(for: method)
    let logger = context.logger

    guard mode != .none else {
      // Unregister rather than no-op: the setting having been on before is exactly the
      // case where there is something to remove, including a registration made by a build
      // that predates the launcher, which is a different entry.
      LaunchAtLogin.unregisterEverything()
  ...
  private static func raiseApprovalAlert(context: Reporting) async {
    await context.alerts.raise(
      UserAlert(
        severity: .info,

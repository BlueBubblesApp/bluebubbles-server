...
actor PrivateAPIGatedService: Service, GatedService, ConfigurableService {
  ...
  func start() async throws {
    ...
    // Any previous runtime goes FIRST, because `self.runtime` is about to be replaced and
    // whatever it held would then be unreachable. The registry supervises this service with
    // a retry policy, so a start that throws is followed by another start, and every
    // attempt used to overwrite the reference to the one before: `PrivateAPIRuntime.start`
    // BINDS THE TRANSPORT before it injects (see `startUnbounded`), so each orphan was a
    // bound socket and an event-loop group held for the life of the process. With the
    // configured five attempts that is five of them, and the socket path is unlinked on the
    // way in, so the orphans hold unreachable inodes nothing will ever close.
    if let existing = runtime {
      logger.debug("Stopping the previous Private API runtime before restarting")
      await existing.stop()
      self.runtime = nil
    }
    ...
    let runtime = PrivateAPIRuntime(
      ...
      // Its own label, not the host's root logger: the runtime hands this to the socket
      // transport and the client too, and a request/reply line is filtered by subsystem.
      logger: Logger(label: "bluebubbles.privateapi")
    )
    ...
    pump = Task {
      for await event in client.events {
        ...
        // Typing is per keystroke burst and would drown the rest at debug. The key is
        // a chat GUID for chat-scoped events, so it is redacted.
        logger.log(
          level: server.name == .typingIndicator ? .trace : .debug,
          "Announcing helper event",
          metadata: [
            "event": .string(server.name.rawValue),
            "chat": .string(key.map(Redaction.chatGUID) ?? "-"),
          ])
    ...
    do {
      ...
      // The transport is already bound by the time injection can fail, and the pump and the
      // published client are already attached, so a throw here leaves a half-started service
      // that nothing else will take down: the registry retries `start()` and never calls
      // `stop()` in between. `stop()` is exactly the teardown that is wanted, so it is the
      // one that runs rather than a second copy of it that could drift.
      await stop()
      throw error
    }
  }

...
actor WebhookDeliveryService: Service, ConfigurableService {
  ...
  func start() async throws {
    ...
    guard !ntfyEventNames.isEmpty else {
      // A topic with no events is the one ntfy configuration that looks complete on the
    ...
    logger.info(
      "ntfy attached",
      metadata: ["events": .stringConvertible(ntfyEventNames.count)])

...
public struct ServerComposition {
  ...
  public static func build(
    ...
    let logSink = storage.logSink
    let logger = storage.logger
    let appDatabase = storage.appDatabase
    let secrets = storage.secrets
    let settings = storage.settings

    // Refuse to build on an unadopted Electron install. This does NOT migrate.
    //
    // It used to. The import ran right here, automatically, on whatever it found — which
    // meant an upgrade rewrote a user's settings and moved their credentials with no notice
    // and no way to decline. Worse, a run that failed half-way had a single Bool to record
    // itself in, so the next launch redid the whole thing, and redoing the settings import
    // is not harmless: it writes every key it finds with no comparison and reverts anything
    // changed since.
    //

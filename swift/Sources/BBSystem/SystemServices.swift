...
public enum LaunchAtLogin {
  ...
  public enum Mode: String, Sendable, CaseIterable {
    ...
    ///
    /// The only mode. A per-user LaunchAgent was offered here and never worked — both entry
    /// points required a plist name that no call site passed, and `SMAppService.agent` reads
    /// only from `Contents/Library/LaunchAgents/`, where nothing was ever shipped. It is gone
    /// rather than fixed, because the thing it was wanted for is supervision and a login item
    /// helper provides that without the agent's cost: Background Task Management names a

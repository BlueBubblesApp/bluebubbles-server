//  SystemServices
//  System facilities called natively rather than through a subprocess.
//
//  A shell-out costs a fork+exec, parses text that was never meant to be an API, and fails
//  in ways that read as "the command produced unexpected output" rather than as the thing
...
///
/// Not `caffeinate -i -m -s -w <pid>`: a subprocess whose lifetime is tied to ours by pid can
/// outlive an uncleanly killed server and hold the machine awake indefinitely with nothing
/// left to explain why.
///
...
///
/// `SMAppService` rather than `launchctl` plus a hand-written LaunchAgent plist, which means
...
public enum LaunchAtLogin {
  ...
  public enum Mode: String, Sendable, CaseIterable {
    ...
    ///
    /// The only mode. A per-user LaunchAgent is deliberately not offered: the thing it would
    /// be wanted for is supervision, and the login item helper provides that without the
    /// agent's cost. Background Task Management names a
    /// plist-based agent from the signing certificate's Organization field, which for an
    /// individual Apple Developer enrolment is the developer's legal name. A user would see a
    /// person in System Settings > Login Items rather than BlueBubbles.
    ///
    /// `SMAppService.daemon` is not an option either, and for a harder reason: it runs
    /// pre-login as root, where the user's login keychain is neither unlocked nor theirs, so
    /// every `SecItemCopyMatching` returns `errSecInteractionNotAllowed` and the server comes
    /// up unable to read its own password.
  ...
  /// The launcher bundle inside this application, if it is there.
  ///
  /// `SMAppService.loginItem` reports `.notFound` for a missing helper, which is
  /// indistinguishable from "not registered yet" at the call site, so the presence check is
  /// done here, where the answer can be acted on.
  public static var launcherIsBundled: Bool {
    FileManager.default.fileExists(
      atPath: Bundle.main.bundleURL
        .appendingPathComponent(LauncherContract.launcherBundlePath).path)
  }

  /// The login item this build registers.
  ///
  /// The LAUNCHER when it is bundled, and the app itself otherwise. Registering the launcher
  /// is what buys supervision: it is a separate process, so it survives the app and can bring
  /// it back, where `SMAppService.mainApp` only ever starts the app once at login and has no
  /// opinion about it afterwards.
  ///
  /// The fallback is not a silent degradation: `Tools/dev-bundle.sh` and a release both ship
  /// the launcher, so it applies to a hand-assembled bundle only, and `registeredService`
  /// says which is in use.
  private static var service: SMAppService {
    launcherIsBundled
      ? SMAppService.loginItem(identifier: LauncherContract.launcherBundleIdentifier)
      : SMAppService.mainApp
  }

  public static func status(mode: Mode) -> Status {
    guard mode != .none else { return .notRegistered }
  ...
  @discardableResult
  public static func register(mode: Mode) throws -> Status {
    switch mode {
    case .none:
      unregisterEverything()
      return .notRegistered
    case .loginItem:
      // An install upgraded from a build that registered the app directly still has THAT
      // registration, and it is a separate entry as far as Background Task Management is
      // concerned. Left behind it becomes a second Login Items row that starts a second copy
      // of the app, which the single-instance lock then refuses: the user sees a login item
      // that appears to do nothing.
      if launcherIsBundled { try? SMAppService.mainApp.unregister() }
      try service.register()
      return status(mode: mode)
    }
  }

  public static func unregister(mode: Mode) throws {
    switch mode {
    case .none: break
    case .loginItem: try service.unregister()
    }
  }

  /// Removes BOTH registrations, whichever this build would create.
  ///
  /// Turning the setting off has to clear what an earlier version registered as well as what
  /// this one would, or "off" leaves the app still starting at login.
  public static func unregisterEverything() {
    try? SMAppService.mainApp.unregister()
    try? SMAppService.loginItem(identifier: LauncherContract.launcherBundleIdentifier)
      .unregister()
  }
}

/// Whether the launcher is running and will bring this application back.
///
/// The question a restart has to answer before choosing HOW to restart, and it is about the
/// world rather than about the build: a bundled launcher that is not running supervises
/// nothing, and a login item the user disabled in System Settings is exactly that case. So
/// this asks whether the process is there, not whether the bundle is.
public enum LauncherSupervision {

  public static var isActive: Bool {
    ApplicationControl.isRunning(bundleIdentifier: LauncherContract.launcherBundleIdentifier)
  }
}
...
///
/// `NSRunningApplication` rather than `osascript` or `killall "<name>"`. `killall` matches on a
...
/// Read from `Locale` rather than `defaults read -g AppleLanguages`.
///
...
/// symbol and the `pmset displaysleepnow` command; the latter is what the reference
...
public enum ScreenLock {
  ...
  /// Locks the screen.
  ///
  /// `SACLockScreenImmediate` in the private `login.framework`, which is what the menu-bar
  /// "Lock Screen" item calls.
  ...
  /// on a Mac without that setting the fallback dims the screen and leaves it unlocked.
  /// This locks regardless.

//  Permissions
//  macOS permissions as declared descriptors, checked authoritatively.
//
//  A permission that was not granted up front becomes a mysterious failure much later.
//  Three things stop that here:
//
//    1. **The probes are authoritative.** Reading a preferences plist and string-matching
//       the output says nothing about Full Disk Access; the authoritative check is to open
//       `chat.db`: if it opens, we have the access; if it does not, we do not.
//    2. **Checks are continuous.** Flipping a toggle in System Settings has to update the
//       app, or a user who grants a permission still sees it as missing and concludes it
//       did not work.
//    3. **Services gate on them.** A service whose permission is missing must not start
//       and fail obscurely at first use. `ServiceRegistry` refuses to start it and says why.
//
//  **Accessibility is deliberately absent.** Nothing here drives another application's UI,
//  so there is no reason to ask for it: one fewer alarming permission in onboarding.
//
//  See `docs/AUTH.md`.

import AppKit
import BBCore
import BBServiceKit
import Contacts
import Foundation
import Logging
import UserNotifications

public enum PermissionStatus: String, Sendable, Equatable, Codable {
  case granted
  case denied
  /// Never asked. Distinct from `denied`, because one is worth prompting for and the other
  /// needs the user to go to System Settings: advice that is wrong for the other case.
  case notDetermined
  /// Blocked by policy: MDM, parental controls. No prompt will help.
  case restricted
  /// The check could not run. Reported rather than guessed at.
  case unknown

  /// Whether this status is the system actually saying no.
  ///
  /// `.unknown` is deliberately excluded: it means the check could not run, not that
  /// anything was taken away. Anything that alerts a user, or tells them a feature has
  /// stopped working, must ask this rather than `!= .granted`.
  public var isDefiniteRefusal: Bool {
    switch self {
    case .denied, .restricted, .notDetermined: true
    case .granted, .unknown: false
    }
  }
}

public struct Permission: Sendable, Identifiable {
  public let id: PermissionID
  public let title: String
  /// One user-facing sentence, always shown. A permission request without a reason is a
  /// permission users deny.
  public let why: String
  public let requirement: PermissionRequirement
  /// Deep link to the exact pane, not "open System Settings and find it".
  public let settingsPane: URL?
  /// Full Disk Access needs the app added by hand and then relaunched. Both are stated.
  public let requiresRelaunch: Bool
  /// Whether the app can prompt, or the user must go to System Settings themselves.
  public let canPrompt: Bool

  public init(
    id: PermissionID,
    title: String,
    why: String,
    requirement: PermissionRequirement,
    settingsPane: URL? = nil,
    requiresRelaunch: Bool = false,
    canPrompt: Bool = false
  ) {
    self.id = id
    self.title = title
    self.why = why
    self.requirement = requirement
    self.settingsPane = settingsPane
    self.requiresRelaunch = requiresRelaunch
    self.canPrompt = canPrompt
  }
}

public struct PermissionState: Sendable, Equatable {
  public let id: PermissionID
  public let status: PermissionStatus
  public let checkedAt: Date

  public init(id: PermissionID, status: PermissionStatus, checkedAt: Date = Date()) {
    self.id = id
    self.status = status
    self.checkedAt = checkedAt
  }
}

// MARK: - Deep links

public enum SettingsPane {

  /// Pane identifiers changed in Ventura, which is below our macOS 14 floor, so one set of
  /// URLs covers every supported release and no version branching is needed.
  static func url(_ anchor: String) -> URL? {
    URL(string: "x-apple.systempreferences:com.apple.preference.security?\(anchor)")
  }

  public static let fullDiskAccess = url("Privacy_AllFiles")
  public static let automation = url("Privacy_Automation")
  public static let contacts = url("Privacy_Contacts")
  public static let notifications = URL(
    string: "x-apple.systempreferences:com.apple.preference.notifications")
}

// MARK: - Probes

/// The actual checks, behind a protocol so the service can be tested without the real system
/// state, which on a CI machine is neither granted nor deniable.
public protocol PermissionProbing: Sendable {
  func fullDiskAccess() async -> PermissionStatus
  func automation(bundleIdentifier: String) async -> PermissionStatus
  func contacts() async -> PermissionStatus
  func notifications() async -> PermissionStatus
  func systemIntegrityProtectionDisabled() async -> PermissionStatus

  /// Prompts, where prompting is possible. Returns nil when there is nothing to ask:
  /// Full Disk Access and SIP have no dialog, so the caller re-reads the real status
  /// rather than being handed an invented one.
  func requestAccess(to id: PermissionID, bundleIdentifier: String) async -> PermissionStatus?
}

extension PermissionProbing {
  /// Default for probes that only observe, which is every test double.
  public func requestAccess(to id: PermissionID, bundleIdentifier: String) async
    -> PermissionStatus?
  {
    nil
  }
}

public struct SystemPermissionProbe: PermissionProbing {

  private let chatDatabasePath: String

  public init(
    chatDatabasePath: String = NSHomeDirectory() + "/Library/Messages/chat.db"
  ) {
    self.chatDatabasePath = chatDatabasePath
  }

  /// Authoritative: open the file we actually need.
  ///
  /// `FileManager.isReadableFile` is not enough: it consults the filesystem permission
  /// bits, which say yes even when TCC will refuse the open. Only opening it settles the
  /// question, and it is exactly the operation the server performs anyway.
  public func fullDiskAccess() async -> PermissionStatus {
    guard FileManager.default.fileExists(atPath: chatDatabasePath) else {
      // No Messages database at all. Not a permission problem, and reporting it as one
      // would send the user to the wrong settings pane.
      return .unknown
    }
    do {
      let handle = try FileHandle(forReadingFrom: URL(fileURLWithPath: chatDatabasePath))
      defer { try? handle.close() }
      // Read a byte: opening can succeed lazily on some paths.
      _ = try handle.read(upToCount: 1)
      return .granted
    } catch {
      return .denied
    }
  }

  /// A real tri-state.
  ///
  /// `askUserIfNeeded: false` matters: passing true would show a consent dialog as a side
  /// effect of *checking*, which is how a status page ends up prompting the user every time
  /// they look at it.
  /// **OFF THE COOPERATIVE POOL, AND BOUNDED.** Both halves are load-bearing.
  ///
  /// `AEDeterminePermissionToAutomateTarget` is a synchronous C call that makes an XPC
  /// round trip to `tccd`. Calling it directly from an `async` function blocks a thread the
  /// Swift concurrency runtime owns and expects back, and the runtime has only as many of
  /// those as the machine has cores. When `tccd` does not answer, startup would reach this
  /// point and stop: the HTTP listener bound (NIO runs its own threads) while every async
  /// task behind it starves, so the port accepts connections that are never served. From
  /// the outside that looks like a hung server with no error anywhere.
  ///
  /// `tccd` fails to answer for reasons that have nothing to do with this app. It stalls
  /// most readily for a process it cannot attribute to a bundle: an unbundled `swift run`
  /// binary is attributed to the terminal that spawned it, which is precisely the
  /// development configuration.
  ///
  /// So: run it on a thread that is ours to block, and give up after `deadline`. An
  /// unanswered probe is `.unknown`, which is what the type is for.
  public func automation(bundleIdentifier: String) async -> PermissionStatus {
    // Through the gate, which is what stops a stalled `tccd` costing a thread per call.
    // See `StalledProbeGate`.
    guard await Self.probeGate.beginProbe(bundleIdentifier) else { return .unknown }
    let status = await Self.boundedSyncProbe(
      deadline: Self.tccProbeDeadline,
      onAbandon: { await Self.probeGate.markStalled(bundleIdentifier) },
      onLateAnswer: { await Self.probeGate.clearStalled(bundleIdentifier) },
      probe: {
        var target = AEAddressDesc()
        let created = bundleIdentifier.withCString { pointer -> OSErr in
          AECreateDesc(typeApplicationBundleID, pointer, strlen(pointer), &target)
        }
        guard created == noErr else { return .unknown }
        defer { AEDisposeDesc(&target) }

        switch AEDeterminePermissionToAutomateTarget(&target, typeWildCard, typeWildCard, false) {
        case noErr: return .granted
        case OSStatus(errAEEventNotPermitted): return .denied
        case OSStatus(errAEEventWouldRequireUserConsent): return .notDetermined
        case OSStatus(procNotFound): return .unknown  // the target is not installed
        default: return .unknown
        }
      })
    await Self.probeGate.endProbe(bundleIdentifier)
    return status
  }

  /// Long enough that a busy but working `tccd` still answers, short enough that a wedged
  /// one does not hold up a server start.
  static let tccProbeDeadline = Duration.seconds(5)

  /// Runs a blocking probe on a dedicated thread, answering `.unknown` if it overruns.
  ///
  /// The thread is deliberately abandoned rather than cancelled on timeout: a blocked
  /// `mach_msg` cannot be interrupted, and pretending otherwise would swap one hang for a
  /// crash. It is one thread, it ends when `tccd` finally replies, and nothing waits on it.
  ///
  /// **The deadline and the probe race for ONE continuation, and that shape is the fix.**
  /// This was a `withTaskGroup` with the probe in one child and the sleep in the other,
  /// taking whichever finished first. It read as bounded and was not: `withCheckedContinuation`
  /// ignores cancellation, so the probe child stayed blocked, and a task group AWAITS ITS
  /// REMAINING CHILDREN before it returns. `cancelAll()` bought nothing and the whole call
  /// still took as long as `tccd` did. The failure it exists to prevent, a server whose port
  /// is bound while every async task behind it starves, was therefore still live, and the
  /// comment claiming nothing waits on the thread was the reason nobody looked again.
  ///
  /// `Timeout.withTimeout` is not the tool here, for the reason its own header gives: this
  /// has to ANSWER rather than throw, because an unanswered permission probe is a real
  /// state (`.unknown`) rather than an error.
  /// - Parameter onAbandon: called when the deadline wins, i.e. a thread has been left
  ///   blocked. Called at most once.
  /// - Parameter onLateAnswer: called if that abandoned thread ever does return. Called at
  ///   most once, and only when `onAbandon` already fired.
  static func boundedSyncProbe(
    deadline: Duration,
    onAbandon: (@Sendable () async -> Void)? = nil,
    onLateAnswer: (@Sendable () async -> Void)? = nil,
    probe: @escaping @Sendable () -> PermissionStatus
  ) async -> PermissionStatus {
    let abandoned = Flag()
    return await withCheckedContinuation { continuation in
      let answer = FirstAnswer(continuation)

      let deadlineTask = TaskBox()

      let thread = Thread {
        let status = probe()
        if answer.resolve(status) {
          // We won, so there is nothing left to time out. Cancelling matters because this
          // runs under a loop: leaving the deadline task to "end on its own either way"
          // left one sleeping task per tick, each waking `deadline` later to discover it
          // had already lost. At a two-second cadence that doubled the loop's wakeups for
          // no work at all, which on a throttled Mac is what keeps the package out of its
          // deeper sleep states.
          deadlineTask.cancel()
        }
        // The thread got there eventually. Whoever gave up on it wants to know, because
        // that is the signal that `tccd` is answering again.
        if abandoned.isSet, let onLateAnswer {
          Task { await onLateAnswer() }
        }
      }
      thread.stackSize = 512 * 1024
      thread.start()

      // Unstructured on purpose: it must be able to outlive the call it bounds, since the
      // whole point is returning while the probe is still blocked.
      deadlineTask.set(
        Task {
          // A cancelled sleep means the probe answered first; there is nothing to resolve.
          do { try await Task.sleep(for: deadline) } catch { return }
          // `resolve` answers only the first caller, so this tells us whether the deadline
          // actually won rather than merely fired after the probe had answered.
          if answer.resolve(.unknown) {
            abandoned.set()
            await onAbandon?()
          }
        })
    }
  }

  /// Holds the deadline task so the probe thread can cancel it the moment it answers.
  ///
  /// The thread is started before the task exists and can win that race, so a cancel that
  /// arrives early is remembered and applied when the task lands.
  private final class TaskBox: @unchecked Sendable {
    private let lock = NSLock()
    private var task: Task<Void, Never>?
    private var isCancelled = false

    func set(_ task: Task<Void, Never>) {
      let alreadyCancelled = lock.withLock {
        if isCancelled { return true }
        self.task = task
        return false
      }
      if alreadyCancelled { task.cancel() }
    }

    func cancel() {
      let task: Task<Void, Never>? = lock.withLock {
        isCancelled = true
        defer { self.task = nil }
        return self.task
      }
      task?.cancel()
    }
  }

  /// A one-way boolean, readable from the probe thread and set from the deadline task.
  private final class Flag: @unchecked Sendable {
    private let lock = NSLock()
    private var value = false
    var isSet: Bool { lock.withLock { value } }
    func set() { lock.withLock { value = true } }
  }

  /// The gate that keeps a stalled `tccd` from costing a thread every two seconds.
  ///
  /// `boundedSyncProbe` deliberately ABANDONS a blocked thread, because a blocked `mach_msg`
  /// cannot be interrupted. That is right for one call and wrong under a loop:
  /// `PermissionsService` re-checks every two seconds forever, so a `tccd` that has stopped
  /// answering produced roughly thirty abandoned threads a minute, each holding a
  /// half-megabyte stack and a kernel thread port, until the process hit the per-task thread
  /// limit and died. Reproduced with a probe that never returns: thread count climbs
  /// linearly and nothing is ever reclaimed.
  ///
  /// Two rules, both about not making it worse:
  ///
  /// 1. **One probe at a time per target.** A second caller arriving while one is in flight
  ///    is answered `.unknown` rather than given its own thread. Coalescing them onto the
  ///    same task would be tidier and is wrong here: the point is to spawn nothing.
  /// 2. **Nothing at all while stalled.** Once a deadline has abandoned a thread, further
  ///    calls answer `.unknown` immediately and spawn nothing, until that abandoned thread
  ///    finally returns and clears the flag. So a stalled `tccd` costs exactly one leaked
  ///    thread in total rather than one per tick, and recovery is automatic.
  actor StalledProbeGate {
    private var inFlight: Set<String> = []
    private var stalled: Set<String> = []

    /// Whether the caller may spawn a probe thread for this target.
    func beginProbe(_ target: String) -> Bool {
      guard !stalled.contains(target), !inFlight.contains(target) else { return false }
      inFlight.insert(target)
      return true
    }

    func endProbe(_ target: String) { inFlight.remove(target) }

    func markStalled(_ target: String) {
      stalled.insert(target)
      inFlight.remove(target)
    }

    func clearStalled(_ target: String) { stalled.remove(target) }

    /// For tests.
    func isStalled(_ target: String) -> Bool { stalled.contains(target) }
  }

  static let probeGate = StalledProbeGate()

  /// Resumes a continuation exactly once, whoever gets there first.
  ///
  /// A continuation resumed twice is a crash, and here two independent things race to resume
  /// it: an abandoned thread that may answer minutes later, and a deadline that has already
  /// given up on it. Both call `resolve`; only the first does anything.
  private final class FirstAnswer: @unchecked Sendable {
    private let lock = NSLock()
    private var continuation: CheckedContinuation<PermissionStatus, Never>?

    init(_ continuation: CheckedContinuation<PermissionStatus, Never>) {
      self.continuation = continuation
    }

    /// Returns whether THIS call was the one that answered, so the deadline can tell
    /// "I gave up on a blocked thread" from "I fired after the probe had already answered".
    @discardableResult
    func resolve(_ status: PermissionStatus) -> Bool {
      lock.lock()
      let pending = continuation
      continuation = nil
      lock.unlock()
      // Resumed outside the lock: a continuation resumes the awaiting task, and doing that
      // while holding a lock the resumed work might take is how this would deadlock.
      pending?.resume(returning: status)
      return pending != nil
    }
  }

  /// Prompts for a permission the app is allowed to ask for.
  ///
  /// Kept separate from the probes on purpose. A check that prompts as a side effect turns
  /// a status page into a nag: the user opens Permissions, gets three dialogs, and learns
  /// to dismiss them without reading. Asking happens only when someone presses a button
  /// that says so.
  ///
  /// Returns the status AFTER the prompt resolves, so a caller can act on the answer
  /// rather than polling for it.
  public func requestAccess(to id: PermissionID, bundleIdentifier: String) async
    -> PermissionStatus?
  {
    switch id {
    case .contacts:
      // The completion handler fires on an arbitrary queue; the continuation is what
      // makes it awaitable without blocking one.
      _ = await withCheckedContinuation { continuation in
        CNContactStore().requestAccess(for: .contacts) { granted, _ in
          continuation.resume(returning: granted)
        }
      }
      return await contacts()

    case .notifications:
      guard Bundle.main.bundleIdentifier != nil else { return .unknown }
      _ = try? await UNUserNotificationCenter.current()
        .requestAuthorization(options: [.alert, .sound])
      return await notifications()

    case .automationMessages:
      // The ONLY place `askUserIfNeeded: true` is passed. There is no separate request
      // API for Apple Events (the consent dialog is a side effect of the check) so
      // requesting means deliberately doing the thing the probes avoid.
      var target = AEAddressDesc()
      let created = bundleIdentifier.withCString { pointer -> OSErr in
        AECreateDesc(typeApplicationBundleID, pointer, strlen(pointer), &target)
      }
      guard created == noErr else { return .unknown }
      defer { AEDisposeDesc(&target) }
      _ = AEDeterminePermissionToAutomateTarget(&target, typeWildCard, typeWildCard, true)
      return await automation(bundleIdentifier: bundleIdentifier)

    default:
      // Nothing else can be prompted for. Full Disk Access requires the user to add
      // the app by hand in System Settings (macOS shows no dialog at all) and SIP is
      // a Recovery-mode setting. Returning `nil` says "there was nothing to ask" so
      // the caller re-reads the real status rather than trusting a made-up answer.
      return nil
    }
  }

  public func contacts() async -> PermissionStatus {
    switch CNContactStore.authorizationStatus(for: .contacts) {
    case .authorized: .granted
    case .denied: .denied
    case .restricted: .restricted
    case .notDetermined: .notDetermined
    @unknown default: .unknown
    }
  }

  public func notifications() async -> PermissionStatus {
    // `UNUserNotificationCenter.current()` does not fail gracefully outside an app
    // bundle: it raises `NSInternalInconsistencyException`, which is not catchable from
    // Swift and takes the process down. The server runs unbundled in development and
    // under a launch agent, so this is a normal configuration, not an edge case.
    //
    // A bundle identifier is the thing it actually requires, so that is what is checked.
    guard Bundle.main.bundleIdentifier != nil else { return .unknown }

    let settings = await UNUserNotificationCenter.current().notificationSettings()
    switch settings.authorizationStatus {
    case .authorized, .provisional, .ephemeral: return .granted
    case .denied: return .denied
    case .notDetermined: return .notDetermined
    @unknown default: return .unknown
    }
  }

  /// SIP status, for the Private API explainer.
  ///
  /// `csr_check` is resolved dynamically because it is not in any public header: declaring
  /// it would be relying on a private symbol at link time, which breaks the build if it is
  /// ever removed. Resolved at runtime, its absence degrades to the `csrutil` fallback.
  ///
  /// Read-only: this is never something the app offers to change. Disabling SIP is a
  /// Recovery-mode operation and the app has no business attempting it.
  public func systemIntegrityProtectionDisabled() async -> PermissionStatus {
    typealias CSRCheck = @convention(c) (UInt32) -> Int32
    // CSR_ALLOW_UNRESTRICTED_FS. Returns 0 when the protection is PERMITTED: that is,
    // when SIP is not enforcing it.
    let unrestrictedFilesystem: UInt32 = 1 << 1

    if let symbol = dlsym(dlopen(nil, RTLD_NOW), "csr_check") {
      let check = unsafeBitCast(symbol, to: CSRCheck.self)
      return check(unrestrictedFilesystem) == 0 ? .granted : .denied
    }
    return await csrutilFallback()
  }

  /// Shelling out only when the symbol is gone.
  private func csrutilFallback() async -> PermissionStatus {
    // Through `Subprocess`, so nothing parks a cooperative-pool thread for the length of
    // the call.
    guard
      let result = try? await Subprocess.run(
        "/usr/bin/csrutil", ["status"],
        output: .standardOutputOnly, timeout: .seconds(10)
      )
    else { return .unknown }

    let output = result.text.lowercased()
    if output.contains("disabled") { return .granted }
    if output.contains("enabled") { return .denied }
    return .unknown
  }
}

// MARK: - The service

public actor PermissionsService {

  /// How often live status re-checks while a permission page is on screen AND the app is
  /// frontmost. Frequent enough that flipping a System Settings toggle updates the app while
  /// the user is still looking at it: the single biggest usability gap today.
  ///
  /// Both halves are required, because frontmost alone is not evidence anyone is reading the
  /// answer. The app was paying this cadence while the user sat on the dashboard, the log
  /// viewer, or any settings tab — none of which display a permission status.
  public static let liveRefreshInterval: Duration = .seconds(2)

  /// How often it re-checks otherwise — backgrounded, or headless, which is every server
  /// install.
  ///
  /// A tick is not free: it opens chat.db and reads a byte, spawns a half-megabyte-stack
  /// thread for an XPC round trip to `tccd`, `dlopen`s a framework, and asks the
  /// notification centre. Measured at 0.8ms and, with the deadline task it used to leak,
  /// about sixty of this process's sixty-six idle wakeups a minute. Nothing observes the
  /// answer when no one is looking at the page, and the reason the fast cadence exists —
  /// "granted it in System Settings and switched back" — is precisely the moment the service
  /// re-enters the live cadence, which re-checks immediately anyway. So the fast interval is
  /// not what makes that case feel instant; `enterLiveIfNeeded` is.
  public static let idleRefreshInterval: Duration = .seconds(60)

  private let probe: any PermissionProbing
  private let logger: Logger
  private let onChange: @Sendable (PermissionID, PermissionStatus, PermissionStatus) async -> Void

  private var states: [PermissionID: PermissionState] = [:]
  private var monitorTask: Task<Void, Never>?
  /// Defaults to false so a headless server — which never calls `setForeground` — takes
  /// the idle cadence without having to know this exists.
  private var isForeground = false
  /// Whether a view that actually DISPLAYS permission status is on screen.
  ///
  /// Defaults to false for the same reason `isForeground` does: a server with no UI
  /// attached never calls `setWatching` and must not be the one configuration that pays the
  /// fast cadence forever.
  private var isWatched = false
  private var observers: [UUID: AsyncStream<[PermissionID: PermissionStatus]>.Continuation] = [:]

  public init(
    probe: any PermissionProbing = SystemPermissionProbe(),
    logger: Logger = Logger(label: "bluebubbles.permissions"),
    onChange: @escaping @Sendable (PermissionID, PermissionStatus, PermissionStatus) async -> Void =
      { _, _, _ in }
  ) {
    self.probe = probe
    self.logger = logger
    self.onChange = onChange
  }

  /// The declared list. Order is the order onboarding presents them.
  public nonisolated var permissions: [Permission] { Self.catalogue }

  /// The same list, reachable without an instance.
  ///
  /// A service manifest declares a `PermissionID` and nothing else: the human title lives
  /// here, in the layer that can probe the permission, so anything rendering a manifest's
  /// permissions needs this lookup and has no reason to hold a running service to get it.
  public static let catalogue: [Permission] = [
    Permission(
      id: .fullDiskAccess,
      title: "Full Disk Access",
      why: "Read your Messages database",
      requirement: .required,
      settingsPane: SettingsPane.fullDiskAccess,
      // The step users most often get half-right: the app must be added by hand
      // and then relaunched before the grant takes effect.
      requiresRelaunch: true
    ),
    Permission(
      id: .automationMessages,
      title: "Automation → Messages",
      why: "Send messages without the Private API",
      // Required only when the Private API is unavailable; with it, sending does
      // not go through AppleScript at all.
      requirement: .feature("Sending without the Private API"),
      settingsPane: SettingsPane.automation,
      canPrompt: true
    ),
    Permission(
      id: .contacts,
      title: "Contacts",
      why: "Show names instead of phone numbers",
      requirement: .recommended,
      settingsPane: SettingsPane.contacts,
      canPrompt: true
    ),
    Permission(
      id: .notifications,
      title: "Notifications",
      why: "Alert you when something needs attention",
      requirement: .recommended,
      settingsPane: SettingsPane.notifications,
      canPrompt: true
    ),
    Permission(
      id: .systemIntegrityProtection,
      title: "System Integrity Protection disabled",
      why: "Enables reactions, edit and unsend, typing indicators, and group management",
      requirement: .feature("Private API"),
      // Deliberately no settings pane: this is a Recovery-mode operation and there
      // is nothing to deep-link to.
      settingsPane: nil
    ),
  ]

  /// A permission's human title, or its raw identifier when this binary does not know it.
  ///
  /// The fallback matters: the set of permissions is open, so a third-party manifest can
  /// name one we have never heard of, and dropping it from the list would hide exactly the
  /// declaration the list exists to show.
  public static func title(for id: PermissionID) -> String {
    catalogue.first { $0.id == id }?.title ?? id.rawValue
  }

  // MARK: Checking

  /// Messages.app: the only automation target the server drives.
  static let messagesBundleIdentifier = "com.apple.MobileSMS"

  /// Prompts for a permission, then re-checks it.
  ///
  /// The re-check is the point: the prompt's own return value tells you what the user
  /// tapped, while the status is what the system actually recorded, and for Automation
  /// those can differ. Storing the re-checked value keeps the page honest.
  @discardableResult
  public func request(_ id: PermissionID) async -> PermissionStatus {
    _ = await probe.requestAccess(
      to: id,
      bundleIdentifier: Self.messagesBundleIdentifier
    )
    return await check(id)
  }

  public func check(_ id: PermissionID) async -> PermissionStatus {
    let status: PermissionStatus
    switch id {
    case .fullDiskAccess: status = await probe.fullDiskAccess()
    case .automationMessages:
      status = await probe.automation(bundleIdentifier: "com.apple.MobileSMS")
    case .contacts: status = await probe.contacts()
    case .notifications: status = await probe.notifications()
    case .systemIntegrityProtection: status = await probe.systemIntegrityProtectionDisabled()
    default: status = .unknown
    }

    let previous = states[id]?.status

    // An inconclusive probe does not overwrite what we already know.
    //
    // `.unknown` means the check could not run: a `tccd` that did not answer inside
    // `tccProbeDeadline`, most often. Storing it throws away a good answer and makes the
    // next successful probe look like a change, so the UI flickered "not available" and
    // the home dashboard flashed a missing-permission banner on a permission that was
    // granted throughout. Keeping the last known answer is both more accurate and quieter;
    // a permission that is genuinely gone reports `.denied`, not `.unknown`.
    guard !(status == .unknown && previous != nil && previous != .unknown) else {
      return previous ?? status
    }
    states[id] = PermissionState(id: id, status: status)

    if let previous, previous != status {
      // A permission revoked after setup (which happens on OS upgrades) is reported
      // at the moment it breaks rather than surfacing later as unexplained failures.
      logger.info(
        "Permission changed",
        metadata: [
          "permission": .string(id.rawValue),
          "from": .string(previous.rawValue),
          "to": .string(status.rawValue),
        ])
      await onChange(id, previous, status)
    }
    return status
  }

  @discardableResult
  public func checkAll() async -> [PermissionID: PermissionStatus] {
    var result: [PermissionID: PermissionStatus] = [:]
    for permission in permissions {
      result[permission.id] = await check(permission.id)
    }
    broadcast(result)
    return result
  }

  /// The results of every check this service runs, for observers that want the answer
  /// without asking the question themselves.
  ///
  /// The monitor probes and everyone else listens. Each automation probe spawns a thread to
  /// ask TCC, so a second loop asking the same question would double the expensive one.
  public func stream() -> AsyncStream<[PermissionID: PermissionStatus]> {
    let id = UUID()
    return AsyncStream { continuation in
      observers[id] = continuation
      // SEEDED with what is already known, rather than only reporting the next change.
      //
      // This service starts before everything else and checks immediately, so by the time
      // any UI subscribes the answers are already here — and a subscriber that waited for
      // the next broadcast waited out the IDLE cadence for them: sixty seconds.
      //
      // What that looked like: `PermissionsModel.unsatisfiedRequiredCount` reads
      // `statuses[id] ?? .notDetermined`, and exactly one permission in the catalogue is
      // `.required`, so an empty map is indistinguishable from "Full Disk Access is
      // missing". Home opened saying one required permission was missing on a Mac that had
      // granted it. Opening the Permissions page set `isWatched`, which re-checks at once,
      // which broadcast — so the page was green and Home was right again on the way back,
      // and the symptom read as the count being stale rather than never delivered.
      //
      // Every other follow in this app seeds; see `AlertsModel.attach` and
      // `followPublishedAddress`. This was the one that did not.
      if !states.isEmpty {
        continuation.yield(states.mapValues(\.status))
      }
      continuation.onTermination = { [weak self] _ in
        Task { await self?.removeObserver(id) }
      }
    }
  }

  private func removeObserver(_ id: UUID) {
    observers[id] = nil
  }

  private func broadcast(_ states: [PermissionID: PermissionStatus]) {
    for continuation in observers.values { continuation.yield(states) }
  }

  public func status(of id: PermissionID) -> PermissionStatus {
    states[id]?.status ?? .unknown
  }

  public func allStates() -> [PermissionState] {
    permissions.compactMap { states[$0.id] }
  }

  /// Whether every REQUIRED permission is granted. What onboarding gates on.
  public func requiredPermissionsSatisfied() -> Bool {
    permissions
      .filter { $0.requirement.isRequired }
      .allSatisfy { states[$0.id]?.status == .granted }
  }

  public func unsatisfiedRequired() -> [Permission] {
    permissions.filter { $0.requirement.isRequired && states[$0.id]?.status != .granted }
  }

  // MARK: Live status

  /// Starts continuous re-checking.
  ///
  /// This is what makes the page feel correct: granting a permission in System Settings and
  /// switching back shows it granted, with no relaunch and no navigating away and back.
  public func startMonitoring() {
    guard monitorTask == nil else { return }
    monitorTask = Task { [weak self] in
      while !Task.isCancelled {
        // ENDS when the owner goes, rather than ticking on forever against a nil.
        //
        // `[weak self]` stopped the loop keeping the service alive and did nothing about the
        // loop itself: `await self?.checkAll()` on a released owner is a no-op that sleeps
        // two seconds and tries again, for the life of the process. Each live iteration also
        // spawns a half-megabyte-stack thread per bounded probe, so an app that stops and
        // starts the server accumulated one of these every time.
        //
        // Checked here rather than in a `deinit`, which on an actor may not touch isolated
        // state: the strong reference is taken and dropped within the iteration.
        guard let self else { return }
        await self.checkAll()
        try? await Task.sleep(for: await self.refreshInterval)
      }
    }
  }

  public func stopMonitoring() {
    monitorTask?.cancel()
    monitorTask = nil
  }

  /// Whether the fast cadence is currently earned.
  ///
  /// Both halves, and neither is sufficient. Frontmost without a page displaying status is
  /// the dashboard or the log viewer, where nothing renders the answer. A page on screen in
  /// a backgrounded app is a window behind another window, where the same is true — and
  /// where the user is, by definition, somewhere else, so activation will re-check the
  /// moment they come back.
  var isLive: Bool { isForeground && isWatched }

  /// The cadence the loop is currently entitled to.
  var refreshInterval: Duration {
    isLive ? Self.liveRefreshInterval : Self.idleRefreshInterval
  }

  /// Tells the service whether the app is frontmost.
  ///
  /// Entering the live cadence re-checks at once rather than waiting out a sleep, which
  /// makes "grant it in System Settings, switch back" faster than the old fixed cadence
  /// rather than slower. Leaving it changes nothing immediately: the in-flight sleep
  /// finishes and the next one reads the idle interval.
  public func setForeground(_ active: Bool) {
    guard active != isForeground else { return }
    let wasLive = isLive
    isForeground = active
    enterLiveIfNeeded(wasLive: wasLive)
  }

  /// Tells the service whether a view that displays permission status is on screen.
  ///
  /// Called by the pages that render it — the permissions settings tab and the onboarding
  /// step — rather than inferred, because the service cannot see the view hierarchy and
  /// guessing from "the app is open" is exactly the over-approximation this replaces.
  public func setWatching(_ watching: Bool) {
    guard watching != isWatched else { return }
    let wasLive = isLive
    isWatched = watching
    enterLiveIfNeeded(wasLive: wasLive)
  }

  /// Restarts the loop when, and only when, the cadence has just become live.
  ///
  /// The restart is what delivers the immediate re-check: `startMonitoring`'s loop probes
  /// before its first sleep. Falling OUT of live deliberately does not restart — there is
  /// nothing urgent to report and the in-flight sleep costs nothing to finish.
  private func enterLiveIfNeeded(wasLive: Bool) {
    guard isLive, !wasLive, monitorTask != nil else { return }
    stopMonitoring()
    startMonitoring()
  }

  // MARK: Preflight

  /// Whether a service may start.
  ///
  /// Wired into `ServiceRegistry`'s permission gate, so a service whose permission is
  /// missing reports `.inactive` with a precise reason instead of failing obscurely the
  /// first time it touches something.
  public func permissionCheck() -> @Sendable (PermissionID) async -> Bool {
    { [weak self] id in
      guard let self else { return false }
      return await self.check(id) == .granted
    }
  }
}

extension PermissionID {
  /// Read-only, and not a permission the app can request: it is a Recovery-mode setting.
  /// Modelled here anyway so the Private API's requirement is expressed the same way as
  /// every other prerequisite rather than as a special case.
  public static let systemIntegrityProtection = PermissionID("sip-disabled")
}

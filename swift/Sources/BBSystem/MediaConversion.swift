...
//
//  In-process rather than `sips`, `afconvert` and `mdls`: each of those spawns a process,
//  writes through a temporary file, and reports failure as unparsed text on stderr.
//
...
public enum ImageConverter {
  ...
  ///
  /// Reads the header only, rather than spawning `mdls` and parsing its output, so it costs
  /// the same for a 50MB photo as for a thumbnail.
...
public enum AudioConverter {
  ...
  ///
  /// iMessage voice notes arrive as CAF, which most clients cannot

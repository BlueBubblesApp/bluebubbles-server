...
//
//  **This is not where the running server keeps its certificate.** The Keychain is;
//  see `CertificateKeychainStore`. What remains here is the READ half: `TLSProvisioning`
//  consults these paths when the Keychain is empty, adopts what it finds, and calls `clear()`.
//  `install` and `recordExpiration` survive for the tests that build such an install, and for
//  nothing else. Do not add a caller that writes here at runtime; a second store means a
//  renewal can update one copy while the reader consults the other.
//
//  The `0700` directory and `0600` key still apply while the files exist, which is the same
//  protection SSH gives `~/.ssh/id_*`: enough against another user, and nothing at all
//  against another process running as this one. That gap is why the material moved.
//
...
public struct CertificateStore: Sendable {
  ...
  /// The Electron server's own location, unchanged, and this server's too, which is why
  /// material being present here does not by itself mean an install is an upgrade.
  public static var defaultDirectory: URL { ApplicationSupport.certificates }
  ...
  private func write(_ contents: String, to url: URL, permissions: Int) throws {
    do {
      // The UMASK is narrowed for the duration, so the file is never briefly world-readable.
      //
      // An atomic write creates a new file at the default mask and the `setAttributes`
      // below then tightens it — a window, small but real, during which a private key sits
      // at 0644. The containing directory is already 0700, which is what actually keeps
      // another user out, so this closes the last gap rather than the only one.
      let previousMask = umask(0o077)
      defer { umask(previousMask) }

      try contents.write(to: url, atomically: true, encoding: .utf8)
      // After the write as well: an atomic write replaces the file, the replacement does
      // not inherit the permissions of what it replaced, and the umask only removes bits
      // rather than setting the ones we want.
      try FileManager.default.setAttributes(

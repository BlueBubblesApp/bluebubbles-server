...
public enum SystemInfo {
  ...
  ///
  /// Read from the plist directly rather than through `PlistBuddy`. Index 0 is preserved
  /// rather than searched: clients have been
  ...
  ///
  /// `getifaddrs`, matching the filtering the reference does with `os.networkInterfaces`:
  /// the requested family only, and nothing internal. Link-local IPv6 is dropped as well:
  /// Node's `internal` flag does not catch `fe80::`, so the reference publishes
  ...
  /// reports configuration rather than drift) so this parses `sntp`. Shelling out is the
  /// decision, and it stands on there being no alternative, not on the reference doing it.
  ///
  ...
  /// the part before `+/-`. The pattern is transcribed rather than reinvented for one
  /// narrow reason: the parsed NUMBER reaches `server/info`, so the two have to agree on what
  /// `sntp` output means: not because the regex itself is a contract. Rewrite it freely if it
  /// keeps producing the same offset for the same output.

...
//
//  `swift-certificates` is Apple-maintained and the key generation goes through
//  `swift-crypto`, so no key material is produced by hand.
//
...
public enum CertificateAuthority {
  ...
  ///
  /// Exists so the SAN can be ASSERTED rather than assumed. A certificate whose names are
  /// wrong is indistinguishable from a correct one everywhere except at a client: it parses,
  /// it binds, the server logs success, and every connection fails to verify. The renewal
  /// path dropped the published address for exactly that reason and nothing noticed.
  ///
  /// IP addresses come back in their textual form, matching how they were put in.
  public static func subjectAlternativeNames(certificatePEM: String) throws -> [String] {
    let certificate = try Certificate(pemEncoded: certificatePEM)
    guard let extensions = try certificate.extensions.subjectAlternativeNames else { return [] }
    return extensions.compactMap { name in
      switch name {
      case .dnsName(let value): value
      case .ipAddress(let bytes): Self.textualAddress(bytes)
      default: nil
      }
    }
  }

  /// An ASN.1 SAN IP address as text. Four bytes is IPv4, sixteen is IPv6.
  private static func textualAddress(_ value: ASN1OctetString) -> String? {
    let bytes = Array(value.bytes)
    switch bytes.count {
    case 4: return bytes.map(String.init).joined(separator: ".")
    case 16:

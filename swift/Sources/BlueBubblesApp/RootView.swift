...
struct ServerStatusBar: View {
  ...
  var body: some View {
    VStack(alignment: .leading, spacing: 8) {
      ...
      // WHAT startup is doing, under "Starting…". A single opaque phase covers opening two
      // databases, probing the Messages schema and bringing a dozen services up, and on a
      // slow Mac that is minutes of a label that never changes: reported as "no indication
      // of what was going on". See `StartupStage`.

  ...
  var body: some View {
    NavigationSplitView {
      ...
      detail
        ...
        // which is how this started. The window title is the one piece of branding that
        ...
        .navigationSubtitle(model.selection.rawValue)
        .toolbar {
    ...
    .task(id: model.phase) {
      ...
      //
      // This used to fire from a bare `.task` that raced `model.start()`, so the
      // walkthrough could appear over a server that had not been built — and its
      // permission step would read nothing. Keying on the phase also keeps it from
...
struct MenuBarContent: View {
  ...
  var body: some View {
    ...
    Button(menuStartStopTitle) {
      Task {
        switch model.phase {
        case .migrationRequired: openWindow(id: "main")
        case .running: await model.stop()
        default: await model.start()
        }
      }

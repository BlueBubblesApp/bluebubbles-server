...
struct BlueBubblesApp: App {
  ...
  /// Owned by the delegate, NOT by this scene.
  ///
  /// `@State` here would tie the model to the app's scenes, and the thing that goes wrong
  /// then is not the model's lifetime but the STARTUP's: see `AppDelegate`, which is where
  /// the server is now started from and why.
  ...
  var body: some Scene {
    Window("BlueBubbles", id: RootView.windowID) {
      RootView(model: model)
        // Parks `openWindow` where `applicationShouldHandleReopen` can reach it; see
        // `MainWindowOpener`.
        .background(MainWindowOpenerInstaller())
        ...
        .task {
          // The ONLY part of launching that needs a window: dismissing one.
          //
          // Everything else (the signal handlers, the activation policy, starting the
          // server) now happens in `AppDelegate.applicationDidFinishLaunching`, because
          // this `.task` does not always run. It belongs to the window's content view, and
          // the window is not guaranteed to exist: AppKit opens no window at launch when
          // argv leaves a stray positional argument behind, which `--headless --set k=v`
          // does every time. See `AppDelegate` for the measurement.
          //
          // `.accessory` permits windows, so the main one is dismissed rather than left
          // on screen: headless means no window, not no menu bar.
...
final class AppDelegate: NSObject, NSApplicationDelegate {
  ...
  /// The root model, owned HERE.
  ///
  /// It used to be a `weak var` the scene assigned to itself from the main window's `.task`.
  /// That made the delegate's view of the app depend on a window having appeared, which is
  /// exactly the assumption `applicationDidFinishLaunching` below exists to stop making.
  let model = AppModel()

  /// Starts the server, from the one place that always runs.
  ///
  /// **A SwiftUI `Window` scene is not guaranteed to open, so its `.task` is not a safe place
  /// to launch from.** AppKit opens no window at all when argv still holds a positional
  /// argument after it has paired the `-key value` arguments into `NSArgumentDomain`: it
  /// reads the leftover as a file to open. `--headless --set k=v` produces one every time,
  /// because `--headless` takes no value, so AppKit pairs it with `--set` and `k=v` is left
  /// over. Measured on a live build: `--headless` alone starts normally, `--headless --set
  /// socket_port=15879` never runs the window's `.task` at all, and a bare `BlueBubbles
  /// hello` does the same. With startup living in that task, the result was a process that
  /// looked alive, served nothing, and logged NOTHING, because the failure path only sets
  /// `phase` and headless has no window to show a phase in.
  ///
  /// This message does arrive at an `@NSApplicationDelegateAdaptor` object, contrary to what
  /// this file used to say: verified by probe on macOS 26, alongside
  /// `applicationWillFinishLaunching`. `applicationShouldTerminate` always did.
  func applicationDidFinishLaunching(_ notification: Notification) {
    // Before `start`, so a signal arriving during a slow startup still finds something to
    // shut down. Touching this installs it; it is a global, so it happens exactly once.
    _ = terminationSignals

    if LaunchOptions.current.isHeadless {
      // `.accessory`, NOT `.prohibited`.
      //
      // `.prohibited` suppresses the Dock icon AND every other way the app can present
      // itself (including the menu bar) so a headless server would run with no indication
      // whatsoever that it is running: no Dock icon, no status item, nothing to click to
      // stop it. `.accessory` is what "no Dock icon" actually means, and it is the same
      // policy the `hide_dock_icon` setting uses (see AppBehaviour.applyDockVisibility).
      NSApplication.shared.setActivationPolicy(
        AppBehaviourPolicy.activationPolicy(headless: true)
      )
    }

    // Who opened the app, not "the app opened this rather than the button". The startup
    // delay and starting minimised both hide the window from somebody who may well be
    // watching it; see `LauncherContract.automaticLaunchArgument`.
    model.beginStart(isUnattended: LaunchOptions.current.wasLaunchedAutomatically)
  }
  ...
  ///
  /// The body used to be `activate(ignoringOtherApps:)` and nothing else, which is precisely
  /// the default it claims to replace: it brings the app forward and opens no window. So
  /// after ⌘W, or after `--headless` closed the window at launch, clicking the Dock icon did
  /// nothing at all — and clicking the icon is the only way back.
  ///
  /// `openWindow` is an environment value and a delegate cannot read one, which is why this
  /// goes through `MainWindowOpener`: a view that CAN read it parks the action there. If no
  /// window has ever been shown the action is absent, and activating is the honest fallback.
...
/// Carries SwiftUI's `openWindow` out to the AppKit delegate.
///
/// `@Environment(\.openWindow)` is readable only from a `View`, and
/// `applicationShouldHandleReopen` is on the delegate. The action is a value that stays valid
/// after its window closes, so a view stores it once and the delegate calls it later.
@MainActor
enum MainWindowOpener {

  private static var action: (@MainActor () -> Void)?

  static func install(_ openWindow: @escaping @MainActor () -> Void) {
    action = openWindow
  }

  /// Opens the main window, or does nothing when no view has ever installed an action:
  /// a launch that has never shown a window has nothing to reopen.
  static func open() { action?() }
}

/// Installs the opener. A zero-size view rather than a modifier on `RootView` so it runs
/// once with the scene and does not re-register on every redraw.
private struct MainWindowOpenerInstaller: View {
  ...
  var body: some View {
    Color.clear
      ...
      .onAppear { MainWindowOpener.install { openWindow(id: RootView.windowID) } }
  }
}

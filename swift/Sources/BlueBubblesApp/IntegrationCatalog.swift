...
enum IntegrationCatalog {
  ...
  /// Every service the server knows, manageable or not.
  ///
  /// The ONE place the app reads the built-in list. Every screen that needs a manifest (the
  /// connection row, the health strip, Home, onboarding) asks this type, so a connection
  /// method the registry loads from somewhere else is found by all of them when this line
  /// changes, and by none of them if any screen kept its own lookup. A test refuses a
  /// second one.
  private static var all: [ServiceManifest] { BuiltInManifests.all }
  ...
  static var manageable: [ServiceManifest] {
    all.filter(\.isUserManageable)
  }

  /// Every connection method, in no particular order. `ConnectionMethodChoices` orders them.
  static var connectionMethods: [ServiceManifest] {
    all.filter { $0.category == .reverseProxy }
  }
  ...
  /// By name, except that the recommended connection method leads its category: the
  /// list is where someone chooses, and the suggestion belongs at the top of it.
  static func manifests(in category: ServiceCategory) -> [ServiceManifest] {
    manageable.filter { $0.category == category }.sorted { lhs, rhs in
      let lhsRecommended = ConnectionMethodChoices.isRecommended(lhs)
      let rhsRecommended = ConnectionMethodChoices.isRecommended(rhs)
      if lhsRecommended != rhsRecommended { return lhsRecommended }
      return lhs.name < rhs.name
    }
  }

  /// Any known service, by id: including the ones the Integrations screen does not list,
  /// so a status row can name Keep Awake while it starts. A screen that pushes a detail
  /// page checks `isUserManageable` itself.
  static func manifest(_ id: ServiceIdentifier) -> ServiceManifest? {
    all.first { $0.id == id }
  }

  /// What to call a service in a sentence, falling back to its identifier.
  ///
  /// The fallback is not decoration: `manifest(_:)` answers nil for a service registered
  /// without a manifest in the catalogue, and a startup line reading "Starting …" with a
  /// blank in it is worse than one naming a raw id.
  static func name(of id: ServiceIdentifier) -> String {
    manifest(id)?.name ?? id.rawValue
  }

...
extension AppModel {
  ...
  var backgroundActivities: [BackgroundActivity] {
    ...
    // 6. A remote install: a client asked, and the app is about to relaunch without any
    //    window of its own. The strip is the only place someone at the Mac learns why the
    //    app is about to vanish.
    if let version = updater.installingVersion {
      found.append(
        BackgroundActivity(
          id: "update-install",
          name: "Updates",
          state: .describing("Installing \(version) and relaunching…")
        ))
    }

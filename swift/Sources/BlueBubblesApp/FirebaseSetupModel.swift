...
final class FirebaseSetupModel {
  ...
  /// Awaits whatever operation is currently running, if any.
  ///
  /// The model publishes its state in steps: `rejectedFiles` first, `activity` back to
  /// `.idle` a suspension later, so an observer that samples one of them has no way to know
  /// the operation is over. `.idle` cannot answer that on its own either, because it is also
  /// the state the model starts in. This is the one signal that means finished, and it is
  /// exact: `start` assigns `runTask` synchronously, so a caller that returns from
  /// `importFiles` and awaits this cannot miss it.
  ///
  /// Named for `EventBus.settle()`, which exists for the same reason.
  func settle() async { await runTask?.value }
  ...
  private func runProvisioning(
    ...
    do {
      ...
      // Both of those types define `body { description }`, so this is the same string
      // they produced by hand, and a better one for anything else that reaches here.
      let text = DiagnosticText.sentence(for: error)
      report(.failure, text)
  ...
  func sendTest(push: (any PushSetupProviding)?) {
    start(push: push) { [weak self] push in
      ...
      await self?.finish(
        status: nil,
        message: delivered > 0
          ? "Sent to \(delivered.counted("device"))."
  ...
  private func start(
    ...
    runTask = Task { [weak self] in
      do {
        try await body(await Self.readyInterface(push))
        ...
        // `PushSetupError` and `ProvisioningError` both carry a sentence written for a
        // person, and both define `body { description }`, so the one rule reaches the
        // same string this chain spelled out, and a better one for anything else.
        let text = DiagnosticText.sentence(for: error)
  ...
  /// A push interface with its service running, if it can be.
  ///
  /// Configured-but-not-running is a state the user can land in and cannot get out of: the
  /// gate reads the Keychain once, at startup, and swallows a read that FAILS as "there are
  /// no credentials", which is what a first launch does while the access dialog is still on
  /// screen. Push declines for the life of the process, the page reads the same credentials
  /// a moment later and reports the project as set up, and every button that needs the
  /// service answers "Push notifications are not running on this server".
  ///
  /// So the two are reconciled here, once per action, rather than left to the user to guess
  /// that restarting the server would fix it. Re-fetched afterwards because `PushInterface`
  /// captures the service it was built with: reloading through the one in hand would start
  /// the service and then act through a value that still holds nil.
  ///
  /// The condition mirrors the GATE, not this page's idea of "set up". Two different
  /// questions wear the name `isConfigured`: `PushStatus.isConfigured` means both files are
  /// present, while `PushDeliveryService.canRun` asks `PushCredentialStore.isConfigurable`,
  /// which is the service account ALONE. A server with a key and no `google-services.json`
  /// can send notifications: it is `hasServiceAccount` that decides whether a reload has
  /// any chance of starting push, and requiring both would leave exactly that server stuck.
  /// With no key at all, "not running" is correct and a reload would decline again.
  private static func readyInterface(_ push: any PushSetupProviding) async -> PushInterface {
    let interface = await push.pushInterface()
    guard !interface.isRunning, await interface.status().hasServiceAccount else {
      return interface
    }
    await interface.reload()
    return await push.pushInterface()
  }
  ...
  private func offer(projects: [FirebaseProjectSummary]) {
    ...
    note(
      projects.isEmpty
        ...
        : "Found \(projects.count.counted("existing project")).")
  }

...
/// The toolbar row.
///
/// `Button` + `openURL` rather than `Link`. A `Link` is the more semantic control and it
/// loses two things that matter more here: it renders in the accent colour while every other
/// toolbar control is monochrome, and it gets no hover highlight, so three of the four
/// controls in the group would light up on hover and these would not.

...
struct ContactsView: View {
  ...
  private var visible: [ContactRowItem] {
    ContactFiltering.visible(rows, search: search, sortOrder: sortOrder)
  }
  ...
  ///
  /// Counts the MATCHES rather than reading `visible`, which sorted them first: sorting is
  /// four times the cost of filtering, and nothing about a count depends on the order. This
  /// was the call site that kept the page at two full passes per keystroke.
  private var countSummary: String {
    let total = contacts.count
    guard !search.isEmpty else { return total.counted("contact") }
    return "\(ContactFiltering.matches(rows, search: search).count) of "

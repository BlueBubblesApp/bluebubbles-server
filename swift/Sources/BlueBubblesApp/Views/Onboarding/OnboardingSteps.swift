...
private struct PrivateAPIStep: View {
    ...
    VStack(alignment: .leading, spacing: 16) {
      if sipDisabled {
        ...
        if let store = model.settingsStore {
          // Both toggles, because they are two independent capabilities in two different
          // apps — `PrivateAPIGatedService.canRun()` starts on EITHER, and FaceTime does
          // not need the Messages helper. Offering only Messages here left the FaceTime
          // half of the feature discoverable solely by finding the settings tab.
          section([Settings.enablePrivateAPI.erased], store: store)
          // Hand-off rides with the toggle it depends on rather than waiting for a
          // Settings visit: it is the FaceTime capability a CLIENT actually uses, and
          // its own help says it needs the switch above it. Registry order, which is
          // also the settings tab's order.
          section(
            [
  ...
  /// One section, headed, described and divided exactly as the settings tab does it.
  ///
  /// The section name and its sentence both come from the declarations rather than from
  /// literals here — a renamed section or a reworded subtitle changes in one place, and this
  /// screen cannot end up calling the same switch something the settings page does not.
  ///
  /// An empty list renders nothing rather than a headed card with no rows in it, which is
  /// also what makes the `first` read safe.
  @ViewBuilder
  private func section(_ settings: [AnySetting], store: SettingsStore) -> some View {
    if let first = settings.first {
      SettingsSection(
        first.presentation.section,
        subtitle: SettingsView.description(for: first.presentation.section)
      ) {

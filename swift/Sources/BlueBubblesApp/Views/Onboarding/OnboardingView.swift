...
struct OnboardingView: View {
  ...
  private var passwordRejection: String? {
    ...
    do {
      ...
      // `validate` only throws `Rejection`, so this is the unreachable arm, but an
      // unreachable arm that would print a Swift value at somebody is still the wrong
      // thing to leave in a field they are typing into.
      return DiagnosticText.sentence(for: error)
    }
  }

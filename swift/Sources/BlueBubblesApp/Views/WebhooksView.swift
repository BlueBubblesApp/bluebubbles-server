...
struct WebhooksView: View {
  ...
  // The matching and the sentence are `WebhookDeliverySummary`, not `private func`s here:
  // the id-reuse rule in particular is worth a test, and could not have one on a View.
  private func delivery(for hook: Webhook) -> WebhookDeliveryState? {
    WebhookDeliverySummary.delivery(for: hook, in: deliveries)
  }

  private func describe(_ state: WebhookDeliveryState) -> String {
    WebhookDeliverySummary.describe(state)
  }

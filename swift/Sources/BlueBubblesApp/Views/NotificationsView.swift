...
struct NotificationsView: View {
  ...
  private var list: some View {
    ScrollView {
      // LAZY. A plain `VStack` builds every row the moment the drawer opens, and each row is
      // a blurred card: the model seeds 200 and the centre holds up to 500, so opening the
      // drawer composited up to 500 glass layers in one frame. The log viewer and the chat
      // picker are both lazy already; this was the one list that was not.
  ...
  private func perform(_ action: AlertAction, on alert: UserAlert) async {
    ...
    case .installUpdate:
      // Sparkle's window, not a download from here: the release notes and the choice
      // to skip belong on that window, and this button only brings it forward.

...
struct ConnectionMethodRow: View {
  ...
  private var toolNote: SettingsFootnote? {
    ...
    // A copy IS here and is not being used, which is a different sentence from "not
    // downloaded yet" and a much more confusing state to meet without one: the user knows
    // they installed it. `copyOnThisMacSummary` is the tool manager's own wording, so this
    // row and the Integrations page never describe the same copy differently.
    if let copy = toolStatus?.copyOnThisMacSummary {
      return SettingsFootnote(text: copy, symbol: "externaldrive", tone: .warning)
    }
    return SettingsFootnote(

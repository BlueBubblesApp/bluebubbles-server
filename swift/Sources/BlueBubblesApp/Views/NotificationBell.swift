...
struct NotificationBell: View {
  ...
  var body: some View {
    Button {
      model.alerts.isDrawerPresented.toggle()
    } label: {
    ...
    .help(
      model.alerts.unreadCount > 0
        ? "\(model.alerts.unreadCount.counted("unread notification"))"
        : "Notifications"
    )
    // A tooltip is not a label: it needs a pointer to hover, which is exactly what a VoiceOver user does
    // not have. The count is the VALUE rather than part of the name, so the button keeps one
    // name as its badge changes.
    .accessibilityLabel("Notifications")
    .accessibilityValue(
      model.alerts.unreadCount == 0
        ? "No unread"
        : "\(model.alerts.unreadCount) unread"
    )
    // Bound to the model, so the Guides page can open this drawer rather than describing
    // it. Written out rather than `$model.alerts.…` because `alerts` is a `let` on
    // `AppModel`, so `@Bindable` cannot project through it: the same shape `RootView` uses
    // for the migration and onboarding sheets.
    .popover(
      isPresented: Binding(
        get: { model.alerts.isDrawerPresented },
        set: { model.alerts.isDrawerPresented = $0 }
      ),
      arrowEdge: .bottom
    ) {
      NotificationsPopover(model: model, dismiss: { model.alerts.isDrawerPresented = false })

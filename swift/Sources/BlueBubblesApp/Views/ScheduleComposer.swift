...
struct ScheduleComposer: View {
  ...
  ///
  /// It read `(try? await …) ?? []`, so a server that REFUSED and a Mac with no
  /// conversations arrived at the picker as the same empty array — and `emptyResults`
  /// below already tried to tell those apart and could not, because nothing reaching it
  ...
  private var emptyResults: some View {
    // Three different situations that used to look identical, because the read collapsed
    // all of them into an empty array. Now they are distinguishable, so they are told
    // apart: the read failed, the read has not happened, or it happened and there is
    // genuinely nothing to show.
    if let failure = conversations.state.errorMessage {

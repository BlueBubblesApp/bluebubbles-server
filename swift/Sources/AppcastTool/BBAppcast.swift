...
struct Add: ParsableCommand {
  ...
  var notes: String?
  @Option(help: "URL of the full version history, for Sparkle's Version History button.")
  var fullReleaseNotesLink: String?
  @Option(
    help: "Sparkle channel, e.g. beta. Omit for a release every install should take.")
  var channel: String?
  ...
  func run() throws {
    ...
    // ABSENT and UNREADABLE are different, and collapsing them loses every prior release.
    //
    // `try?` treated both as "start a new feed". Absent is the correct first-release case:
    // the root `appcast.xml` does not exist until the first one is cut. A read error on a
    // file that IS there is not — it would publish a feed containing one entry, silently
    // cutting every installed client off from the update history Sparkle uses to say what
    // changed. The parser already throws on malformed XML; only the read was unguarded.
    var feed: Appcast
    if FileManager.default.fileExists(atPath: appcast) {
      let existing = try Data(contentsOf: URL(fileURLWithPath: appcast))
    ...
    feed.items.append(
      AppcastItem(
        ...
        title: shortVersion,
        publishedAt: Date(),
        // A notes path that was GIVEN and cannot be read is an error, not empty notes.
        // `try?` published a release whose "what changed" section was silently blank
        // because someone mistyped the path, and the release is drafted by then.
        releaseNotesHTML: try notes.map {
        ...
        fullReleaseNotesLink: fullReleaseNotesLink,
        channel: channel
      )
    )

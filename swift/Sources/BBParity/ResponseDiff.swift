...
/// blind spot is real: an entry for `os_version` would let `"Version 26.5.2 (Build 25F84)"`
/// pass against the reference's `"26.5.2"`, because both are strings.
///
...
/// they must match exactly and are not here.
...
/// would let a decision hide among the noise.
///
...
///
/// **The key is a PATH, not a field name, and that distinction is the point.** These were
/// bare leaf names, matched against whatever key the diff happened to be looking at, at any
/// depth of any response. So declaring `install` for the one object on
/// `GET /server/update/check` also accepted an `install` key appearing on a message, on a
/// chat, on a handle, inside an attachment, on every route in the API. An exception written
/// for one field on one route silently became a hole across the whole surface, and nothing
/// in the entry said so.
///
/// A key is now the full path the diff reports, with array indices erased: `data.local_ipv6s`
/// accepts that field on the server-info envelope and nowhere else. A field that genuinely
/// IS everywhere, because it comes from a serialiser that many routes embed, is written
/// `**.name` and says so out loud. The default is narrow; breadth is declared.
public let acceptedDifferences: [String: String] = [
  ...
  "data.local_ipv6s": "link-local addresses are dropped; the reference publishes them",
  ...
  "**.associatedMessageEmoji":
    "the emoji of an emoji tapback; the reference never read the column",
  ...
  // `GET /server/update/check`. What the app is doing about an update it already found
  // (downloading, scheduled for the install hour, installing), so a client that asked for
  // the install sees progress rather than "available" again. Null when nothing is pending.
  // The reference had no updater state to report.
  "data.install": "pending-install state on the update check; the reference had none",
]

/// The FIELD names in `acceptedDifferences`, without their paths.
///
/// For the checks that ask "is this added field declared anywhere?" rather than "is it
/// accepted here?". `SendShapeTests` is the caller: it compares two field sets and has no
/// path to match against.
public let acceptedFieldNames: Set<String> = Set(
  acceptedDifferences.keys.map { key in
...
public enum ResponseDiff {
  ...
  public static func compare(
    ...
    //
    // An addition we CHOSE is declared in `acceptedDifferences` and skipped here: here,
    // not only in the value check, or declaring an added key would do nothing.
    for key in actualKeys.subtracting(expectedKeys).sorted()
    where !isAccepted("\(path)\(key)", by: accepted) {
    ...
    for key in expectedKeys.intersection(actualKeys).sorted() {
      ...
      if isAccepted(childPath, by: accepted) { continue }
      ...
      case (let left as [Any], let right as [Any]):
        ...
        //
        // An `else` here would hide everything inside the array behind a number that
        // reads like an environment difference: on `contact`, an added key, two dropped
        // keys, a missing nested `id` and a null `displayName` all behind "552 vs 587".
        // Two servers holding identical data is the rare case, not the common one: a
        // length mismatch must never suppress what is inside the array.
  ...
  /// Whether a reported path is one of the differences we chose.
  ///
  /// The path is normalised first: `data.messages[3].scheduleType` and
  /// `data.messages[7].scheduleType` are the same declaration, so the index is erased before
  /// matching. A `**.` pattern matches the last segment anywhere, which is what a field
  /// carried by a serialiser many routes embed needs; anything else must match the whole
  /// path, which is what a field on one route gets.
  static func isAccepted(_ path: String, by accepted: [String: String]) -> Bool {
    let normalised = normalise(path)
    if accepted[normalised] != nil { return true }
    let leaf = String(normalised.split(separator: ".").last ?? "")
    return !leaf.isEmpty && accepted["**." + leaf] != nil
  }

  /// `data.messages[3].guid` -> `data.messages.guid`.

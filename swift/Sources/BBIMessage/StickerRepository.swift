...
public struct StickerRepository: Sendable {
  ...
  // The columns each query wants, as (alias, column) pairs.
  //
  // **`stickers.stickerdb` is Apple's, it is NEWER than chat.db, and it had no schema
  // profile.** The header of this file says as much: the store did not exist before Sonoma,
  // so a column here is if anything more likely to move than one in `chat.db`, and every
  // chat.db query goes through `SchemaProfile.select`. These named fourteen columns outright,
  // with no introspection and no fallback, and this file's own tests say the queries are
  // exercised against a live Mac rather than in the suite. That Mac runs macOS 26. If any of
  // those columns arrived after Sonoma, every sticker route answers 500 with "no such
  // column" on the deployment floor, which is exactly the failure `supportsScheduledMessages`
  // was added to prevent for Send Later.
  //
  // So the list is filtered by what the table actually has: a column the store does not carry
  // is selected as NULL, which the row decoder already handles, and the feature degrades by
  // one field instead of failing altogether.
  private static let stickerColumnsWanted: [(String, String)] = [
    ("pk", "Z_PK"), ("type", "ZTYPE"), ("identifier", "hex(ZIDENTIFIER)"),
    ("externalURI", "ZEXTERNALURI"), ("name", "ZNAME"),
    ("accessibilityName", "ZACCESSIBILITYNAME"), ("searchText", "ZSEARCHTEXT"),
    ("byteCount", "ZBYTECOUNT"), ("effect", "ZEFFECT"),
    ("createdAt", "ZCREATIONDATE"), ("lastUsedAt", "ZLASTUSEDDATE"),
    ("libraryIndex", "ZLIBRARYINDEX"), ("attributionName", "ZATTRIBUTIONNAME"),
    ("attributionBundleID", "ZATTRIBUTIONBUNDLEIDENTIFIER"),
  ]

  private static let representationColumnsWanted: [(String, String)] = [
    ("sticker", "ZSTICKER"), ("identifier", "hex(ZIDENTIFIER)"), ("role", "ZROLE"),
    ("uti", "ZUTI"), ("width", "ZSIZE_W"), ("height", "ZSIZE_H"),
    ("byteCount", "ZBYTECOUNT"), ("isPreferred", "ZISPREFERRED"),
  ]

  /// `alias.COLUMN AS name` for every column the table has, `NULL AS name` for the rest.
  private static func selectList(
    _ wanted: [(String, String)], available: Set<String>, alias: String
  ) -> String {
    wanted.map { name, expression in
      // `hex(ZIDENTIFIER)` names its column inside the call; everything else is the column.
      let bare =
        expression.hasPrefix("hex(")
        ? String(expression.dropFirst(4).dropLast()) : expression
      guard available.contains(bare.uppercased()) else { return "NULL AS \(name)" }
      return expression.hasPrefix("hex(")
        ? "hex(\(alias).\(bare)) AS \(name)"
        : "\(alias).\(bare) AS \(name)"
    }
    .joined(separator: ", ")
  }

  /// Built once per repository, from the store in front of us.
  private func stickerColumns() async throws -> String {
    let available = Set(
      try await database.columns(of: "ZMANAGEDSTICKER").map { $0.uppercased() })
    return columnCache.stickerList {
      Self.selectList(Self.stickerColumnsWanted, available: available, alias: "s")
    }
  }

  private func representationColumns() async throws -> String {
    let available = Set(
      try await database.columns(of: "ZMANAGEDREPRESENTATION").map { $0.uppercased() })
    return columnCache.representationList {
      Self.selectList(Self.representationColumnsWanted, available: available, alias: "r")
    }
  }

  /// Memoised per repository: the store's schema cannot change under a running server.
  private final class ColumnCache: @unchecked Sendable {
    private let lock = NSLock()
    private var sticker: String?
    private var representation: String?

    func stickerList(_ build: () -> String) -> String {
      lock.lock()
      defer { lock.unlock() }
      if let sticker { return sticker }
      let list = build()
      sticker = list
      return list
    }

    func representationList(_ build: () -> String) -> String {
      lock.lock()
      defer { lock.unlock() }
      if let representation { return representation }
      let list = build()
      representation = list
      return list
    }
  ...
  private let columnCache = ColumnCache()
  ...
  public func stickers(
    ...
    let columns = try await stickerColumns()
    let representationList = try await representationColumns()

    return try await database.read { db in
      let rows = try Row.fetchAll(
        db,
        sql: """
          SELECT \(columns) FROM ZMANAGEDSTICKER s
      ...
      return Self.assemble(
        rows,
        representations: try Self.representations(
          db, stickerKeys: keys, columns: representationList))
    }
  }
  ...
  public func sticker(identifier: String) async throws -> StickerRow? {
    ...
    return try await database.read { db in
      ...
      return Self.assemble(
        [row],
        representations: try Self.representations(
          db, stickerKeys: [key], columns: representationList)
      ).first
    }
  }
  ...
  /// URI, exactly as it does for the saved sticker a recent points back at. Measured:
  /// answering with the donation's identifier makes every read of it 404.
  ///
  ...
  private static func representations(
    _ db: Database, stickerKeys: [Int], columns: String
  ) throws -> [Int: [StickerRepresentationRow]] {
    guard !stickerKeys.isEmpty else { return [:] }
    let placeholders = stickerKeys.map { _ in "?" }.joined(separator: ", ")
    let rows = try Row.fetchAll(
      db,
      sql: """
        SELECT \(columns) FROM ZMANAGEDREPRESENTATION r

...
//
//  Two things about the registry are not obvious:
...
struct HomebrewBottleIndex: Sendable, Equatable {
  ...
  /// one built on the OLDEST release the index carries, because a bottle built on Sonoma
  /// runs on Sequoia and Tahoe while the reverse is not promised. That is also what makes
  /// a pinned digest stable: the choice depends on what the index carries, not on which
  /// Mac is asking.
  ///
  /// It is a preference, not a guarantee about the host. Homebrew builds for its supported
  /// macOS releases only, so once it stops building for this server's floor release the
  /// oldest bottle in a NEW version's index is newer than a Mac still on that floor. A
  /// static Go binary such as tailscaled tolerates that; nothing here checks it, and the
  /// version probe at install time is what would catch a bottle that will not run.

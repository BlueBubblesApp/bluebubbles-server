...
public struct ToolInstaller: Sendable {
  ...
  ///
  /// Six phases in a fixed order (see the header) each its own function below, so the
  /// order reads here in twenty lines and the detail of any one phase is in one place.
  public func install(
    ...
    // 1. Download.
    let archive = try await download(release, of: descriptor, into: scratch, progress: progress)
    ...
    progress(.verifying)
    let digest = try await verifyDigest(of: archive, descriptor: descriptor, release: release)
    ...
    //    the tarball around it, and BEFORE `makeRunnable`, which is the order this file's
    //    header states: "download, verify, unpack, prove it runs".
    //
    //    The two were the other way round. `makeRunnable` chmods 755 and strips the
    //    quarantine flag, so an unverified binary was made executable and de-quarantined,
    //    then verified. The window was small — inside a scratch directory a `defer` removes,
    //    and the EXECUTION (`probeVersion`) did come after verification — but the two lines
    //    are trivially swappable and the order was the thing being claimed.
    progress(.verifying)
    ...
    // 6. Adopt it.
    progress(.activating)
    let installedExecutable = try adopt(
      unpacked, of: descriptor, release: release, version: version, layout: layout
    )
  ...
  /// The downloaded archive, and the name it was downloaded under, which is also the name
  /// a vendor's checksums file lists it by.
  private struct DownloadedArchive {
    let file: URL
    let name: String
  }

  /// Phase 1. Fetches the release asset into `scratch`, reporting progress as it goes.
  private func download(
    _ release: ResolvedRelease,
    of descriptor: ManagedToolDescriptor,
    into scratch: URL,
    progress: @escaping @Sendable (ToolInstallPhase) -> Void
  ) async throws -> DownloadedArchive {
    let downloadName =
    ...
    return DownloadedArchive(file: archiveFile, name: downloadName)
  }

  /// Phase 2. Hashes the archive and holds the digest against every pin that applies,
  /// strongest first. Returns the digest, which is recorded on the install either way.
  private func verifyDigest(
    of archive: DownloadedArchive,
    descriptor: ManagedToolDescriptor,
    release: ResolvedRelease
  ) async throws -> String {
    let digest = try sha256(of: archive.file, toolID: descriptor.id)
    ...
    if let checksumsURL = release.checksumsURL {
      try await verifyChecksum(
        descriptor, digest: digest, assetName: archive.name, checksumsURL: checksumsURL
      )
      ...
      throw ToolError.checksumMissing(tool: descriptor.id, asset: archive.name)
    }
    return digest
  }

  /// Phase 6. Moves the unpacked tree into its version directory, makes the executable and
  /// its companions runnable there, and points `current` at it. Returns the executable as
  /// it is installed.
  private func adopt(
    _ unpacked: URL,
    of descriptor: ManagedToolDescriptor,
    ...
    // their prefix: subtracting one from the other yields `…/privatefaketool`, and the
    // install fails on a file that exists.
    guard
  ...
  private func sha256(of file: URL, toolID: String) throws -> String {
    ...
    // `try`, not `try?`. An I/O error part-way through exited the loop silently and
    // finalised the hash of a TRUNCATED file. Against a pinned digest that fails safe, the
    // mismatch throws; with no pin the wrong digest was recorded on the install as fact,
    // and every later comparison was against a hash of a file that never existed.
    var hasher = SHA256()
  ...
  ///
  /// The probe itself lives in `ToolVersionProbe`, shared with discovery. What differs here
  /// is only what is done with the answer: this is refusing an install, so a binary that
  /// will not start is fatal.
  private func probeVersion(
    _ descriptor: ManagedToolDescriptor, executable: URL
  ) async throws -> String? {
    switch await ToolVersionProbe.run(descriptor, executable: executable.path) {
    case .ran(let version): return version
    case .versionUnreadable: return nil
    case .wouldNotRun(let reason):
      throw ToolError.versionProbeFailed(tool: descriptor.id, reason: reason)
    }
  }
}

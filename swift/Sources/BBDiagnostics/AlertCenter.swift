...
public actor AlertCenter: AlertRaising {
  ...
  /// prove a 30-day cutoff works is to wait 30 days.
  ...
  /// Observers of withdrawals. The drawer seeds itself once and then appends from `stream()`,
  /// so without this an alert the server withdrew stayed on screen until the next re-read.
  private var dismissalContinuations: [UUID: AsyncStream<[UUID]>.Continuation] = [:]
  ...
  public func raise(_ alert: UserAlert) async {
      ...
      let index = alerts.firstIndex(where: { $0.id == existingID })
    {
      ...
      // cutoff comes from `clock.now` would leave the injected clock controlling only one
    ...
    // The dedupe key is RESERVED before the await below, not after it.
    //
    // The check above is synchronous, but `store.insert` suspends, and this actor releases
    // across it. Two concurrent raises with the same key both missed the index, both
    // inserted, and the index ended up naming only one — so the flapping proxy that this
    // coalescing exists for produced exactly the duplicate rows it was written to prevent,
    // in the one case it was written for.
    //
    // Reserved with the id this alert will carry, so the loser of the race coalesces onto
    // the winner on its next raise rather than inserting a third.
  ...
  /// - Parameter limit: how many to return, newest first. Clamped: `prefix` traps on a
  ///   negative length, and this is reached straight from `?limit=` on the alert routes, so
  ///   `?limit=-1` stopped the process. Guarded here rather than only at the two handlers,
  ///   for the same reason `FileSink.tail` is: the route was the only caller when the same
  ///   bug was fixed there, and this is the sibling that was missed.
  public func all(limit: Int = 100) -> [UserAlert] {
    Array(alerts.prefix(max(limit, 0)))
  }
  ...
  ///
  /// It does not tombstone it. One model, not two: a dismissed alert is gone,
  ...
  public func dismiss(_ id: UUID) async {
    ...
    broadcastDismissal([id])
  }

  /// Withdraws every alert whose dedupe key starts with `prefix`.
  ///
  /// For a condition that has CLEARED, as opposed to one a person has dealt with. A
  /// connection method waiting on a sign-in raises the link; once the sign-in happens the
  /// notice is not read, it is wrong, and leaving it in the drawer tells the person to do
  /// something they have done. The prefix is the service's own, so a method withdrawing its
  /// pending steps cannot reach another's.
  public func dismiss(dedupeKeyPrefix prefix: String) async {
    let matching = alerts.filter { $0.dedupeKey?.hasPrefix(prefix) == true }
    guard !matching.isEmpty else { return }
    let removed = Set(matching.map(\.id))
    for alert in matching {
      if let key = alert.dedupeKey { indexByDedupeKey[key] = nil }
      do { try await store?.delete(alert.id) } catch { logStoreFailure(error) }
    }
    alerts.removeAll { removed.contains($0.id) }
    broadcastDismissal(Array(removed))
  }

  public func clear() async {
    let removed = alerts.map(\.id)
    alerts.removeAll()
    indexByDedupeKey.removeAll()
    do { try await store?.deleteAll() } catch { logStoreFailure(error) }
    broadcastDismissal(removed)
  }
  ...
  /// The identifiers of alerts withdrawn, in batches, as they go.
  public func dismissals() -> AsyncStream<[UUID]> {
    let id = UUID()
    return AsyncStream { continuation in
      dismissalContinuations[id] = continuation
      continuation.onTermination = { [weak self] _ in
        Task { await self?.removeDismissalContinuation(id) }
      }
    }
  }
  ...
  private func removeDismissalContinuation(_ id: UUID) {
    dismissalContinuations[id] = nil
  }
  ...
  private func broadcastDismissal(_ ids: [UUID]) {
    guard !ids.isEmpty else { return }
    for continuation in dismissalContinuations.values {
  ...
  private func trim() async {
    ...
    // BOTH branches clean the dedupe index, and BOTH announce what they removed.
    //
    // The retention branch did neither. A drawer seeds itself once and then follows
    // `stream()` and `dismissals()`, so an alert trimmed here stayed on screen until the
    // window was reopened: the exact bug the dismissal stream was added to fix, arriving by
    // a different route. And leaving the index entry behind means the next raise of that
    // dedupe key updates a row that is no longer in `alerts`, so it never appears at all.
    var removed: [UUID] = []

    let expired = alerts.filter { $0.lastOccurredAt < cutoff }
    for alert in expired {
      if let key = alert.dedupeKey { indexByDedupeKey[key] = nil }
      removed.append(alert.id)
    }
    alerts.removeAll { $0.lastOccurredAt < cutoff }

    if alerts.count > capacity {
      for alert in alerts[capacity...] {
        if let key = alert.dedupeKey { indexByDedupeKey[key] = nil }
        removed.append(alert.id)
      }

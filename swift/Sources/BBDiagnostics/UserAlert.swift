...
//
//  Two systems: swift-log never produces a user-visible item, and alerts are raised
//  explicitly. Coupling the two turns every internal error into a user-facing notification
...
public struct Diagnostics: Sendable {
  ...
  public init(
    ...
    self.code = code
    self.domain = domain
    // REDACTED HERE, not at each call site, because this field is the one nobody thinks
    // about: it is almost always `String(describing: error)`, and what that contains is
    // decided by whoever wrote the error rather than by the person raising the alert. A
    // `URLError` puts `NSErrorFailingURLKey=<the whole URL>` in it, query string included,
    // and `redactedReport()` prints this verbatim into the bundle people paste into chat
    // and issue trackers. The webhook alert redacted the URL in its body and its context
    // and then handed the raw one over in this field.
    //
    // Central rather than per-caller for the same reason `Redaction` exists at all: a rule
    // every raise site has to remember is one that holds until somebody adds a raise site.
    self.underlyingDescription = underlyingDescription.map(Redaction.urls(in:))
...
///
/// An enum rather than a free string. A string would be written by whoever raises the
/// alert, in a different module, and a new spelling would compile, render a button, and land
/// on the wrong page. Every case here is a page the app has, and the app's routing switch is
/// exhaustive.
...
public enum AlertAction: Sendable, Hashable {
  ...
  /// Brings the app's updater forward for a release it has already found.
  ///
  /// Raised only by the app, which owns Sparkle; a headless server has nothing to bring
  /// forward and never raises it. The button opens Sparkle's own window (release notes,
  /// Install, Remind Me Later, Skip), so the decision is still made there, with the notes
  /// in view, rather than on a one-line notice.
  case installUpdate

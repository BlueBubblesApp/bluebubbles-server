...
public struct RotatingFileLogHandler: LogHandler {
  ...
  public func log(event: LogEvent) {
    ...
    if !merged.isEmpty {
      let rendered =
        merged
        .sorted { $0.key < $1.key }
        // REDACTED HERE, once, rather than at ninety-four call sites.
        //
        // The value most likely to carry a URL nobody decided to log is an ERROR's own
        // description: `String(describing:)` is written by whoever wrote the error, and a
        // `URLError` carries `NSErrorFailingURLKey=https://…` with the whole query string —
        // which is where clients routinely put the server password. Ninety-four sites log
        // an error that way, and asking each to remember is how one of them does not.
        //
        // `Diagnostics.init` already does exactly this for the alert path, and for the same
        // reason. Cheap: `urls(in:)` returns its input untouched when there is no `://` in
        // it, which is almost every line.
        .map { "\($0.key)=\(Redaction.urls(in: "\($0.value)"))" }
        .joined(separator: " ")
      line += " {\(rendered)}"
    }
  ...
  private static let formatter: DateFormatter = {
    ...
    // POSIX locale, pinned calendar, and the local time zone stated rather than inherited.
    //
    // A `dateFormat` with the user's locale left in place is not the fixed format it looks
    // like: `yyyy` renders the era year under a Japanese or Buddhist calendar region, and
    // `HH` can follow a twelve-hour preference. A log line is the artefact people paste
    // into an issue and the format is documented as stable, so it has to be stable
    // everywhere rather than on the machines that happen to be in `en_US`.
    //
    // The time zone stays LOCAL on purpose: somebody reading their own log is matching it
...
public final class FileSink: @unchecked Sendable {
  ...
  private func open() {
    ...
    // REPORTED. Without this, a file that could not be opened left `handle` nil, and every
    // subsequent `try? handle?.write` was a no-op FOREVER, with nothing anywhere saying the
    // log had stopped being written. That is the worst possible failure for a log: the one
    // place you would look for the reason is the thing that broke. It goes to the console
    // sink, which is the only one guaranteed to still work.
    if handle == nil {
      FileHandle.standardError.write(
        Data(
          "bluebubbles: could not open the log file at \(url.path); file logging is off\n"
            .utf8))
    }
  ...
  /// Empties the log: the current file is truncated and its rotated copies are deleted.
  ///
  /// Synchronous on the writer's queue, so it is a barrier: every line written before the
  /// call is gone and every line written after it lands in the fresh file. Followers are
  /// not told; the one viewer there is drops its own tail beside this call, and a stream
  /// that only ever appends has no way to say "forget what you have". Best effort, like the
  /// rest of this type: a file that cannot be truncated is left as it is.
  public func clear() {
    queue.sync {
      try? handle?.close()
      handle = nil
      let manager = FileManager.default
      for index in 1...max(1, keepRotations) {
        try? manager.removeItem(at: url.appendingPathExtension("\(index)"))
      }
  ...
  ///
  /// Clamped here as well as at the route. `Array.suffix` traps on a negative length, and
  /// the guarantee belongs with the function that needs it rather than only with the one
  /// caller that happens to remember: a second caller is how the first trap came back.
  public func tail(lines count: Int) -> [String] {
    let wanted = max(count, 0)
    guard wanted > 0 else { return [] }
    // Read from the END rather than loading the file.
    //
    // This used to read the whole thing into a String and split it, INSIDE `queue.sync`,
    // which is the same serial queue every write goes through: a client asking for ten lines
    // stalled every logger in the process while up to ten megabytes were read, decoded and
    // split. It backs a route any authenticated client can call, in a loop if it likes.
    //
    // Reading backwards in chunks costs the tail and nothing else. The queue is still taken,
    // because the writer and this must not interleave, but it is held for a few kilobytes
    // instead of the file.
    return queue.sync { Self.lastLines(of: url, count: wanted) }
  }

  /// The last `count` lines of a file, read backwards.
  ///
  /// Returns them in file order, and keeps the trailing empty element the split used to
  /// produce, because the route's caller strips it and a client would otherwise see the last
  /// line disappear.
  static func lastLines(of url: URL, count: Int) -> [String] {
    ...
    var position = end
    var buffer = Data()
    // One more than asked for: the read has to reach the newline BEFORE the first line
    // wanted, or that line comes back truncated at whatever the chunk boundary cut.
    let wantedNewlines = count + 1

    while position > 0 {
      let size = UInt64(chunkSize) > position ? Int(position) : chunkSize
      position -= UInt64(size)
      guard (try? handle.seek(toOffset: position)) != nil,
        let chunk = try? handle.read(upToCount: size)
      else { break }
      buffer = chunk + buffer
      if buffer.filter({ $0 == UInt8(ascii: "\n") }).count >= wantedNewlines { break }
    }

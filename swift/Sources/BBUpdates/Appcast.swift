...
public struct AppcastItem: Sendable, Equatable {
  ...
  /// Where the whole history lives, for Sparkle's "Version History" button on the
  /// up-to-date alert. The GitHub releases page; one link for every item.
  public var fullReleaseNotesLink: String?
  /// Sparkle's `sparkle:channel`. Nil is the default channel every install reads; a named
  /// one (`beta`) is offered only to installs that opted in. See `Appcast.newestItem`.
  ...
  public init(
    ...
    fullReleaseNotesLink: String? = nil,
    channel: String? = nil
  ) {
...
public struct Appcast: Sendable, Equatable {
  ...
  /// The highest version in the feed, on the default channel.
  ///
  ...
  public var newestItem: AppcastItem? { newestItem(allowingChannels: []) }

  /// The highest version an install reading `channels` as well as the default may take.
  ///
  /// The same rule Sparkle applies: an item with no channel is for everyone, an item on a
  /// named channel only for installs that allow it. A beta that is OLDER than the current
  /// release is never offered, because the comparison is by version across both.
  public func newestItem(allowingChannels channels: Set<String>) -> AppcastItem? {
    items
      .filter { $0.channel.map(channels.contains) ?? true }
      .max { SemanticVersion($0.version) < SemanticVersion($1.version) }
  }
}
  ...
  public func xmlString(generatedAt: Date = Date()) -> String {
    ...
    for item in ordered {
      ...
      if let channel = item.channel, !channel.isEmpty {
        lines.append("            <sparkle:channel>\(Self.escape(channel))</sparkle:channel>")
      }
      if let link = item.fullReleaseNotesLink, !link.isEmpty {
        lines.append(
          "            <sparkle:fullReleaseNotesLink>\(Self.escape(link))"

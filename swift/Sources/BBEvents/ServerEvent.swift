...
//
//  Routing is a property of the event, not of whoever happens to be emitting it.
//
//  See `docs/EVENTS.md`.
...
///
/// In the reference these decisions live as extra boolean arguments to `emitMessage`
...
public struct EventRouting: Sendable {
  ...
  /// Per-event routing.
  ///
  /// **The reference's two push suppressions are NOT here.** They are a rule about
  /// FIREBASE, not about notifications in general (the reference delivers both events to
  /// webhooks) so they live in `FirebaseProvider.referenceSubscription`, next to the
  /// transport they describe. Applied at the bus they would take `typing-indicator` and
  /// `new-findmy-location` away from ntfy, which under v1 (where ntfy is a webhook) receives
  /// them. `allowsPush` is the class gate and is open for every event; a suppression that
  /// genuinely applies to every notification transport would belong here.
  ///
  ...
  public static func policy(for name: EventName) -> EventRouting {
    ...
    case .newFindMyLocation:
      ...
      //
      // The 250ms gap is the reference's handler `waitMs(250)` between items in a batch,
      ...
      // The RATE limit lives here: it protects Apple from this server's polling, whatever
      // the event is delivered over. The push suppression is Firebase's alone.
      EventRouting(

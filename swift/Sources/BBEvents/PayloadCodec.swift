...
public struct CodecNegotiator: Sendable {
  ...
  public func resolve(for capabilities: TargetCapabilities) -> any EventPayloadCodec {
    // An unrecognised preference falls back to the WEAKEST codec, not the strongest.
    //
    // `?? 0` is index zero of `preferenceOrder`, which is `sealedV2`: the ceiling that is
    // supposed to make default-off structural would have opened all the way up for any
    // preference string this build does not know, which is precisely the case where the
    // server's intent is least clear. Failing to the legacy end keeps the property the
    // comment above claims.
    let ceilingIndex =
      CodecIdentifier.preferenceOrder.firstIndex(of: serverPreference)
      ?? (CodecIdentifier.preferenceOrder.count - 1)

...
//  holding a socket open. One sink, so there is one copy of the routing policy and one
//  answer to "which events do I get". ntfy is a Firebase replacement: someone configuring
//  it is leaving Google, not subscribing a URL, so it follows push routing, not webhook
//  routing.
//
//  ## What belongs where
//
...
//  whatever the operator configured; a webhook has none. A single number above this line
...
public actor NotificationSink: EventSink {
  ...
  private func fanOut(_ event: ServerEvent) async throws {
    for provider in providers.values.sorted(by: { $0.providerID < $1.providerID }) {
      // A subscription miss is silent: it is configuration working as asked. Not ready
      // is the state "notifications stopped" usually turns out to be, so it says so.
      guard provider.subscription.includes(event.name) else { continue }
      guard await provider.isReady else {
        logger.debug(
          "Notification provider not ready; skipping",
          metadata: [

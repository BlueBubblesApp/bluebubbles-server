...
//  and in the message case through AppleScript as well) and both have to answer the same
//  two questions:
//
//    1. **What if no helper is connected?** One refusal, `requirePrivateAPI`, rather than a
//       copy per interface.
//    2. **What if the backend refuses?** AppleScript throws `MessageSendError` and the
//       helper throws `PrivateAPIError`; left alone, both fall through to the renderer's
//       fallback and reach clients as a generic 500 `Server Error`, indistinguishable from
//       the server itself having broken.
//
//  Both answers live here, so an interface added later gets them by conforming rather than
//  by remembering.
//
...
protocol MessagesBackedInterface: Sendable {
  /// The slice of the helper contract THIS interface uses, as a composition of roles from
  /// `BBPrivateAPIContract`.
  ///
  /// An associated type rather than `any PrivateAPI`, so each conformer states the roles it
  /// actually calls and a test can hand it a double built for those alone. Every conformer
  /// is still satisfied by a real `any PrivateAPI`: the composed protocol refines all
  /// sixteen roles, and an existential upcasts to anything it refines, so the composition
  /// root passes the same client it always did.
  associatedtype Helper: Sendable
  var privateAPI: Helper? { get }
...
extension MessagesBackedInterface {
  ...
  func requirePrivateAPI(for feature: String) throws -> Helper {
  ...
  ///
  /// `operation` defaults to `#function`, which Swift evaluates at the CALL SITE, so every
  /// log line below names the interface method that was running (`sendText(_:)`,
  /// `markRead(guid:)`) without any caller having to say so.
  func throughMessages<T>(
    operation name: String = #function, _ operation: () async throws -> T
  ) async throws -> T {
    do {
      ...
      // the `helperUnavailable` from `requirePrivateAPI` above. Passed through untouched,
      // and logged at debug: it is the caller's mistake, but it was invisible.
      logger.debug(
        "An operation through Messages was refused",
        metadata: [

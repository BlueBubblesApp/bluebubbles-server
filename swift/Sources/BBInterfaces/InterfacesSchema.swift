...
public enum InterfacesSchema: SchemaContributor {
  ...
  public static func registerSchema(in migrator: inout DatabaseMigrator) {
    ...
    // The first date a recurring message was aimed at, which is what calendar arithmetic
    // has to count from. `scheduled_for` moves to the next occurrence every time the
    // message fires, so a series anchored only on it drifts: the 31st becomes the 28th in
    // February and stays the 28th forever after. Counted from here, it is the 31st again
    // in March. Backfilled from `scheduled_for` so a series that predates the column
    // anchors on the next date it already had, the best information there is.
    //
    // Namespaced, unlike everything above it: this is the first migration in this module
    // written after the seam existed. See `SchemaContributor.schemaNamespace`.
    migrator.registerMigration("interfaces.addScheduledMessageAnchor") { db in
      try db.alter(table: "scheduled_message") { table in
        table.add(column: "first_scheduled_for", .datetime)
      }
      try db.execute(sql: "UPDATE scheduled_message SET first_scheduled_for = scheduled_for")
    }
  }
}

...
//  auth) lives in `BBHandlers/HandlerCapabilities.swift`, so this module does not import the
//  auth layer to name two protocols nothing here uses.
//
...
/// The event bus, for anything that emits events or registers a sink on it.
public protocol EventPublishing: Sendable {
  var events: EventBus { get }
}

/// The sink that fans an event out to the notification providers (ntfy today).
///
/// Separate from `EventPublishing` because registering the sink and configuring what is
/// behind it are different jobs: the bus is what every emitter touches, and this is what
/// only the service that owns notification delivery does.
public protocol NotificationSinkProviding: Sendable {
  var notifications: NotificationSink { get }
}

/// How a payload is encoded for a particular client or endpoint.
///
/// Its own capability rather than a member of `ServerStatusProviding`, which refines it: a
/// delivery service negotiates codecs and has no business reading this Mac's system
/// information, and `server/info` wants both.
public protocol CodecProviding: Sendable {
...
/// The server's OWN database: settings, webhooks, devices, scheduled messages.
///
/// Distinct from `MessageSourceProviding`, which is Apple's `chat.db` and is read-only.
public protocol AppDatabaseProviding: Sendable {
  var appDatabase: AppDatabase { get }
}

/// The `chat.db` read path and the serializer that projects rows from it.
///
/// Both are optional for one reason (Full Disk Access may not have been granted) so they
/// are one capability rather than two that are always absent together. A holder that has
/// `messages` has message access; there is no third state.
public protocol MessageSourceProviding: Sendable {
  var messages: MessageRepository? { get }
  var serializer: MessageSerializer? { get }
}
...
/// The Keychain-backed secret store.
///
/// Its own capability, refined by `DeviceRegistering`: the push service needs credentials
/// and has no business with the device table, and the registration route wants both.
public protocol SecretStoreProviding: Sendable {
  var secrets: any SecretStore { get }
}

/// Push registration. `secrets` comes from `SecretStoreProviding` above, which this refines
/// because the consumer serving the Firebase client configuration wants it next to the
/// device it just registered.
public protocol DeviceRegistering: SecretStoreProviding {
  var devices: DeviceRepository { get }
}

/// Registered push devices: their tokens, and pruning the ones Firebase has rejected.
///
/// Distinct from `DeviceRegistering`, which is the table. This is the behaviour over it, and
/// the push delivery path wants only this half.
public protocol DeviceDirectoryProviding: Sendable {
  var deviceDirectory: DeviceDirectory { get }
}
...
public protocol FaceTimeProviding: Sendable {
  /// The FaceTime operations, over whichever helper is connected right now. Built per call
  /// for the reason `PrivateAPIProviding` is a function: the helper comes and goes.
  func faceTimeInterface() async -> FaceTimeInterface
  /// The coordinator itself: the app's maintenance page and the Private API service reach
  /// it directly to stop watchers and run the periodic cleanup. A handler does not.
...
public protocol FindMyProviding: Sendable {
  /// The FindMy operations, over whichever helper is connected right now, and over the one
  /// shared runtime: the gates on reaching Apple are global on purpose.
  func findMyInterface() async -> FindMyInterface
}
...
public protocol ServerStatusProviding: CodecProviding {
...
///
/// Implemented by the SwiftUI app, which owns Sparkle, and consumed by
/// `POST /server/update/install`. The seam exists so the endpoint is written once against
/// a capability rather than against a specific host: the headless CLI has nothing to
/// relaunch and never supplies one, and the endpoint says so.
public protocol UpdateInstalling: Sendable {
  func beginUpdate(to item: AppcastItem) async
  /// What is happening to an update already found, or nil when nothing is pending.
  var installState: UpdateInstallState? { get async }

  /// Waits for the download `beginUpdate` started, for callers that asked to be told.
  ///
  /// `POST /server/update/install?wait=true` is the only caller. The reference awaits
  /// `autoUpdater.downloadUpdate()` before answering (`serverRouter.ts:37-40`), and this
  /// server used to accept the flag and answer immediately: a client that asked to be told
  /// when the download finished was told at once, and could not tell the difference between
  /// "downloaded" and "started".
  ///
  /// Bounded rather than open-ended, because the caller is an HTTP request: a download that
  /// stalls must not hold a connection until the route's own timeout kills it with no
  /// answer at all. The outcome says which happened, so the handler can report it rather
  /// than claim success.
  func awaitDownload(timeout: Duration) async -> UpdateDownloadOutcome
}

/// How waiting for an update download ended.
public enum UpdateDownloadOutcome: Sendable, Equatable {
  /// Sparkle has the update on disk and will install it.
  case downloaded
  case failed(String)
  /// Still going when the wait ran out. Not an error: the download continues, and the
  /// client is told that rather than told it finished.
  case timedOut
  /// Nothing was downloading by the time the wait began, which is what an updater that
  /// showed its own window to the person at the Mac reports.
  case notDownloading
}
...
/// The once-per-version `server-update` announcer, for every path that checks the feed:
/// the API, the app's fallback check and Sparkle's own find. One object so the three cannot
/// each tell the clients about the same release.
public protocol UpdateAnnouncing: Sendable {
  var updateAnnouncer: UpdateAnnouncer { get }
}

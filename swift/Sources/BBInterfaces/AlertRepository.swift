...
//
//  Its columns map one-to-one onto `UserAlert`, down to `occurrence_count`, `read_at` and
//  `dismissed_at`, which only mean anything because rows outlive the process; `AlertCenter`
//  applies a thirty-day retention window to them.
//
//  `is_durable` is the one column `createAlerts` did not have, added by its own migration.
//  See `UserAlert.isDurable` for what it decides.

...
public struct ChatInterface: MessagesBackedInterface {
  ...
  public func messages(
    ...
    // No helper passed, and that is not a narrowing workaround: `query` is a pure chat.db
    // read — it goes to the repository and then to `project`, neither of which touches
    // Messages. Handing one over implied a dependency this call does not have. The role
    // split is what made that visible; the argument was inert before it too.

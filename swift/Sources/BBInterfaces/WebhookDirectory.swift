...
//
//  Two things that are one cohesive job: the delivery path needs the targets, and the
//  settings page needs a "send a test" button that records its outcome the same way a real
//  delivery would.
//
...
public struct WebhookDirectory: Sendable {
  ...
  /// The registered endpoints, now and after every change to them. See
  /// `WebhookRepository.changes`.
  public func registrations() -> AsyncThrowingStream<[Webhook], any Error> {
    repository.changes()
  }
  ...
  public func sendTest(id: Int64) async -> WebhookDeliveryState {
    ...
    do {
      ...
      await deliveries.record(
        id: id, url: target.url, event: event.name.rawValue, outcome: .failed(reason)
      )
      logger.warning(
        "Webhook test failed",
        metadata: [

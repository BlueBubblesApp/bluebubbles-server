...
public enum InterfaceError: BBError, Equatable {
  ...
  /// An integration the operator switched off, asked for anyway.
  ///
  /// Distinct from `unavailable`, which is 503 and means "configured, but not usable right
  /// now", a chat.db that will not open. This is a decision somebody made, it is not going
  /// to resolve on its own, and the remedy is a switch. It answers 403 for the reason
  /// `FindMyHandlers.requireFeature` does: the route exists and the caller is not allowed to
  /// use it, which is a different thing from a path that is not there.
  case serviceDisabled(String, service: String)
  ...
  public var code: String {
    ...
    case .serviceDisabled: "interface.service_disabled"
  ...
  public var title: String {
    ...
    case .serviceDisabled: "That is switched off on this server"
    }
  }
  ...
  public var body: String {
    ...
    case .serviceDisabled(let detail, _): detail

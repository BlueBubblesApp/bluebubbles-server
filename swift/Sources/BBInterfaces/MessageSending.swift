...
extension MessageInterface {
  ...
  // contract on all three send routes: a client reads back the text, the date, the handle
  // and the chats it just sent to.
  //
  ...
  ///
  /// Invisible to every test that does not go through Messages: a fixture database has its
  /// joins already written, so the race cannot occur there.
  ///
  ...
  private func awaitJoined(
    ...
    if joined == nil {
      // The silent sixty seconds. The send happened; what the client gets back is the
      // bare identifiers, or a row with `chats: []`, and this is the only record of why.
      logger.debug(
        "Sent message did not appear joined to its chat before the deadline",
        metadata: [
          "rowSeen": .stringConvertible(lastSeen != nil),
          "limitS": .stringConvertible(Int(policy.limit.seconds)),
        ])
    }
  ...
  ///
  /// Attachments are included even though a text send has none and an attachment send's row
  /// is written before its transfer completes: the reference's recorded attachment send
  /// carries the attachment, its dimensions and its `transferState`, and a client uses them
  /// to render what it just sent. A text send simply gets `[]`, which costs one query.
  ...
  // `replyToGUID`, `partIndex`: one calling convention for one operation family.
  //
  ...
  public func sendText(_ request: SendTextRequest) async throws -> SendOutcome {
    ...
    let started = logSending(
      "text", backend: backend, chat: request.chatGUID,
      extra: [
        "forced": .stringConvertible(request.forcedBackend != nil),
        "chars": .stringConvertible(request.text.count),
        "subject": .stringConvertible(request.subject != nil),
        "effect": .string(request.effectID ?? "-"),
        "reply": .string(request.replyToGUID ?? "-"),
        "scheduled": .stringConvertible(request.scheduledFor != nil),
      ])
    ...
    case .appleScript:
      ...
      {
        // Which feature, for the log: the 400 the client gets names them all.
        var needs: [String] = []
        if request.subject != nil { needs.append("subject") }
        if request.effectID != nil { needs.append("effect") }
        if request.replyToGUID != nil { needs.append("reply") }
        if !request.formatting.isEmpty { needs.append("formatting") }
        if request.scheduledFor != nil { needs.append("scheduled") }
        logger.debug(
          "Refused a send AppleScript cannot make",
          metadata: ["needs": .string(needs.joined(separator: ","))])
  ...
  // MARK: - The two lines a send writes

  /// "Sending message", once the backend is decided. The chat is redacted (a direct chat's
  /// GUID is the other party's address); the text, subject and file path never appear,
  /// only their size and whether they were given.
  func logSending(
    _ kind: String, backend: SendBackend, chat: String, extra: Logger.Metadata = [:]
  ) -> ContinuousClock.Instant {
    var metadata: Logger.Metadata = [
      "kind": .string(kind),
      "backend": .string(backend.rawValue),
      "chat": .string(Redaction.chatGUID(chat)),
    ]
    metadata.merge(extra) { _, new in new }
    logger.debug("Sending message", metadata: metadata)
    return .now
  }

  /// "Message sent", with what the backend reported and whether the row was hydrated in
  /// time. `hydrated: false` beside a GUID is the deadline case `awaitJoined` logs.
  func logSent(_ outcome: SendOutcome, since started: ContinuousClock.Instant) -> SendOutcome {
    logger.debug(
      "Message sent",
      metadata: [
        "backend": .string(outcome.backend.rawValue),
        "guid": .string(outcome.messageGUID ?? "-"),
        "hydrated": .stringConvertible(outcome.message != nil),
  ...
  public func sendAttachment(_ request: SendAttachmentRequest) async throws -> SendOutcome {
    ...
    // Size and extension, never the name: a file name is whatever the person called it.
    let size = (try? FileManager.default.attributesOfItem(atPath: filePath)[.size] as? Int) ?? 0
    let started = logSending(
      "attachment", backend: backend, chat: chatGUID,
      extra: [
        "bytes": .stringConvertible(size),
        "ext": .string((filePath as NSString).pathExtension.lowercased()),
        "audio": .stringConvertible(request.isAudioMessage),
      ])
  ...
  public func sendMultipart(_ request: SendMultipartRequest) async throws -> SendOutcome {
    ...
    let started = logSending(
      "multipart", backend: .privateAPI, chat: request.chatGUID,
      extra: [
        "parts": .stringConvertible(parts.count),
        "files": .stringConvertible(parts.filter { $0.attachmentPath != nil }.count),
        "subject": .stringConvertible(request.subject != nil),
        "effect": .string(request.effectID ?? "-"),
        "reply": .string(request.replyToGUID ?? "-"),
      ])

...
//
//  A repository even though the table has one caller, because the rule is worth being able
//  to state without exceptions: every table in the app database is owned by a repository in
//  this folder, and a table's statements are not somewhere else because that is where they

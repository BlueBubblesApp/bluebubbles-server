...
public struct AdminInterface: Sendable {
  ...
  /// Four counts, each nil when `only` excluded it.
  ///
  /// Optional rather than zero-defaulted, so "not asked for" and "none" stay distinct: the
  /// reference omits the key entirely in the first case, and a client reading `chats: 0`
  /// where it should have read nothing would show an empty server.
  public struct Totals: Sendable, Equatable {
    ...
    public init(handles: Int?, messages: Int?, chats: Int?, attachments: Int?) {
  ...
  ///
  /// A key is OMITTED when `only` did not ask for it, which is what the reference does
  /// (`getDatabaseTotals` builds its result object one `if` at a time) — not emitted as
  /// zero, which would be a wrong answer rather than an absent one.
  public static func serialize(_ totals: Totals) -> JSONValue {
    var object: [String: JSONValue] = [:]
    if let handles = totals.handles { object["handles"] = .int(handles) }
    if let messages = totals.messages { object["messages"] = .int(messages) }
    if let chats = totals.chats { object["chats"] = .int(chats) }
    if let attachments = totals.attachments { object["attachments"] = .int(attachments) }
    return .object(object)
  }

  /// What the database holds, as four counts.
  ///
  /// - Parameter only: the reference's `only` filter, already normalised to singular
  ///   lower-case names (`handle`, `message`, `chat`, `attachment`). Nil means all four.
  ///
  ///   Honouring it is not tidiness: each of these is a full table scan, and a client
  ///   asking for the chat count on a 420,000-message database was paying for the message,
  ///   handle and attachment scans as well. The route accepted the parameter and counted
  ///   everything regardless.
  public func counts(only: Set<String>? = nil) async throws -> Totals {
    ...
    // Run concurrently. Sequentially this is four full table scans one after another, and
    // on a large database that is the difference between a responsive stats page and a
    // visibly stalled one.
    async let handles = wanted("handle") ? messages.handleCount() : nil
    async let chats = wanted("chat") ? messages.chatCount() : nil
    async let all = wanted("message") ? messages.messageCount() : nil
    async let attachments = wanted("attachment") ? messages.attachmentCount() : nil
  ...
  public func createWebhook(url: String, events: [String]) async throws -> Webhook {
    ...
    let webhook = try await webhookStore.upsert(url: url, events: events)
    logWebhook("Webhook created", id: webhook.id, url: url, events: events)
    return webhook
  }

  public func updateWebhook(id: Int64, url: String?, events: [String]?) async throws -> Webhook {
    if let url { try Self.validate(url: url) }
    let webhook = try await webhookStore.update(id: id, url: url, events: events)
    logWebhook("Webhook updated", id: id, url: webhook.url, events: webhook.subscribedEvents)
    return webhook
  }

  /// The URL is redacted: it routinely carries the server password in its query.
  private func logWebhook(_ what: String, id: Int64?, url: String, events: [String]) {
    logger.info(
      "\(what)",
      metadata: [
      ...
      backups.compactMap { row in
        guard let value = try? JSONValue.parse(row.payload) else {
          logger.warning(
            ...
            metadata: [
              "kind": .string(row.kind), "backup": .string(row.name),
            ])
          return nil
        }
        return value
  ...
  /// - Parameter name: which backup to delete. NOT optional: the repository treats a nil
  ///   name as "every backup of this kind", which is a reasonable primitive and a terrible
  ///   thing to reach from a route. The only caller is the delete route, and it requires
  ///   the name; see the comment there.
  public func deleteBackup(kind: BackupKind, name: String) async throws {
    ...
    guard deleted > 0 else {
      throw InterfaceError.notFound("no \(kind.rawValue) named \(name)")
    }
  }
}

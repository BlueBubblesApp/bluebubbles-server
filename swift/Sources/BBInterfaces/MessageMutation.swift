...
extension MessageInterface {
  ...
  public func react(
    ...
    logMutation(
      "Reacting to a message", guid: targetGUID, chat: chatGUID,
      extra: ["reaction": .string(reaction), "partIndex": .stringConvertible(partIndex)])
  ...
  public func sendSticker(
    ...
    try await requireMessage(targetGUID)
    logMutation(
      "Sending a sticker", guid: targetGUID, chat: chatGUID,
      extra: ["partIndex": .stringConvertible(partIndex), "tapback": .stringConvertible(asTapback)])
  ...
  public func edit(
    ...
    logMutation(
      "Editing a message", guid: guid, chat: chat.rawValue,
      extra: [
        "partIndex": .stringConvertible(partIndex), "chars": .stringConvertible(newText.count),
      ])
  ...
  public func unsend(guid: String, partIndex: Int) async throws -> SendOutcome {
    ...
    let chat = try await owningChat(of: guid)
    logMutation(
      "Unsending a message", guid: guid, chat: chat.rawValue,
      extra: ["partIndex": .stringConvertible(partIndex)])
  ...
  public func notify(guid: String) async throws -> SendOutcome {
    ...
    let chat = try await owningChat(of: guid)
    logMutation("Notifying anyway", guid: guid, chat: chat.rawValue)
    try await throughMessages {
  ...
  /// One debug line per mutation, on entry. A message GUID is opaque; the chat is
  /// redacted because a direct chat's GUID is the other party's address. Never the text.
  func logMutation(
    _ what: String, guid: String, chat: String, extra: Logger.Metadata = [:]
  ) {
    ...
    logger.debug("\(what)", metadata: metadata)
  }

...
public struct PushStatus: Sendable, Equatable {
  ...
  /// Why a credential could not be READ, when one could not: a locked Keychain, an
  /// entitlement lost to a rebuild, a stored document that no longer parses. Distinct from
  /// the two `has…` flags being false, which means nothing has been imported: the first is
  /// fixed by unlocking or re-importing, the second by setting up, and a screen that showed
  /// both as "not set up" sent people through setup to fix a Keychain.
  public let credentialProblem: String?
...
public enum PushSetupError: BBError, Equatable, CustomStringConvertible {
  ...
  /// were dropped at once. A single catch-all case would tell someone who dropped a damaged
  /// `google-services.json` to go and download a service account key instead.
...
public struct PushInterface: Sendable {
  ...
  /// Whether the push SERVICE is running, as opposed to whether push is configured.
  ///
  /// The two come apart, and the gap is a real failure rather than a transient: the gate
  /// (`PushDeliveryService.canRun`) reads the Keychain once at startup, and a Keychain read
  /// that FAILS (locked, or waiting on the access dialog this bundle's signature triggers)
  /// is swallowed as "no credentials". Push then declines permanently while the credential
  /// store, readable a moment later, reports the project as fully set up. Every action here
  /// that needs the service reported "Push notifications are not running on this server" on
  /// a page displaying the project id.
  public var isRunning: Bool { service != nil }

  /// Re-runs the gate through the registry, so a service that declined can start.
  ///
  /// Public because the setup UI has to be able to recover the state above without the user
  /// restarting the whole server. Restarting through the registry is the only thing that
  /// works: there is no instance to ask, which is the whole problem.
  public func reload() async { await reloadPush() }
  ...
  public func status() async -> PushStatus {
    ...
    // Read, not `try?`-ed. A Keychain that refuses is not the same answer as "nothing has
    // been imported", and the screen has to say which one it is; see
    // `PushStatus.credentialProblem`. Absence still reads as nil, as it always did.
    let clientConfig: FirebaseClientConfig?
    var credentialProblem: String?
    do {
      clientConfig = try await credentials.clientConfig()
    } catch {
      clientConfig = nil
      credentialProblem = DiagnosticText.sentence(for: error)
    }
    let hasClientConfig = clientConfig != nil

    let account: ServiceAccount?
    do {
      account = try await credentials.serviceAccount()
    } catch {
      account = nil
      credentialProblem = DiagnosticText.sentence(for: error)
    }
    ...
    // SETTING, and a service reports the value it was started with; read from there, the
    // toggle would snap back to the pre-restart value while the save itself succeeded. The
    // service's copy describes what the poller is currently doing; the store describes what
    // the user asked for, and the control is bound to the latter.
  ...
  ///
  /// The user-facing control over the Firebase command channel; `PushService` honours the
  /// setting this writes.
  ...
  public func inspect(_ urls: [URL]) async -> CredentialInspection {
    ...
    for url in urls {
      ...
      // `try?` as a probe: "is this file a service account?" A no falls through to the
      // client-config attempt below, whose own failure is what gets reported.
  ...
  ///
  /// Takes the nonce the listener will accept, because the two must agree: a URL built from
  /// a fresh configuration carries a `state` the callback then refuses. `signIn` asks the
  /// server it just started for this rather than calling here.
  public nonisolated func authorizationURL(for purpose: OAuthPurpose, nonce: String) -> URL {
    OAuthConfiguration().authorizationURL(for: purpose, nonce: nonce)
  }
  ...
  public func signIn(
    ...
    do {
      try await server.start(for: purpose)
    } catch {
      throw PushSetupError.signInFailed(reason: String(describing: error))
    }
    // AWAITED on every exit, not detached. `defer { Task { … } }` returns immediately and
    // schedules the stop for whenever, which produces exactly the failure the defer exists
    // to prevent: the listener is still on 8641 when the next attempt tries to bind it, and
    // that attempt reports `signInFailed` for a reason that has nothing to do with signing
    // in. Somebody who abandons the browser and presses the button again gets a port
    // conflict blamed on their sign-in.
    // The URL comes from the SERVER, so it carries the nonce that listener will accept.
    // Building it from a fresh configuration here would send the user to a URL whose state
    // the callback then refuses.
    await openBrowser(await server.authorizationURL(for: purpose))
  ...
  public func inspectProject(
    ...
    // Propagated, not `try?`-ed: a Keychain that refused here would read as "no key held",
    // and the plan built on that offers to mint a new key and delete the one we cannot see.
  ...
  public func provision(
    ...
    // Propagated, not `try?`-ed, for the same reason as `inspectProject`: a read that fails
    // must stop the provisioning run, not quietly turn "reuse the held key" into "mint one".
    var heldKey: Data?

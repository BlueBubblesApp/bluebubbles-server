...
extension MessageInterface {
  ...
  /// The session a message belongs to: the one the caller named, or a fresh one.
  ///
  /// **An unparsable id is refused, not replaced.** This was
  /// `sessionID.flatMap(UUID.init(uuidString:)) ?? UUID()`, so a malformed id minted a NEW
  /// session: a reply to a game in progress went out as a fresh invitation, answered 200,
  /// with the move lost and nothing saying so. Game Pigeon threads a whole game through this
  /// value, which is what makes a silent substitution worse here than a refusal.
  ///
  /// An ABSENT id still mints one, because that is what a first message is.
  static func session(for sessionID: String?) throws -> UUID {
    guard let sessionID, !sessionID.isEmpty else { return UUID() }
    guard let parsed = UUID(uuidString: sessionID) else {
      throw InterfaceError.invalidRequest(
        "`sessionId` is not a UUID; omit it to start a new session")
    }
    return parsed
  }
  ...
  public func sendAppMessage(
    ...
    let session = try Self.session(for: sessionID)

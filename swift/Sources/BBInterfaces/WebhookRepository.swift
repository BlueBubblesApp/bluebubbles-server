...
public struct Webhook: Sendable, Codable, FetchableRecord, PersistableRecord {
  ...
  /// The `events` column decoded, or nil when it cannot be. The one place that column is
  /// interpreted. Nil is DISTINCT from an empty list: the first is a row this server cannot
  /// act on, which the webhooks page names as such, and the second never occurs: `encode`
  /// writes `["*"]` for "no list". `try?` here because nil IS the report.
  public var decodedEvents: [String]? {
    (try? JSONValue.parse(Data(events.utf8)))?.arrayValue?.compactMap(\.stringValue)
  }

  /// What the delivery path subscribes this endpoint to. An undecodable column subscribes
...
public struct WebhookRepository: Sendable {
  ...
  ///
  /// Whichever path wrote (this process's API, the settings window, a client over HTTP)
  /// the follower sees the new list, so a page showing it never has to re-read on a timer.
  /// The first element is the current table. Throws only if the database itself fails,
  /// which a follower reports the way a failed `all()` is reported.
  ///
  /// A plain stream rather than GRDB's own sequence type, so a caller follows the table
  /// without importing GRDB: the same reason `read` takes a closure rather than handing
  /// out the queue.
  public func changes() -> AsyncThrowingStream<[Webhook], any Error> {

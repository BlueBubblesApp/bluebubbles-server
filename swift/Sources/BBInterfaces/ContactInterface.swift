...
public struct ContactInterface: Sendable {
  ...
  /// - Parameter isEnabled: whether the Contacts integration is switched on, asked EACH
  ///   time rather than captured as a value. `AppContext` caches the interfaces it builds
  ///   and rebuilds them only when the Private API republishes, so a stored flag would go
  ///   on answering with whatever the setting said when the server started.
  public init(
    index: ContactIndex,
  ...
  /// Refuses every contact operation while the integration is switched off.
  ///
  /// On the INTERFACE rather than in the handlers, per this module's rule that a decision
  /// belongs one level below the transport: "off" then means the same thing to the API, to
  /// the app, and to anything added later. The index itself is untouched and still on disk,
  /// so switching the integration back on serves it again without re-indexing.
  private func requireEnabled() async throws {
    guard await isEnabled() else {
      throw InterfaceError.serviceDisabled(
        "The Contacts integration is switched off on this server, so it is not indexing "
          + "your address book and will not serve contacts. Turn it on under Integrations.",
        service: "app.bluebubbles.core.contacts"
      )
    }
  }
  ...
  /// - Parameters are clamped for the same reason every other paged read here clamps: the
  ///   value reaches `LIMIT ?`, and SQLite reads a negative LIMIT as no limit at all, so
  ///   `?limit=-1` returned the entire address book in one response.
  public func list(limit: Int = 1000, offset: Int = 0) async throws -> [ContactRecord] {
    try await requireEnabled()
    return try await index.page(limit: min(max(1, limit), 1000), offset: max(0, offset))
  }

  public func count() async throws -> Int {
    try await requireEnabled()
    return try await index.count()
  }

  /// Avatars for a page of contacts, keyed by contact id, for `extraProperties=avatar`.
  ///
  /// **Fetched per contact, on demand, and only when asked.** The ingest deliberately does
  /// not request `CNContactImageDataKey` (see `ContactsIngest.indexKeys`: bulk-loading every
  /// avatar is the memory pathology it avoids) so the index has no image data to hand back.
  /// That decision stands; what was missing is this, the on-demand path for the one page a
  /// client actually asked about. Before it, `extraProperties` was a documented parameter —
  /// in this server's own OpenAPI, the only one of twenty-six it advertised and never read —
  /// that silently did nothing, and the client fell back to one HTTP request per contact.
  ///
  /// A contact whose image cannot be read is simply absent from the result: the wire field
  /// defaults to `""`, which is what the reference sends for an avatar-less contact, so a
  /// failure here costs a picture and never the request.
  ///
  /// - Parameter extraProperties: the request's raw value, comma-separated as
  ///   `parseWithQuery` takes it. Matched on a contained substring like every other relation
  ///   list on this API.
  public func serialize(page records: [ContactRecord], extraProperties: String?) async
    -> [JSONValue]
  {
    guard Self.wantsAvatars(extraProperties) else {
      return records.map { Self.serialize($0) }
    }
    let images = await avatars(for: records)
    return records.map { Self.serialize($0, avatar: images[$0.id]) }
  }

  static func wantsAvatars(_ raw: String?) -> Bool {
    guard let raw else { return false }
    return raw.lowercased().split(separator: ",").contains {
      $0.trimmingCharacters(in: .whitespaces).contains("avatar")
    }
  }

  func avatars(for records: [ContactRecord]) async -> [String: String] {
    ...
    for record in records {
      ...
      result[record.id] = data.base64EncodedString()
    }
    return result
  }
  ...
  public func contact(id: String) async throws -> ContactRecord? {
    try await requireEnabled()
    return try await index.contact(id: id)
  }
  ...
  public func displayNames(for addresses: [String]) async throws -> [String: String] {
    try await requireEnabled()
    return try await index.findContacts(addresses: addresses)
  ...
  ///
  /// Identity keys off `externalID` when the client provides one, so two different people
  /// with the same name do not collide into one row.
  ...
  public func update(id: String, body: JSONValue) async throws -> ContactRecord {
    try await requireEnabled()
    // 400, NOT 404, and the sentence is the reference's own.
    //
    // `contactRouter.update` and `.delete` both call through `findDbContact`, which throws a
    // plain `Error("No contact found with the ID: <id>")`; the router's `catch` turns any
    // error into `new BadRequest({ error: ex?.message })`. So the reference answers 400
    // "Validation Error" here, and this answered 404 "Database Error" — a different status
    // AND a different error type for the same request. A client branching on either sees a
    // failure mode that does not exist on the reference.
    //
    // The corpus records both routes against a contact that IS present, so the replay only
    // ever compared the success path; the miss was reachable but never looked at.
    guard let existing = try await index.contact(id: id) else {
      throw InterfaceError.invalidRequest("No contact found with the ID: \(id)")
    }
  ...
  public func delete(id: String) async throws {
    try await requireEnabled()
    // 400 with the reference's sentence; see `update` above for why.
    guard let existing = try await index.contact(id: id) else {
      throw InterfaceError.invalidRequest("No contact found with the ID: \(id)")
    }
  ...
  public func create(batch body: JSONValue) async throws -> BatchOutcome<ContactRecord> {
    ...
    logBatch("Contacts created", succeeded: created.count, failed: failures.count)
  ...
  > {
    ...
    logBatch("Contacts updated", succeeded: updated.count, failed: failures.count)
  ...
  public func delete(batch body: JSONValue) async throws -> BatchOutcome<String> {
    ...
    logBatch("Contacts deleted", succeeded: deleted.count, failed: failures.count)
    return BatchOutcome(succeeded: deleted, failures: failures)
  }

  /// Counts only. A contact is a person by definition, so nothing from the record goes in.
  private func logBatch(_ what: String, succeeded: Int, failed: Int) {
    logger.info(
      "\(what)",
      metadata: [
        "succeeded": .stringConvertible(succeeded),
  ...
  public func refresh() async throws -> ContactsIngestResult {
    try await requireEnabled()
    // Reaching here with the integration switched ON and no ingestor means the service did
    // not start: a Contacts permission it was refused, most likely. This used to say the
    // permission had not been granted whatever the cause, which was wrong the other way
    // too: with the integration off it sent people to System Settings to check a permission
    // they already had. `requireEnabled` above now answers that case, so this can be
    // specific about the one that is left.
    guard let ingestor else {
      throw InterfaceError.unavailable(
        "The address book has not been indexed. The Contacts service did not start; check "
          + "that Contacts access is granted under Settings › Permissions."
      )
    }
    let result = try await ingestor.reindexAll()
    logger.info(
      "Re-indexed the address book on request",
      metadata: [
        "indexed": .stringConvertible(result.indexed),
        "skipped": .stringConvertible(result.skipped),
        "ms": .stringConvertible(result.duration.milliseconds),
      ])
    return result
  }
  ...
  /// conditional ones.
  ///
  /// Measured against a live Electron server. The parity harness stops at a length
  /// mismatch and never reaches the elements, and two machines' address books never have
  /// the same count, so this shape has to be checked by hand or by `ContactWireShapeTests`.
  ///
  ...
  public static func serialize(_ record: ContactRecord, avatar: String? = nil) -> JSONValue {
    ...
    // that number and is null for our local records.
    object.set(
      "phoneNumbers",
      .array(
    ...
    // reference defaults it with `isNotEmpty(avatar) ? base64(avatar) : ""`, and a client
    // rendering an avatar expects the key.

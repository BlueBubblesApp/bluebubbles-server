...
### How long each check takes, so you can tell slow from stuck

Measured on an Apple Silicon development Mac at 73 targets and 2,439 tests. Treat them as
orders of magnitude, not benchmarks; the point is the SHAPE: nothing here takes minutes.

| Check | Warm | From `swift package clean` |
|---|---|---|
| `swift build` | ~7s | ~55s |
| Strict build (`-Xswiftc -warnings-as-errors …`) | ~20s | ~55s |
| `swift format lint --strict --recursive Sources Tests Helper` | ~6s | same |
| `python3 Tools/package-graph/check.py` | under 1s | same |
| `swift test` | ~30s | ~35s |

**`swift test` past about 90 seconds is not a slow build, it is a hang.** The suite ITSELF
runs in about 10 seconds; the rest is building test targets, and that is bounded by the cold
number above. This is worth stating because the natural assumption is the opposite one: a
five-minute `swift test` was read as "it must still be compiling" and was in fact a stream
that had gone quiet, in `FileSinkFollowTests`. Two ways to tell in one command:

```bash
ps -Ao etime,%cpu,comm | grep -E 'swift-frontend|swiftpm-testing-helper'
```

`swift-frontend` processes burning CPU means it is compiling. `swiftpm-testing-helper` alive
at 0% CPU, with the output not growing, means a test is waiting on something that will never
arrive. **An `await` on an `AsyncStream` or a continuation is the usual culprit**, and it hangs
the whole process rather than failing one test, so the run reports nothing about which test is
stuck. A test that awaits a stream bounds the wait (`withTimeout` in `BBCore`) so a
regression fails with a name on it instead. `FileSinkFollowTests` is the worked example.

One trap in the ordering: the plain build and the strict build share `.build` but use different
compiler flags, so they invalidate each other. Running the five checks in the order given above
therefore pays one test-target rebuild inside `swift test`; that is normal and is included in
the numbers.
...
# Not Swift, and not previously written down anywhere, which is how a contributor ran the
# five checks above and was failed by one nobody had mentioned. Pinned versions, because a
# new release of either should not fail a build that changed neither.
#   pip install 'ruff==0.16.7' 'shellcheck-py==0.11.0.1'
find Packaging Tools -name '*.sh' -not -path '*/.build/*' -print0 \
  | xargs -0 shellcheck -x --source-path=SCRIPTDIR
ruff check Tools --exclude '**/.build' --select E9,F,B,SIM115

# The workflows' own inline shell, and the two out-of-tree tool packages.
actionlint ../.github/workflows/swift-*.yml
swift build --package-path Tools/observation-probe
swift build --package-path Tools/send-probe
```
...
- **Order matters**: schemas are inferred from the corpus, and the document is built from the
  schemas. Infer first.
- **`shellcheck` and `ruff` gate every pull request too.** They are not Swift, so they are easy
  to forget, and for a while they appeared in no document at all: the gate failed people for a
  check the instructions did not mention. `ruff`'s rule set is deliberately narrow — syntax
  errors, pyflakes, bugbear, and the one file-handle rule — because the point is catching a
  script that is broken, not failing a commit that touched no Python on a preference.
- **`swift build --package-path Tools/send-probe` rewrites that package's `Package.resolved`.**
  That is a real diff, and it is noise: restore it unless you meant to move the pin.
...
The corpus lives in `Fixtures/http/` (186 files) and **is committed**. It was local-only while
...
Deterministic and byte-identical on regeneration, so they never appear as diff noise, which is
why they are **committed** rather than rebuilt: `swift test` then needs no Python. The
generator's `--self-test` proves the determinism. This paragraph said the opposite for a
long time, and the `.gitignore` rule it rested on named a directory that has never existed.

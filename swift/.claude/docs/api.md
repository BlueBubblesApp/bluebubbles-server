...
- `message` is **not** uniform. Fifty-two routes carry their own string; the rest fall through
...
- A required field that is absent answers `"The <field> field is required."`: validatorjs's
  own wording, which is what the reference generates from a `required` rule. Those sentences now
  come from a real validation layer rather than from a handful of hand-written checks: see
  below.
...
`Sources/BBHTTPAPI/RequestValidation.swift` is the engine and `ValidationRules.swift` is the
per-route table, run from `HTTPServer.dispatch` after the body is collected and before the
handler — the same place the reference's validator middleware sits, so a refusal never reaches
a handler and never touches Messages.

**The rules are a transcription of `validators/*.ts`**, keyed by `HandlerID` so that routes
sharing a controller share their validator exactly as the reference's do. Only the FIRST
failure is reported, in declaration order (fields in the order the reference lists them, then
that field's rules in order), because that is what `getFirstError` does and the sentence a
client is shown depends on it. `ValidationRuleParityTests` diffs the table against
`Tests/CompatibilityTests/Fixtures/node-validation-rules.json`, generated by
`Tools/validation-rules/extract.py`; regenerate it when the Node validators change.

**This layer can only break a client by being too STRICT**, which is the whole of its design
constraint: a rule that is too lax preserves today's behaviour, while one that is too strict
turns a working request into a 400. validatorjs is far laxer than it looks, and each of these
is load-bearing rather than a curiosity:

- A non-implicit rule **does not run at all** unless the value would pass `required`, so `""`,
  `"   "`, `null` and an absent key satisfy `string`, `numeric`, `boolean`, `in` and
  `min`/`max` by never reaching them. `{"limit": ""}` is a 200.
- `numeric` is `Number()` coercion, not a type check: `"100"` passes. This is the same property
  `RequestValues`' leniency exists for, and the one a "sensible" reimplementation breaks first.
- `in` passes for any falsy value: `sort: ""` satisfies `in:ASC,DESC`.
- `required` tests `String(val)` with whitespace stripped, so `0` and `false` pass and an empty
  ARRAY fails.
- `min`/`max` measure the NUMBER for a numeric field, the LENGTH for an array and the CHARACTER
  COUNT otherwise — one rule, three meanings, and the message changes with it.

`RequestValidationTests` pins each of those as an ACCEPTED input rather than a refused one, for
the same reason. Downstream of the gate nothing changed: `RequestValues` stays lenient, so
everything that passes reads exactly as it did before.

One field is knowingly not reproduced. `WebhookValidator.getWebhookRules` declares
`id: "number"`, which is not a validatorjs rule, and an unregistered rule throws — so the
reference answers **500** for any request that actually sends `id`. Transcribing that would be
transcribing a crash. It is declared in `ValidationRuleParityTests.knowinglyDivergent`, and a
divergence that is not on that list fails the build.
...
`where` clause is arbitrary SQL from an authenticated client. We accept the same requests and
run none of that SQL: `MessageFilter` (`Sources/BBIMessage/`) parses each `{statement, args}`
pair into one of eight typed cases and `MessageRepository.messagePredicate` renders it.

The eight are every statement the BlueBubbles client sends, transcribed from it: row-id `>`
and `<=` (incremental sync), `text LIKE … COLLATE NOCASE`, `associated_message_guid IS NULL`,
`chat.guid =`, `is_from_me =`, `handle.id =` (search) and `guid IN (:...guids)` (notification
hydration). Matching is on SHAPE — placeholders normalize to `?` — so a client naming its
bound value differently still works.

**A statement outside the set is a 400 naming it, never a silent pass.** `where` was
previously accepted, parsed by nobody, and applied to neither the rows nor `metadata.total`.
A client asking for "messages after ROWID n" got the newest `limit` messages and a total
counting the whole database; measured on a 420,679-message chat.db, the app's incremental
sync turned a five-message delta into 417 pages of 2.3 MB and timed out. Search was equally
broken and nobody had reported it yet.

**`total` is counted under the same predicate as the page.** `messagePredicate` is shared by
the listing and the count precisely so they cannot drift; a filtered page beside an
unfiltered total is a pagination bug that reads as a hang.

Only `/message/query` takes filters. `/chat/query` accepts a `where` in the reference and no
client sends one, so it is not implemented; add it here if that changes.

### The LAN is trusted by default, and that is about LOCKOUT, not authentication

`trust_local_network` defaults ON. It never bypasses the password:
`AuthenticationMiddleware.authenticate` is `admit` (blocklist and rate limiter) then
`verifyCredential`, and the allowlist is read only by `AccessControl.evaluate` and
`recordFailure` — nothing in the credential chain consults the client address. All it removes
is the automatic lockout for RFC1918 peers.

It defaults on because the failure it prevents is invisible. Loopback can never be blocked
(`ProxyTrustPolicy.permanentAllowlist`), so with it off a phone on the same desk locks itself
out after ten bad passwords while `curl` on the Mac keeps working, and a blocked caller gets a
**401 deliberately identical to a wrong password**, so nothing on the wire says what happened.
Reproduced: ten wrong passwords from the LAN address, after which the CORRECT password
answered 401 from the LAN and 200 from loopback. The reference has no rate limiter, no
blocklist and no such setting, so an install with this on is still strictly more hardened than
any v1 server. Turning it off costs a hostile device already inside the network its unlimited
guesses, which is the reason the switch is a plain toggle on the Security page.

### Every v1 parameter is applied or refused, and `RequestParameterParityTests` checks it

`where` was the first of twelve. An audit of the whole v1 surface — parse both route tables,
extract what each Node router destructures and what each handler here names, join, verify by
hand — found eleven more on nine routes:

| Route | Was ignored |
|---|---|
| `GET /chat/:guid/message` | `message.attributedBody`, `message.messageSummaryInfo`, `message.payloadData`. The app's FULL SYNC sends all three, so a sync imported no edits, no unsends, no rich links and no attributed body |
| `POST /message/:guid/edit` | `backwardsCompatibilityMessage`. The handler read `backwardsCompatMessage`, the reference's INTERNAL name; the wire name is what the client sends and what our own schema documents, so the user's fallback text was discarded on every edit |
| `POST /handle/query` | `address` and `with`. A client asking for one correspondent got an unfiltered page of a thousand |
| `GET /contact`, `/contact/external/:id`, `POST /contact/query` | `extraProperties` |
| `GET /message/count/me`, `/count/updated` | `chatGuid` |
| all three `/message/count*` | `minRowId`, `maxRowId` |
| `GET /server/statistics/totals` | `only` — four full table scans where one was asked for |
| `GET /backup/theme`, `/backup/settings` | `name` |
| `GET /webhook` | `id`, `url` |
| `GET /attachment/:guid/blurhash` | `width`, `height`, `quality` |

Two things the audit also settled, both in the reference's favour:

- **`after`/`before` are INCLUSIVE.** The reference compares `>=` and `<=`; this compared `>`
  and `<`, so a message whose timestamp is exactly the boundary was dropped — and the
  incremental sync passes its last sync time as `after`, which makes that the message never
  sent and never asked for again.
- **`/message/count/updated` really does require `after`.** It looked like a divergence of
  ours until `messageValidator.ts` turned up `after: "required|numeric|min:0"` and the
  recorded fixture `get_api_v1_message_count_updated-5baa61-400.json` confirmed the 400.
  **Read the validators as well as the routers**: a router that destructures a parameter
  without checking it may still have a validator that requires it.

**Two parameters are knowingly inert, and say so where they land.** `convertAttachments`
reaches `AttachmentSerializerConfig.convert` and is read by nothing: in the reference it
pre-warms the converted file as a side effect of serializing and changes no field on the
wire, and this server converts on download instead. `quality` on the blurhash route selects
Electron's resize filter, which has no Core Graphics counterpart. Both are listed in
`RequestParameterParityTests.knowinglyInert` with the reason, and the test still requires the
name to appear in the sources so a reader finds the explanation rather than an absence.

### `messageSummaryInfo` and `payloadData` are TRANSFORMED, not passed through

Both columns are binary plists, and the reference runs them through `node-typedstream`'s
`BPlistReader`, which does four things `PropertyListSerialization` does not:

1. renames the short Apple keys — `ec`→`editedContent`, `ep`→`editedParts`, `otr`→
   `originalTextRange`, `t`→`text`, `d`→`date`, `euh`→`editingUserHandle`, `amc`/`ams`/`bcg`;
2. typedstream-DECODES every nested blob, so an edit's history is `{encodings, values}` with
   real attributed strings rather than base64;
3. collapses a dictionary whose only key is `"0"` to its value, so a single-part message's
   `editedContent` is a bare array;
4. collapses `{lo, le}` to `[location, length]`.

`rp` is deliberately NOT renamed, and the client reads `json["retractedParts"] ?? json["rp"]`
because of it. That asymmetry is diagnostic: when this transformation was missing, unsends
worked and edits did not — the client's `editedParts` parsed as `[]`, `buildMessageParts`
never entered its edit branch, and an edited message rendered as if it had never been edited.

The expectations in `MessageSummaryInfoWireTests` were produced by running
`node-typedstream@1.4.0` over a real row and transcribing its output, not derived from our
code. The parity corpus cannot catch this: its eight non-null `messageSummaryInfo` values
contain no edited message, because the recorded `message/:id/edit` response is `[{"ust":
true}]` — captured before Messages had written the history.

**Plist UIDs are `{"UID": n}`.** A keyed-archiver UID has no Swift or Objective-C surface —
it answers no selector, bridges to no `NSNumber`, and `CFKeyedArchiverUIDGetValue` is
CoreFoundation SPI. `PropertyListWire.uidValue` reads it by re-encoding the lone object as a
one-object binary plist and taking the value out of the bytes, which is public API over a
documented format and handles the two-byte UIDs past 255 that a naive reader truncates.
Serializing them as `null`, as this did, does not lose a value: in an `NSKeyedArchiver`
archive every reference between objects is a UID, so it loses the whole object graph, and a
rich-link `payloadData` arrives as a flat pile of strings.

**One known divergence, deliberate.** The reference decodes a NESTED attributed string with
`BinaryDecoding.all`, so an attribute whose value is undecodable `NSData` survives as a bare
array of octets; the top-level `attributedBody` uses `.decodable` and drops it, which is what
`AttributedBodyWireFormat.legacy` reproduces. We apply the caller's format in both places, so
such an attribute inside an edit is dropped too. Measured over sixty live rows carrying one
blob or the other: that accounts for the only difference in the set, on one message, in
`__kIMCalendarEventAttributeName` — a data-detector attribute the client never reads
(`Attributes.fromMap` names four keys and this is not one). Matching would mean emitting a
byte array, a shape this server produces nowhere else. Pinned by
`MessageSummaryInfoWireTests`.

### `server_version` is a FEATURE LEVEL, not this build's version

Clients fold it into `major * 100 + minor * 21 + patch` and gate fourteen features on
thresholds of that integer (`server_details.dart`). This server's own `0.1.1` scores **22**,
below every one of them, so a client was told it could use none of what this server
implements — no edit or unsend, no scheduled messages, no handle sync, no group management,
and the oldest incremental-sync algorithm.

So `GET /server/info` reports `ServerVersion.advertised`: the version of the Electron server
whose behaviour this one implements (`1.9.9`, scoring 298). `ServerVersion.current` stays the
real bundle version and is what `UpdateChecker` compares against the appcast — conflating the
two would break updates in both directions. `ServerVersionAdvertisementTests` holds the
advertised value against the client's gate table, so a new gate above 298 fails the build
rather than shipping dark.
...
- `AdditiveRoutes` (`RouteTable.swift:506`): routes that are not in the reference table. Mounted
...
handshake can reuse these stages without pulling in Hummingbird types.

**What the socket actually shares is `AccessControlService` itself**, not the
`AuthenticationStage` wrapper: `EngineIOServer` is handed the same instance the middleware
holds and runs the same sequence (`identity` → `evaluate` → authenticate →
`recordSuccess`/`recordFailure`) at both of its authentication points, the query handshake
and the deferred socket.io v4 CONNECT. One controller across both transports is the property
that matters, because two would mean two counters and twice the guesses.

This was not true until recently, and the gap was invisible: a wrong password on the socket
wrote a log line and nothing else, so no failure was counted, no block was ever computed and
no alert naming the address was raised. A client blocked on HTTP could keep guessing on the
socket indefinitely. `SocketAccessControlTests` covers both paths, including that a refusal
for being blocked is indistinguishable from a wrong password, and that a shared tunnel cannot
be locked out by one bad client behind it; `SocketAccessControlWiringTests` asserts the
composition root still passes the controller, because its absence fails no other test.

**An unauthenticated session is cheap and short-lived.** The deferred path has to create a
session before any credential is seen, so two limits bound what that is worth to an attacker:
`maximumAwaitingAuth` (32) caps how many may exist at once, and `authGraceTimeout` (10s) is how
long one may sit before its CONNECT. Both are on `EngineIOServer.Configuration`. The grace is
deliberately unrelated to `pingTimeout` (120s), which governs an ESTABLISHED session that may
legitimately go quiet; an unauthenticated one has no such claim. Expired waiters are swept on
the way IN as well as on the maintenance tick, which is what keeps the cap from refusing a real
client on an attacker's behalf, and a refusal at the cap is byte-identical to a wrong password.
`UnauthenticatedSessionTests` covers both properties.

...
### The helper does nothing outside its own app

A dylib constructor runs wherever the dylib is loaded, and that is not only Messages or
FaceTime: `DYLD_INSERT_LIBRARIES` is inherited by every process the relaunched app spawns,
and the package test bundle links the Messages helper, so its constructor ran in SwiftPM's
discovery helper and in the test runner. Each connected to the live server's socket and was
refused with `-67050`, one warning per `swift test`. `HelperHostGuard.shouldRun` is checked
first thing in both constructors: the helper starts only when `Bundle.main.bundleIdentifier`
is its intended app, or when `BLUEBUBBLES_HELPER_SOCKET` is set, which is a test or a
development server that has pointed it somewhere of its own on purpose.
...
The fields inside a command are typed the same way. `WireKey` (also in the contract) is
every payload key on the wire: request fields, reply fields, event fields and the frame
envelope, declared once with the shipping helper's spelling as its raw value.
`PrivateAPIClient` builds payloads with `.object([.chatGuid: …])`, each dispatch reads them
with `data[.chatGuid]`, and every reply is a `WireObject` built from the same keys, so a
field has one spelling and renaming a case breaks both sides at once. `HelperEventName`
does the same for the event names. The string subscript and string-keyed `object` case still
exist for the frame decoder and for tests that spell the wire out on purpose;
`HelperVocabularyTests` scans the files on either side of the socket and fails on a literal
key, which is what keeps `WireKey` the only spelling. What it does not do is tie a field to an
action: that pairing is still the client method against its dispatch case, checked by
...
belongs to Apple. Only the server could host it, and the server has no launchd job at all: it
starts as a login item, not an agent, so there is no `MachServices` array to put a name in. (When
a launch agent WAS offered the objection was weaker but still fatal: it would have coupled the
Private API to an optional registration and invited launchd to on-demand-launch a second server
instance.) The macOS-14 API-vintage objection no longer applies; reason 1 stands alone.
...
**There is no rung at all for the outgoing send sound, and the reason is worth keeping.** The
sound is not played by Messages.app: it is played by **imagent**, in
`+[IMDSoundUtilities playMessageSentSoundIfNeeded:]` (IMDaemonCore), which this helper — being
injected into Messages.app — can never reach. Three hooks were built and measured against live
sends before that was established: `IMDaemonController` (a plain `NSObject` with no
`forwardInvocation:`, so an added method is never reached), the `IMDistributingProxy` its
`remoteDaemon` really is, and `-[SMSApplicationSoundHelper playSendSoundForMessage:]` in
ChatKit. All three intercepted zero requests while the Mac audibly played the sound each time.
`-[IMDaemonAnyProtocol playSendSoundForMessageGUID:callerOrigin:]` is a decoy: it names the
sound and the message and is declared identically on 14.6.1, 15.6.1 and 26.5.2, and this send
path never sends it.

What settled it was the system log, not the header dump:

```
imagent [com.apple.Messages:IMDSoundUtilities] read value: [YES] FROM playSoundsKey com.apple.MobileSMS
imagent [com.apple.Messages:IMDSoundUtilities] Play message sent sound type: SMS, for message: <guid>
```

The two levers inside imagent are both wrong for us. `+_isPlaySoundEnabled` reads
`PlaySoundsKey` in `com.apple.MobileSMS` — the Mac-wide Messages "play sound effects"
preference, not something that can be scoped to one send, and a server that wrote it around
each send would leave a person's sounds off for good the first time it crashed mid-send.
`+_soundTypeForMessage:` returns "no sound" for a scheduled-message edit, an SMS downgrade, a
sound already played, and an **auto-reply** — and `isAutoReply` is a real property, stored in
`chat.db` and carried on the v1 wire, so setting it would mute the sound by lying to every
client about what the message is. Somebody who does not want a server Mac chirping turns
Messages ▸ Settings ▸ General ▸ Play sound effects off, which is the same switch imagent reads.

The macOS 26 typing indicator would sit on rung 4
(`CKConversationListStandardCell.setShowTypingIndicator:`), walking
`cell → conversation → chat → guid` through `NSSelectorFromString` inside a `@try`, firing only
while that list cell exists. Observing the UI to recover a signal the message layer stopped
emitting is a fidelity and robustness downgrade. **It is a known defect to replace, not a
pattern to copy.**

**WOULD, not does: rung 4 is not implemented.** This paragraph was written in the present
tense and read as a live fallback. There is no swizzling anywhere in `Helper/` —
`method_exchangeImplementations`, `class_replaceMethod`, `class_addMethod`,
`method_setImplementation` and `imp_implementationWithBlock` return zero hits between them —
and `IMCoreBridge` says so in place. Typing observation is rungs 1 to 3 only, which matters
because rung 2 is where an injected handler receives message items, and the registration
handshake reports which rung attached. If typing is missing on a release, that is the ladder
running out, not this fallback failing.
...
maps nowhere. dyld declines the insert SILENTLY: the app launches, nothing is logged, and the
Private API is just absent, so this is asserted rather than trusted:

```bash
swift build --arch arm64e --product BlueBubblesHelper
lipo -archs .build/arm64e-apple-macosx/debug/libBlueBubblesHelper.dylib   # want: arm64e
...
**Both packaging scripts build the helpers separately from everything else, and both check the
result with `lipo` before copying.** `Packaging/build-app.sh` builds them `arm64e + x86_64`
(the app and the CLI stay `arm64 + x86_64`); `Tools/dev-bundle.sh` builds the single slice this
Mac's Messages runs. That second build is load-bearing: a helper taken from the ordinary
`swift build` output is `arm64` and can never load. Both scripts copy BOTH dylibs:
`PrivateAPIGatedService` looks for `libBlueBubblesFaceTimeHelper.dylib` in
`Contents/Frameworks/` by name, and a bundle without it has no FaceTime routes at all.
